...
Deno.test("shapeT212Portfolio — a renamed ticker surfaces its price under the CURRENT symbol", () => {
  ...
  // …and the position itself now syncs under the CURRENT symbol too.
  assertEquals(holdings["META"], { shares: 4, cost: 300 });
  assertEquals(holdings["RKLB"], { shares: 100, cost: 5 });
});
...
Deno.test("shapeT212Portfolio — prices = EVERY recognised holding's currentPrice (generic map)", () => {
  ...
  const { holdings, prices } = shapeT212Portfolio(raw);
  // Price map carries the allow-list ETF AND the US stocks.
  assertEquals(prices["VUAA.L"], 98.42);
  assertEquals(prices["AAPL"], 234.5);
  assertEquals(prices["HOOD"], 31);
  // Holdings now carry EVERY reported position, not just the two DCA'd
  // ETFs. Narrowing this map is what hid 55 shares of PLTR the board had
  // recorded as sold out. SAEM.L is still present at zero because the
  // allow-list is seeded — a sold-out DCA target has to arrive as an
  // explicit 0 rather than silently vanishing.
  assertEquals(Object.keys(holdings).sort(), ["AAPL", "HOOD", "SAEM.L", "VUAA.L"]);
  assertEquals(holdings["AAPL"], { shares: 3, cost: 200 });
  assertEquals(holdings["HOOD"], { shares: 9, cost: 30 });
  assertEquals(holdings["SAEM.L"], { shares: 0, cost: 0 });
});
...
Deno.test("shapeT212Portfolio — /equity/positions nested instrument shape (ticker + averagePricePaid)", () => {
  ...
  // Every reported position syncs, under either field naming.
  assertEquals(holdings["AAPL"], { shares: 3, cost: 200 });
  assertEquals(prices["VUAA.L"], 98.42);
  assertEquals(prices["AAPL"], 234.5);
});
...
Deno.test("shapeT212Order — a dropped item names the field it was missing", () => {
  // The order backfill stalled in production with "50 items, 0 parsed
  // (top-level keys: fill,order)" — the right envelope, nothing stored,
  // and no way to tell which field the older pages lacked without
  // guessing at someone else's schema. Every rejection now reports
  // itself, so one run of the stuck page answers the question.
  const skips: string[] = [];
  const base = {
    fill: { id: "1", quantity: 2, price: 10, filledAt: "2026-07-20T14:30:00Z" },
    order: { instrument: { ticker: "AAPL_US_EQ" }, side: "BUY", status: "FILLED" },
  };
  // The happy path stores and reports nothing.
  assertEquals(shapeT212Order(base, "invest", skips)?.ticker, "AAPL");
  assertEquals(skips.length, 0);

  const reasonFor = (item: unknown): string => {
    const out: string[] = [];
    shapeT212Order(item, "invest", out);
    return out[0] ?? "";
  };
  assertEquals(
    reasonFor({ ...base, order: { ...base.order, side: undefined } }),
    "no-side:missing",
  );
  assertEquals(
    reasonFor({ ...base, order: { ...base.order, instrument: {} } }),
    "no-ticker",
  );
  assertEquals(
    reasonFor({ ...base, fill: { ...base.fill, quantity: 0 } }),
    "no-quantity",
  );
  assertEquals(
    reasonFor({ ...base, fill: { ...base.fill, price: 0 } }),
    "no-price",
  );
  assertEquals(
    reasonFor({ ...base, fill: { ...base.fill, filledAt: "not a date" } }),
    "no-fill-time",
  );
