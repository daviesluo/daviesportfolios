...
type PortfolioStats = {
  ...
  bestDayUsd: number; worstDayUsd: number; maxOrderUsd: number; peakOpenUsd: number; curveMonthly: [string, number][];
};
...
function combine(sleeves: Sleeve[]): PortfolioStats {
  ...
  if (!days.length) {
    return { members: 0, capitalUsd: 0, pnlUsd: 0, ret: 0, retOnBook: 0, maxDD: 0, retOverDD: 0, days: 0, from: "", to: "", deployment: 0, turnoverPerYear: 0, bestDayUsd: 0, worstDayUsd: 0, maxOrderUsd: 0, peakOpenUsd: 0, curveMonthly: [] };
  }
  let eq = capital, peak = capital, maxDD = 0, best = -Infinity, worst = Infinity, peakOpen = 0;
  ...
  return {
    ...
    peakOpenUsd: Number(peakOpen.toFixed(2)),
    curveMonthly: curve,
  };
}
...
if (import.meta.main) {
  ...
  type CoinRule = {
    ...
    tradeStats: { count: number; medianGross: number; medianHoldBars: number; medianHoldDays: number; shareGrossOver: Record<string, number> } | null;
  };
      ...
      for (const w of windowsFor(bars.length)) {
        ...
        coinRules[w.name][`${rule}·${symbol}`] = {
          ...
          tradeStats: tl.length
            ? {
              count: tl.length, medianGross: Number(median(gross).toFixed(4)),
  ...
  function buildCore(spec: Spec, wname: "A" | "B", weight: ((i: number) => number) | null = null): Core {
    ...
    if (spec.symbol == null) {
      ...
      const r = runRotation(basketDaily, w.oosFrom, w.oosTo, p, costs, rotStops);
      return {
  ...
  for (const wname of ["A", "B"] as const) for (const spec of specs) cores[wname].push(buildCore(spec, wname));
  ...
  function sleevesFor(arm: ArmName, wname: "A" | "B"): Sleeve[] {
    ...
    for (const row of LIVE_ROWS) {
      ...
      if (row.kind === "rotation-1d") {
        const c = cores[wname].find((x) => x.id === rowSpecs[0].id)!;
        out.push({ ...c, slotUsd: rowSpecs[0].rowDeployable });
        continue;
      }
      const w = weightsFor(arm, wname, row);
      for (const spec of rowSpecs) {
        const base = cores[wname].find((x) => x.id === spec.id)!;
        const c = arm === "volScaledPerEntry" ? volCores[wname].find((x) => x.id === spec.id)! : base;
        out.push({ ...c, slotUsd: spec.rowDeployable * w[spec.symbol!] });
      }
    }
    return out;
  }
  ...
  for (const wname of ["A", "B"] as const) {
    ...
    for (const arm of ARMS) {
      ...
      arms[arm] = {
        ...combine(sl),
        needsMaxOrderRaisedTo: Number(Math.max(MAX_ORDER_USD, ...sl.map((s) => s.slotUsd)).toFixed(2)),
  ...
  // ── question 2: how the book is split between the rows ──────────────────
  ...
  const revxScale = BOOK_USD / LIVE_ROWS.filter((r) => r.venue === "revx").reduce((a, r) => a + shippedCapital[r.id], 0);
  const rowPlans: RowPlan[] = [
    ...
    {
      name: "revxOnly_sameBook",
      capital: Object.fromEntries(LIVE_ROWS.map((r) => [r.id, r.venue === "revx" ? Number((shippedCapital[r.id] * revxScale).toFixed(2)) : 0])),
      note: "the Kraken rows' $180 redistributed to the Revolut X rows in their existing proportions; the same $400 at work",
    },
    { name: "revxOnly_smallerBook", capital: { "trend-4h": 100, "trend-1h": 40, "momentum-1d": 40, "rotation-1d": 40, "trend-4h-kraken": 0, "momentum-1d-kraken": 0, "rotation-1w-kraken": 0 }, note: "the Kraken rows dropped and their money NOT redeployed — $220 at work, the rest in cash" },
    { name: "dropRotation", capital: { "trend-4h": 100, "trend-1h": 40, "momentum-1d": 40, "rotation-1d": 0, "trend-4h-kraken": 100, "momentum-1d-kraken": 40, "rotation-1w-kraken": 0 }, note: "both rotation rows dropped, nothing redeployed" },
    { name: "revxNoRotation", capital: { "trend-4h": 100, "trend-1h": 40, "momentum-1d": 40, "rotation-1d": 0, "trend-4h-kraken": 0, "momentum-1d-kraken": 0, "rotation-1w-kraken": 0 }, note: "Revolut X only, no rotation — $180 at work" },
    { name: "trend4hOnly", capital: { "trend-4h": 100, "trend-1h": 0, "momentum-1d": 0, "rotation-1d": 0, "trend-4h-kraken": 0, "momentum-1d-kraken": 0, "rotation-1w-kraken": 0 }, note: "go-live.md's recommendation: one row, five $20 slots" },
    { name: "trend4hPlusMomentum", capital: { "trend-4h": 100, "trend-1h": 0, "momentum-1d": 40, "rotation-1d": 0, "trend-4h-kraken": 0, "momentum-1d-kraken": 0, "rotation-1w-kraken": 0 }, note: "the two slowest Revolut X rules" },
    { name: "trend4hBothVenues", capital: { "trend-4h": 100, "trend-1h": 0, "momentum-1d": 0, "rotation-1d": 0, "trend-4h-kraken": 100, "momentum-1d-kraken": 0, "rotation-1w-kraken": 0 }, note: "the same rule on both venues, to price the twin" },
    { name: "trend4hRevx200", capital: { "trend-4h": 200, "trend-1h": 0, "momentum-1d": 0, "rotation-1d": 0, "trend-4h-kraken": 0, "momentum-1d-kraken": 0, "rotation-1w-kraken": 0 }, note: "the one row at double size — $40 a slot, which needs max_order_usd raised" },
  ];
  ...
  function planSleeves(plan: RowPlan, arm: ArmName, wname: "A" | "B"): Sleeve[] {
    ...
    for (const row of LIVE_ROWS) {
      ...
      if (cap <= 0) continue;
      const rowSpecs = specs.filter((s) => s.rowId === row.id);
      if (row.kind === "rotation-1d") { out.push({ ...cores[wname].find((x) => x.id === rowSpecs[0].id)!, slotUsd: cap }); continue; }
      const w = weightsFor(arm, wname, row);
      for (const spec of rowSpecs) {
        const c = arm === "volScaledPerEntry" ? volCores[wname].find((x) => x.id === spec.id)! : cores[wname].find((x) => x.id === spec.id)!;
        out.push({ ...c, slotUsd: cap * w[spec.symbol!] });
      }
    }
    return out;
  }
  ...
  const equalSlotOf = (spec: Spec): number => spec.rowDeployable / (spec.symbol == null ? 1 : spec.symbolsInRow);
  const shippedSleeves = (wname: "A" | "B"): Sleeve[] => cores[wname].map((c) => ({ ...c, slotUsd: equalSlotOf(specs.find((s) => s.id === c.id)!) }));
  ...
  for (const wname of ["A", "B"] as const) {
    ...
    q2[wname] = {
      ...
      plans: Object.fromEntries(rowPlans.map((p) => [p.name, { capital: p.capital, note: p.note, equalSlots: combine(planSleeves(p, "equal", wname)) }])),
    };
  }
  ...
  for (const plan of rowPlans) for (const arm of ["equal", "evidencePrior", "concentrated", "invVolSlots"] as ArmName[]) {
