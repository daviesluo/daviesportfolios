// THE JEV GATE ON THE TWO PAPER ROWS — trend-1h and momentum-1d, priced the way
// `backtest_jev.ts --replay measured` priced the live candidate's (`jev_v2.json`).
//
// Migration 0047 put the v2 question and `enterMin` 0.45 on all three rows, and
// `tick.ts` gates every one of them the same way: `combineDecision` with the
// row's own `enterMin`, on entries only. The real model's replies to every entry
// state of the two paper rules were measured the same night
// (`jev_answers_v2_other.json`: trend-1h's 54 states and momentum-1d's 189, five
// calls each), but only the live candidate's gate was ever priced. This prices
// the other two, exactly as that one was: at every entry signal where `tick.ts`
// would ask the model (flat, the rulebook says enter, not cooling down) one of
// the five replies the model gave to that exact state is drawn uniformly and put
...
//     supabase/functions/agents/backtest_jev_other.ts \
...
//
// Writes `<out>/jev_v2_other.json` and NOTHING else, in about fifteen minutes
// on one core. There is no wall clock in the output: two runs over the same
...
// parameters, `lookbackDays` 30, the 8 % floor, `enterMin` 0.45.
//
...
/** Read one rule's states from `jev_answers_v2_other.json`, and refuse anything but its whole entry space, echo-verified. */
function loadOtherAnswers(text: string, kind: OtherKind, symbols: readonly string[]): OtherBook {
  const j = JSON.parse(text) as { provenance?: { question?: { version?: string }; fixedOnEveryState?: Record<string, string> } } & Record<string, unknown>;
  if (j.provenance?.question?.version !== "v2") throw new Error(`answers: the wording is ${j.provenance?.question?.version}; this study prices v2`);
...
async function otherRowsStudy(args: Record<string, string>): Promise<void> {
  ...
  const OUT_NAME = "jev_v2_other.json";
  const DRAWS = Number(args.draws ?? 2000), NULL_DRAWS = Number(args["null-draws"] ?? 1000);
  /** The threshold the rows run: `agent_strategies.params.enterMin` on both since migration 0047. Not a flag: this prices what runs. */
  const ENTER_MIN = JEV_ENTER_MIN;
  ...
  const hashNow = async () => ({
    backtestTs: await sha(btUrl), backtestJevTs: await sha(jevUrl), backtestJevOtherTs: await sha(selfUrl),
    agentsStrategyTs: await sha(rbUrl), answersJson: await sha(answersPath), set2Json: await sha(set2Path), testingSetJson: await sha(testingSetPath),
  });
  ...
  for (const k of kinds) books[k] = loadOtherAnswers(answersText, k, PAPER_SYMBOLS);
  const bookOf = (kind: OtherKind) => books[kind]!;
  // The share of each state's replies that refuse at the priced threshold, and the states whose replies straddle it.
  const qAt: Partial<Record<OtherKind, Map<string, number>>> = {}, straddling: Partial<Record<OtherKind, Set<string>>> = {};
  for (const k of kinds) {
    qAt[k] = new Map([...bookOf(k).bySymbol.entries()].map(([key, rs]) => [key, vetoShare(rs, ENTER_MIN)]));
  ...
  for (const row of rows) for (const cond of CONDITIONS) for (const w of scoredWindows(row, cond)) for (let k = 0; k < SHADOW_DRAWS; k++) {
    ...
    const a = runArm(row, cond, w, (sym) => {
      const gated = otherJevPolicy(row.kind, bookOf(row.kind), mulberry32(seedOf("shadow-would", row.id, cond.id, w, sym, k)), ENTER_MIN, true);
      const shadow = otherJevPolicy(row.kind, bookOf(row.kind), mulberry32(seedOf("shadow", row.id, cond.id, w, sym, k)), ENTER_MIN, false);
  ...
  for (const row of rows) {
    ...
    for (const cond of CONDITIONS) {
      ...
      for (const w of scoredWindows(row, cond)) {
        const ii = mcRun(row, cond, w, DRAWS, (sym, k) => otherJevPolicy(row.kind, bookOf(row.kind), mulberry32(seedOf("ii", row.id, cond.id, w, sym, k)), ENTER_MIN, true));
  ...
  // ── the answers, read on their own: what the priced threshold does to each state, and every other threshold ──
  ...
  const report = {
    study: "the fixed Jev gate (the v2 wording at enterMin 0.45) on the two PAPER rows, trend-1h and momentum-1d — the rulebook against the rulebook ∧ the gate on four windows × the four evaluations §3.19 requires, each gate against a random veto that takes the same number of entries",
    script: "supabase/functions/agents/backtest_jev_other.ts — imports runGated, loadMeasuredSeries, combine, episodeNullPolicy, seedOf, mulberry32 and the statistics from backtest_jev.ts, which it does not modify",
    rows: rows.map((r) => ({ ...r, slotUsd: Number(Math.min(r.capitalUsd / r.slots, MAX_ORDER_USD).toFixed(2)) })),
    thresholds: { enterMin: ENTER_MIN, cautionExit: CAUTION_EXIT, source: "JEV_ENTER_MIN in agents_strategy.ts — agent_strategies.params.enterMin on trend-1h and momentum-1d since migration 0047; cautionExit is tick.ts's literal" },
    answers: { file: answersPath, sha256: hashesStart.answersJson, provenance: answersProvenance, atThreshold: answersAtThreshold, thresholdScan: { note: "per threshold, from the answers alone: states whose five replies are ALL under it (refused every call), SOME (call by call) and NONE (never refused); combineDecision refuses when P < enterMin. Nothing here chooses a threshold.", ...thresholdScan } },
    replay: {
    ...
    caveats: [
      "The answers are today's model (typesafe/jev-1.13-20260917 through OpenRouter, measured 2026-09-23 00:24 UTC with the v2 wording for each rule). Asking it about a state computed from 2022 candles is legitimate — the state is ten categorical words and carries no date — but it is what the model says NOW.",
      "Five replies per state. At 0.45 trend-1h's 54 states are all decided the same way on every call; momentum-1d has 7 states of 189 whose replies straddle 0.45, drawn call by call.",
      "No missing answer and no echo failure was measured in 1,215 calls, and none is modelled; each would be a veto in production.",
      ...
      "The null is matched on ENTRIES, not on time in the market. On momentum-1d under the shipped stop a refused null episode is a whole stretch of positive momentum, while the gate mostly delays an entry until the 4-hour picture turns, so the null holds the coins less of the time than the gate (compare null.nullDeploymentMean with null.gateDeploymentMean): its per-window comparison there mixes selection with exposure, favouring the gate in rising windows and the null in falling ones. On trend-1h and under the trail the two are matched much more closely.",
    ...
    sourceIntegrity: { ...hashesStart, note: "SHA-256 of backtest.ts, backtest_jev.ts (imported, unmodified), this script, agents_strategy.ts, the answers file and the two published files the rule arms are checked against, at the start of the run; the run throws if any changed before it finished" },
  };
