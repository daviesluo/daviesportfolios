"""Assemble ROUND's committed input. The rule is round_test.py.

Prints counts only: never a price against the line through its neighbors.
Keyless. Resumes from $PM_DATA/round.

usage: round_inputs.py <out.json.gz>
"""
import gzip
import json
import os
...
def cache_dir():
    d = os.path.join(pmnet.DATA, "round")
...
def _share_pace():
    """Several events at once. The gaps are this pull's, set before any line is drawn.
...
def one(ev):
    ...
    for m in (raw or {}).get("markets") or []:
        ...
        rec = {
            "condition": m.get("conditionId"), "strike": strike, "shown": shown, "tick": tick, "rate": rate,
            "payout_yes": float(prices[0]), "payout_no": float(prices[1]), "closed": closed,
        }
        if shown is not None and int(strike) % ROUND == 0:
            rec["prints"] = prints_of(m.get("conditionId"), td)
        markets.append(rec)
...
def main():
    ...
    print("wrote", outp, "events", len(rows), "round_strikes", rounds, "incomplete", incomplete, round(time.time() - t0, 1), flush=True)
