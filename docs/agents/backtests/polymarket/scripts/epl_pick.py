"""Fetch the pre-match prints a frozen Premier League rule names.

The print window is the hour before kickoff. Keyless. Resumes from $PM_DATA/epl/<rule>.

usage: epl_pick.py <rule module> <epl_inputs.json.gz> <out.json.gz>
"""
import gzip
import importlib
import json
...
def cache_dir(rule):
    d = os.path.join(pmnet.DATA, "epl", rule)
    os.makedirs(d, exist_ok=True)
    return d


def prints_or_cache(rule, ev, cond, t0, t1):
...
def main():
    ...
    for ev in data["events"]:
        ...
        td = end - score.LEAD
        prints = prints_or_cache(name, ev, market["condition"], td, end)
