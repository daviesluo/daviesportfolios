...
> the five). The draft (`0051_go_live.sql.draft`) now adds
...
2. Move `docs/agents/0051_go_live.sql.draft` to
   `supabase/migrations/0051_go_live.sql` and push — check first that
   0051 is still the next free number (`ls supabase/migrations/`; up to
   `0050` are taken and applied, and a file under a used number is
