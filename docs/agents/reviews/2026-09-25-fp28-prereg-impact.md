# Pre-registration: IMPACT, volume lined up with the move (fp28)

Written 2026-09-25 after the 2023 screen and before any 2024–2026 return of this
rule was computed. Frozen at 2026-09-25T02:52:42Z by the sha256 in
`2026-09-25-fp28-prereg-impact.sha256`.

## Why this one

The screen in `2026-09-25-fp28-protocol.md` scored one idea on 2023 only.
IMPACT passed: 37 trades, mean net +86.5868 bps after 20 bps of fees, total
+$32.0371 on $100 a trade, against a null p95 of +74.5602 bps (200 draws,
seed 20250925). This file is the test that decides whether that pass was
the year.

fp5 through fp27 stay dead, including LS-FADE, BODY and BAL-CHG. Nothing in
them is retuned here. A day whose volume and moves do not line up is not a
candidate. The level of the address count is not scored, and BAL-CHG's 80th
is not a candidate. BODY's 80th is not a candidate.

## What was seen

The 2023 screen, the BTC book at this freeze (`bookTicker` BTCUSDT, bid
84273.51, ask 84273.52, half-spread 5.933062130678438e-08), and the code
that reproduces the screen. No hourly open, close or quote volume from 2024
onward has been read for this rule. No daily open from 2024 onward has been
read except the 2024-01-01 open stored in the screen file, which is the exit
print of the last 2023 entry.
...
* Inside one UTC day, the population correlation of the 24 hourly quote
  volumes and the 24 absolute close-to-open returns. All 24 hours must
  exist. The open and the close must be positive. Quote volume must not be
  negative. If either series does not vary, the day is missing. The sign of
  an hour is not used.
* It fires when the correlation is strictly above its own trailing-90-day
  90th percentile, at least 90 earlier prints. The percentile is
...
* Reproduction, not the bar: entries in 2023, at the screen's fee with no
  spread. Must come back as 37 trades and +$32.0371 within one cent. If it
