# Pre-registration: OPEN-DRIVE, the first hour (fp44)
...
`2026-09-25-fp44-prereg-drive.sha256`.

## Why this one

The screen in `2026-09-25-fp44-protocol.md` scored one idea on 2023 only.
OPEN-DRIVE passed: 36 trades, mean net +80.7544 bps after 20 bps of fees,
total +$29.0716 on $100 a trade, against a null p95 of +72.6224 bps (200
draws, seed 20250925). This file is the test that decides whether that pass
was the year.

fp5 through fp43 stay dead, including LS-FADE, BODY, BAL-CHG, IMPACT, PEAK,
STREAK's screen is a different rule and is not an input here, and the eight
screens fp30 through fp37. Nothing in them is retuned here. A weak first
hour is not a candidate. The 80th of a failed screen is not a candidate.
IMPACT's 80th and 95th are not candidates. PEAK's close under the prior
30-day high, and its 80th, are not candidates. BODY's 80th is not a
candidate. The level of the address count is not scored, and BAL-CHG's 80th
is not a candidate.
...
the code that reproduces the screen. No hourly open or close from 2024
onward has been read. No daily open from 2024 onward has been read except
the 2024-01-01 open stored in the screen file, which is the exit print of
the last 2023 entry.
...
reproduction check is what shows it did not move. Later hours are not an
input. A missing later hour does not drop the day. The known hole at
2023-03-24 13:00 UTC does not drop 2023-03-24. Yesterday's close does not
enter.

* The first hour only: that hour's close divided by that hour's open, minus
  1. The hour must exist. The open and the close must be positive.
* It fires when that return is strictly above its own trailing-90-day 90th
  percentile, at least 90 earlier prints. The percentile is
  `sorted[floor(0.90 × (n − 1))]`. There is no extra gate that the return
  be positive.
...
* Reproduction, not the bar: entries in 2023, at the screen's fee with no
  spread. Must come back as 36 trades and +$29.0716 within one cent. If it
...
* OOS2: entries on [2025-01-01, 2026-09-25). The last entry is 2026-09-24
  ...
  The 2026-09-25 daily bar stores that open only.
