# Pre-registration: AVG-CHG, the change in average trade size (fp48)

Written 2026-09-25 after the 2023 screen and before any 2024–2026 return of this
rule was computed. Frozen at 2026-09-25T03:47:48Z by the sha256 in
`2026-09-25-fp48-prereg-size.sha256`.

## Why this one

The screen in `2026-09-25-fp48-protocol.md` scored one idea on 2023 only.
AVG-CHG passed: 48 trades, mean net +77.6367 bps after 20 bps of fees, total
+$37.2656 on $100 a trade, against a null p95 of +60.9592 bps (200 draws,
seed 20250925). This file is the test that decides whether that pass was
the year.

fp5 through fp47 and fp49 through fp53 stay dead, including LS-FADE, BODY,
BAL-CHG, IMPACT, PEAK, STREAK, OPEN-DRIVE, and the screens fp30 through fp45.
Nothing in them is retuned here. A day whose average size did not rise is
not a candidate. The 80th of a failed screen is not a candidate. IMPACT's
80th and 95th are not candidates. STREAK's and OPEN-DRIVE's 80th and 95th
are not candidates. PEAK's close under the prior 30-day high, and its 80th,
are not candidates. BODY's 80th is not a candidate. The level of the address
count is not scored, and BAL-CHG's 80th is not a candidate.

## What was seen

The 2023 screen, the BTC book at this freeze (`bookTicker` BTCUSDT, bid
84180.31000000, ask 84180.32000000, half-spread 5.939630903800506e-08), and
the code that reproduces the screen. No quote volume and no trade count from
2024 onward has been read. No daily open from 2024 onward has been read
except the 2024-01-01 open stored in the screen file, which is the exit
print of the last 2023 entry. That bar's quote and trade count are stored
as 0.
...
reproduction check is what shows it did not move. A day that did not grow
stays in the history. The neighbours keep that gate: the value must be
strictly above the threshold and strictly above 0.

* Average size is quote volume divided by the trade count. The statistic is
  today's size divided by yesterday's size, minus 1. Both sizes must be
  positive. Yesterday is exactly one day earlier.
* It fires when that change is strictly above its own trailing-90-day 90th
...
* Reproduction, not the bar: entries in 2023, at the screen's fee with no
  spread. Must come back as 48 trades and +$37.2656 within one cent. If it
...
* OOS2: entries on [2025-01-01, 2026-09-25). The last entry is 2026-09-24
  ...
  The 2026-09-25 quote and trade count are stored as 0.
...
They were not used to choose the 90th. Each one still requires the change to be
