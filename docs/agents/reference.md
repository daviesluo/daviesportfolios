...
### 3.43 Cross-sectional return skew: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp14-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp14/screen_2023.json`). Population
skewness of the basket's 8h returns, above its own trailing 90th, long spot
...
### 3.47 BTC–ETH return correlation: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp18-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp18/screen_2023.json`). The 30-day
population correlation of BTC and ETH daily returns, above its own trailing
90th, long spot for the next day: 26 trades, mean +57.9 bps, null p95
+81.2 bps. The gross move was +78.0 bps. Both daily grids have no hole.
**Nothing here is fit to add.** The count stays 30. The mean was positive
and lost to the null. The low-correlation tail and the 80th are not
candidates.

### 3.48 Close location in the day's range: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp19-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp19/screen_2023.json`). Where the close
sits between that day's low and high, above its own trailing 90th, long spot
for the next day: 41 trades, mean +12.1 bps, null p95 +70.2 bps. The gross
move was +32.1 bps. The daily grid has no hole. **Nothing here is fit to
...
### 3.51 Hourly volume concentration: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp22-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp22/screen_2023.json`). The Herfindahl
of the day's 24 hourly quote shares, above its own trailing 90th, long spot
the next day: 29 trades, mean +68.6 bps, null p95 +79.2 bps. The gross move
was +88.7 bps. One hour is missing, 2023-03-24 13:00 UTC, and that day is
not a print. **Nothing here is fit to add.** The count stays 30. The mean
...
### 3.75 The last close versus the day's average: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp46-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp46/screen_2023.json`). The last
hourly close divided by the equal-weighted mean of that day's 24 closes,
minus 1, above its own trailing 90th, long spot the next day: 35 trades,
mean +37.5 bps, null p95 +78.4 bps, +$13.12. The gross move was +57.6 bps.
The hourly grid has the same one hole, and that day is not a print. 456
days have a distance. **Nothing here is fit to add.** The count cleared and
the mean was positive; it lost to the null. A close under the day's average,
and the 80th, are not candidates.

### 3.76 Hours that break the running high: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp47-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp47/screen_2023.json`). The count of
hours whose high is strictly above the running maximum of earlier highs that
day, above its own trailing 90th, long spot the next day: 27 trades, mean
+31.4 bps, null p95 +81.3 bps, +$8.49. The gross move was +51.5 bps. The
...
stays there. The mean was positive and under the null. Fewer new highs, and
the 80th, are not candidates.

### 3.77 The rise in average trade size: the screen passes, the later years do not (2026-09-25)

The protocol (`reviews/2026-09-25-fp48-protocol.md`) was committed before the
2023 screen. Two runs matched (`backtests/fp48/screen_2023.json`). Today's
quote per trade divided by yesterday's, minus 1, above its own trailing 90th
and strictly above zero, long spot the next day: 48 trades, mean +77.6 bps,
null p95 +61.0 bps, +$37.27. The file's mean is +77.6367 bps, the null is
+60.9592 bps, and the total is +$37.2656. The gross move was +97.8 bps. The
null matches §3.58 because both rules have 48 trades on the same BTC pool
and the same seed. The daily grid has no hole. The first day has no prior
size, so there are 456 changes. The pre-registration
(`reviews/2026-09-25-fp48-prereg-size.md`, sha256 `e5463632…`, frozen
2026-09-25T03:47:48Z, half-spread 5.939630903800506e-08) was committed before
any later-year quote or trade count. Two out-of-sample runs matched
(`backtests/fp48/size_oos.json`). 2023 reproduces: 48 trades, +$37.2656.
Entries from 2024-01-01 through 2026-09-24: 116 trades, −$14.8577, mean
−$0.128083, null p95 +$0.2669. 2024 is +$3.839 and 2025-01-01 through
2026-09-24 is −$18.6967. Doubled costs are −$38.0048. November 2024 is
+$20.8241; without it the total is −$35.6818. Annualised on the locked $100
it is −5.43%. The 80th neighbor loses (−$25.3009, 202 trades). The 95th is
positive (+$5.685, 64 trades). The later daily grid has 999 bars and no
hole. **Nothing here is fit to add.** The total is negative, the second
window is negative, the mean loses to the null, and doubled costs lose. A
positive neighbour does not save it. A day the average size did not rise,
and the 80th, are not candidates.

### 3.78 Hours that closed above the prior hour: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp49-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp49/screen_2023.json`). The share of
hours 1 through 23 whose close is strictly above the previous hour's close,
above its own trailing 90th, long spot the next day: 25 trades, mean +127.8
bps, null p95 +87.1 bps, +$31.94. The file's mean is +127.755 bps. The gross
move was +148.0 bps. The hourly grid has the same one hole, and that day is
not a print. 456 days have a share. **Nothing here is fit to add.** The mean
was positive and above the null. The count is under 30 and stays there, so
the later years were not pulled. Fewer rising hourly closes, and the 80th,
are not candidates.

### 3.79 The hour of the high: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp50-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp50/screen_2023.json`). The hour index
of the day's highest high, earliest tie, above its own trailing 90th, long
spot the next day: 19 trades, mean +50.3 bps, null p95 +91.0 bps, +$9.56.
The file's null is +90.987 bps. The gross move was +70.4 bps. The hourly
grid has the same one hole, and that day is not a print. 456 days have an
index. **Nothing here is fit to add.** The count is under 30 and stays
there. The mean was positive and under the null. An earlier high, and the
80th, are not candidates.

### 3.80 Hours with a low above the open: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp51-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp51/screen_2023.json`). The count of
the 24 hours whose low is strictly above that day's open, above its own
trailing 90th, long spot the next day: 22 trades, mean +50.5 bps, null p95
+93.0 bps, +$11.12. The file's null is +92.975 bps. The gross move was +70.7
bps. The null matches §3.82 because both rules have 22 trades on the same
BTC pool and the same seed. The hourly grid has the same one hole, and that
day is not a print. 456 days have a count. **Nothing here is fit to add.**
The count is under 30 and stays there. The mean was positive and under the
null. Fewer hours above the open, and the 80th, are not candidates.

### 3.81 The rise in quote volume: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp52-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp52/screen_2023.json`). Today's quote
volume divided by yesterday's, minus 1, above its own trailing 90th and
strictly above zero, long spot the next day: 45 trades, mean +61.8 bps, null
p95 +64.3 bps, +$27.81. The gross move was +81.9 bps. The daily grid has no
hole. The first day has no prior quote, so there are 456 changes. **Nothing
here is fit to add.** The count cleared and the mean was positive; it lost
to the null. A day quote volume did not rise, and the 80th, are not
candidates.

### 3.82 Hours between the high and the low: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp53-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp53/screen_2023.json`). The absolute
difference in hour index between the day's high and the day's low, earliest
tie, above its own trailing 90th, long spot the next day: 22 trades, mean
−33.1 bps, null p95 +93.0 bps, −$7.29. The file's null is +92.975 bps. The
gross move was −13.2 bps. The null matches §3.80 because both rules have 22
trades on the same BTC pool and the same seed. The hourly grid has the same
one hole, and that day is not a print. 456 days have a gap. **Nothing here
is fit to add.** The count is under 30 and stays there. The mean was
negative and under the null. A smaller clock gap, and the 80th, are not
candidates.

### 3.83 Where the day's quote sits: the screen passes, the later years do not (2026-09-25)

The protocol (`reviews/2026-09-25-fp54-protocol.md`) was committed before the
2023 screen. The one-element quote-field tuple was fixed before any bar was
pulled. Two runs matched (`backtests/fp54/screen_2023.json`). The
quote-weighted mean of the hour index, 0 through 23, above its own trailing
90th, long spot the next day: 38 trades, mean +66.7 bps, null p95 +65.9 bps,
+$25.33. The file's mean is +66.6518 bps, the null is +65.8998 bps, and the
total is +$25.3277. The gross move was +86.8 bps. The hourly grid has the
same one hole, and that day is not a print. 456 days have a clock. The
pre-registration (`reviews/2026-09-25-fp54-prereg-clock.md`, sha256
`d0083afe…`, frozen 2026-09-25T04:06:10Z, half-spread 5.9397938896041255e-08)
was committed before any later-year hourly quote. Two out-of-sample runs
matched (`backtests/fp54/clock_oos.json`). 2023 reproduces: 38 trades,
+$25.3277. Entries from 2024-01-01 through 2026-09-24: 109 trades,
−$42.8954, mean −$0.393536, null p95 +$0.2948. 2024 is −$14.7672 and
2025-01-01 through 2026-09-24 is −$28.1282. Doubled costs are −$64.588.
August 2026 is +$6.8278; without it the total is −$49.7232. Annualised on
the locked $100 it is −15.69%. The 80th neighbor loses (−$73.9054, 203
trades). The 95th loses (−$39.7305, 71 trades). The later hourly grid has
23,952 bars and no hole. The later daily grid has 999 bars and no hole.
**Nothing here is fit to add.** Both later windows are negative, the mean
loses to the null, and doubled costs lose. A day whose quote printed
earlier, and either neighbour, are not candidates.

### 3.84 The close over the day's low: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp55-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp55/screen_2023.json`). The close
divided by the same day's low, minus 1, above its own trailing 90th, long
spot the next day: 40 trades, mean +0.0707 bps, null p95 +67.6 bps, +$0.03.
The file's total is +$0.0283. The gross move was +20.1 bps. The daily grid
has no hole. The window has 457 prints. **Nothing here is fit to add.** The
count cleared. The mean was positive and far under the null. A close sitting
on the low, and the 80th, are not candidates.

### 3.85 Hours that break the running close: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp56-protocol.md`) was committed before any
return. The one-element close-field tuple was fixed before any bar was
pulled. Two runs matched (`backtests/fp56/screen_2023.json`). The count of
hours after the first whose close is strictly above the running maximum of
earlier closes, above its own trailing 90th, long spot the next day: 28
trades, mean +54.9 bps, null p95 +84.2 bps, +$15.36. The file's mean is
+54.862 bps. The gross move was +75.0 bps. The hourly grid has the same one
hole, and that day is not a print. 456 days have a count. **Nothing here is
fit to add.** The count is under 30 and stays there. The mean was positive
and under the null. Fewer new closing highs, and the 80th, are not
candidates.

### 3.86 The rise in base volume: the screen passes, the later years do not (2026-09-25)

The protocol (`reviews/2026-09-25-fp57-protocol.md`) was committed before the
2023 screen. Two runs matched (`backtests/fp57/screen_2023.json`). Today's
base volume divided by yesterday's, minus 1, above its own trailing 90th and
strictly above zero, long spot the next day: 44 trades, mean +65.2 bps, null
p95 +64.0 bps, +$28.69. The file's mean is +65.1976 bps, the null is
+63.9818 bps, and the total is +$28.6869. The gross move was +85.3 bps. The
daily grid has no hole. The first day has no prior base volume, so there are
456 changes. The pre-registration (`reviews/2026-09-25-fp57-prereg-base.md`,
sha256 `1b455c48…`, frozen 2026-09-25T04:06:10Z, half-spread
5.9397938896041255e-08) was committed before any later-year base volume. Two
out-of-sample runs matched (`backtests/fp57/base_oos.json`). 2023
reproduces: 44 trades, +$28.6869. Entries from 2024-01-01 through
2026-09-24: 102 trades, −$6.934, mean −$0.067981, null p95 +$0.2877. 2024 is
+$7.8662 and 2025-01-01 through 2026-09-24 is −$14.8002. Doubled costs are
−$27.2998. February 2026 is +$10.5814; without it the total is −$17.5154.
Annualised on the locked $100 it is −2.54%. The 80th neighbor loses
(−$50.133, 210 trades). The 95th loses (−$6.0329, 65 trades). The later
daily grid has 999 bars and no hole. **Nothing here is fit to add.** The
second window is negative, the mean loses to the null, and doubled costs
lose. A day base volume did not rise, and either neighbour, are not
candidates.

### 3.87 The later of the high and the low: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp58-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp58/screen_2023.json`). The later of
the hour index of the highest high and the hour index of the lowest low,
earliest tie, above its own trailing 90th, long spot the next day: 19
trades, mean −33.3 bps, null p95 +91.0 bps, −$6.32. The file's null is
+90.987 bps. The gross move was −13.3 bps. The null matches §3.79 because
both rules have 19 trades on the same BTC pool and the same seed. The hourly
grid has the same one hole, and that day is not a print. 456 days have an
index. **Nothing here is fit to add.** The count is under 30 and stays
there. The mean was negative and under the null. An earlier later-extreme,
and the 80th, are not candidates.

### 3.88 Base volume in rising hours: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp59-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp59/screen_2023.json`). The share of
base volume in hours 1 through 23 whose close is strictly above the previous
hour's close, above its own trailing 90th, long spot the next day: 30
trades, mean +62.5 bps, null p95 +75.2 bps, +$18.74. The file's null is
+75.189 bps. The gross move was +82.6 bps. The null matches §3.60 because
both rules have 30 trades on the same BTC pool and the same seed. The hourly
grid has the same one hole, and that day is not a print. 456 days have a
share. **Nothing here is fit to add.** The count cleared and the mean was
positive; it lost to the null. A day whose rising hours held less of the
base, and the 80th, are not candidates.

### 3.89 The open over yesterday's low: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp60-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp60/screen_2023.json`). Today's open
divided by yesterday's low, minus 1, above its own trailing 90th, long spot
the next day: 41 trades, mean +5.8 bps, null p95 +70.2 bps, +$2.39. The
file's null is +70.1636 bps. The gross move was +25.9 bps. The null matches
§3.63 because both rules have 41 trades on the same BTC pool and the same
seed. The daily grid has no hole. The first day has no prior low, so there
are 456 distances. **Nothing here is fit to add.** The count cleared and the
mean was positive; it lost to the null. An open that is not far above
yesterday's low, and the 80th, are not candidates.

### 3.90 The last hour's return: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp61-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp61/screen_2023.json`). The 23:00
hour's close divided by that hour's open, minus 1, above its own trailing
90th, long spot the next day: 43 trades, mean +58.9 bps, null p95 +66.2 bps,
+$25.33. The file's null is +66.2261 bps. The gross move was +79.1 bps. The
null matches §3.62 because both rules have 43 trades on the same BTC pool
and the same seed. The hourly grid has the same one hole. That hole is not
the 23:00 hour, so the day stays a print. 457 days have a last-hour return.
**Nothing here is fit to add.** The count cleared and the mean was positive;
...
### 3.100 The open inside today's range: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp71-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp71/screen_2023.json`). Today's open
minus today's low, divided by today's high minus today's low, above its own
trailing 90th, long spot the next day: 38 trades, mean −19.6 bps, null p95
+65.9 bps, −$7.44. The file's gross is +0.4021 bps, the mean is −19.5787
bps, the null is +65.8998 bps, and the total is −$7.4399. The null matches
§3.83 because both rules have 38 trades on the same BTC pool and the same
seed. The daily grid has no hole. The masked horizon is not a print, so the
window has 457 locations. **Nothing here is fit to add.** The count cleared.
The mean was negative and under the null. An open that is not high in
today's range, and the 80th, are not candidates.

### 3.101 The return of the low's hour: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp72-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp72/screen_2023.json`). The return of
the earliest hour of the low, above its own trailing 90th, long spot the
next day: 36 trades, mean +51.9 bps, null p95 +72.6 bps, +$18.70. The file's
mean is +51.9369 bps, the null is +72.6224 bps, and the total is +$18.6973.
The gross move was +72.1 bps. The null matches §3.73 because both rules have
36 trades on the same BTC pool and the same seed. The hourly grid has the
same one hole, and that day is not a print. 456 days have a return. A low
in hour 23 stays a print. **Nothing here is fit to add.** The count cleared
and the mean was positive; it lost to the null. A weak return in the low's
hour, and the 80th, are not candidates.

### 3.102 The quote share before the high: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp73-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp73/screen_2023.json`). Quote volume
in the hours strictly before the earliest hour of the high, divided by the
day's quote, above its own trailing 90th, long spot the next day: 27 trades,
mean +75.3 bps, null p95 +81.3 bps, +$20.34. The file's mean is +75.3179
bps, the null is +81.2895 bps, and the total is +$20.3358. The gross move
was +95.5 bps. The null matches §3.98 because both rules have 27 trades on
the same BTC pool and the same seed. The hourly grid has the same one hole,
and that day is not a print. 456 days have a share. 28 days cleared the
90th. The 2022-12-30 print would enter on 2022-12-31, outside the screen.
**Nothing here is fit to add.** The count is under 30 and stays there. The
mean was positive and under the null. A smaller quote share before the high,
and the 80th, are not candidates.

### 3.103 The return from hour 0 to hour 6: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp74-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp74/screen_2023.json`). Hour 6's close
divided by hour 0's open, minus 1, above its own trailing 90th, long spot
the next day: 36 trades, mean +58.0 bps, null p95 +72.6 bps, +$20.87. The
file's mean is +57.984 bps, the null is +72.6224 bps, and the total is
+$20.8742. The gross move was +78.1 bps. The null matches §3.73 because
both rules have 36 trades on the same BTC pool and the same seed. The hourly
grid has the same one hole. That hole is after hour 6, so the day stays a
print. 457 days have a morning return. **Nothing here is fit to add.** The
count cleared and the mean was positive; it lost to the null. A weak run
from hour 0 to hour 6, and the 80th, are not candidates.

### 3.104 Crosses of today's open: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp75-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp75/screen_2023.json`). The count of
today's hourly closes that cross today's open, above its own trailing 90th,
long spot the next day: 24 trades, mean +25.4 bps, null p95 +86.8 bps,
+$6.10. The file's gross is +45.478 bps, the null is +86.8142 bps, and the
total is +$6.0977. The 24 entry days are the same days as §3.97, so the
fill record matches that screen. The level here is today's open. The null
matches §3.97 because both rules have 24 trades on the same BTC pool and
the same seed. The hourly grid has the same one hole, and that day is not a
print. 456 days have a count. **Nothing here is fit to add.** The count is
under 30 and stays there. The mean was positive and under the null. Fewer
crosses, and the 80th, are not candidates.

### 3.105 The widest hour over the day's range: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp76-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp76/screen_2023.json`). The widest of
the 24 hourly ranges divided by the day's range, above its own trailing
90th, long spot the next day: 28 trades, mean −27.5 bps, null p95 +84.2 bps,
−$7.70. The file's mean is −27.5026 bps, the null is +84.168 bps, and the
total is −$7.7007. The gross move was −7.5 bps. The null matches §3.85
because both rules have 28 trades on the same BTC pool and the same seed.
The hourly grid has the same one hole, and that day is not a print. 456
days have a ratio. **Nothing here is fit to add.** The count is under 30
and stays there. The mean was negative and under the null. A day whose
widest hour held less of the range, and the 80th, are not candidates.

### 3.106 The day's VWAP over the open: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp77-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp77/screen_2023.json`). The day's
quote volume divided by its base volume, divided by the open, minus 1,
above its own trailing 90th, long spot the next day: 44 trades, mean −14.4
bps, null p95 +64.0 bps, −$6.32. The file's mean is −14.359 bps, the null
is +63.9818 bps, and the total is −$6.3179. The gross move was +5.6 bps.
The null matches §3.86 because both rules have 44 trades on the same BTC
pool and the same seed. The daily grid has no hole. The masked horizon is
not a print, so the window has 457 distances. **Nothing here is fit to add.**
The count cleared. The mean was negative and under the null. A VWAP that is
not far above the open, and the 80th, are not candidates.

### 3.107 The lowest hourly close over the open: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp78-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp78/screen_2023.json`). The lowest of
the 24 hourly closes divided by the day's open, minus 1, above its own
trailing 90th, long spot the next day: 26 trades, mean +49.3 bps, null p95
+81.2 bps, +$12.82. The file's mean is +49.2951 bps, the null is +81.2211
bps, and the total is +$12.8167. The gross move was +69.4 bps. The null
matches §3.92 because both rules have 26 trades on the same BTC pool and
the same seed. The hourly grid has the same one hole, and that day is not a
print. 456 days have a distance. **Nothing here is fit to add.** The count
is under 30 and stays there. The mean was positive and under the null. A
day whose worst hourly close fell further, and the 80th, are not candidates.

### 3.108 The busiest hour's return: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp79-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp79/screen_2023.json`). The return of
the earliest hour with the day's maximum quote volume, above its own
trailing 90th, long spot the next day: 35 trades, mean +14.2 bps, null p95
+78.4 bps, +$4.97. The file's mean is +14.1953 bps, the null is +78.4258
bps, and the total is +$4.9684. The gross move was +34.2 bps. The null
matches §3.75 because both rules have 35 trades on the same BTC pool and
the same seed. The hourly grid has the same one hole, and that day is not a
print. 456 days have a return. **Nothing here is fit to add.** The count
cleared and the mean was positive; it lost to the null. A quieter busiest
hour, and the 80th, are not candidates.

### 3.109 Where the VWAP sits in the day's range: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp80-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp80/screen_2023.json`). Quote volume
divided by base volume, placed between the day's low and high, above its own
trailing 90th, long spot the next day: 40 trades, mean −30.4 bps, null p95
+67.6 bps, −$12.17. The file's mean is −30.4286 bps, the null is +67.5914
bps, and the total is −$12.1714. The gross move was −10.5 bps. The null
matches §3.84 because both rules have 40 trades on the same BTC pool and
the same seed. The daily grid has no hole. The masked horizon is not a
print, so 457 days have a location. **Nothing here is fit to add.** The
count cleared and the mean was negative. A VWAP lower in the range, and the
80th, are not candidates.

### 3.110 The largest gap inside the day: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp81-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp81/screen_2023.json`). The largest
gap from an hourly close to the next hour's open, above its own trailing
90th, long spot the next day: 26 trades, mean +74.8 bps, null p95 +81.2 bps,
+$19.45. The file's mean is +74.8086 bps, the null is +81.2211 bps, and the
total is +$19.4502. The gross move was +95.0 bps. The null matches §3.92
because both rules have 26 trades on the same BTC pool and the same seed.
The hourly grid has the same one hole, and that day is not a print. 456 days
have a gap. **Nothing here is fit to add.** The count is under 30 and stays
there. The mean was positive and under the null. A smaller largest gap, and
the 80th, are not candidates.

### 3.111 The quote share of hours 12 through 17: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp82-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp82/screen_2023.json`). Quote volume
in hours 12 through 17 divided by the day's quote volume, above its own
trailing 90th, long spot the next day: 35 trades, mean +44.3 bps, null p95
+78.4 bps, +$15.52. The file's mean is +44.3483 bps, the null is +78.4258
bps, and the total is +$15.5219. The gross move was +64.5 bps. The null
matches §3.75 because both rules have 35 trades on the same BTC pool and
the same seed. The hourly grid has the same one hole, and that day is not a
print. 456 days have a share. **Nothing here is fit to add.** The count
cleared and the mean was positive; it lost to the null. A smaller share in
that block, and the 80th, are not candidates.

### 3.112 The close of the high's hour: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp83-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp83/screen_2023.json`). The close of
the earliest hour of the high, divided by the day's open, minus 1, above its
own trailing 90th, long spot the next day: 37 trades, mean +31.0 bps, null
p95 +74.6 bps, +$11.49. The file's mean is +31.0467 bps, the null is
+74.5602 bps, and the total is +$11.4873. The gross move was +51.1 bps. The
null matches §3.94 because both rules have 37 trades on the same BTC pool
and the same seed. The hourly grid has the same one hole, and that day is
not a print. 456 days have a distance. **Nothing here is fit to add.** The
count cleared and the mean was positive; it lost to the null. A high hour
that closed nearer the open, and the 80th, are not candidates.

### 3.113 The quote-weighted hourly return: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp84-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp84/screen_2023.json`). The
quote-weighted mean of the 24 hourly returns, above its own trailing 90th,
long spot the next day: 35 trades, file mean +21.85 bps, null p95 +78.4 bps,
+$7.65. The file's null is +78.4258 bps and the total is +$7.6475. The gross
move was +41.9 bps. The null matches §3.75 because both rules have 35 trades
on the same BTC pool and the same seed. The hourly grid has the same one
hole, and that day is not a print. 456 days have a return. **Nothing here is
fit to add.** The count cleared and the mean was positive; it lost to the
null. A lower weighted return, and the 80th, are not candidates.

### 3.114 The median hourly close over the open: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp85-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp85/screen_2023.json`). The lower
median of the 24 hourly closes divided by the day's open, minus 1, above its
own trailing 90th, long spot the next day: 33 trades, mean +41.1 bps, null
p95 +74.8 bps, +$13.55. The file's mean is +41.0741 bps, the null is
+74.7946 bps, and the total is +$13.5544. The gross move was +61.2 bps. The
null matches §3.96 because both rules have 33 trades on the same BTC pool
and the same seed. The hourly grid has the same one hole, and that day is
not a print. 456 days have a distance. **Nothing here is fit to add.** The
count cleared and the mean was positive; it lost to the null. A lower
median, and the 80th, are not candidates.

### 3.115 The overlap of consecutive hours: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp86-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp86/screen_2023.json`). The mean
overlap of the 23 consecutive hourly ranges, above its own trailing 90th,
long spot the next day: 23 trades, mean −13.2 bps, null p95 +85.9 bps,
−$3.03. The file's mean is −13.1798 bps, the null is +85.9259 bps, and the
total is −$3.0314. The gross move was +6.8 bps. The null matches §3.52
because both rules have 23 trades on the same BTC pool and the same seed.
The hourly grid has the same one hole, and that day is not a print. 456 days
have an overlap. **Nothing here is fit to add.** The count is under 30 and
stays there. The mean was negative. A day whose hours overlapped less, and
the 80th, are not candidates.

### 3.116 The close above the day's VWAP: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp87-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp87/screen_2023.json`). How far the
close finished above the day's VWAP, in units of the day's range, above its
own trailing 90th, long spot the next day: 43 trades, mean +54.7 bps, null
p95 +66.2 bps, +$23.52. The file's gross is +74.8196 bps, the mean is
+54.6901 bps, the null is +66.2261 bps, and the total is +$23.5167. The null
matches §3.62 because both rules have 43 trades on the same BTC pool and the
same seed. The daily grid has 458 bars and no hole. The 2024-01-01 bar is
not a print, so 457 days have a stretch. **Nothing here is fit to add.** The
count cleared and the mean was positive; it lost to the null. A close that
finished nearer the VWAP, and the 80th, are not candidates.

### 3.117 The low above yesterday's high: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp88-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp88/screen_2023.json`). How far today's
low held above yesterday's high, in units of yesterday's range, above its
own trailing 90th, long spot the next day: 39 trades, file mean +14.9609
bps, null p95 +66.261 bps, +$5.8347. The file's gross is +35.0109 bps. No
earlier screen had 39 trades. §3.121 prints the same null. The daily grid
has 458 bars and no hole. The first day has no yesterday, and the 2024-01-01
bar is not a print, so 456 days have a hold. **Nothing here is fit to add.**
The count cleared and the mean was positive; it lost to the null. A low that
did not hold above yesterday's high, and the 80th, are not candidates.

### 3.118 The worst hourly return: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp89-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp89/screen_2023.json`). The lowest of
the 24 hourly returns, above its own trailing 90th, long spot the next day:
23 trades, mean −16.3 bps, null p95 +85.9 bps, −$3.75. The file's gross is
+3.6829 bps, the mean is −16.3045 bps, the null is +85.9259 bps, and the
total is −$3.75. The null matches §3.52 because both rules have 23 trades on
the same BTC pool and the same seed. The hourly grid has the same one hole,
and that day is not a print. 456 days have a worst return. **Nothing here is
fit to add.** The count is under 30 and stays there. The mean was negative.
A weaker worst hour, and the 80th, are not candidates.

### 3.119 The last hour's average trade: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp90-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp90/screen_2023.json`). The last hour's
quote per trade over the day's quote per trade, minus 1, above its own
trailing 90th, long spot the next day: 31 trades, file mean +18.9592 bps,
null p95 +71.8264 bps, +$5.8774. The file's gross is +39.0172 bps. The null
matches §3.44 because both rules have 31 trades on the same BTC pool and the
same seed. The hourly grid has the same one hole, and that day is not a
print. 456 days have a ratio. **Nothing here is fit to add.** The count
cleared and the mean was positive; it lost to the null. A smaller last-hour
average trade, and the 80th, are not candidates.

### 3.120 Quote at the high over quote at the low: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp91-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp91/screen_2023.json`). Quote volume in
the earliest hour of the high, divided by quote volume in the earliest hour
of the low, above its own trailing 90th, long spot the next day: 30 trades,
mean +3.2 bps, null p95 +75.189 bps, +$0.96. The file's gross is +23.2417
bps, the mean is +3.2152 bps, and the total is +$0.9646. The null matches
§3.60 because both rules have 30 trades on the same BTC pool and the same
seed. The hourly grid has the same one hole, and that day is not a print.
456 days have a ratio. **Nothing here is fit to add.** The count cleared and
the mean was positive; it lost to the null. A quieter high hour, and the
80th, are not candidates.

### 3.121 The opening hour's quote over the last hour's: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp92-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp92/screen_2023.json`). Hour 0's quote
volume divided by hour 23's quote volume, above its own trailing 90th, long
spot the next day: 39 trades, mean −8.6 bps, null p95 +66.261 bps, −$3.35.
The file's gross is +11.4092 bps, the mean is −8.5936 bps, and the total is
−$3.3515. The null matches §3.117 because both rules have 39 trades on the
same BTC pool and the same seed. Hours 1 through 22 are not read, so the day
with the hole is still a print. The hole was not filled in. 457 days have a
ratio. **Nothing here is fit to add.** The count cleared and the mean was
negative. An opening hour that was quieter than the last hour, and the 80th,
are not candidates.

### 3.122 The last hour's share of the day's range: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp93-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp93/screen_2023.json`). Hour 23's range
divided by the range of the 24 hours, above its own trailing 90th, long spot
the next day: 32 trades, mean +29.9 bps, null p95 +72.3 bps, +$9.58. The
file's gross is +50.0109 bps, the mean is +29.9309 bps, the null is +72.2613
bps, and the total is +$9.5779. The null matches §3.49 because both rules
have 32 trades on the same BTC pool and the same seed. The hourly grid has
the same one hole, and that day is not a print. 456 days have a share.
**Nothing here is fit to add.** The count cleared and the mean was positive;
it lost to the null. A last hour that printed less of the day's range, and
the 80th, are not candidates.

### 3.123 The last close over the first hour's close: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp94-protocol.md`) was committed before any
return. Two runs matched (`backtests/fp94/screen_2023.json`). Hour 23's close
divided by hour 0's close, minus 1, above its own trailing 90th, long spot
the next day: 47 trades, mean +13.7 bps, null p95 +66.6563 bps, +$6.42. The
file's gross is +33.6986 bps, the mean is +13.6513 bps, and the total is
+$6.4161. No earlier screen had 47 trades, so the null has no earlier file
to match. It is fp5's 200-draw p95 on the same BTC pool and the same seed.
Hours 1 through 22 are not read, so the day with the hole is still a print.
The hole was not filled in. 457 days have a return. **Nothing here is fit to
add.** The count cleared and the mean was positive; it lost to the null. A
...
### 3.222 Ask notional above bid notional, then a session short: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp193-protocol.md`) was committed before any
return (`4fb0448`). Two runs matched (`backtests/fp193/screen_2023.json`,
sha256 `d9dc206d…`). The last book snapshot's ask notional strictly above
its bid notional, then a short of the next session from the open to the
close: 112 trades, mean −61.9342 bps, null p95 −13.7277 bps, −$69.3663.
The file's gross is −42.0381 bps. **Nothing here is fit to add.** The count
cleared and the mean was negative. The null is every spot session short,
not an unconditional long. The three missing book days were not filled in.
An equal book does not fire. This is not the bid-up and ask-down change.
The sell is the next open.

### 3.223 The index return above the spot return, then an open-to-open short: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp194-protocol.md`) was committed before any
return (`4fb0448`). Two runs matched (`backtests/fp194/screen_2023.json`,
sha256 `a9abb8b0…`). The index close/open strictly above the spot close/open,
then a short from the next open to the following open: 177 trades, mean
−55.7059 bps, null p95 −22.5105 bps, −$98.5994. The file's gross is
−35.7974 bps. **Nothing here is fit to add.** The count cleared and the
mean was negative. The null is every open-to-next-open short, not an
unconditional long. This is not the spot close set against the index close.
The open on 2024-01-01 is a cover only. The sell is the next open.

### 3.224 The perpetual average above the spot average, then a two-leg session: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp195-protocol.md`) was committed before any
return (`4fb0448`). Two runs matched (`backtests/fp195/screen_2023.json`,
sha256 `0e6582f0…`). The USDT perpetual's average trade strictly above the
spot average, then long spot and short that perpetual, each open to close:
120 trades, mean −32.7757 bps, null p95 −33.4027 bps, −$39.3308. The file's
gross is +7.1987 bps. **Nothing here is fit to add.** The count cleared.
The mean was negative, so it does not pass. The mean is above that null.
The null is the same hedge on every day both legs exist, not an
unconditional long. Funding cash is not added. A hundred dollars sits on
each leg. This is not the spot close against its own average, and it is
not the perpetual average against the mark.

### 3.225 The coin-margined close above spot, then a two-leg session: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp196-protocol.md`) was committed before any
return (`4fb0448`). Two runs matched (`backtests/fp196/screen_2023.json`,
sha256 `ac64dc6f…`). The coin-margined close strictly above the spot close,
then long spot and short that perpetual, each open to close: 151 trades,
mean −32.2977 bps, null p95 −33.6862 bps, −$48.7695. The file's gross is
+7.6777 bps. **Nothing here is fit to add.** The count cleared. The mean
was negative, so it does not pass. The mean is above that null. The null
is the same hedge on every day both legs exist, not an unconditional long.
Funding cash is not added. A hundred dollars sits on each leg. A count
taken before the freeze, with no profit computed, was 147. The scored book
is 151. The rule was not changed. This is not the coin-margined close set
against the USDT perpetual close.

### 3.226 The 16:00 funding rate above the 08:00 rate, then an overnight short: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp197-protocol.md`) was committed before any
return (`4fb0448`). Two runs matched (`backtests/fp197/screen_2023.json`,
sha256 `9efbc829…`). The 16:00 USDT funding rate strictly above the same
day's 08:00 rate, then a short from that day's close to the next open: 124
trades, mean −19.9748 bps, null p95 −19.956 bps, −$24.7688. The file's
gross is +0.0052 bps. **Nothing here is fit to add.** The count cleared
and the mean was negative and under the null. The null is every overnight
short, not an unconditional long. The 00:00 rate is not read. Funding cash
is not added. The close is the fill, not an input. The open on 2024-01-01
is a cover only.

### 3.227 Rising coin open interest, then a perpetual session short: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp198-protocol.md`) was committed before any
return (`4fb0448`). Two runs matched (`backtests/fp198/screen_2023.json`,
sha256 `1f181522…`). The last coin open interest strictly above the first
print, then a short of the next USDT-perpetual session from the open to the
close: 185 trades, mean −49.2392 bps, null p95 −29.906 bps, −$91.0925. The
file's gross is −29.3178 bps. **Nothing here is fit to add.** The count
cleared and the mean was negative and under the null. The null is every
USDT-perpetual session short, not an unconditional long. The quantity is
coins, not dollars. This is not the dollar open-interest overnight long.
Funding cash is not added. The sell is the next open.

### 3.228 The mark return above the index return, then a perpetual open-to-open short: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp199-protocol.md`) was committed before any
return (`4fb0448`). Two runs matched (`backtests/fp199/screen_2023.json`,
sha256 `f7f9e3cd…`). The mark close/open strictly above the index
close/open, then a short of the USDT perpetual from the next open to the
following open: 184 trades, mean −34.898 bps, null p95 −26.6463 bps,
−$64.2124. The file's gross is −14.9479 bps. **Nothing here is fit to add.**
The count cleared and the mean was negative and under the null. The null
is every such perpetual short, not an unconditional long. Funding cash is
not added. This is not the perpetual average set against the mark close.
The perpetual open on 2024-01-01 is a cover only. The sell is the next open.

### 3.229 The USDT taker share above the coin-margined share, then a two-leg session: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp200-protocol.md`) was committed before any
return (`4fb0448`). Two runs matched (`backtests/fp200/screen_2023.json`,
sha256 `4dca03c2…`). The USDT taker-buy share strictly above the
coin-margined share, then long the coin-margined perpetual and short the
USDT perpetual, each open to close: 202 trades, mean −34.4919 bps, null
p95 −33.7986 bps, −$69.6737. The file's gross is +5.4791 bps. **Nothing
here is fit to add.** The count cleared and the mean was negative and
under the null. The null is the same spread on every day both legs exist,
not an unconditional long. Funding cash is not added. A hundred dollars
sits on each leg. A count taken before the freeze, with no profit computed,
...
### 3.246 Monday's open bought, Friday's open sold: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp217-protocol.md`) was committed before any
return (`3acc6ef`). Two runs matched (`backtests/fp217/screen_2023.json`,
sha256 `5cbfcdee…`). Buy the USDT-perpetual open on Monday and sell Friday's
open: 52 trades, mean +73.4461 bps, null p95 +202.8863 bps, +$38.192. The
file's gross is +93.6132 bps. **Nothing here is fit to add.** The count
cleared and the mean was positive; it lost to the null. One leg. The weekend
is not inside the hold. Funding cash is not added. The null is that four-day
...
The protocol (`reviews/2026-09-25-fp219-protocol.md`) was committed before any
return (`3acc6ef`). Two runs matched (`backtests/fp219/screen_2023.json`,
sha256 `b42af92a…`). Buy the front coin-margined quarterly when 7 to 35 days
remain and sell the expiry open: 112 trades, mean +936.6832 bps, null p95
+1588.5237 bps, +$1049.0852. The file's gross is +958.5785 bps. **Nothing
here is fit to add.** The count cleared and the mean was positive; it lost
to the null. One leg. The March 2024 contract was not traded. A bar after
expiry was not stored. Funding cash is not added. The null holds that
contract to expiry whenever at least seven days remain. A count taken before
the freeze, with no profit computed, was 112. The scored book is 112. The
rule was not changed.

### 3.249 Five down sessions, then a seven-day long: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp220-protocol.md`) was committed before any
return (`3acc6ef`). Two runs matched (`backtests/fp220/screen_2023.json`,
sha256 `97963d87…`). At least five of the seven finished sessions closed
below their open, then buy the USDT perpetual for seven days: 81 trades,
mean +276.0868 bps, null p95 +284.8539 bps, +$223.6303. The file's gross is
+296.6595 bps. **Nothing here is fit to add.** The count cleared and the
mean was positive; it lost to the null. One leg. A high and a low were not
read. Funding cash is not added. The null is that seven-day long on every
day the seven sessions exist. A count taken before the freeze, with no
profit computed, was 81. The scored book is 81. The rule was not changed.

### 3.250 A finished up-week on spot, then a seven-day spot long: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp221-protocol.md`) was committed before any
return (`3acc6ef`). Two runs matched (`backtests/fp221/screen_2023.json`,
sha256 `940c1f82…`). Spot's close finished strictly above its close seven
days earlier, then buy spot for seven days: 204 trades, mean +174.683 bps,
null p95 +238.791 bps, +$356.3532. The file's gross is +195.0527 bps.
**Nothing here is fit to add.** The count cleared and the mean was positive;
it lost to the null. One spot leg. Funding cash is not added. The null is
that seven-day spot long on every day the two closes exist. A count taken
before the freeze, with no profit computed, was 204. The scored book is 204.
The rule was not changed.

### 3.251 Friday's open bought, the next Friday's open sold: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp222-protocol.md`) was committed before any
return (`3acc6ef`). Two runs matched (`backtests/fp222/screen_2023.json`,
sha256 `16951b0d…`). Buy the USDT-perpetual open on Friday and sell the next
Friday's open: 51 trades, mean +186.6858 bps, null p95 +345.5275 bps,
+$95.2098. The file's gross is +207.0796 bps. **Nothing here is fit to add.**
The count cleared and the mean was positive; it lost to the null. One leg.
The weekend sits inside the seven-day hold. Funding cash is not added. The
null is that seven-day long on every day. A count taken before the freeze,
with no profit computed, was 51. The scored book is 51. The rule was not
changed.

### 3.252 The 16:00 funding rate fell over seven days, then a long: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp223-protocol.md`) was committed before any
return (`3acc6ef`). Two runs matched (`backtests/fp223/screen_2023.json`,
sha256 `61ae3dca…`). The 16:00 funding rate finished strictly below the 16:00
rate seven days earlier, then buy the USDT perpetual for seven days: 135
trades, mean +259.1592 bps, null p95 +276.2519 bps, +$349.8649. The file's
gross is +279.6981 bps. **Nothing here is fit to add.** The count cleared
and the mean was positive; it lost to the null. One leg. Funding cash is not
added. Only the 16:00 print is stored. The null is that seven-day long on
every day both prints exist. A count taken before the freeze, with no profit
computed, was 135. The scored book is 135. The rule was not changed.

### 3.253 The coin-margined week lagged, then a seven-day short: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp224-protocol.md`) was committed before any
return (`3acc6ef`). Two runs matched (`backtests/fp224/screen_2023.json`,
sha256 `043f064c…`). The coin-margined seven-day return finished strictly
below the USDT return, then short that coin-margined perpetual for seven
days: 171 trades, mean −115.8579 bps, null p95 −128.2059 bps, −$198.117. The
file's gross is −96.0698 bps. **Nothing here is fit to add.** The count
cleared. The mean was negative, so it does not pass. The mean is above that
null. One short leg. The USDT book is a signal, not a second leg. Funding
cash is not added. A missing coin-margined day was not filled in. The null
...
### 3.255 The front quarterly held ten days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp225-protocol.md`) was committed before any
return (`ec08ec7`). Two runs matched (`backtests/fp225/screen_2023.json`,
sha256 `d5ff2883…`). Buy the front quarterly when 21 to 35 days remain and
sell ten days later, before expiry: 56 trades, mean +216.0632 bps, null p95
+454.864 bps, +$120.9954. The file's gross is +236.5158 bps. **Nothing here
is fit to add.** The count cleared and the mean was positive; it lost to the
null. One leg. The sale is not the expiry open. Funding cash is not added.
The null is that ten-day long whenever the sale is still before expiry. A
count taken before the freeze, with no profit computed, was 56. The scored
book is 56. The rule was not changed.

### 3.256 A finished spot pullback, held ten days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp226-protocol.md`) was committed before any
return (`ec08ec7`). Two runs matched (`backtests/fp226/screen_2023.json`,
sha256 `07e1d377…`). Yesterday's spot close was above the close twenty days
earlier and below the close five days earlier, then buy spot for ten days:
72 trades, mean +298.2792 bps, null p95 +426.7908 bps, +$214.761. The file's
gross is +318.8964 bps. **Nothing here is fit to add.** The count cleared and
the mean was positive; it lost to the null. One spot leg. The entry open is
not an input. Funding cash is not added. The null is that ten-day spot long
on every day. A count taken before the freeze, with no profit computed, was
72. The scored book is 72. The rule was not changed.

### 3.257 A finished coin-margined rise, short eight days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp227-protocol.md`) was committed before any
return (`ec08ec7`). Two runs matched (`backtests/fp227/screen_2023.json`,
sha256 `065b8584…`). Yesterday's coin-margined close was above the close
fifteen days earlier, then short that perpetual for eight days: 223 trades,
mean −161.2475 bps, null p95 −157.8331 bps, −$359.582. The file's gross is
−141.5503 bps. **Nothing here is fit to add.** The count cleared. The mean
was negative, so it does not pass. The mean is under that null. One short
leg. Funding cash is not added. The null is that eight-day short on every
day. A count taken before the freeze, with no profit computed, was 223. The
scored book is 223. The rule was not changed.

### 3.258 A finished thirty-day decline, held five days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp228-protocol.md`) was committed before any
return (`ec08ec7`). Two runs matched (`backtests/fp228/screen_2023.json`,
sha256 `9f288885…`). Yesterday's USDT close was below the close thirty days
earlier, then buy the USDT perpetual for five days: 122 trades, mean +48.4442
bps, null p95 +201.1551 bps, +$59.1019. The file's gross is +68.5612 bps.
**Nothing here is fit to add.** The count cleared and the mean was positive;
it lost to the null. One leg. Funding cash is not added. The null is that
five-day long on every day. A count taken before the freeze, with no profit
computed, was 122. The scored book is 122. The rule was not changed.

### 3.259 Spot led, then a fourteen-day spot short: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp229-protocol.md`) was committed before any
return (`ec08ec7`). Two runs matched (`backtests/fp229/screen_2023.json`,
sha256 `9a8d7413…`). Spot's finished ten-day return was above the USDT
return, then short spot for fourteen days: 177 trades, mean −401.5948 bps,
null p95 −254.4862 bps, −$710.8227. The file's gross is −382.3787 bps.
**Nothing here is fit to add.** The count cleared. The mean was negative, so
it does not pass. The mean is under that null. One spot short. The USDT book
is a signal, not a second leg. Funding cash is not added. The null is that
fourteen-day spot short on every day. A count taken before the freeze, with
no profit computed, was 177. The scored book is 177. The rule was not changed.

### 3.260 A coin-margined decline that lagged spot: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp230-protocol.md`) was committed before any
return (`ec08ec7`). Two runs matched (`backtests/fp230/screen_2023.json`,
sha256 `cd228903…`). The finished twelve-day coin-margined return was negative
...
scored book is 96. The rule was not changed.

### 3.261 A newly listed quarterly, held eleven days: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp231-protocol.md`) was committed before any
return (`ec08ec7`). Two runs matched (`backtests/fp231/screen_2023.json`,
sha256 `e8bfe70c…`). Buy the most recently listed quarterly during its first
...
freeze, with no profit computed, was 40. The scored book is 40. The rule was
not changed.

### 3.262 A finished nine-day rise, short nine days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp232-protocol.md`) was committed before any
return (`ec08ec7`). Two runs matched (`backtests/fp232/screen_2023.json`,
sha256 `5633a66d…`). Yesterday's USDT close was above the close nine days
earlier, then short the USDT perpetual for nine days: 206 trades, mean
−175.5027 bps, null p95 −177.8873 bps, −$361.5356. The file's gross is
−155.834 bps. **Nothing here is fit to add.** The count cleared. The mean
was negative, so it does not pass. The mean is above that null. One short
...
### 3.265 Ten higher spot closes, held eight days: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp233-protocol.md`) was committed before any
return (`55f8480`). Two runs matched (`backtests/fp233/screen_2023.json`,
sha256 `8e98355f…`). Yesterday's spot close was above each of the ten prior
...
### 3.266 A four-day rise then a four-day fall, short eleven days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp234-protocol.md`) was committed before any
return (`55f8480`). Two runs matched (`backtests/fp234/screen_2023.json`,
sha256 `762e961e…`). A finished four-day rise was followed by a finished
four-day fall, then short the USDT perpetual for eleven days: 82 trades,
mean −102.668 bps, null p95 −139.9677 bps, −$84.1878. The file's gross is
−82.8535 bps. **Nothing here is fit to add.** The count cleared. The mean
was negative, so it does not pass. The mean is above that null. One short
leg. Funding cash is not added. The null is that eleven-day short on every
day. A count taken before the freeze, with no profit computed, was 82. The
scored book is 82. The rule was not changed.

### 3.267 Spot, USDT and coin-margined each rose for six days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp235-protocol.md`) was committed before any
return (`55f8480`). Two runs matched (`backtests/fp235/screen_2023.json`,
sha256 `566b04fa…`). Spot, the USDT perpetual and the coin-margined perpetual
each finished a six-day rise, then buy the coin-margined perpetual for seven
days: 203 trades, mean +175.5563 bps, null p95 +244.2694 bps, +$356.3792.
The file's gross is +195.9277 bps. **Nothing here is fit to add.** The count
cleared and the mean was positive; it lost to the null. One coin-margined
leg. Spot and the USDT book are signals, not a second leg. Funding cash is
not added. The null is that seven-day long on every day. A count taken
before the freeze, with no profit computed, was 203. The scored book is 203.
The rule was not changed.

### 3.268 A two-day rise inside an eight-day fall: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp236-protocol.md`) was committed before any
return (`55f8480`). Two runs matched (`backtests/fp236/screen_2023.json`,
sha256 `289e9e51…`). A finished two-day rise sat inside a finished eight-day
fall, then buy the USDT perpetual for ten days: 59 trades, mean +310.5619
bps, null p95 +479.0876 bps, +$183.2315. The file's gross is +331.2037 bps.
**Nothing here is fit to add.** The count cleared and the mean was positive;
it lost to the null. One USDT-perpetual leg. Funding cash is not added. The
null is that ten-day long on every day. A count taken before the freeze,
with no profit computed, was 59. The scored book is 59. The rule was not
changed.

### 3.269 Two finished three-day rises, held four days: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp237-protocol.md`) was committed before any
return (`55f8480`). Two runs matched (`backtests/fp237/screen_2023.json`,
sha256 `ff0517d5…`). Two finished three-day rises, back to back, then buy
...
is 112. The rule was not changed.

### 3.270 Yesterday's open above the prior close: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp238-protocol.md`) was committed before any
return (`55f8480`). Two runs matched (`backtests/fp238/screen_2023.json`,
sha256 `6c37947a…`). Yesterday's USDT open finished above the close of the
day before, then buy the USDT perpetual for six days: 96 trades, mean
+178.2574 bps, null p95 +235.1411 bps, +$171.1271. The file's gross is
+198.6343 bps. **Nothing here is fit to add.** The count cleared and the
mean was positive; it lost to the null. One USDT-perpetual leg. Funding
cash is not added. The null is that six-day long on every day. A count
taken before the freeze, with no profit computed, was 96. The scored book
is 96. The rule was not changed.

### 3.271 A smaller absolute three-day return: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp239-protocol.md`) was committed before any
return (`55f8480`). Two runs matched (`backtests/fp239/screen_2023.json`,
sha256 `d169cae5…`). The absolute three-day USDT return finished smaller
than the absolute three-day return before it, then buy the USDT perpetual
for thirteen days: 179 trades, mean +331.9343 bps, null p95 +451.1356 bps,
+$594.1624. The file's gross is +352.6189 bps. **Nothing here is fit to
add.** The count cleared and the mean was positive; it lost to the null.
The comparison uses closes. One USDT-perpetual leg. Funding cash is not
added. The null is that thirteen-day long on every day. A count taken
before the freeze, with no profit computed, was 179. The scored book is
179. The rule was not changed.

### 3.272 Coin-margined return finished above spot: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp240-protocol.md`) was committed before any
return (`55f8480`). Two runs matched (`backtests/fp240/screen_2023.json`,
sha256 `d26ae687…`). The coin-margined eight-day return finished above
spot's eight-day return, then buy spot for five days: 177 trades, mean
+175.8273 bps, null p95 +178.24 bps, +$311.2143. The file's gross is
+196.1993 bps. **Nothing here is fit to add.** The count cleared and the
mean was positive; it lost to the null. One spot leg. The coin-margined
...
### 3.275 A finished three-day spot decline, held sixteen days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp241-protocol.md`) was committed before any
return (`6f1213a`). Two runs matched (`backtests/fp241/screen_2023.json`,
sha256 `a99c2d50…`). Spot's finished three-day return was negative, then buy
the coin-margined perpetual for sixteen days: 150 trades, mean +448.313 bps,
null p95 +566.0566 bps, +$672.4695. The file's gross is +469.2306 bps.
**Nothing here is fit to add.** The count cleared and the mean was positive;
it lost to the null. One coin-margined leg. Spot is a signal, not a second
leg. Funding cash is not added. The null is that sixteen-day long on every
day. A count taken before the freeze, with no profit computed, was 150. The
scored book is 150. The rule was not changed.

### 3.276 A finished two-day USDT rise, short nine days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp242-protocol.md`) was committed before any
return (`6f1213a`). Two runs matched (`backtests/fp242/screen_2023.json`,
sha256 `ea3818b9…`). The USDT perpetual's finished two-day return was
positive, then short spot for nine days: 198 trades, mean −262.305 bps, null
p95 −170.1131 bps, −$519.364. The file's gross is −242.8102 bps. **Nothing
here is fit to add.** The count cleared. The mean was negative, so it does
not pass. The mean is under that null. One spot short. The USDT book is a
signal, not a second leg. Funding cash is not added. The null is that
nine-day short on every day. A count taken before the freeze, with no profit
computed, was 198. The scored book is 198. The rule was not changed.

### 3.277 Spot's seven-day return ahead of coin-margined: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp243-protocol.md`) was committed before any
return (`6f1213a`). Two runs matched (`backtests/fp243/screen_2023.json`,
sha256 `2c5f17ad…`). Spot's finished seven-day return was above the
coin-margined return, then buy the USDT perpetual for twelve days: 171
trades, mean +372.2267 bps, null p95 +437.1806 bps, +$636.5077. The file's
gross is +392.992 bps. **Nothing here is fit to add.** The count cleared and
the mean was positive; it lost to the null. One USDT-perpetual leg. Spot and
the coin-margined book are signals, not a second leg. Funding cash is not
added. The null is that twelve-day long on every day. A count taken before
the freeze, with no profit computed, was 171. The scored book is 171. The
rule was not changed.

### 3.278 A quieter nine-day coin-margined move, held six days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp244-protocol.md`) was committed before any
return (`6f1213a`). Two runs matched (`backtests/fp244/screen_2023.json`,
sha256 `cdf7e8e5…`). The absolute nine-day coin-margined return finished
smaller than the absolute nine-day return before it, then buy that perpetual
for six days: 161 trades, mean +88.4583 bps, null p95 +230.6992 bps,
+$142.4179. The file's gross is +108.6554 bps. **Nothing here is fit to
add.** The count cleared and the mean was positive; it lost to the null. The
comparison uses closes. One coin-margined leg. Funding cash is not added.
The null is that six-day long on every day. A count taken before the freeze,
with no profit computed, was 161. The scored book is 161. The rule was not
changed.

### 3.279 A twenty-day coin-margined decline, short fifteen days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp245-protocol.md`) was committed before any
return (`6f1213a`). Two runs matched (`backtests/fp245/screen_2023.json`,
sha256 `d1969fae…`). The coin-margined twenty-day return finished negative,
then short the USDT perpetual for fifteen days: 123 trades, mean −239.3876
bps, null p95 −259.8823 bps, −$294.4467. The file's gross is −219.8468 bps.
**Nothing here is fit to add.** The count cleared. The mean was negative, so
it does not pass. The mean is above that null. One USDT short. The
coin-margined book is a signal, not a second leg. Funding cash is not added.
The null is that fifteen-day short on every day. A count taken before the
freeze, with no profit computed, was 123. The scored book is 123. The rule
was not changed.

### 3.280 Three finished two-day spot rises, held three days: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp246-protocol.md`) was committed before any
return (`6f1213a`). Two runs matched (`backtests/fp246/screen_2023.json`,
sha256 `65e67393…`). Three finished two-day spot rises, back to back, then
...
no profit computed, was 55. The scored book is 55. The rule was not changed.

### 3.281 Two finished five-day coin-margined declines, held eighteen days: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp247-protocol.md`) was committed before any
return (`6f1213a`). Two runs matched (`backtests/fp247/screen_2023.json`,
sha256 `6b1fd9b5…`). Two finished five-day coin-margined declines, back to
...
profit computed, was 64. The scored book is 64. The rule was not changed.

### 3.282 A one-day USDT bounce inside a five-day decline: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp248-protocol.md`) was committed before any
return (`6f1213a`). Two runs matched (`backtests/fp248/screen_2023.json`,
sha256 `e915e5d9…`). Yesterday's USDT close finished above the prior close
and below the close five days earlier, then buy spot for twenty-one days: 53
trades, mean +572.7312 bps, null p95 +820.5322 bps, +$303.5475. The file's
gross is +593.8978 bps. **Nothing here is fit to add.** The count cleared
and the mean was positive; it lost to the null. One spot leg. The USDT book
is a signal, not a second leg. Funding cash is not added. The null is that
...
### 3.285 Both books finished a four-day decline, held nineteen days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp249-protocol.md`) was committed before any
return (`dfa98e6`). Two runs matched (`backtests/fp249/screen_2023.json`,
sha256 `9706cbfc…`). Spot and the coin-margined book both finished a
four-day decline, then buy the USDT perpetual for nineteen days: 152
trades, mean +475.1995 bps, null p95 +621.3966 bps, +$722.3032. The file's
gross is +496.1709 bps. **Nothing here is fit to add.** The count cleared
and the mean was positive; it lost to the null. One USDT-perpetual leg.
Spot and the coin-margined book are signals, not extra legs. Funding cash
is not added. The null is that nineteen-day long on every day. A count
taken before the freeze, with no profit computed, was 152. The scored book
is 152. The rule was not changed. This is not a testing row.

### 3.286 A slower ten-day USDT rise, short twenty-six days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp250-protocol.md`) was committed before any
return (`dfa98e6`). Two runs matched (`backtests/fp250/screen_2023.json`,
sha256 `e7f5e940…`). A finished ten-day USDT rise was positive and smaller
than the ten-day rise before it, then short the coin-margined perpetual for
twenty-six days: 61 trades, mean −618.731 bps, null p95 −385.7913 bps,
−$377.4259. The file's gross is −599.9496 bps. **Nothing here is fit to
add.** The mean is negative and under the null. One coin-margined leg. The
USDT book is a signal, not a second leg. Funding cash is not added. The
null is that twenty-six-day short on every day. A count taken before the
freeze, with no profit computed, was 61. The scored book is 61. The rule
was not changed. This is not a testing row.

### 3.287 Spot lagged the USDT book, short seventeen days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp251-protocol.md`) was committed before any
return (`dfa98e6`). Two runs matched (`backtests/fp251/screen_2023.json`,
sha256 `6b2e7f8a…`). Spot's finished six-day return was below the USDT
six-day return, then short the USDT perpetual for seventeen days: 172
trades, mean −339.1838 bps, null p95 −299.0266 bps, −$583.3961. The file's
gross is −319.8428 bps. **Nothing here is fit to add.** The mean is
negative and under the null. One USDT-perpetual leg. Spot is a signal, not
a second leg. Funding cash is not added. The null is that seventeen-day
short on every day. A count taken before the freeze, with no profit
computed, was 172. The scored book is 172. The rule was not changed. This
is not a testing row.

### 3.288 A quieter five-day move inside twenty days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp252-protocol.md`) was committed before any
return (`dfa98e6`). Two runs matched (`backtests/fp252/screen_2023.json`,
sha256 `e85e99ef…`). The absolute five-day spot return finished smaller
than the absolute twenty-day spot return, then buy the coin-margined
perpetual for twenty days: 238 trades, mean +529.3541 bps, null p95
+631.1015 bps, +$1259.8627. The file's gross is +550.4338 bps. **Nothing
here is fit to add.** The count cleared and the mean was positive; it lost
to the null. The comparison uses closes. One coin-margined leg. Spot is a
signal, not a second leg. Funding cash is not added. The null is that
twenty-day long on every day. A count taken before the freeze, with no
profit computed, was 238. The scored book is 238. The rule was not changed.
This is not a testing row.

### 3.289 An up day after a down day, held twenty-two days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp253-protocol.md`) was committed before any
return (`dfa98e6`). Two runs matched (`backtests/fp253/screen_2023.json`,
sha256 `19c4e30c…`). A finished down day and then a finished up day, then
buy spot for twenty-two days: 93 trades, mean +550.0871 bps, null p95
+755.778 bps, +$511.581. The file's gross is +571.2084 bps. **Nothing here
is fit to add.** The count cleared and the mean was positive; it lost to
the null. One spot leg. Funding cash is not added. The null is that
twenty-two-day long on every day. A count taken before the freeze, with no
profit computed, was 93. The scored book is 93. The rule was not changed.
This is not a testing row.

### 3.290 A new three-day rise, held twenty-three days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp254-protocol.md`) was committed before any
return (`dfa98e6`). Two runs matched (`backtests/fp254/screen_2023.json`,
sha256 `f74f3224…`). A finished three-day spot rise followed a three-day
window that was not a rise, then buy the coin-margined perpetual for
twenty-three days: 83 trades, mean +526.5491 bps, null p95 +859.5851 bps,
+$437.0358. The file's gross is +547.6233 bps. **Nothing here is fit to
add.** The count cleared and the mean was positive; it lost to the null.
One coin-margined leg. Spot is a signal, not a second leg. Funding cash is
not added. The null is that twenty-three-day long on every day. A count
taken before the freeze, with no profit computed, was 83. The scored book
is 83. The rule was not changed. This is not a testing row.

### 3.291 Two finished down days, short nineteen days: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp255-protocol.md`) was committed before any
return (`dfa98e6`). Two runs matched (`backtests/fp255/screen_2023.json`,
sha256 `e3e98044…`). Two finished down days on spot, back to back, then
short spot for nineteen days: 80 trades, mean −449.5724 bps, null p95
−272.4165 bps, −$359.6579. The file's gross is −430.4524 bps. **Nothing
here is fit to add.** The mean is negative and under the null. One spot
leg. Funding cash is not added. The null is that nineteen-day short on
every day. A count taken before the freeze, with no profit computed, was
80. The scored book is 80. The rule was not changed. This is not a testing
row.

### 3.292 An older coin-margined rise and a later spot rise: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp256-protocol.md`) was committed before any
return (`dfa98e6`). Two runs matched (`backtests/fp256/screen_2023.json`,
sha256 `1e47bd94…`). A finished six-day coin-margined rise had already
ended, and the later four-day spot return was positive, then buy the USDT
perpetual for twenty-five days: 102 trades, mean +807.2796 bps, null p95
+826.0551 bps, +$823.4252. The file's gross is +828.9158 bps. **Nothing
here is fit to add.** The count cleared and the mean was positive; it lost
to the null. One USDT-perpetual leg. Spot and the coin-margined book are
signals, not extra legs. Funding cash is not added. The null is that
twenty-five-day long on every day. A count taken before the freeze, with
...
### 3.294 A finished spot decline, coin-margined against spot: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp257-protocol.md`) was committed before any
return (`73270f4`). Two runs matched (`backtests/fp257/screen_2023.json`,
sha256 `314500be…`). A finished four-day spot decline, then buy the
coin-margined book for nine days: 156 trades, mean +233.222 bps, null p95
+230.2271 bps, +$363.8263. The file's gross is +253.7089 bps. **The 2023
screen passes. This is not a testing row. The later years are not scored.**
One coin-margined leg. The other trade is the nine-day spot long on the same
entries, and the cutoff is that trade's mean. A missing open was not taken
from the other book. Funding cash is not added. A count taken before the
freeze, with no profit computed, was 156 against 156. The scored book is 156.
The rule was not changed. A Binance daily open is not a Revolut X fill.

### 3.295 A finished USDT rise, short against long: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp258-protocol.md`) was committed before any
return (`73270f4`). Two runs matched (`backtests/fp258/screen_2023.json`,
sha256 `0d9d4de4…`). A finished twelve-day USDT rise, then short that book
for thirteen days: 214 trades, mean −248.6318 bps, null p95 +288.6992 bps,
−$532.0721. The file's gross is −229.1096 bps. **Nothing here is fit to add.**
The mean is negative and under the null. One USDT leg. The other trade is the
thirteen-day long on the same entries. Funding cash is not added. A count
taken before the freeze, with no profit computed, was 214 against 214. The
scored book is 214. The rule was not changed. This is not a testing row.

### 3.296 A spot short against the same short started later: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp259-protocol.md`) was committed before any
return (`73270f4`). Two runs matched (`backtests/fp259/screen_2023.json`,
sha256 `5db612af…`). A finished six-day spot decline, then short spot for ten
days: 144 trades, mean −293.4708 bps, null p95 −203.409 bps, −$422.5979. The
file's gross is −274.0383 bps. **Nothing here is fit to add.** The mean is
negative and under the null. One spot leg. The other trade is that ten-day
short started ten days later. A signal was kept only when both windows could
fill. Funding cash is not added. A count taken before the freeze, with no
profit computed, was 144 against 144. The scored book is 144. The rule was
not changed. This is not a testing row.

### 3.297 Spot ahead of the coin-margined book: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp260-protocol.md`) was committed before any
return (`73270f4`). Two runs matched (`backtests/fp260/screen_2023.json`,
sha256 `6b1705a1…`). Spot's finished eight-day return was above the
coin-margined return, then buy the USDT book for sixteen days: 170 trades,
mean +530.253 bps, null p95 +533.5308 bps, +$901.43. The file's gross is
+551.3345 bps. **Nothing here is fit to add.** The count cleared and the mean
was positive; it lost to the null. One USDT leg. The other trade is the
sixteen-day coin-margined long on the same entries. Funding cash is not added.
A count taken before the freeze, with no profit computed, was 170 against 170.
The scored book is 170. The rule was not changed. This is not a testing row.

### 3.298 The coin-margined book lagging spot: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp261-protocol.md`) was committed before any
return (`73270f4`). Two runs matched (`backtests/fp261/screen_2023.json`,
sha256 `9876b165…`). The finished eleven-day coin-margined return was below
spot, then short the USDT book for six days: 169 trades, mean −158.4795 bps,
null p95 −160.716 bps, −$267.8303. The file's gross is −138.7768 bps.
**Nothing here is fit to add.** The mean is negative and above the null. One
USDT leg. The other trade is the six-day coin-margined short on the same
entries. Funding cash is not added. A count taken before the freeze, with no
profit computed, was 169 against 169. The scored book is 169. The rule was
not changed. This is not a testing row.

### 3.299 The larger five-day move against the quieter one: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp262-protocol.md`) was committed before any
return (`73270f4`). Two runs matched (`backtests/fp262/screen_2023.json`,
sha256 `ae1db46c…`). The absolute five-day spot return finished larger than
the absolute fifteen-day return, then buy the coin-margined book for ten
days: 108 trades, mean +342.0814 bps, null p95 +351.58 bps, +$369.448. The
file's gross is +362.7863 bps. **Nothing here is fit to add.** The count
cleared and the mean was positive; it lost to the null. The other regime has
240 fills and a mean of +264.3076 bps. The cutoff is the house p95 of samples
of 108, not that mean. One coin-margined leg. Equals went to neither. The
shorter regime was assigned to the rule from the span count, before any
profit. The windows were not moved. Funding cash is not added. A count taken
before the freeze, with no profit computed, was 108 against 240. The scored
book is 108. The rule was not changed after the freeze. This is not a testing
row.

### 3.300 A new two-day rise against the same long started later: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp263-protocol.md`) was committed before any
return (`73270f4`). Two runs matched (`backtests/fp263/screen_2023.json`,
sha256 `f8392a33…`). A finished two-day spot rise followed a two-day window
that was not a rise, then buy spot for fifteen days: 87 trades, mean
+437.5697 bps, null p95 +329.1846 bps, +$380.6856. The file's gross is
+458.4657 bps. **The 2023 screen passes. This is not a testing row. The later
years are not scored.** One spot leg. The other trade is that fifteen-day
long started fifteen days later, and the cutoff is that trade's mean. A
signal was kept only when both windows could fill. Funding cash is not added.
A count taken before the freeze, with no profit computed, was 87 against 87.
The scored book is 87. The rule was not changed. A Binance daily open is not
a Revolut X fill.

### 3.301 Both books finished a five-day decline: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp264-protocol.md`) was committed before any
return (`73270f4`). Two runs matched (`backtests/fp264/screen_2023.json`,
sha256 `457e995d…`). Spot and the USDT book both finished a five-day decline,
then buy the USDT book for fourteen days: 154 trades, mean +474.1374 bps,
null p95 +476.1511 bps, +$730.1716. The file's gross is +495.1066 bps.
**Nothing here is fit to add.** The count cleared and the mean was positive;
it lost to the null. One USDT leg. The other trade is the fourteen-day
coin-margined long on the same entries. Funding cash is not added. A count
...
### 3.305 A seven-day decline that closed up, long against short: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp265-protocol.md`) was committed before any
return (`dd2cb6e`). Two runs matched (`backtests/fp265/screen_2023.json`,
sha256 `1d3e890c…`). A finished seven-day spot decline ended with the last
finished day up, then buy spot for eleven days: 63 trades, mean +404.7314
bps, null p95 −355.5073 bps, +$254.9808. The file's gross is +425.5617 bps.
**The 2023 screen passes. This is not a testing row. The later years are not
scored.** One spot leg. The other trade is the eleven-day short on the same
entries, and the cutoff is that trade's mean. Funding cash is not added. A
count taken before the freeze, with no profit computed, was 63 against 63.
The scored book is 63. The rule was not changed. A Binance daily open is not
a Revolut X fill.

### 3.306 The coin-margined close finished below spot: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp266-protocol.md`) was committed before any
return (`dd2cb6e`). Two runs matched (`backtests/fp266/screen_2023.json`,
sha256 `59cff905…`). Yesterday's coin-margined close finished below
yesterday's spot close, then buy the coin-margined book for twelve days: 206
trades, mean +447.4904 bps, null p95 +443.2493 bps, +$921.8302. The file's
gross is +468.4063 bps. **The 2023 screen passes. This is not a testing row.
The later years are not scored.** One coin-margined leg. The other trade is
the twelve-day spot long on the same entries, and the cutoff is that trade's
mean. A missing open was not taken from the other book. Funding cash is not
added. A count taken before the freeze, with no profit computed, was 206
against 206. The scored book is 206. The rule was not changed. A Binance
daily open is not a Revolut X fill.

### 3.307 A faster three-day USDT decline, short against the same short later: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp267-protocol.md`) was committed before any
return (`dd2cb6e`). Two runs matched (`backtests/fp267/screen_2023.json`,
sha256 `9d77933f…`). A finished three-day USDT decline was strictly faster
than the prior three-day window, then short that book for eight days: 118
trades, mean −130.3164 bps, null p95 −144.3478 bps, −$153.7733. The file's
gross is −110.5572 bps. **Nothing here is fit to add.** The mean is negative
and above the null. One USDT leg. The other trade is that eight-day short
started eight days later. A signal was kept only when both windows could
fill. Funding cash is not added. A count taken before the freeze, with no
profit computed, was 118 against 118. The scored book is 118. The rule was
not changed. This is not a testing row.

### 3.308 A louder three-day move against the quieter one: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp268-protocol.md`) was committed before any
return (`dd2cb6e`). Two runs matched (`backtests/fp268/screen_2023.json`,
sha256 `22382308…`). The absolute three-day spot return finished larger than
the absolute three-day return that ended three days earlier, then buy the
USDT book for twenty-one days: 170 trades, mean +601.172 bps, null p95
+551.9241 bps, +$1021.9923. The file's gross is +622.3955 bps. **The 2023
screen passes. This is not a testing row. The later years are not scored.**
One USDT leg. The other trade buys that book for twenty-one days when the
recent absolute return finished smaller. Equals go to neither. The other
trade has 175 fills and a mean of +529.6739 bps; the cutoff is the house
p95, +551.9241. Funding cash is not added. A count taken before the freeze,
with no profit computed, was 170 against 175. The scored book is 170. The
rule was not changed. A Binance daily open is not a Revolut X fill.

### 3.309 A finished rise that had already ended: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp269-protocol.md`) was committed before any
return (`dd2cb6e`). Two runs matched (`backtests/fp269/screen_2023.json`,
sha256 `9b43a28e…`). A finished nine-day spot rise had already ended, and the
latest nine-day spot return was not a rise, then buy the coin-margined book
for five days: 89 trades, mean +88.9967 bps, null p95 +88.2984 bps,
+$79.2071. The file's gross is +109.1949 bps. **The 2023 screen passes. This
is not a testing row. The later years are not scored.** One coin-margined
leg. The other trade is the five-day USDT long on the same entries, and the
cutoff is that trade's mean. A missing open was not taken from the other
book. Funding cash is not added. A count taken before the freeze, with no
profit computed, was 89 against 89. The scored book is 89. The rule was not
changed. A Binance daily open is not a Revolut X fill.

### 3.310 A twenty-day rise with a five-day dip, short against long: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp270-protocol.md`) was committed before any
return (`dd2cb6e`). Two runs matched (`backtests/fp270/screen_2023.json`,
sha256 `e996f477…`). The finished twenty-day coin-margined return was
positive and the finished five-day return was negative, then short that book
for seventeen days: 69 trades, mean −581.0178 bps, null p95 +676.4821 bps,
−$400.9023. The file's gross is −562.1609 bps. **Nothing here is fit to add.**
The mean is negative and under the null. One coin-margined leg. The other
trade is the seventeen-day long on the same entries, and the cutoff is that
trade's mean. Funding cash is not added. A count taken before the freeze,
with no profit computed, was 69 against 69. The scored book is 69. The rule
was not changed. This is not a testing row.

### 3.311 A thirty-day decline, short against the same short later: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp271-protocol.md`) was committed before any
return (`dd2cb6e`). Two runs matched (`backtests/fp271/screen_2023.json`,
sha256 `02a448fe…`). The finished thirty-day spot return was negative, then
short spot for four days: 123 trades, mean −46.5219 bps, null p95 −138.7914
bps, −$57.2219. The file's gross is −26.595 bps. **Nothing here is fit to
add.** The mean is negative and above the null. One spot leg. The other
trade is that four-day short started four days later. A signal was kept only
when both windows could fill. Funding cash is not added. A count taken
before the freeze, with no profit computed, was 123 against 123. The scored
book is 123. The rule was not changed. This is not a testing row.

### 3.312 Spot's ten-day return above the USDT book: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp272-protocol.md`) was committed before any
return (`dd2cb6e`). Two runs matched (`backtests/fp272/screen_2023.json`,
sha256 `0dbe0342…`). Spot's finished ten-day return was strictly above the
USDT ten-day return, then short the coin-margined book for twenty-four days:
169 trades, mean −635.1923 bps, null p95 −635.7899 bps, −$1073.4749. The
file's gross is −616.4439 bps. **Nothing here is fit to add.** The mean is
negative and above the null. One coin-margined leg. The other trade is the
twenty-four-day USDT short on the same entries, and the cutoff is that
trade's mean. A missing open was not taken from the other book. Funding cash
is not added. A count taken before the freeze, with no profit computed, was
...
### 3.316 Two non-overlapping USDT rises, against the same long later: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp273-protocol.md`) was committed before any
return (`1057661`). Two runs matched (`backtests/fp273/screen_2023.json`,
sha256 `c7d4a538…`). Two non-overlapping fourteen-day USDT rises, then buy
that book for nine days: 131 trades, mean +231.0159 bps, null p95 +278.6329
bps, +$302.6308. The file's gross is +251.4984 bps. **Nothing here is fit to
add.** The mean is positive and under the null. One USDT leg. The other
trade is that nine-day long started nine days later. A signal was kept only
when both windows could fill. The cutoff is that other trade's mean. A count
taken before the freeze, with no profit computed, was 131 against 131. The
scored book is 131. The rule was not changed. This is not a testing row.

### 3.317 Yesterday's USDT close above spot, short against short: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp274-protocol.md`) was committed before any
return (`1057661`). Two runs matched (`backtests/fp274/screen_2023.json`,
sha256 `2b8096ae…`). Yesterday's USDT close finished above yesterday's spot
close, then short the USDT book for eighteen days: 51 trades, mean −501.1465
bps, null p95 −502.9563 bps, −$255.5847. The file's gross is −482.1298 bps.
**Nothing here is fit to add.** The mean is negative and above the null. One
USDT leg. The other trade is the eighteen-day spot short on the same
entries, and the cutoff is that trade's mean. A count taken before the
freeze, with no profit computed, was 51 against 51. The scored book is 51.
The rule was not changed. This is not a testing row.

### 3.318 A six-day coin-margined decline, short against the same short later: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp275-protocol.md`) was committed before any
return (`1057661`). Two runs matched (`backtests/fp275/screen_2023.json`,
sha256 `5afe2cd4…`). A finished six-day coin-margined decline, then short
that book for sixteen days: 134 trades, mean −452.7271 bps, null p95
−206.7926 bps, −$606.6543. The file's gross is −433.6134 bps. **Nothing here
is fit to add.** The mean is negative and under the null. One coin-margined
leg. The other trade is that sixteen-day short started sixteen days later.
A signal was kept only when both windows could fill. The cutoff is that
other trade's mean. A count taken before the freeze, with no profit
computed, was 134 against 134. The scored book is 134. The rule was not
changed. This is not a testing row.

### 3.319 Both books finished a two-day rise: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp276-protocol.md`) was committed before any
return (`1057661`). Two runs matched (`backtests/fp276/screen_2023.json`,
sha256 `f0f7d8ba…`). Spot and the USDT book both finished a two-day rise,
then buy spot for twenty-seven days: 187 trades, mean +849.0499 bps, null
p95 +849.9417 bps, +$1587.7233. The file's gross is +870.7697 bps. **Nothing
here is fit to add.** The mean is positive and under the null. One spot leg.
The other trade is the twenty-seven-day USDT long on the same entries, and
the cutoff is that trade's mean. A count taken before the freeze, with no
profit computed, was 187 against 187. The scored book is 187. The rule was
not changed. This is not a testing row.

### 3.320 A cooler twenty-day coin-margined rise, against the same long later: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp277-protocol.md`) was committed before any
return (`1057661`). Two runs matched (`backtests/fp277/screen_2023.json`,
sha256 `12fe3d00…`). A finished twenty-day coin-margined rise was strictly
smaller than the prior twenty-day rise, then buy that book for eight days:
66 trades, mean +125.0161 bps, null p95 +241.5348 bps, +$82.5106. The file's
gross is +145.2864 bps. **Nothing here is fit to add.** The mean is positive
and under the null. One coin-margined leg. The other trade is that eight-day
long started eight days later. A signal was kept only when both windows
could fill. The cutoff is that other trade's mean. A count taken before the
freeze, with no profit computed, was 66 against 66. The scored book is 66.
The rule was not changed. This is not a testing row.

### 3.321 Both books finished a nine-day decline, short against short: none clears (2026-09-25)

The protocol (`reviews/2026-09-25-fp278-protocol.md`) was committed before any
return (`1057661`). Two runs matched (`backtests/fp278/screen_2023.json`,
sha256 `b322df26…`). Spot and the USDT book both finished a nine-day
decline, then short the USDT book for twenty-eight days: 141 trades, mean
−565.9823 bps, null p95 −567.0602 bps, −$798.0351. The file's gross is
−547.0954 bps. **Nothing here is fit to add.** The mean is negative and
above the null. One USDT leg. The other trade is the twenty-eight-day
coin-margined short on the same entries, and the cutoff is that trade's
mean. A count taken before the freeze, with no profit computed, was 141
against 141. The scored book is 141. The rule was not changed. This is not
a testing row.

### 3.322 A finished fifteen-day spot rise, against the same long later: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp279-protocol.md`) was committed before any
return (`1057661`). Two runs matched (`backtests/fp279/screen_2023.json`,
sha256 `a63ea6da…`). A finished fifteen-day spot rise, then buy spot for
three days: 225 trades, mean +60.2883 bps, null p95 +48.0585 bps,
+$135.6487. The file's gross is +80.429 bps. **The 2023 screen passes. This
is not a testing row. The later years are not scored.** One spot leg. The
other trade is that three-day long started three days later, and the cutoff
is that trade's mean. The mean clears it by 12.2298 bps. A signal was kept
only when both windows could fill. A count taken before the freeze, with no
profit computed, was 225 against 225. The scored book is 225. The rule was
not changed. A Binance daily open is not a Revolut X fill.

### 3.323 Spot's twelve-day return below the USDT book: the 2023 screen passes (2026-09-25)

The protocol (`reviews/2026-09-25-fp280-protocol.md`) was committed before any
return (`1057661`). Two runs matched (`backtests/fp280/screen_2023.json`,
sha256 `7e5cd11a…`). Spot's finished twelve-day return was strictly below
the USDT twelve-day return, then buy spot for twenty-six days: 174 trades,
mean +657.8146 bps, null p95 +657.6838 bps, +$1144.5975. The file's gross is
+679.1516 bps. **The 2023 screen passes. This is not a testing row. The
later years are not scored.** One spot leg. The other trade is the
twenty-six-day USDT long on the same entries, and the cutoff is that
trade's mean. The mean clears it by 0.1308 bps. That gap is the same class
as PICK, UNDER and STALE. A count taken before the freeze, with no profit
