...
14. **The venue's market data is the account's region, always.** Revolut X keeps two books per pair (UK / EEA) and this account trades the UK one; a quote or a candle from the other book is not a price this account can get, and reading one produced the only trade the dislocation rule ever made (§3.5). Every public call names `region=UK`, a row from another region is dropped, and the probe shows which book the loop is reading. The same discipline applies to any venue that publishes more than one book, and any fact of that kind written into this reference is a requirement on the client with a pin, the day it is written. **And the public pair list is both regions added together** (checked 2026-09-26, for PR7): `configuration/pairs` lists 455 pairs, 322 in both regions, 71 UK-only and 62 EEA-only; the UK side is the 393 books the signed pair list shows. USDC/EUR, the one stablecoin book against a currency PR5 and PR6 had not tried, is EEA-only: `/2.0/public/order-book/USDC-EUR?region=UK` answers 400 "Couldn't find currency pair USDC/EUR" (the EEA book answers 200), and its UK tape held no print in 366 days. Every EUR-quoted book is EEA-only and every GBP one UK-only; there is no USDT/EUR or EUR stablecoin in either region. So this account's stablecoin-against-currency books are exactly USDC/GBP and USDT/GBP (PR5, paper) and USDC/USD and USDT/USD (PR6, failed). Before a study plans on a pair, it asks the UK book for it.
...
36. **RW runs on paper for fourteen days (2026-09-24, migration `0053`, `agents/pmrw.ts`).** §3.33's RW passed its forward bar on 268 minutes of one day; the plan for Polymarket (the ledger's item) runs a candidate on paper over days before anything is built for orders, and Davies said build it. The spec is frozen at `reviews/2026-09-24-polymarket-rw-paper-spec.md`: 2026-09-25 00:00 → 2026-10-09 00:00 UTC, the portfolio re-selected each UTC day ($300, whole markets by first-round reward per dollar), the same six conditions over the fourteen day totals.
    ...
    - **RW-E is pre-registered beside it (2026-09-26, `reviews/2026-09-26-polymarket-rw-end-prereg.md`).** Split by how far each market's scheduled end was from the day it was chosen, the run to 09-26 16:16 UTC put the whole stress loss in the markets that end that day (15 markets: rewards $147.84, fills −$128.53, stress −$77.61; every other market together +$60.86 of stress). RW-E is RW without those market-days, judged on 2026-09-27 → 10-08 by an exact replay of the engine's own stored tables through `stepRw`, after the replay reproduces RW's `pm_rw_days`; its bar is RW's six conditions over twelve days plus a seventh, that its stress total beats RW's. It changes nothing that runs, and is reported with RW's verdict.
    - **RW-E runs beside RW from 2026-09-26 (migration `0056`, `agents/pmrw_e.ts`, Davies: "fix it where it can be fixed").** RW's own test stays frozen; `agents?action=pmrw-e` replays RW's stored minutes every minute (every five until `0060`), from RW's start, in two arms: RW itself, whose closed days must equal `pm_rw_days` to under a cent (the check), and RW-E. Since 2026-09-26 RW-E is a TESTING row of its own, "Reward quotes (no same-day)", with RW's page (Davies: "两个testing策略"); the section that showed both arms on RW's page was then taken off both pages on his word, and RW-E's page keeps only a warning for when the check fails. Each of the two rows is funded $1,000 on the page (`RW_FUNDED_USD`, Davies 2026-09-26: a cap like every other strategy's): its cap, its FUNDED and the base of its today and realised percents. The rule keeps its own $300 of quotes a day, the days table keeps what the markets have at work, and the verdict's capital is the spec's. What is at work has run from $165 to $798 a day so far (the $798 is RW's open 26 Sep), under the cap; on a day it goes over, the page says so (`rwOverCapText`), because the frozen rule does not read the cap and a $1,000 account could not have placed every quote the paper engine placed. Its row is `rweArmSummary`, RW's own `rwSummary` run on the replay's `e` arm, with RW's fills less those of the market-days RW-E leaves out (pinned against the engine run without them, figure for figure; the Polymarket card sums the two rows). On paper the replay is RW-E's own book, not a copy: every market is decided independently, so in a market it quotes RW-E's decisions are RW's, and where its inventory differs it runs the rule itself. **One deviation from the pre-registration's method, recorded here:** a minute is applied from what the engine RECORDED of it (its decision in `pm_rw_minutes` and its fills in `pm_rw_fills`, in the order of their prints), not by running `stepRw` again on the stored inputs, because the inputs cannot say which of a minute's prints were published by the time the engine decided it, nor under which day's selection a minute at midnight was decided. A market is independent of every other, so while RW-E holds what RW holds in it, its minute is RW's; where the two hold different amounts (only in a market quoted again after an exclusion), the minute runs through `stepRw` on its stored book and prints, and the market is named in `diverged`. The rule, the data and the bar are unchanged.
