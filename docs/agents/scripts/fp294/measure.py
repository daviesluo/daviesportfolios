"""Score the fp294 screen. 2023 entries only.

    python3 docs/agents/scripts/fp294/measure.py
"""
...
DATA = Path(os.environ.get("FP294_DATA", "/tmp/fp294/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp294-protocol.md"
NOTE = 'Fair value of the USDT book is the coin-margined close. The finished USDT close was at least five basis points under that fair value. The next USDT open is bought and sold six days later. The other trade buys the USDT book for six days when that discount is absent. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side.'
...
def run() -> None:
    ...
    if c.IDEA != 'DISCM' or c.MIN_N != 30 or c.HOLD_DAYS != 6 or c.NULL_KIND != 'other_regime':
    ...
    payload = {
        ...
        "code_sha256": {
            "fp294_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp294_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp294/screen_2023.json"
