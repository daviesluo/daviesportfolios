"""Score the fp265 screen. 2023 entries only.

    python3 docs/agents/scripts/fp265/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp265-protocol.md"
NOTE = 'A finished seven-day spot decline ended with an up day. The next spot open is bought and sold eleven days later. The other trade sells that open for the same eleven days. One leg. Funding cash is not added. The null is not that eleven-day long on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'LASTUP' or c.MIN_N != 30 or c.HOLD_DAYS != 11:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp265_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp265_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp265/screen_2023.json"
