"""The search fp265. A finished seven-day spot decline, and the last finished day closed up. Buy spot for eleven days. The other trade sells spot for those same eleven days.
...
def _signal(spot, um, cm, entry) -> bool:
    move = finished(spot, entry, 7)
    newer = _px(spot, entry - fp5.DAY_MS, 1)
    older = _px(spot, entry - 2 * fp5.DAY_MS, 1)
    return (
        move is not None
