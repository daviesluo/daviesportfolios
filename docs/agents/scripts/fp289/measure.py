"""Score the fp289 screen. 2023 entries only.

    python3 docs/agents/scripts/fp289/measure.py
"""
...
DATA = Path(os.environ.get("FP289_DATA", "/tmp/fp289/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp289-protocol.md"
NOTE = 'Fair value of the USDT book is the spot close. The finished USDT close was at least five basis points under that fair value. The next USDT open is bought and sold nine days later. The other trade buys the USDT book for nine days when that discount is absent. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side.'
...
def run() -> None:
    ...
    if c.IDEA != 'DISCU' or c.MIN_N != 30 or c.HOLD_DAYS != 9 or c.NULL_KIND != 'other_regime':
    ...
    payload = {
        ...
        "code_sha256": {
            "fp289_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp289_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp289/screen_2023.json"
