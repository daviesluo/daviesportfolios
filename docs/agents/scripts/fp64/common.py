"""The sixty-fourth search (fp64). The fill and the null are fp5's.

Nothing here reads the network or a file. An hourly bar is (open, close).
A daily bar is (open,). The signal is hour 15's close divided by hour 8's
open, minus 1. Hours outside 8 through 15 are not read. The fill reads a
later daily open.
"""
...
def mid_at(
    ...
    """Middle-block return. None when one of hours 8 through 15 is missing."""
    if day >= end_ms:
        return None
    for i in range(FIRST, LAST + 1):
        ...
        if bar is None or len(bar) < 2 or bar[0] <= 0 or bar[1] <= 0:
            return None
    open_ = hours[day + FIRST * HOUR_MS][0]
...
def mid_signal_days(
    ...
    return _upper(mid_prints(hours, end_ms), q)
...
def mid_trades(
    ...
    """Enter the next daily open after a strong middle block. Hold one day."""
    trades = []
    for day in mid_signal_days(hours, q, end_ms):
