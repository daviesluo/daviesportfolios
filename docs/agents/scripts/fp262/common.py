"""The search fp262. When the absolute five-day spot return finished larger than the absolute fifteen-day return, buy the coin-margined book for ten days. The other trade buys that book for ten days when the five-day absolute return finished smaller.
...
RULE_SIDE = 'long'
NULL_SIDE = 'long'
FILES = {'spot': ('spot1d/BTCUSDT.json', False), 'cm': ('cm1d/BTCUSD_PERP.json', True)}
PULLS = [('spot', 'BTCUSDT', 'spot1d', False), ('cm', 'BTCUSD_PERP', 'cm1d', True)]
...
def _rule_signal(spot, um, cm, entry) -> bool:
    a = finished(spot, entry, 5)
    b = finished(spot, entry, 15)
