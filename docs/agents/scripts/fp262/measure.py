"""Score the fp262 screen. 2023 entries only.

    python3 docs/agents/scripts/fp262/measure.py
"""
...
DATA = Path(os.environ.get("FP262_DATA", "/tmp/fp262/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp262-protocol.md"
...
def run() -> None:
    ...
    if c.IDEA != 'OTHER' or c.MIN_N != 30 or c.HOLD_DAYS != 10:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp262_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp262_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp262/screen_2023.json"
