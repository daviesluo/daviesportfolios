"""Score the fp254 screen. 2023 entries only.

    python3 docs/agents/scripts/fp254/measure.py
"""
...
DATA = Path(os.environ.get("FP254_DATA", "/tmp/fp254/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp254-protocol.md"
NOTE = 'The finished three-day spot return turned positive after a three-day window that was not positive. The next coin-margined open is bought and sold twenty-three days later. One coin-margined leg. Funding cash is not added. The null is that twenty-three-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    if not c._signal(books["spot"], books["cm"], trade["entry_ms"]):
        raise SystemExit("the new three-day rise was not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'NEWUP' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp254_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp254_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp254/screen_2023.json"
