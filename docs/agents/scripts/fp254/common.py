"""The search fp254. A finished three-day spot rise after a three-day window that was not a rise, then a twenty-three-day coin-margined long.
...
not added. The null is that same 23-day long on every day.
"""
...
def _signal(spot: dict, cm: dict, entry: int) -> bool:
    ...
    near = finished(spot, entry, 3)
    prev = finished(spot, entry - 3 * fp5.DAY_MS, 3)
