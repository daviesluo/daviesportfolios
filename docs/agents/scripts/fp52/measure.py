"""Score the fp52 screen. 2023 only.

    python3 docs/agents/scripts/fp52/measure.py
"""
...
DATA = Path(os.environ.get("FP52_DATA", "/tmp/fp52/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp52-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "QVOL-CHG", c.qvol_trades(daily), pool, c.MIN_N,
        "Rise in quote volume, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp52_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp52_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp52/screen_2023.json"
