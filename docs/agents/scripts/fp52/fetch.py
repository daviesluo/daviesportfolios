"""Pull BTCUSDT bars for the fp52 screen.

A bar is the open and the quote volume. The 2024-01-01 quote is stored as 0.

No later year is requested. The 2024-01-01 bar keeps its open. Its quote is masked.

    python3 docs/agents/scripts/fp52/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp52-research"}
...
def main() -> None:
    ...
    save(rows)
    print(f"fp52 BTCUSDT 1d {len(rows)} first {min(rows)} last {max(rows)}")
