"""Score the fp172 screen. 2023 entries only.

    python3 docs/agents/scripts/fp172/measure.py
"""
...
DATA = Path(os.environ.get("FP172_DATA", "/tmp/fp172/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp172-protocol.md"
NOTE = "A down day. Next day, buy the open, and sell at that day's low when the low trades there, otherwise sell the close."
...
def main() -> None:
    ...
    for trade in trades:
        ...
        if not bars[day][3] < bars[day][0]:
            raise SystemExit("a trade was not a down day")
        level = bars[day][2]
        open_px, _high, low, close_px = bars[trade["entry_ms"]]
        if open_px <= level:
            exit_px = open_px
        elif low <= level:
            exit_px = level
        ...
        if abs(trade["net"] - c.fp5.net_return(open_px, exit_px)) > 1e-12:
            raise SystemExit("a fill is not the stop")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp172_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp172_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp172/screen_2023.json"
