"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp216/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp216-research"}
...
def main() -> None:
    ...
    project(cm, (0, 1), last, "cmsig/BTCUSD_PERP.json", True)
    project(mark, (0, 1), last, "mark1d/BTCUSDT.json", True)
    project(spot, (0, 1), session, "spot1d/BTCUSDT.json", True)
    project(cm, (0, 1), session, "cm1d/BTCUSD_PERP.json", True)
    print("fp216 " + f"spot {len(spot)} cm {len(cm)} mark {len(mark)}")
