"""Score the fp280 screen. 2023 entries only.

    python3 docs/agents/scripts/fp280/measure.py
"""
...
DATA = Path(os.environ.get("FP280_DATA", "/tmp/fp280/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp280-protocol.md"
NOTE = "Spot's finished twelve-day return was strictly below the USDT twelve-day return. The next spot open is bought and sold twenty-six days later. The other trade buys the USDT book for those same twenty-six days. One leg. Funding cash is not added. The null is not that spot long with the comparison turned off."
...
def run() -> None:
    ...
    if c.IDEA != 'LAGSP' or c.MIN_N != 30 or c.HOLD_DAYS != 26:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp280_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp280_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp280/screen_2023.json"
