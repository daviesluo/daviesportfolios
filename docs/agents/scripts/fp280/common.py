"""The search fp280. Spot's finished twelve-day return was strictly below the USDT twelve-day return. Buy spot for twenty-six days. The other trade buys the USDT book for those same twenty-six days. The entry open is not an input. One leg.
...
def _signal(spot, um, cm, entry) -> bool:
    spot_ret = finished(spot, entry, 12)
    um_ret = finished(um, entry, 12)
    return spot_ret is not None and um_ret is not None and spot_ret < um_ret
