...
"""Pins for the fp194 rule. No file and no return from 2023 is read."""
...
def test_index_outran() -> None:
    ...
    if abs(trades[0]["gross"] - (11.0 / 10.0 - 1.0)) < 1e-12:
        raise SystemExit("the gross is the signal day")
    if c.signal_trades(bars, {signal: (10.0, 11.0)}):
        raise SystemExit("an equal return fired")
    if c.signal_trades(bars, {signal: (10.0, 10.5)}):
        raise SystemExit("a slower index fired")
    last_signal = c.fp5.SCREEN_END_MS - 2 * c.fp5.DAY_MS
    cover = {
        last_signal: (10.0, 11.0),
        last_signal + c.fp5.DAY_MS: (100.0, 90.0),
        c.fp5.SCREEN_END_MS: (80.0, 70.0),
    }
    held = c.signal_trades(cover, {last_signal: (10.0, 13.0)})
    ...
    blocked = {c.fp5.SCREEN_END_MS - c.fp5.DAY_MS: (10.0, 11.0)}
    if c.signal_trades(
        {c.fp5.SCREEN_END_MS - c.fp5.DAY_MS: (10.0, 11.0), c.fp5.SCREEN_END_MS: (100.0, 80.0)},
        {c.fp5.SCREEN_END_MS - c.fp5.DAY_MS: (10.0, 13.0)},
    ):
        raise SystemExit("a 2024 open was an entry")
    if c.signal_trades({c.fp5.SCREEN_END_MS: (100.0, 80.0)}, blocked):
        raise SystemExit("a 2024 open was an entry")
    pool = c.pool_nets(bars)
    if len(pool) != 2 or abs(pool[0] - float(want)) > 1e-12:
        raise SystemExit("the null is not every open-to-open short")
