"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp194/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp194-research"}
...
def main() -> None:
    ...
    print("fp194 " + f"spot {len(spot)} last {max(spot)} index {len(index)} last {max(index)}")
