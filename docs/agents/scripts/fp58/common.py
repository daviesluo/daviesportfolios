"""The fifty-eighth search (fp58). The fill and the null are fp5's.

Nothing here reads the network or a file. An hourly bar is (high, low).
A daily bar is (open,). The signal is the later of the hour of the highest
high and the hour of the lowest low. A tie keeps the earlier hour. The
prices do not enter. The fill reads a later daily open.
"""
...
def lastext_at(
    ...
    """Hour index of the later extreme. None when an hour is missing."""
    if day >= end_ms:
        return None
    hi_i = lo_i = None
    hi = lo = None
...
def lastext_prints(
    ...
    for day in range(start, end_ms, fp5.DAY_MS):
        value = lastext_at(hours, day, end_ms)
        if value is None:
            continue
...
def lastext_signal_days(
    hours: dict[int, tuple], q: float = 0.90, end_ms: int = fp5.SCREEN_END_MS,
) -> list[int]:
    return _upper(lastext_prints(hours, end_ms), q)
...
def lastext_trades(
    ...
    """Enter the next daily open after a day whose later extreme printed late. Hold one day."""
    trades = []
    for day in lastext_signal_days(hours, q, end_ms):
