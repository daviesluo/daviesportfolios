"""Score the fp58 screen. 2023 only.

    python3 docs/agents/scripts/fp58/measure.py
"""
...
DATA = Path(os.environ.get("FP58_DATA", "/tmp/fp58/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp58-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "LAST-EXT", c.lastext_trades(daily, hours), pool, c.MIN_N,
        "Hour of the later of the high and the low, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp58_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp58_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp58/screen_2023.json"
