"""The search fp310. Fair value is the KuCoin BTC-USDT daily close. The finished coin-margined close was at least five basis points under that close, so the next coin-margined open is a price that finished cheap to KuCoin. Buy the coin-margined book for eleven days. The other trade buys the coin-margined book for eleven days on these same prices when that discount is absent.
...
FILES = {
    ...
    FAIR_KEY: ('kc1d/BTC-USDT.json', False),
}
PULLS = [
    ('cm', 'BTCUSD_PERP', 'cm1d', True),
    ('kc', 'BTC-USDT', 'kc1d', False),
]
