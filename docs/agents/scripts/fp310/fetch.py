"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp310/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp310-research"}
...
def pull_kucoin() -> dict[int, tuple]:
    url = (
        "https://api.kucoin.com/api/v1/market/candles?type=1day&symbol=BTC-USDT"
        # endAt is exclusive, so the last stored day needs one extra step.
        f"&startAt={c.LOOKBACK_MS // 1000}&endAt={c.FAIR_LAST_MS // 1000 + 86400}"
    )
    payload = json.loads(get(url))
    if payload.get("code") != "200000":
        raise SystemExit("KuCoin refused the candles")
    rows: dict[int, tuple] = {}
    ...
    check_fair(rows, "KuCoin")
    save("kc1d/BTC-USDT.json", rows)
    return rows
...
def main() -> None:
    ...
    fair = pull_kucoin()
    print(f"fp310 {len(traded)} {len(fair)}")
