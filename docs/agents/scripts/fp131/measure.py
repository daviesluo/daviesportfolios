"""Score the fp131 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp131/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp131-protocol.md"
NOTE = "English Wikipedia pageviews of Bitcoin that UTC day, above their own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_book(DATA / "wiki/bitcoin.json")
    ...
    row = c.fp5.summarise("WIKI", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp131_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp131_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "wiki": sha256(DATA / "wiki/bitcoin.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp131/screen_2023.json"
