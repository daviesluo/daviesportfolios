"""Score the fp241 screen. 2023 entries only.

    python3 docs/agents/scripts/fp241/measure.py
"""
...
DATA = Path(os.environ.get("FP241_DATA", "/tmp/fp241/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp241-protocol.md"
NOTE = 'A finished three-day spot decline. The next coin-margined open is bought and sold sixteen days later. One coin-margined leg. Funding cash is not added. The null is that sixteen-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    move = c.finished(books["spot"], trade["entry_ms"], 3)
    if move is None or not move < 0.0:
        raise SystemExit("the three-day spot decline was not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'DROP' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp241_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp241_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp241/screen_2023.json"
