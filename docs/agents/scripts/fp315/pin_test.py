...
"""Pins for the fp315 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "EZERO" or c.HOLD_DAYS != 2 or c.NULL_KIND != "other_regime":
        raise SystemExit("the idea moved")
    if c.MODE != "regime" or c.RULE_SIDE != "long" or c.RULE_BOOK != "cm":
        raise SystemExit("the leg moved")
    if c.FAIR_KIND != "index" or c.COIN != "ETHUSD_PERP":
    ...
    sofr = {}
    fund[entry - 8 * 3_600_000] = -0.0002
    fund[off - 8 * 3_600_000] = 0.0001
    # A positive rate stamped on the entry must not erase the earlier negative print.
    fund[entry] = 0.01
