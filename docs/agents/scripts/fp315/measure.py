"""Score the fp315 screen. 2023 entries only.

    python3 docs/agents/scripts/fp315/measure.py
"""
...
DATA = Path(os.environ.get("FP315_DATA", "/tmp/fp315/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp315-protocol.md"
NOTE = "Fair value is the price index inside Binance's funding formula for ETHUSD_PERP. The last settled funding rate is negative, so that contract finished below the index. Buy the ETH coin-margined book for two days. The other trade buys it for two days when that rate is not negative. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side. The traded open is the fill only. Another venue's close is not an input. A book ticker is not read."
...
def run() -> None:
    book = load_opens(DATA / "cm1d" / "ETHUSD_PERP.json")
    fund = load_fund(DATA / "fund" / "ETHUSD_PERP.json")
    inputs = {
        "cm": sha256(DATA / "cm1d" / "ETHUSD_PERP.json"),
        "fund": sha256(DATA / "fund" / "ETHUSD_PERP.json"),
    }
    sofr = {}

    trades = c.rule_trades(book, fund, {})
    nulls = c.null_trades(book, fund, {})
    spans = c.rule_spans(book, fund, {})
    pool_spans = c.null_spans(book, fund, {})
    ...
    for trade in trades:
        ...
        if not c._rule_on(book, fund, {}, trade["entry_ms"]):
            raise SystemExit("the signal was not already finished")
    for trade in nulls:
        check_common(trade)
        check_prices(trade, book)
    check_shape(spans, pool_spans, book)
    if c.IDEA != "EZERO" or c.MIN_N != 30 or c.HOLD_DAYS != 2 or c.FAIR_KIND != "index":
    ...
    payload = {
        ...
        "code_sha256": {
            "fp315_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp315_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp315/screen_2023.json"
