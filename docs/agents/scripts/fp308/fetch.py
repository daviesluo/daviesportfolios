"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp308/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp308-research"}
...
def pull_okx() -> dict[int, tuple]:
    after = c.fp5.SCREEN_END_MS
    rows: dict[int, tuple] = {}
    for _page in range(20):
        ...
        if payload.get("code") != "0":
            raise SystemExit("OKX refused the candles")
        data = payload["data"]
        if not data:
            break
        for row in data:
        ...
        if oldest <= c.LOOKBACK_MS:
            break
        after = oldest
        time.sleep(0.15)
    check_fair(rows, "OKX")
    save("ox1d/BTC-USDT.json", rows)
    return rows
...
def main() -> None:
    ...
    fair = pull_okx()
    print(f"fp308 {len(traded)} {len(fair)}")
