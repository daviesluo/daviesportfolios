"""Pull BTCUSDT hourly quote volume and daily opens for the fp22 screen.
...
entry. A later open is a bug. Prices are not stored on the hourly file.

    python3 docs/agents/scripts/fp22/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp22-research"}
...
def main() -> None:
    ...
    print(
        f"fp22 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
