"""Score the fp22 screen. 2023 only.

    python3 docs/agents/scripts/fp22/measure.py
"""
...
DATA = Path(os.environ.get("FP22_DATA", "/tmp/fp22/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp22-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "VOL-HHI", c.hhi_trades(daily, hours), pool, c.MIN_N,
        "Herfindahl of the day's 24 hourly quote shares, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp22_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp22_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp22/screen_2023.json"
