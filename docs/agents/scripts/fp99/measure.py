"""Score the fp99 screen. 2023 only.

    python3 docs/agents/scripts/fp99/measure.py
"""
...
DATA = Path(os.environ.get("FP99_DATA", "/tmp/fp99/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp99-protocol.md"
...
def main() -> None:
    if os.environ.get("FP99_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")

    alt_path = DATA / "spot1d" / "ADAUSDT.json"
    ...
    for trade in trades:
        if trade["coin"] != "ADAUSDT":
            raise SystemExit("a fill is not ADA")
    ...
    pool = c.fp5.pool_from_bars({"ADAUSDT": alt}, ["ADAUSDT"], c.fp5.DAY_MS)
    row = c.fp5.summarise(
        "ADA-Q", trades, pool, c.MIN_N,
        "ADA's daily quote volume over BTC's, above its own trailing 90th. Next day, long ADA.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp99_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp99_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp99/screen_2023.json"
