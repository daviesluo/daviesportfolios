"""Score the fp46 screen. 2023 only.

    python3 docs/agents/scripts/fp46/measure.py
"""
...
DATA = Path(os.environ.get("FP46_DATA", "/tmp/fp46/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp46-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "CLOSE-MEAN", c.mean_trades(daily, hours), pool, c.MIN_N,
        "Last close over the equal-weighted mean of the day's closes, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp46_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp46_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp46/screen_2023.json"
