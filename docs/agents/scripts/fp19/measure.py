"""Score the fp19 screen. 2023 only.

    python3 docs/agents/scripts/fp19/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp19-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "CLOSE-LOC", c.loc_trades(btc), pool, c.MIN_N,
        "Close location in the day's range above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp19_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp19_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp19/screen_2023.json"
