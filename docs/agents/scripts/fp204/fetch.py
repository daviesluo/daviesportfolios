"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp204/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp204-research"}
...
def main() -> None:
    ...
    mark = pull_price("mark", (1, 4), "mark1d/BTCUSDT.json", last)
    cm_sig = pull_klines("cm", "BTCUSD_PERP", "cmret/BTCUSD_PERP.json", (1, 4), last, (2022, 12))
    cm = pull_klines("cm", "BTCUSD_PERP", "cm1d/BTCUSD_PERP.json", (1,), session, (2022, 12))
    cm = add_exit(cm, "cm", "BTCUSD_PERP", (1,), "cm1d/BTCUSD_PERP.json")
    um = pull_klines("um", "BTCUSDT", "um1d/BTCUSDT.json", (1,), session, (2022, 12))
    um = add_exit(um, "um", "BTCUSDT", (1,), "um1d/BTCUSDT.json")
    print("fp204 " + f"mark {len(mark)} cmret {len(cm_sig)} cm {len(cm)} um {len(um)}")
