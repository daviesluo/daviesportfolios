"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp322/fetch.py
"""
...
DATA = Path(os.environ.get("FP322_DATA", "/tmp/fp322/data"))
VISION = "https://data.binance.vision"
UA = {"User-Agent": "Mozilla/5.0 daviesportfolios-fp322-research"}
...
def pull_rate() -> dict[int, str]:
    if c.FAIR_KIND != "rate":
        raise SystemExit("this pull is the exchange rate")
    base = (
        "https://www.binance.com/bapi/earn/v1/public/pos/cftoken/project/"
        "exchange-rate/history?projectId=BETH&pageSize=100&pageIndex="
    )
    rows: dict[int, str] = {}
    page = 1
    while page <= 30:
        payload = json.loads(get(base + str(page)).decode())
        if not payload.get("success"):
            raise SystemExit("the exchange-rate history was refused")
        batch = payload["data"]
        if not batch:
            break
        oldest = None
        for row in batch:
            if "exchangeRate" not in row or "calcDate" not in row:
                raise SystemExit("an exchange-rate row is missing a field")
            ts = int(row["calcDate"])
            px = row["exchangeRate"]
            if Decimal(px) <= 0:
                raise SystemExit("an exchange rate is not a price")
            oldest = ts if oldest is None else min(oldest, ts)
            if ts >= c.fp5.SCREEN_END_MS:
                continue
            if ts in rows and rows[ts] != px:
                raise SystemExit("two exchange rates at one stamp")
            rows[ts] = px
        if oldest is not None and oldest <= c.FAIR_FIRST:
            break
        if len(batch) < 100:
            break
        page += 1
    ...
    if (
        ...
        raise SystemExit("the exchange rate does not have the frozen shape")
...
def main() -> None:
    if os.environ.get("FP322_OOS"):
        raise SystemExit("this pull does not request a later year")
    opens = pull_opens()
    fair = pull_rate()
    print(f"fp322 {len(opens)} {len(fair)}")
