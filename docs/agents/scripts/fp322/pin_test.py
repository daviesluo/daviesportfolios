...
"""Pins for the fp322 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "WBRAT" or c.HOLD_DAYS != 2 or c.NULL_KIND != "other_regime":
        raise SystemExit("the idea moved")
    if c.MODE != "regime" or c.RULE_SIDE != "long" or c.RULE_BOOK != "spot":
        raise SystemExit("the leg moved")
    if c.FAIR_KIND != "rate" or c.COIN != "WBETHETH" or c.THRESHOLD_BPS != Decimal("25"):
    ...
    book = {}
    fair = {}
    # Previous close is the signal. The entry open is the fill only.
    book[entry - day] = (Decimal("100"), Decimal("100"))
    book[entry] = (Decimal("50"), Decimal("200"))
    book[entry + c.HOLD_DAYS * day] = (Decimal("55"), Decimal("200"))
    book[off - day] = (Decimal("10"), Decimal("100"))
    ...
    # Exactly twenty-five basis points under the fair value. The entry day's close is rich.
    fair[entry - 86_400_000] = Decimal("100.25")
    # A later stamp sitting on the entry must not replace that print.
    fair[entry] = Decimal("1")
    # Twenty-four basis points is the edge turned off, even though the entry close is cheap.
    fair[off - 86_400_000] = Decimal("100.24")
    fair[hole - 1000] = Decimal("100")
    ...
    late = dict(fair)
    late[entry] = Decimal("1")
    if [t["entry_ms"] for t in c.rule_trades(book, late)] != [entry]:
        raise SystemExit("a fair value stamped on the entry was used")
    missing = dict(fair)
    del missing[entry - 86_400_000]
    missing[entry] = Decimal("200")
    ...
    used_open = dict(book)
    used_open[entry - day] = (Decimal("1000"), Decimal("100"))
    ...
    rich_close[entry] = (Decimal("50"), Decimal("100"))
