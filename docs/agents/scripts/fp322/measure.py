"""Score the fp322 screen. 2023 entries only.

    python3 docs/agents/scripts/fp322/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp322-protocol.md"
NOTE = (
    "Fair value is Binance's published WBETH exchange rate, ETH per WBETH. The "
    "previous WBETHETH close is at least twenty-five basis points under that "
    "rate. Buy the spot book for two days. The other trade buys it for two days "
    "when that discount is absent. Same prices, same side, the edge turned off. "
    "One leg. The traded open is the fill only. The entry day's close is not an "
...
def run() -> None:
    ...
    if c.IDEA != "WBRAT" or c.MIN_N != 30 or c.HOLD_DAYS != 2 or c.THRESHOLD_BPS != Decimal("25"):
    ...
    payload = {
        ...
        "code_sha256": {
            "fp322_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp322_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp322/screen_2023.json"
