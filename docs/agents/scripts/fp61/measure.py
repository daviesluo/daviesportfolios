"""Score the fp61 screen. 2023 only.

    python3 docs/agents/scripts/fp61/measure.py
"""
...
DATA = Path(os.environ.get("FP61_DATA", "/tmp/fp61/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp61-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "LAST-HOUR", c.lasth_trades(daily, hours), pool, c.MIN_N,
        "Last hour's return, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp61_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp61_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp61/screen_2023.json"
