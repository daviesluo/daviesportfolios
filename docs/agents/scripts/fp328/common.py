"""The search fp328. Fair value is relative producer prices. The US finished-goods producer price index and the euro-area producer price index are each rebased to December 2022, scaled by the ECB reference rate of 2022-12-30, and divided by the Coin Metrics BUSD PriceUSD that had already completed. The result is quoted in the same unit as EURBUSD. The previous EURBUSD close is at least five hundred basis points under that price. Buy the spot book for two days. The other trade buys it for two days when that discount is absent.
...
BLS_PUB_DAY = 20
EA_SOURCE = "ecb"
ECB_SERIES = "STS/M.I9.N.PRON.NS0020.4.000"
ECB_KEY = "STS.M.I9.N.PRON.NS0020.4.000"
ECB_LAG = 2
ECB_PUB_DAY = 10
NOTE = (
    "Fair value is 1.0666 times the US finished-goods PPI, rebased to December 2022, divided by the euro-area producer price index rebased to the same month, divided by the Coin Metrics BUSD price that had already completed. The previous EURBUSD close is at least five hundred basis points under that price. Buy the spot book for two days. The other trade buys it for two days when that discount is absent. Same prices, same side, the edge turned off. One leg. The traded open is the fill only. The entry day's close is not an input. Another venue's close is not an input. A book ticker is not read."
)
