"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp209/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp209-research"}
...
def main() -> None:
    ...
    spot = pull_klines("spot", "BTCUSDT", (1, 4, 5, 7), session, (2022, 12))
    um = pull_klines("um", "BTCUSDT", (1, 4, 5, 7), session, (2022, 12))
    project(spot, (1, 2, 3), last, "spotsig/BTCUSDT.json", True)
    project(um, (1, 2, 3), last, "umsig/BTCUSDT.json", True)
    project(spot, (0, 1), session, "spot1d/BTCUSDT.json", True)
    project(um, (0, 1), session, "um1d/BTCUSDT.json", True)
    print("fp209 " + f"spot {len(spot)} um {len(um)}")
