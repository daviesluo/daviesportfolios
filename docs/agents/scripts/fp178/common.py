"""The search fp178. An upper wick longer than the body, then a one-day short.
...
def signal_days(bars: dict[int, tuple], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Up days whose upper wick is strictly longer than the body."""
    out = []
    for day in sorted(bars):
        ...
        if close_px > open_px and (high - close_px) > (close_px - open_px):
            out.append(day)
    return out
