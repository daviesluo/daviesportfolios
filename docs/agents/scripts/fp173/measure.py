"""Score the fp173 screen. 2023 entries only.

    python3 docs/agents/scripts/fp173/measure.py
"""
...
DATA = Path(os.environ.get("FP173_DATA", "/tmp/fp173/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp173-protocol.md"
NOTE = 'The day sits strictly inside the high and the low of three days earlier. Next day, long BTC from the open to the close.'
...
def main() -> None:
    if os.environ.get("FP173_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    if c.IDEA != 'COIL' or c.MIN_N != 30:
        raise SystemExit("the frozen name or the count moved")

    if c.HORIZON != 3:
        raise SystemExit("the horizon moved")
    ...
    for trade in trades:
        ...
        old = day - 3 * c.fp5.DAY_MS
        if not (bars[day][1] < bars[old][1] and bars[day][2] > bars[old][2]):
            raise SystemExit("a trade was not inside the range of three days ago")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp173_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp173_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp173/screen_2023.json"
