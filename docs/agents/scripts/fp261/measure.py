"""Score the fp261 screen. 2023 entries only.

    python3 docs/agents/scripts/fp261/measure.py
"""
...
DATA = Path(os.environ.get("FP261_DATA", "/tmp/fp261/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp261-protocol.md"
NOTE = 'The finished eleven-day coin-margined return was below the finished eleven-day spot return. The next USDT open is sold and bought back six days later. The other trade sells the coin-margined book for those same six days. One leg. Funding cash is not added. The null is not that USDT short with the comparison turned off.'
...
def run() -> None:
    ...
    if c.IDEA != 'LAGB' or c.MIN_N != 30 or c.HOLD_DAYS != 6:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp261_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp261_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp261/screen_2023.json"
