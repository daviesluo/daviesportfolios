"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp206/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp206-research"}
...
def pull_funding() -> dict[int, float]:
    rows: dict[int, float] = {}
    last_legal = c.fp5.SCREEN_END_MS - c.fp5.EIGHT_H_MS
    floor = c.fp5.SCREEN_END_MS - c.fp5.DAY_MS - c.fp5.EIGHT_H_MS
    for ym in months((2023, 1), (2023, 12)):
        url = (
            f"{VISION}/data/futures/cm/monthly/fundingRate/BTCUSD_PERP/"
            f"BTCUSD_PERP-fundingRate-{ym}.zip"
        )
        for row in read_zip_rows(url):
            if not row or row[0] in ("calc_time", "fundingTime") or len(row) < 3:
                continue
            bucket = c.fp5.bucket_8h(int(row[0]))
            if bucket is None or bucket >= c.fp5.SCREEN_END_MS or bucket > last_legal:
                continue
            hour = bucket % c.fp5.DAY_MS
            if hour not in (0, 2 * c.fp5.EIGHT_H_MS):
                continue
            if bucket in rows:
                raise SystemExit(f"two funding prints share {bucket}")
            rows[bucket] = float(row[2])
    if floor not in rows or (floor - 2 * c.fp5.EIGHT_H_MS) not in rows:
        raise SystemExit("the last published day is missing a 00:00 or 16:00 print")
    if any(ts >= c.fp5.SCREEN_END_MS for ts in rows):
        raise SystemExit("a 2024 funding print was stored")
    save("funding/BTCUSD_PERP.json", rows)
    return rows
...
def main() -> None:
    ...
    cm = pull_klines("cm", "BTCUSD_PERP", "cm1d/BTCUSD_PERP.json", (1, 4), session, (2022, 12))
    cm = add_exit(cm, "cm", "BTCUSD_PERP", (1, 4), "cm1d/BTCUSD_PERP.json")
    funding = pull_funding()
    print("fp206 " + f"cm {len(cm)} last {max(cm)} funding {len(funding)} last {max(funding)}")
