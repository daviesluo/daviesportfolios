"""Score the fp136 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp136/measure.py
"""
...
DATA = Path(os.environ.get("FP136_DATA", "/tmp/fp136/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp136-protocol.md"
NOTE = "Intraday dollar open interest, maximum minus minimum, divided by the last print, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP136_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "metrics/oi_range.json")
    ...
    row = c.fp5.summarise("OIRNG", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp136_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp136_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "oi_range": sha256(DATA / "metrics/oi_range.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp136/screen_2023.json"
