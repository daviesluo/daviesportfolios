"""Pull dollar open interest for the fp136 screen. The level is not stored as the signal. No other metrics column is stored. No funding print is requested. No hourly price bar is requested.

    python3 docs/agents/scripts/fp136/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp136-research"}
...
COLUMN = 3
COLUMN_NAME = "sum_open_interest_value"
METRICS_REL = "metrics/oi_range.json"
...
def pull_metrics() -> dict[int, tuple]:
    ...
    if METRICS_HEADER[COLUMN] != COLUMN_NAME:
        raise SystemExit("the metrics column moved")
    out: dict[int, tuple] = {}
    for ymd in days:
        url = (
