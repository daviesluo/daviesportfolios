"""Score the fp50 screen. 2023 only.

    python3 docs/agents/scripts/fp50/measure.py
"""
...
DATA = Path(os.environ.get("FP50_DATA", "/tmp/fp50/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp50-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "HIGH-HOUR", c.hour_trades(daily, hours), pool, c.MIN_N,
        "Hour index of the day's high, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp50_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp50_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp50/screen_2023.json"
