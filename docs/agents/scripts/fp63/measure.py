"""Score the fp63 screen. 2023 only.

    python3 docs/agents/scripts/fp63/measure.py
"""
...
DATA = Path(os.environ.get("FP63_DATA", "/tmp/fp63/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp63-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "HIGH-BASE", c.highbase_trades(daily, hours), pool, c.MIN_N,
        "Base share of the earliest high hour, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp63_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp63_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp63/screen_2023.json"
