"""Score the fp123 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp123/measure.py
"""
...
DATA = Path(os.environ.get("FP123_DATA", "/tmp/fp123/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp123-protocol.md"
NOTE = "Count of distinct BTC option strikes with positive open contracts at the day's last hour, above its own trailing 90th. Next day, long BTC for one day."
...
def load_book(path: Path) -> dict[int, tuple]:
    ...
    out = {}
    for row in rows:
        if len(row) < 1 + c.WIDTH or (len(row) - 1) % c.WIDTH != 0:
...
def main() -> None:
    if os.environ.get("FP123_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "options" / "strikes.json")
    ...
    row = c.fp5.summarise("STRIKE", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp123_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp123_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "strikes": sha256(DATA / "options" / "strikes.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp123/screen_2023.json"
