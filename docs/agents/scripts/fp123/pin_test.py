"""Pins for the fp123 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    row = (23, 30000.0, 1.0, 23, 30000.0, 2.0, 23, 40000.0, 0.0, 0, 50000.0, 9.0)
    if not near(c.signal_at({day: row}, day), 1.0):
        raise SystemExit("same strike or an earlier hour counted twice")
    two = (23, 30000.0, 1.0, 23, 40000.0, 1.0)
    if not near(c.signal_at({day: two}, day), 2.0):
        raise SystemExit("two strikes counted as one")
    if c.signal_at({day: (23, 30000.0, 0.0)}, day) is not None:
        raise SystemExit("a zero open interest was a strike")
...
def test_tail_and_fill() -> None:
    ...
    rows[last] = (23, 1.0, 1.0, 23, 2.0, 1.0, 23, 3.0, 1.0, 23, 4.0, 1.0, 23, 5.0, 1.0)
