"""Pull the public series the fp123 screen is allowed to see.
...
requested. ETH, BNB, DOGE and XRP option files are not requested.
    ...
    python3 docs/agents/scripts/fp123/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp123-research"}
SPOT_FIELDS = (1,)
...
def pull_options() -> dict[int, tuple]:
    ...
    for ymd in days:
        ...
        for row in reader:
            ...
            if value is None:
                continue
            out_rows.append((float(hour), parsed[1], value))
