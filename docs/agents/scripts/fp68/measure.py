"""Score the fp68 screen. 2023 only.

    python3 docs/agents/scripts/fp68/measure.py
"""
...
DATA = Path(os.environ.get("FP68_DATA", "/tmp/fp68/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp68-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "CROSS", c.cross_trades(daily, hours), pool, c.MIN_N,
        "Crosses of yesterday's close, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp68_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp68_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp68/screen_2023.json"
