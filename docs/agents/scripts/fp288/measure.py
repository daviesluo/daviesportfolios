"""Score the fp288 screen. 2023 entries only.

    python3 docs/agents/scripts/fp288/measure.py
"""
...
DATA = Path(os.environ.get("FP288_DATA", "/tmp/fp288/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp288-protocol.md"
NOTE = 'The lowest of the last eight finished spot closes was strictly above the lowest of the eight closes before those, and the finished eight-day spot return was positive. The next spot open is bought and sold fifteen days later. The other trade buys spot fifteen days later for another fifteen days. Both windows have to fill. One leg. Funding cash is not added. The null is not that fifteen-day long on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'LIFTL' or c.MIN_N != 30 or c.HOLD_DAYS != 15:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp288_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp288_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp288/screen_2023.json"
