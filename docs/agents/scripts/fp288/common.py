"""The search fp288. The lowest of the last eight finished spot closes was strictly above the lowest of the eight closes before those, and the finished eight-day spot return was positive. Buy spot for fifteen days. The other trade buys spot fifteen days later for another fifteen days. Both windows have to fill. The entry open is not an input. One leg.
...
RULE_SIDE = 'long'
NULL_SIDE = 'long'
FILES = {'spot': ('spot1d/BTCUSDT.json', True)}
PULLS = [('spot', 'BTCUSDT', 'spot1d', True)]
...
def _signal(spot, um, cm, entry) -> bool:
    recent = _closes(spot, entry - fp5.DAY_MS, 8)
    prior = _closes(spot, entry - fp5.DAY_MS - 8 * fp5.DAY_MS, 8)
    trend = finished(spot, entry, 8)
    if recent is None or prior is None or trend is None:
        return False
    return min(recent) > min(prior) and trend > 0.0
