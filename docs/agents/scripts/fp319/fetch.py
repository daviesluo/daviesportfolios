"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp319/fetch.py
"""
...
UA = {"User-Agent": "Mozilla/5.0 daviesportfolios-fp319-research"}
...
def pull_funding() -> dict[int, float]:
    ...
    while cursor < end:
        url = f"{HISTORY}?symbol=ETHUSDT&startTime={cursor}&endTime={end}&limit=1000"
...
def main() -> None:
    ...
    opens = pull_opens()
    fund = pull_funding()
    print(f"fp319 {len(opens)} {len(fund)}")

    sofr = pull_sofr()
    print(f"fp319 sofr {len(sofr)}")
