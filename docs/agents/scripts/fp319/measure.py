"""Score the fp319 screen. 2023 entries only.

    python3 docs/agents/scripts/fp319/measure.py
"""
...
DATA = Path(os.environ.get("FP319_DATA", "/tmp/fp319/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp319-protocol.md"
NOTE = "Fair value is the price index grossed up by the already-published SOFR carry. The last settled ETHUSDT funding rate is strictly below that eight-hour SOFR rate, so the contract finished cheap to the cash-and-carry. Buy the ETH USDT book for two days. The other trade buys it for two days when the settled rate is not below that SOFR rate. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side. The traded open is the fill only. Another venue's close is not an input. A book ticker is not read."
...
def run() -> None:
    book = load_opens(DATA / "um1d" / "ETHUSDT.json")
    fund = load_fund(DATA / "fund" / "ETHUSDT.json")
    inputs = {
        "um": sha256(DATA / "um1d" / "ETHUSDT.json"),
        "fund": sha256(DATA / "fund" / "ETHUSDT.json"),
    }
    ...
    if c.IDEA != "ESOFR" or c.MIN_N != 30 or c.HOLD_DAYS != 2 or c.FAIR_KIND != "sofr":
    ...
    payload = {
        ...
        "code_sha256": {
            "fp319_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp319_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp319/screen_2023.json"
