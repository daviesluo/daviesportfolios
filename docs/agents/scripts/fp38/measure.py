"""Score the fp38 screen. 2023 only.

    python3 docs/agents/scripts/fp38/measure.py
"""
...
DATA = Path(os.environ.get("FP38_DATA", "/tmp/fp38/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp38-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "HALF-DIFF", c.half_trades(daily, hours), pool, c.MIN_N,
        "Second-half return minus first-half return, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp38_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp38_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp38/screen_2023.json"
