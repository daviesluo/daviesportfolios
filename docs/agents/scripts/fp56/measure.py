"""Score the fp56 screen. 2023 only.

    python3 docs/agents/scripts/fp56/measure.py
"""
...
DATA = Path(os.environ.get("FP56_DATA", "/tmp/fp56/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp56-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "CLOSE-HIGHS", c.closehigh_trades(daily, hours), pool, c.MIN_N,
        "Count of hours that break the day's running close, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp56_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp56_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp56/screen_2023.json"
