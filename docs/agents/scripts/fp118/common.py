"""The search fp118. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the day's population standard deviation of the top-trader position ratio, divided by its mean. No hourly price bar is read. No funding print is read.
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Population standard deviation of the position ratio, divided by its mean."""
    if day >= end_ms:
        return None
    vals = rows.get(day)
    if vals is None or len(vals) < 2 or any(v <= 0 for v in vals):
        return None
