"""Score the fp118 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp118/measure.py
"""
...
DATA = Path(os.environ.get("FP118_DATA", "/tmp/fp118/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp118-protocol.md"
NOTE = "Population dispersion of the day's top-trader position ratio, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP118_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_series(DATA / "metrics/top_std.json")
    ...
    row = c.fp5.summarise("TOPSTD", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp118_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp118_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "top_std": sha256(DATA / "metrics/top_std.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp118/screen_2023.json"
