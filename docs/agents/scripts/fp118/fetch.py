"""Pull the public series the fp118 screen is allowed to see.

The signal stores every top-trader position ratio in the UTC day. The standard deviation is not stored. Account-count ratios are not stored. No hourly price bar is requested. No funding print is requested.
No alt quote is requested. No bookDepth file is requested. Count long/short
and taker columns are not stored.

    python3 docs/agents/scripts/fp118/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp118-research"}
SPOT_FIELDS = (1,)
...
COLUMNS = (5,)
METRICS_REL = "metrics/top_std.json"
METRICS_MODE = "series"
...
def main() -> None:
    ...
    bars = pull_spot()
    print("fp118 " + f"metrics {len(metrics)} first {min(metrics)} last {max(metrics)}" + "; " + f"BTCUSDT 1d {len(bars)} first {min(bars)} last {max(bars)}")
