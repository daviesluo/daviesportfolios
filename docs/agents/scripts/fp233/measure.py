"""Score the fp233 screen. 2023 entries only.

    python3 docs/agents/scripts/fp233/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp233-protocol.md"
NOTE = "Yesterday's spot close was above each of the ten prior closes. The next open is bought and sold eight days later. One spot leg. Funding cash is not added. The null is that eight-day long on every day."
...
def check_signal(trade: dict, books: dict) -> None:
    ...
    newest = c._px(spot, end, 1)
    prior = [c._px(spot, end - k * c.fp5.DAY_MS, 1) for k in range(1, 11)]
    if newest is None or any(px is None for px in prior) or not newest > max(prior):
        raise SystemExit("the close was not above the prior ten")


def run() -> None:
    end = c.fp5.SCREEN_END_MS
    last = end - c.fp5.DAY_MS
    books = {"spot": load_rows(DATA / "spot1d/BTCUSDT.json", 2, end)}
    trades = c.signal_trades(books['spot'])
    spans = c.signal_spans(books['spot'])
    ...
    pool_spans = c.pool_spans(books['spot'])
    pool = c.pool_nets(books['spot'])
    if c.IDEA != 'HI10' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp233_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp233_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp233/screen_2023.json"
