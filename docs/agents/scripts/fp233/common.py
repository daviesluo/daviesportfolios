"""The search fp233. A finished spot close above the prior ten closes, held eight days.

Nothing here reads the network or a file. The ten closes and yesterday's close have already printed. The entry open is not an input. The position is one
leg. It is not a one-day trade and it is not an overnight trade. Funding
cash is not added. The null is that same 8-day long on every
day.
"""
...
def _signal(spot: dict, entry: int) -> bool:
    end = entry - fp5.DAY_MS
    newest = _px(spot, end, 1)
    if newest is None:
        return False
    prior = []
    for k in range(1, 11):
        px = _px(spot, end - k * fp5.DAY_MS, 1)
        if px is None:
            return False
        prior.append(px)
    return newest > max(prior)
