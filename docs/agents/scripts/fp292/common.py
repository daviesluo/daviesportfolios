"""The search fp292. The basis is the USDT close over the spot close. That basis finished positive and strictly above the basis five days earlier. Sell the USDT book for eight days. The other trade sells it for eight days when that wider premium is absent.
...
RULE_SIDE = 'short'
NULL_SIDE = 'short'
FILES = {'spot': ('spot1d/BTCUSDT.json', False), 'um': ('um1d/BTCUSDT.json', True)}
PULLS = [('spot', 'BTCUSDT', 'spot1d', False), ('um', 'BTCUSDT', 'um1d', True)]
...
def _widen(spot, um, cm, entry):
    now = _basis(um, spot, entry, 0)
    old = _basis(um, spot, entry, 5)
