"""Score the fp292 screen. 2023 entries only.

    python3 docs/agents/scripts/fp292/measure.py
"""
...
DATA = Path(os.environ.get("FP292_DATA", "/tmp/fp292/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp292-protocol.md"
NOTE = 'The basis is the finished USDT close over the finished spot close, minus one. It finished positive and strictly above the basis five days earlier. The next USDT open is sold and bought back eight days later. The other trade sells the USDT book for eight days when that wider premium is absent. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side.'
...
def run() -> None:
    ...
    if c.IDEA != 'WIDEN' or c.MIN_N != 30 or c.HOLD_DAYS != 8 or c.NULL_KIND != 'other_regime':
    ...
    payload = {
        ...
        "code_sha256": {
            "fp292_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp292_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp292/screen_2023.json"
