"""Score the fp245 screen. 2023 entries only.

    python3 docs/agents/scripts/fp245/measure.py
"""
...
DATA = Path(os.environ.get("FP245_DATA", "/tmp/fp245/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp245-protocol.md"
NOTE = 'The coin-margined twenty-day return finished negative. The next USDT open is sold and bought back fifteen days later. One USDT-perpetual leg. Funding cash is not added. The null is that fifteen-day short on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    move = c.finished(books["cm"], trade["entry_ms"], 20)
    if move is None or not move < 0.0:
        raise SystemExit("the twenty-day coin-margined decline was not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'CDEC' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp245_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp245_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp245/screen_2023.json"
