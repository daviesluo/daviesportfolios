"""Score the fp240 screen. 2023 entries only.

    python3 docs/agents/scripts/fp240/measure.py
"""
...
DATA = Path(os.environ.get("FP240_DATA", "/tmp/fp240/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp240-protocol.md"
NOTE = "The coin-margined eight-day return finished above spot's eight-day return. The next spot open is bought and sold five days later. One spot leg. Funding cash is not added. The null is that five-day long on every day."
...
def check_signal(trade: dict, books: dict) -> None:
    cm_move = c.finished(books["cm"], trade["entry_ms"], 8)
    spot_move = c.finished(books["spot"], trade["entry_ms"], 8)
    if cm_move is None or spot_move is None or not cm_move > spot_move:
        raise SystemExit("the coin-margined book did not finish ahead of spot")
...
def run() -> None:
    ...
    trades = c.signal_trades(books['cm'], books['spot'])
    spans = c.signal_spans(books['cm'], books['spot'])
    ...
    pool_spans = c.pool_spans(books['cm'], books['spot'])
    pool = c.pool_nets(books['cm'], books['spot'])
    if c.IDEA != 'LEAD' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp240_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp240_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"spot": sha256(DATA / "spot1d/BTCUSDT.json"), "cm": sha256(DATA / "cm1d/BTCUSD_PERP.json")},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp240/screen_2023.json"
