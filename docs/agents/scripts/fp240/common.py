"""The search fp240. The coin-margined eight-day return finished above spot's, held five days.

Nothing here reads the network or a file. Both eight-day returns end at
yesterday's close. The entry open is not an input. The coin-margined book is
a signal, not a second leg. The position is one spot leg. It is not a
one-day trade and it is not an overnight trade. Funding cash is not added.
The null is that same five-day spot long on every day.
"""
...
def _signal(cm: dict, spot: dict, entry: int) -> bool:
    cm_move = finished(cm, entry, 8)
    spot_move = finished(spot, entry, 8)
    if cm_move is None or spot_move is None:
        return False
    return cm_move > spot_move
...
def pool_spans(cm: dict, spot: dict) -> list[tuple[int, int]]:
    out = []
    for entry in _days():
...
def pool_nets(cm: dict, spot: dict) -> list[float]:
    nets = []
    for entry, _exit_ms in pool_spans(cm, spot):
        trade = _fill(entry, spot)
        if trade is None:
