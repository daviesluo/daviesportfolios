"""Score the fp170 screen. 2023 entries only.

    python3 docs/agents/scripts/fp170/measure.py
"""
...
DATA = Path(os.environ.get("FP170_DATA", "/tmp/fp170/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp170-protocol.md"
NOTE = 'The 08:00 funding rate is strictly the lowest of the three. Next open, long BTC for one day. The funding cash is not added.'
...
def main() -> None:
    ...
    if any(day >= end for day in funding):
        raise SystemExit("a funding day is in 2024")
    ...
    for trade in trades:
        ...
        if not (rate_08 < rate_00 and rate_08 < rate_16):
            raise SystemExit("a trade was not a trough at 08:00")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp170_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp170_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"funding": sha256(DATA / "funding/BTCUSDT.json"), "spot_open": sha256(DATA / "spot1d/BTCUSDT.json")},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp170/screen_2023.json"
