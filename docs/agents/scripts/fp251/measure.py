"""Score the fp251 screen. 2023 entries only.

    python3 docs/agents/scripts/fp251/measure.py
"""
...
DATA = Path(os.environ.get("FP251_DATA", "/tmp/fp251/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp251-protocol.md"
NOTE = "Spot's finished six-day return was below the USDT six-day return. The next USDT open is sold and bought back seventeen days later. One USDT-perpetual leg. Funding cash is not added. The null is that seventeen-day short on every day."
...
def check_signal(trade: dict, books: dict) -> None:
    if not c._signal(books["spot"], books["um"], trade["entry_ms"]):
        raise SystemExit("spot had not lagged the USDT book")
...
def run() -> None:
    ...
    trades = c.signal_trades(books["spot"], books["um"])
    spans = c.signal_spans(books["spot"], books["um"])
    ...
    pool_spans = c.pool_spans(books["spot"], books["um"])
    pool = c.pool_nets(books["spot"], books["um"])
    if c.IDEA != 'LAGU' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp251_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp251_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp251/screen_2023.json"
