"""The search fp251. Spot's finished six-day return lagged the USDT return, then a seventeen-day USDT short.
...
not added. The null is that same 17-day short on every day.
"""
...
def _signal(spot: dict, um: dict, entry: int) -> bool:

    spot_move = finished(spot, entry, 6)
    um_move = finished(um, entry, 6)
...
def pool_spans(spot: dict, um: dict) -> list[tuple[int, int]]:
    out = []
    for entry in _days():
...
def pool_nets(spot: dict, um: dict) -> list[float]:
    nets = []
    for entry, _exit_ms in pool_spans(spot, um):
        trade = _fill(entry, um)
        if trade is None:
