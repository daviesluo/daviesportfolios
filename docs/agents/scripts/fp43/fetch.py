"""Pull BTCUSDT bars for the fp43 screen.

Hourly bars store the open, the close and quote volume. High, low, base volume and trade count are not stored.
...
entry. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp43/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp43-research"}
...
def main() -> None:
    ...
    print(
        f"fp43 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
