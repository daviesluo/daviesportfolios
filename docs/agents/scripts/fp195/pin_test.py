from __future__ import annotations

"""Pins for the fp195 rule. No file and no return from 2023 is read."""

import sys
from decimal import Decimal
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent))

import common as c


def _long(entry: Decimal, exit_px: Decimal) -> Decimal:
    fee = Decimal("0.001")
    return (exit_px * (1 - fee)) / (entry * (1 + fee)) - 1


def _short(entry: Decimal, exit_px: Decimal) -> Decimal:
    fee = Decimal("0.001")
    return (entry * (1 - fee)) / (exit_px * (1 + fee)) - 1


def test_vwap_hedge() -> None:
    if c.IDEA != "VWAPH" or c.MIN_N != 30:
        raise SystemExit("the idea moved")
    signal = c.fp5.SCREEN_START_MS - c.fp5.DAY_MS
    entry = c.fp5.SCREEN_START_MS
    later = entry + c.fp5.DAY_MS
    # Signal-day averages: UM quote/base = 30/1, spot = 20/1. Entry prices differ.
    spot = {
        signal: (1.0, 1.0, 1.0, 20.0),
        entry: (100.0, 110.0, 9.0, 1800.0),
        later: (50.0, 40.0, 1.0, 80.0),
    }
    um = {
        signal: (1.0, 1.0, 1.0, 30.0),
        entry: (200.0, 250.0, 9.0, 900.0),
        later: (80.0, 70.0, 1.0, 40.0),
    }
    trades = c.signal_trades(spot, um)
    if len(trades) != 1 or trades[0]["entry_ms"] != entry or trades[0]["exit_ms"] != entry:
        raise SystemExit("the next hedge did not fill")
    want = _long(Decimal(100), Decimal(110)) + _short(Decimal(200), Decimal(250))
    if abs(trades[0]["net"] - float(want)) > 1e-12:
        raise SystemExit("the hedge fill moved")
    gross = (110.0 / 100.0 - 1.0) + (200.0 / 250.0 - 1.0)
    if abs(trades[0]["gross"] - gross) > 1e-12:
        raise SystemExit("the gross is not both legs")
    if abs(trades[0]["gross"] - (110.0 / 100.0 - 1.0)) < 1e-12:
        raise SystemExit("the short leg was dropped")
    if abs(trades[0]["gross"] - (30.0 / 20.0 - 1.0)) < 1e-12:
        raise SystemExit("the gross is the signal-day average")
    equal = dict(um)
    equal[signal] = (1.0, 1.0, 1.0, 20.0)
    if c.signal_trades(spot, equal):
        raise SystemExit("an equal average fired")
    under = dict(um)
    under[signal] = (1.0, 1.0, 1.0, 10.0)
    if c.signal_trades(spot, under):
        raise SystemExit("a cheaper perpetual average fired")
    late_day = c.fp5.SCREEN_END_MS - c.fp5.DAY_MS
    if c.signal_trades(
        {late_day: (1.0, 1.0, 1.0, 20.0), c.fp5.SCREEN_END_MS: (100.0, 110.0, 1.0, 100.0)},
        {late_day: (1.0, 1.0, 1.0, 30.0), c.fp5.SCREEN_END_MS: (200.0, 250.0, 1.0, 200.0)},
    ):
        raise SystemExit("a 2024 open was an entry")
    pool = c.pool_nets(spot, um)
    if len(pool) != 2 or abs(pool[0] - float(want)) > 1e-12:
        raise SystemExit("the null is not the hedge on every day")
    naked = _long(Decimal(100), Decimal(110))
    if abs(pool[0] - float(naked)) < 1e-9:
        raise SystemExit("the null is an unconditional long")


def main() -> None:
    test_vwap_hedge()
    print("fp195 pins ok")


if __name__ == "__main__":
    main()
