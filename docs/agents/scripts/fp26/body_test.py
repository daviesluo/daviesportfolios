"""The pre-registered BODY test. Scores 2024-01-01 through 2026-09-24.
...
when it was frozen. Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp26/body_test.py
"""
...
PREREG = ROOT / "docs/agents/reviews/2026-09-25-fp26-prereg-body.md"
SIDECAR = ROOT / "docs/agents/reviews/2026-09-25-fp26-prereg-body.sha256"

# Measured 2026-09-25T02:37:30Z from data-api bookTicker BTCUSDT, bid 84514.26 / ask 84514.27.
HALF_SPREAD = 5.916161022824505e-08
...
def main() -> None:
    ...
    if not oos_path.exists():
        os.environ["FP26_OOS"] = "1"
        fetch.pull_oos()
    screen_daily = load_bars(screen_path)
    screen = c.body_trades(screen_daily)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    screen_n = len(screen)
    if screen_n != 46 or abs(screen_total - 49.2172) > 0.01:
        raise SystemExit(f"2023 did not reproduce: n={screen_n} total={screen_total}")

    oos_daily = load_bars(oos_path)
    if max(oos_daily) != OOS_END:
        raise SystemExit("the 2026-09-25 open is missing")
    if min(oos_daily) != OOS_START:
        raise SystemExit("the out-of-sample file does not start on 2024-01-01")
    daily = dict(screen_daily)
    daily.update(oos_daily)
    exit_open = oos_daily[OOS_END][0]
    if daily[OOS_END][1] != exit_open or daily[OOS_END][2] != exit_open or daily[OOS_END][3] != exit_open:
        raise SystemExit("the 2026-09-25 bar stored more than its open")
    primary = c.body_trades(daily, 0.90, FEE, OOS_START, OOS_END)
    doubled = c.body_trades(daily, 0.90, FEE_DOUBLE, OOS_START, OOS_END)
    q80 = c.body_trades(daily, 0.80, FEE, OOS_START, OOS_END)
    q95 = c.body_trades(daily, 0.95, FEE, OOS_START, OOS_END)
    ...
    payload = {
        ...
        "input_sha256": {
            "screen_btc": hashlib.sha256(screen_path.read_bytes()).hexdigest(),
            "oos_btc": hashlib.sha256(oos_path.read_bytes()).hexdigest(),
        },
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp26/body_oos.json"
