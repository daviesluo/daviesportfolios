"""Score the fp88 screen. 2023 only.

    python3 docs/agents/scripts/fp88/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp88-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "HOLD", trades, pool, c.MIN_N,
        "Today's low above yesterday's high, in units of yesterday's range, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp88_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp88_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp88/screen_2023.json"
