"""Pull bars for the fp88 screen.

Daily bars store the open, the high and the low. The close is not stored.
The 2024-01-01 bar keeps its open and stores the high and the low equal to that open,
so the bar can be an exit and cannot be a print.

    python3 docs/agents/scripts/fp88/fetch.py
"""
...
DATA = Path(os.environ.get("FP88_DATA", "/tmp/fp88/data"))
MARKET = "https://data-api.binance.vision"
LOOKBACK_MS = 1_664_582_400_000  # 2022-10-01 00:00 UTC
UA = {"User-Agent": "daviesportfolios-fp88-research"}
...
def main() -> None:
    if os.environ.get("FP88_OOS"):
        raise SystemExit("this pull does not request a later year")
    rows = klines("BTCUSDT", "1d", c.fp5.DAY_MS, c.fp5.SCREEN_END_MS, (1, 2, 3))
    ...
    print(f"fp88 BTCUSDT 1d {len(rows)} first {min(rows)} last {max(rows)}")
