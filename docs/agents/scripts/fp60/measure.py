"""Score the fp60 screen. 2023 only.

    python3 docs/agents/scripts/fp60/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp60-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "OPEN-YLOW", c.ylow_trades(daily), pool, c.MIN_N,
        "Open over the previous day's low, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp60_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp60_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp60/screen_2023.json"
