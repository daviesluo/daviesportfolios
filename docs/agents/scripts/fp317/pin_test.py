...
"""Pins for the fp317 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "USOFR" or c.HOLD_DAYS != 2 or c.NULL_KIND != "other_regime":
        raise SystemExit("the idea moved")
    if c.MODE != "regime" or c.RULE_SIDE != "long" or c.RULE_BOOK != "um":
        raise SystemExit("the leg moved")
    if c.FAIR_KIND != "sofr" or c.COIN != "BTCUSDT":
