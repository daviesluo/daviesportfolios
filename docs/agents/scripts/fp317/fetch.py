"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp317/fetch.py
"""
...
UA = {"User-Agent": "Mozilla/5.0 daviesportfolios-fp317-research"}
...
def main() -> None:
    ...
    opens = pull_opens()
    fund = pull_funding()
    print(f"fp317 {len(opens)} {len(fund)}")

    sofr = pull_sofr()
    print(f"fp317 sofr {len(sofr)}")
