"""Score the fp121 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp121/measure.py
"""
...
DATA = Path(os.environ.get("FP121_DATA", "/tmp/fp121/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp121-protocol.md"
NOTE = "BTC put open contracts over call open contracts at the day's last hour, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP121_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "options" / "pc_oi.json")
    ...
    row = c.fp5.summarise("PCOI", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp121_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp121_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "pc_oi": sha256(DATA / "options" / "pc_oi.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp121/screen_2023.json"
