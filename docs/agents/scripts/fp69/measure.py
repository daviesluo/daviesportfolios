"""Score the fp69 screen. 2023 only.

    python3 docs/agents/scripts/fp69/measure.py
"""
...
DATA = Path(os.environ.get("FP69_DATA", "/tmp/fp69/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp69-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "EXPAND", c.expand_trades(daily, hours), pool, c.MIN_N,
        "Hours that extend the day's range, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp69_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp69_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp69/screen_2023.json"
