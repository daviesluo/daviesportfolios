"""The sixty-ninth search (fp69). The fill and the null are fp5's.

Nothing here reads the network or a file. An hourly bar is (high, low).
A daily bar is (open,). The signal counts hours after the first that
extend the day's running high or the day's running low. An hour that only
beats the previous hour does not count. Closes are not stored. The fill
...
def expand_at(
    ...
    """Hours that extend the running high or the running low. None when an hour is missing."""
    if day >= end_ms:
        return None
    run_hi = None
    run_lo = None
...
def expand_prints(
    ...
    for day in range(start, end_ms, fp5.DAY_MS):
        value = expand_at(hours, day, end_ms)
        if value is None:
            continue
...
def expand_signal_days(
    hours: dict[int, tuple], q: float = 0.90, end_ms: int = fp5.SCREEN_END_MS,
) -> list[int]:
    return _upper(expand_prints(hours, end_ms), q)
...
def expand_trades(
    ...
    """Enter the next daily open after many range extensions. Hold one day."""
    trades = []
    for day in expand_signal_days(hours, q, end_ms):
