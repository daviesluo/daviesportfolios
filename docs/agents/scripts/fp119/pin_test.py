"""Pins for the fp119 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    if not near(c.signal_at({day: (23, 100.0)}, day), 100.0):
        raise SystemExit("the premium sum was wrong")
    if not near(c.signal_at({day: (0, 50.0, 23, 100.0, 23, 40.0)}, day), 140.0):
        raise SystemExit("an earlier hour entered the sum")
    if c.signal_at({day: (23, 0.0)}, day) is not None:
        raise SystemExit("a zero premium was a print")
    if c.signal_at({day: (23, -1.0)}, day) is not None:
        raise SystemExit("a negative premium was a print")
    if c.signal_at({day: (24, 100.0)}, day) is not None:
        raise SystemExit("hour 24 was a print")
    if c.signal_at({day: (23, 1.0, 2.0)}, day) is not None:
