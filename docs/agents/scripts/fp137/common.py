"""The search fp137. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the sum of today's three funding rates minus yesterday's sum. The level is not an input. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Today's three-rate sum minus yesterday's. Yesterday is one day earlier."""
    if day >= end_ms:
        return None
    today = rows.get(day)
    ...
    if today is None or prev is None:
        return None
    if len(today) != 3 or len(prev) != 3:
        return None
    return sum(today) - sum(prev)
