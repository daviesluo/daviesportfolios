...
def test_formula() -> None:
    ...
    rows = {prev: (1.0, 1.0, 1.0), day: (3.0, 3.0, 3.0)}
    if not near(c.signal_at(rows, day), 6.0):
        raise SystemExit("the jump was not the signal")
    if near(c.signal_at(rows, day), 9.0):
        raise SystemExit("the day's sum replaced the jump")
    down = {prev: (1.0, 1.0, 1.0), day: (1.0, -1.0, 0.0)}
    if not near(c.signal_at(down, day), -3.0):
        raise SystemExit("a negative rate was dropped")
    if c.signal_at({day: (1.0, 1.0, 1.0)}, day) is not None:
        raise SystemExit("a day with no yesterday was a print")
    if c.signal_at({prev: (1.0, 1.0), day: (1.0, 1.0, 1.0)}, day) is not None:
        raise SystemExit("two rates were a print")
    flat = {prev: (0.0, 0.0, 0.0), day: (0.0, 0.0, 0.0)}
    if c.signal_prints(flat) != [(day, 0.0)]:
        raise SystemExit("a zero jump left the history")
    later = c.fp5.SCREEN_END_MS
    if c.signal_at({later - c.fp5.DAY_MS: (1.0, 1.0, 1.0), later: (1.0, 1.0, 1.0)}, later) is not None:
        raise SystemExit("a 2024 day was a print")

"""Pins for the fp137 rule. No file and no return from 2023 is read."""
...
def test_tail_and_fill() -> None:
    ...
    for i in range(121):
        ...
        rows[day] = (3.0, 3.0, 3.0) if i == 120 else (1.0, 1.0, 1.0)
