"""Score the fp249 screen. 2023 entries only.

    python3 docs/agents/scripts/fp249/measure.py
"""
...
DATA = Path(os.environ.get("FP249_DATA", "/tmp/fp249/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp249-protocol.md"
NOTE = 'Spot and the coin-margined perpetual both finished a four-day decline. The next USDT open is bought and sold nineteen days later. One USDT-perpetual leg. Funding cash is not added. The null is that nineteen-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    if not c._signal(books["spot"], books["cm"], books["um"], trade["entry_ms"]):
        raise SystemExit("the two declines were not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'JOIN' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp249_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp249_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp249/screen_2023.json"
