"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp318/fetch.py
"""
...
UA = {"User-Agent": "Mozilla/5.0 daviesportfolios-fp318-research"}
...
def save_opens(rows: dict[int, float]) -> None:
    path = DATA / "cm1d" / "BTCUSD_PERP.json"
...
def pull_funding() -> dict[int, float]:
    ...
    while cursor < end:
        url = f"{HISTORY}?symbol=BTCUSD_PERP&startTime={cursor}&endTime={end}&limit=1000"
    ...
    path = DATA / "fund" / "BTCUSD_PERP.json"
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    opens = pull_opens()
    fund = pull_funding()
    print(f"fp318 {len(opens)} {len(fund)}")

    sofr = pull_sofr()
    print(f"fp318 sofr {len(sofr)}")
