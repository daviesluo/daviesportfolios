"""The search fp318. Fair value is the price index grossed up by the already-published SOFR carry. The last settled BTCUSD_PERP funding rate is strictly below that eight-hour SOFR rate, so the contract finished cheap to the cash-and-carry. Buy the coin-margined book for two days. The other trade buys it for two days when the settled rate is not below that SOFR rate.
