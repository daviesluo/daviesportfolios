"""The search fp250. A finished ten-day USDT rise smaller than the ten-day rise before it, then a twenty-six-day coin-margined short.
...
not added. The null is that same 26-day short on every day.
"""
...
def _signal(um: dict, cm: dict, entry: int) -> bool:

    near = finished(um, entry, 10)
    far = finished(um, entry - 10 * fp5.DAY_MS, 10)
    if near is None or far is None:
        return False
    return near > 0.0 and near < far
...
def signal_spans(um: dict, cm: dict) -> list[tuple[int, int]]:
    out = []
    for entry in _days():
        if not _signal(um, cm, entry):
            continue
...
def pool_spans(um: dict, cm: dict) -> list[tuple[int, int]]:
    out = []
    for entry in _days():
...
def signal_trades(um: dict, cm: dict) -> list[dict]:
    trades = []
    for entry, exit_ms in signal_spans(um, cm):
        trade = _fill(entry, cm)
...
def pool_nets(um: dict, cm: dict) -> list[float]:
    nets = []
    for entry, _exit_ms in pool_spans(um, cm):
        trade = _fill(entry, cm)
        if trade is None:
