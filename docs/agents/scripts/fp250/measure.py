"""Score the fp250 screen. 2023 entries only.

    python3 docs/agents/scripts/fp250/measure.py
"""
...
DATA = Path(os.environ.get("FP250_DATA", "/tmp/fp250/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp250-protocol.md"
NOTE = 'The finished ten-day USDT rise was positive and smaller than the ten-day rise before it. The next coin-margined open is sold and bought back twenty-six days later. One coin-margined leg. Funding cash is not added. The null is that twenty-six-day short on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    if not c._signal(books["um"], books["cm"], trade["entry_ms"]):
        raise SystemExit("the slower rise was not already finished")
...
def run() -> None:
    ...
    trades = c.signal_trades(books["um"], books["cm"])
    spans = c.signal_spans(books["um"], books["cm"])
    ...
    pool_spans = c.pool_spans(books["um"], books["cm"])
    pool = c.pool_nets(books["um"], books["cm"])
    if c.IDEA != 'FADE' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp250_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp250_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp250/screen_2023.json"
