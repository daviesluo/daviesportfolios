"""Score the fp41 screen. 2023 only.

    python3 docs/agents/scripts/fp41/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp41-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "STREAK", c.streak_trades(daily), pool, c.MIN_N,
        "Consecutive up closes, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp41_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp41_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp41/screen_2023.json"
