"""The pre-registered STREAK test. Scores 2024-01-01 through 2026-09-24.
...
when it was frozen. Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp41/streak_test.py
"""
...
PREREG = ROOT / "docs/agents/reviews/2026-09-25-fp41-prereg-streak.md"
SIDECAR = ROOT / "docs/agents/reviews/2026-09-25-fp41-prereg-streak.sha256"
...
def main() -> None:
    ...
    if not oos_day_path.exists():
        os.environ["FP41_OOS"] = "1"
        fetch.pull_oos()
    screen_daily = load_bars(screen_day_path, 3)
    screen = c.streak_trades(screen_daily)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    screen_n = len(screen)
    if screen_n != 33 or abs(screen_total - 36.9301) > 0.01:
    ...
    if len(horizon) != 2 or horizon[0] != horizon[1]:
        raise SystemExit("the 2026-09-25 bar stored a close")
    daily = dict(screen_daily)
    daily.update(oos_daily)
    primary = c.streak_trades(daily, 0.90, FEE, OOS_START, OOS_END)
    doubled = c.streak_trades(daily, 0.90, FEE_DOUBLE, OOS_START, OOS_END)
    q80 = c.streak_trades(daily, 0.80, FEE, OOS_START, OOS_END)
    q95 = c.streak_trades(daily, 0.95, FEE, OOS_START, OOS_END)
    ...
    out = ROOT / "docs/agents/backtests/fp41/streak_oos.json"
