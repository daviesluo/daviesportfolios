"""Score the fp33 screen. 2023 only.

    python3 docs/agents/scripts/fp33/measure.py
"""
...
DATA = Path(os.environ.get("FP33_DATA", "/tmp/fp33/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp33-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "LOW-LIFT", c.lift_trades(daily), pool, c.MIN_N,
        "Today's low over yesterday's low, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp33_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp33_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp33/screen_2023.json"
