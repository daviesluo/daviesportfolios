"""Pull daily bars for the fp33 screen.

A bar is the open and the low. The 2024-01-01 low is stored as the open.
...
open is a bug.
    ...
    python3 docs/agents/scripts/fp33/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp33-research"}
...
def main() -> None:
    ...
    for symbol in ("BTCUSDT",):
        ...
        save(symbol, rows)
        print(f"fp33 {symbol} 1d {len(rows)} first {min(rows)} last {max(rows)}")
