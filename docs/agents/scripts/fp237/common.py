"""The search fp237. Two finished three-day rises in a row, held four days.

Nothing here reads the network or a file. Both windows end before the entry
open. The entry open is not an input. It is not four consecutive higher
closes. The position is one leg. It is not a one-day trade and it is not an
overnight trade. Funding cash is not added. The null is that same four-day
long on every day.
"""
...
def _signal(um: dict, entry: int) -> bool:
    near = finished(um, entry, 3)
    far = finished(um, entry - 3 * fp5.DAY_MS, 3)
