"""Score the fp237 screen. 2023 entries only.

    python3 docs/agents/scripts/fp237/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp237-protocol.md"
NOTE = "Two finished three-day rises, back to back. The next USDT open is bought and sold four days later. One USDT-perpetual leg. Funding cash is not added. The null is that four-day long on every day."
...
def check_signal(trade: dict, books: dict) -> None:
    near = c.finished(books["um"], trade["entry_ms"], 3)
    far = c.finished(books["um"], trade["entry_ms"] - 3 * c.fp5.DAY_MS, 3)
    if near is None or far is None or near <= 0.0 or far <= 0.0:
        raise SystemExit("the two rises were not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'STEP' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp237_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp237_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp237/screen_2023.json"
