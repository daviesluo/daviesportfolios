"""Pull the out-of-sample inputs named in the STEP pre-registration.

The USDT perpetual runs from 2024-01-01 through the 2026-09-25 open. That
last bar is an exit open. It is not an entry. A bar on 2026-09-26 is not
requested. Closes after 2026-09-20 are not a signal. Refuses to run until
the pre-registration's sha256 matches.

    python3 docs/agents/scripts/fp237/oos_fetch.py
"""
...
LIVE = "https://www.binance.com/fapi/v1/klines"
UA = {"User-Agent": "daviesportfolios-fp237-research"}
...
def pull() -> None:
    require_frozen()
    if c.HOLD_DAYS != 4 or c.IDEA != "STEP":
    ...
    if min(rows) != OOS_START or max(rows) != EXIT_OPEN:
        raise SystemExit("the USDT perpetual does not span the frozen window")
    save("oos_um1d/BTCUSDT.json", rows)
    print(f"fp237 oos um {len(rows)}", file=sys.stderr)
