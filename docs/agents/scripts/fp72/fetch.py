"""Pull bars for the fp72 screen.

Hourly bars store the open, the low and the close.
...
No later year is requested. Hourly bars stop before the 2024-01-01 open. The daily file keeps that open because it can be an exit. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp72/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp72-research"}
...
def main() -> None:
    ...
    hourly = klines("BTCUSDT", "1h", c.HOUR_MS, hour_end, (1, 3, 4))
    ...
    print(
        f"fp72 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
