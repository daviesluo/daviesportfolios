"""Score the fp72 screen. 2023 only.

    python3 docs/agents/scripts/fp72/measure.py
"""
...
DATA = Path(os.environ.get("FP72_DATA", "/tmp/fp72/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp72-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "LOW-RET", c.lowret_trades(daily, hours), pool, c.MIN_N,
        "Return of the low's hour, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp72_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp72_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp72/screen_2023.json"
