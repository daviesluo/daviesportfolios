"""Pull daily bars for the fp35 screen.

A bar is the open, base volume and quote volume. The 2024-01-01 volumes are stored as 0. The close is not stored.
...
open is a bug.
    ...
    python3 docs/agents/scripts/fp35/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp35-research"}
...
def main() -> None:
    ...
    for symbol in ("BTCUSDT",):
        ...
        save(symbol, rows)
        print(f"fp35 {symbol} 1d {len(rows)} first {min(rows)} last {max(rows)}")
