...
"""Pins for the fp242 rule. No file and no return from 2023 is read."""
...
def test_upri() -> None:
    if c.IDEA != "UPRI" or c.MIN_N != 30 or c.HOLD_DAYS != 9:
    ...
    y = entry - day
    _put(um, y, 1, 110)
    _put(um, y - 2 * day, 1, 100)
    _put(spot, entry, 40, 1)
    _put(spot, entry + 9 * day, 30, 1)
    py = missed - day
    _put(um, py, 1, 90)
    _put(um, py - 2 * day, 1, 100)
    _put(spot, missed, 25, 90)
    _put(spot, missed + 9 * day, 25, 1)
    trades = c.signal_trades(um, spot)
    if len(trades) != 1 or trades[0]["entry_ms"] != entry:
        raise SystemExit("spot was sold without a finished rise")
