"""Score the fp242 screen. 2023 entries only.

    python3 docs/agents/scripts/fp242/measure.py
"""
...
DATA = Path(os.environ.get("FP242_DATA", "/tmp/fp242/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp242-protocol.md"
NOTE = 'A finished two-day USDT rise. The next spot open is sold and bought back nine days later. One spot leg. Funding cash is not added. The null is that nine-day short on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    move = c.finished(books["um"], trade["entry_ms"], 2)
    if move is None or not move > 0.0:
        raise SystemExit("the two-day USDT rise was not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'UPRI' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp242_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp242_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp242/screen_2023.json"
