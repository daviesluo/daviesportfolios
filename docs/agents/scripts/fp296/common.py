"""The search fp296. Fair value of the coin-margined book is the USDT close. The finished coin-margined close was at least five basis points above that fair value. Sell the coin-margined book for sixteen days. The other trade sells it for sixteen days when that premium is absent.
...
def _rule_signal(spot, um, cm, entry) -> bool:
    basis = _basis(cm, um, entry)
    return basis is not None and basis > DISCOUNT


def _null_signal(spot, um, cm, entry) -> bool:
    basis = _basis(cm, um, entry)
    return basis is not None and not (basis > DISCOUNT)
