"""The search fp330. Fair value is relative clothing prices. The US CPI apparel index and the euro-area HICP clothing-and-footwear index are each rebased to December 2022, scaled by the ECB reference rate of 2022-12-30, and divided by the Coin Metrics BUSD PriceUSD that had already completed. The result is quoted in the same unit as EURBUSD. The previous EURBUSD close is at least one thousand basis points under that price. Buy the spot book for two days. The other trade buys it for two days when that discount is absent.
...
BLS_PUB_DAY = 20
EA_SOURCE = "ecb"
ECB_SERIES = "ICP/M.U2.N.030000.4.INX"
ECB_KEY = "ICP.M.U2.N.030000.4.INX"
ECB_LAG = 1
ECB_PUB_DAY = 25
NOTE = (
    "Fair value is 1.0666 times the US CPI apparel index, rebased to December 2022, divided by the euro-area clothing-and-footwear HICP rebased to the same month, divided by the Coin Metrics BUSD price that had already completed. The previous EURBUSD close is at least one thousand basis points under that price. Buy the spot book for two days. The other trade buys it for two days when that discount is absent. Same prices, same side, the edge turned off. One leg. The traded open is the fill only. The entry day's close is not an input. Another venue's close is not an input. A book ticker is not read."
)
