"""Score the fp184 screen. 2023 entries only.

    python3 docs/agents/scripts/fp184/measure.py
"""
...
DATA = Path(os.environ.get("FP184_DATA", "/tmp/fp184/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp184-protocol.md"
NOTE = "The close finished above the high of two days ago and under yesterday's high. Next day, long BTC from the open to the close."
...
def main() -> None:
    ...
    for trade in trades:
        ...
        old = day - 2 * c.fp5.DAY_MS
        if not (bars[day][3] > bars[old][1] and bars[day][3] < bars[day - c.fp5.DAY_MS][1]):
            raise SystemExit("a trade was not above the older high and under yesterday's high")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp184_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp184_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp184/screen_2023.json"
