"""The search fp184. A close above the high of two days ago, under yesterday's high.
...
def signal_days(bars: dict[int, tuple], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Days that closed above the high of two days ago and under yesterday's high."""
    out = []
    for day in sorted(bars):
        prev = day - fp5.DAY_MS
        old = day - 2 * fp5.DAY_MS
        if day >= end_ms or prev not in bars or old not in bars:
            continue
        if len(bars[day]) != 4 or len(bars[prev]) != 4 or len(bars[old]) != 4:
            raise ValueError("a bar is the open, the high, the low and the close")
        close_px = bars[day][3]
        if close_px > bars[old][1] and close_px < bars[prev][1]:
            out.append(day)
    return out
