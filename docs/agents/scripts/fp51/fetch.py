"""Pull BTCUSDT bars for the fp51 screen.
...
No later year is requested. Hourly bars stop before the 2024-01-01 open. The daily file keeps that open because it can be an exit. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp51/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp51-research"}
...
def main() -> None:
    ...
    print(
        f"fp51 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
