"""Score the fp252 screen. 2023 entries only.

    python3 docs/agents/scripts/fp252/measure.py
"""
...
DATA = Path(os.environ.get("FP252_DATA", "/tmp/fp252/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp252-protocol.md"
NOTE = 'The absolute five-day spot return finished smaller than the absolute twenty-day spot return. The next coin-margined open is bought and sold twenty days later. One coin-margined leg. Funding cash is not added. The null is that twenty-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    if not c._signal(books["spot"], books["cm"], trade["entry_ms"]):
        raise SystemExit("the shorter move was not inside the longer one")
...
def run() -> None:
    ...
    if c.IDEA != 'INNER' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp252_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp252_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp252/screen_2023.json"
