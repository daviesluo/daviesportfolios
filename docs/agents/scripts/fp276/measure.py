"""Score the fp276 screen. 2023 entries only.

    python3 docs/agents/scripts/fp276/measure.py
"""
...
DATA = Path(os.environ.get("FP276_DATA", "/tmp/fp276/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp276-protocol.md"
NOTE = 'Spot and the USDT book both finished a two-day rise. The next spot open is bought and sold twenty-seven days later. The other trade buys the USDT book for those same twenty-seven days. One leg. Funding cash is not added. The null is not that spot long with the two rises turned off.'
...
def run() -> None:
    ...
    if c.IDEA != 'BOTHUP' or c.MIN_N != 30 or c.HOLD_DAYS != 27:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp276_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp276_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp276/screen_2023.json"
