"""The search fp276. Spot and the USDT book both finished a two-day rise. Buy spot for twenty-seven days. The other trade buys the USDT book for those same twenty-seven days. The entry open is not an input. One leg.
...
def _signal(spot, um, cm, entry) -> bool:
    spot_ret = finished(spot, entry, 2)
    um_ret = finished(um, entry, 2)
    return spot_ret is not None and um_ret is not None and spot_ret > 0.0 and um_ret > 0.0
