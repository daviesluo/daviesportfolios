"""Score the fp259 screen. 2023 entries only.

    python3 docs/agents/scripts/fp259/measure.py
"""
...
DATA = Path(os.environ.get("FP259_DATA", "/tmp/fp259/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp259-protocol.md"
NOTE = 'A finished six-day spot decline. The next spot open is sold and bought back ten days later. The other trade sells spot ten days later for another ten days. A signal counts only when both windows fill. One leg. Funding cash is not added. The null is not that ten-day short on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'DEFER' or c.MIN_N != 30 or c.HOLD_DAYS != 10:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp259_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp259_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp259/screen_2023.json"
