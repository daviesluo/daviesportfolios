"""The pre-registered DOM-UP test. Scores 2024-01-01 through 2026-09-24.
...
when it was frozen. Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp12/dom_up_test.py
"""
...
PREREG = ROOT / "docs/agents/reviews/2026-09-25-fp12-prereg-dom-up.md"
SIDECAR = ROOT / "docs/agents/reviews/2026-09-25-fp12-prereg-dom-up.sha256"

# Measured 2026-09-25 from data-api bookTicker BTCUSDT, bid 84645.6 / ask 84645.61.
HALF_SPREAD = 5.906981227649864e-08
...
def load_bars(path: Path) -> dict[int, tuple]:
    ...
    return {int(row[0]): tuple(float(x) for x in row[1:6]) for row in rows}
...
def pool_pnl(btc: dict[int, tuple]) -> list[float]:
    out = []
    for t in btc:
        trade = c.fp5._trade(
            "BTCUSDT", t, t + c.fp5.EIGHT_H_MS, btc,
...
def main() -> None:
    ...
    if not (FP12 / "oos_dom8h.json").exists() or not (FP12 / "oos_btc8h.json").exists():
        os.environ["FP12_OOS"] = "1"
        fetch.pull_oos()
    screen_dom = load_bars(FP12 / "dom8h.json")
    screen_btc = load_bars(FP5 / "spot8h" / "BTCUSDT.json")
    screen = c.dom_trades(screen_dom, screen_btc)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    screen_n = len(screen)
    if screen_n != 115 or abs(screen_total - 35.589) > 0.01:
        raise SystemExit(f"2023 did not reproduce: n={screen_n} total={screen_total}")

    dom = {**screen_dom, **load_bars(FP12 / "oos_dom8h.json")}
    btc = {**screen_btc, **load_bars(FP12 / "oos_btc8h.json")}
    if max(dom) >= OOS_END:
        raise SystemExit("a dominance bar opens on or after 2026-09-25")
    primary = c.dom_trades(dom, btc, 0.90, FEE, OOS_START, OOS_END)
    doubled = c.dom_trades(dom, btc, 0.90, FEE_DOUBLE, OOS_START, OOS_END)
    q80 = c.dom_trades(dom, btc, 0.80, FEE, OOS_START, OOS_END)
    q95 = c.dom_trades(dom, btc, 0.95, FEE, OOS_START, OOS_END)
    primary_sum = summarise(primary, pool_pnl(btc))
    ...
    payload = {
        ...
        "input_sha256": {
            "screen_dom": hashlib.sha256((FP12 / "dom8h.json").read_bytes()).hexdigest(),
            "screen_btc": hashlib.sha256((FP5 / "spot8h" / "BTCUSDT.json").read_bytes()).hexdigest(),
            "oos_dom": hashlib.sha256((FP12 / "oos_dom8h.json").read_bytes()).hexdigest(),
            "oos_btc": hashlib.sha256((FP12 / "oos_btc8h.json").read_bytes()).hexdigest(),
        },
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp12/dom_up_oos.json"
