"""Pull BTCUSDT bars for the fp39 screen.

Hourly bars store the high and the low. Open, close, volume and trade count are not stored.
...
entry. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp39/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp39-research"}
...
def main() -> None:
    ...
    hourly = klines("1h", c.HOUR_MS, hour_end, (2, 3))
    ...
    print(
        f"fp39 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
