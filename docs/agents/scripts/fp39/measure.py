"""Score the fp39 screen. 2023 only.

    python3 docs/agents/scripts/fp39/measure.py
"""
...
DATA = Path(os.environ.get("FP39_DATA", "/tmp/fp39/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp39-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "INSIDE", c.inside_trades(daily, hours), pool, c.MIN_N,
        "Count of hourly inside bars, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp39_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp39_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp39/screen_2023.json"
