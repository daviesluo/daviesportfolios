"""Score the fp189 screen. 2023 entries only.

    python3 docs/agents/scripts/fp189/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp189-protocol.md"
NOTE = 'The last aligned USDT funding rate finished strictly under the coin-margined rate. Next day, long BTC from the open to the close. The funding cash is not added.'
...
def main() -> None:
    ...
    um = load_rates(DATA / "funding/BTCUSDT.json")
    cm = load_rates(DATA / "funding/BTCUSD_PERP.json")
    ...
    for trade in trades:
        ...
        bucket = c.last_aligned(day, um, cm)
        if bucket is None or not (um[bucket] < cm[bucket]):
            raise SystemExit("a trade was not USDT funding under coin-margined funding")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp189_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp189_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"spot": sha256(DATA / "spot1d/BTCUSDT.json"), "um_funding": sha256(DATA / "funding/BTCUSDT.json"), "cm_funding": sha256(DATA / "funding/BTCUSD_PERP.json")},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp189/screen_2023.json"
