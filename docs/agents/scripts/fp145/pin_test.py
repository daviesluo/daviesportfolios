...
"""Pins for the fp145 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    rows = {prev: (1.0,), day: (4.0,)}
    if not near(c.signal_at(rows, day), 3.0):
        raise SystemExit("the jump was not the signal")
    if near(c.signal_at(rows, day), 4.0):
        raise SystemExit("the last ratio replaced the jump")
    if near(c.signal_at(rows, day), 1.0):
        raise SystemExit("yesterday replaced the jump")
    if c.signal_at({day: (4.0,)}, day) is not None:
        raise SystemExit("a day with no yesterday was a print")
    if not near(c.signal_at({prev: (2.0,), day: (2.0,)}, day), 0.0):
        raise SystemExit("a flat jump was dropped")
    if c.signal_prints({prev: (2.0,), day: (2.0,)}) != [(day, 0.0)]:
        raise SystemExit("a zero jump left the history")
    quiet = {prev: (1.0,), day: (1.0,)}

    later = c.fp5.SCREEN_END_MS
    if c.signal_at({later: (2.0,)}, later) is not None:
...
def test_tail_and_fill() -> None:
    ...
    for i in range(121):
        ...
        rows[day] = (2.0,) if i == 120 else (1.0,)
