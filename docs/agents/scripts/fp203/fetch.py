"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp203/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp203-research"}
...
def main() -> None:
    ...
    cm = pull_klines("cm", "BTCUSD_PERP", "cm1d/BTCUSD_PERP.json", (1, 4), last, (2022, 12))
    index = pull_price("index", (1, 4), "index1d/BTCUSDT.json", last)
    um = pull_klines("um", "BTCUSDT", "um1d/BTCUSDT.json", (1, 4), session, (2022, 12))
    print("fp203 " + f"cm {len(cm)} index {len(index)} um {len(um)}")
