"""Score the fp135 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp135/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp135-protocol.md"
NOTE = "Median confirmation time divided by the average, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_book(DATA / "confirm/both.json")
    ...
    row = c.fp5.summarise("CONFR", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp135_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp135_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "confirm_both": sha256(DATA / "confirm/both.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp135/screen_2023.json"
