...
bar is requested. No funding print is requested. No mempool chart is requested.
    ...
    python3 docs/agents/scripts/fp135/fetch.py
"""
...
MEDIAN = "https://api.blockchain.info/charts/median-confirmation-time"
AVERAGE = "https://api.blockchain.info/charts/avg-confirmation-time"


def _daily(url: str, unit: str) -> dict[int, float]:
    payload = json.loads(get(url + "?start=2022-10-01&timespan=457days&format=json&sampled=false"))
    if payload.get("unit") != unit:
        raise SystemExit("a confirmation chart changed unit")
    values = payload.get("values")
    if not isinstance(values, list) or not values:
        raise SystemExit("a confirmation chart came back empty")
    out: dict[int, float] = {}
    for point in values:
        stamp_s = int(point["x"])
        ...
        if day in out:
            raise SystemExit("two confirmation prints landed on one day")
        out[day] = as_number(point["y"], "a confirmation point")
    return out


def pull_signal() -> dict[int, tuple]:
    median = _daily(MEDIAN, "Minutes")
    average = _daily(AVERAGE, "minutes")
    days = set(median) & set(average)
    out = {day: (median[day], average[day]) for day in days}
    assert_span(out, c.fp5.SCREEN_END_MS - c.fp5.DAY_MS, "signal")
    save("confirm/both.json", out)
    return out
