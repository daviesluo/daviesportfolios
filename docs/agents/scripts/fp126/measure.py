"""Score the fp126 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp126/measure.py
"""
...
DATA = Path(os.environ.get("FP126_DATA", "/tmp/fp126/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp126-protocol.md"
NOTE = "Sum of absolute delta times open contracts at the day's last hour, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP126_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "options" / "delta.json")
    ...
    row = c.fp5.summarise("DELTA", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp126_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp126_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "delta": sha256(DATA / "options" / "delta.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp126/screen_2023.json"
