"""Pins for the fp126 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    row = (23, -0.5, 4.0, 23, 0.25, 4.0, 0, 9.0, 9.0)
    if not near(c.signal_at({day: row}, day), 3.0):
        raise SystemExit("absolute delta times open contracts was wrong")
    if near(c.signal_at({day: (23, -0.5, 4.0, 23, 0.25, 4.0)}, day), -1.0):
        raise SystemExit("the signed sum replaced the absolute sum")
    if c.signal_at({day: (23, -0.5, 0.0)}, day) is not None:
        raise SystemExit("a zero open interest was a print")
