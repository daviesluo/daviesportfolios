"""Score the fp287 screen. 2023 entries only.

    python3 docs/agents/scripts/fp287/measure.py
"""
...
DATA = Path(os.environ.get("FP287_DATA", "/tmp/fp287/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp287-protocol.md"
NOTE = 'Spot and the USDT book both finished a sixteen-day rise, and the USDT rise was strictly smaller. The next USDT open is bought and sold twenty days later. The other trade buys spot for those same twenty days. One leg. Funding cash is not added. The null is not that USDT long with the comparison turned off.'
...
def run() -> None:
    ...
    if c.IDEA != 'LESSUP' or c.MIN_N != 30 or c.HOLD_DAYS != 20:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp287_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp287_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp287/screen_2023.json"
