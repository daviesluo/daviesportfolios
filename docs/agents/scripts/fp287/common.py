"""The search fp287. Spot and the USDT book both finished a sixteen-day rise, and the USDT rise was strictly smaller. Buy the USDT book for twenty days. The other trade buys spot for those same twenty days. The entry open is not an input. One leg.
...
RULE_SIDE = 'long'
NULL_SIDE = 'long'
FILES = {'spot': ('spot1d/BTCUSDT.json', True), 'um': ('um1d/BTCUSDT.json', True)}
PULLS = [('spot', 'BTCUSDT', 'spot1d', True), ('um', 'BTCUSDT', 'um1d', True)]
...
def _signal(spot, um, cm, entry) -> bool:
    um_ret = finished(um, entry, 16)
    spot_ret = finished(spot, entry, 16)
    return (
        um_ret is not None
        and spot_ret is not None
        and um_ret > 0.0
        and spot_ret > 0.0
        and um_ret < spot_ret
    )
