"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp190/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp190-research"}
...
def pull_share(kind: str, symbol: str, rel: str) -> dict[int, tuple]:
    rows: dict[int, tuple] = {}
    for ym in months():
        url = (
        ...
        for row in read_zip_rows(url):
            ...
            rows[ts] = (volume, taker)
    if not rows:
        raise SystemExit(symbol + " came back empty")
    if max(rows) > c.SIGNAL_LAST_MS:
        raise SystemExit(symbol + " goes past the signal window")
    save(rel, rows)
    return rows
...
def main() -> None:
    ...
    spot = pull_spot((1, 4))
    um = pull_share("um", "BTCUSDT", "um1d/BTCUSDT.json")
    cm = pull_share("cm", "BTCUSD_PERP", "cm1d/BTCUSD_PERP.json")
    print("fp190 " + f"spot {len(spot)} last {max(spot)} um {len(um)} cm {len(cm)}")
