"""Score the fp190 screen. 2023 entries only.

    python3 docs/agents/scripts/fp190/measure.py
"""
...
DATA = Path(os.environ.get("FP190_DATA", "/tmp/fp190/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp190-protocol.md"
NOTE = 'The coin-margined taker-buy share finished strictly above one half and the USDT share strictly under one half. Next day, long BTC from the open to the close.'
...
def main() -> None:
    ...
    um = load_rows(DATA / "um1d/BTCUSDT.json", 2, c.SIGNAL_LAST_MS, required=False)
    cm = load_rows(DATA / "cm1d/BTCUSD_PERP.json", 2, c.SIGNAL_LAST_MS, required=False)
    ...
    for trade in trades:
        ...
        if not (um_vol > 0 and cm_vol > 0 and (cm_tak / cm_vol) > 0.5 and (um_tak / um_vol) < 0.5):
            raise SystemExit("a trade was not the taker split")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp190_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp190_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"spot": sha256(DATA / "spot1d/BTCUSDT.json"), "um": sha256(DATA / "um1d/BTCUSDT.json"), "cm": sha256(DATA / "cm1d/BTCUSD_PERP.json")},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp190/screen_2023.json"
