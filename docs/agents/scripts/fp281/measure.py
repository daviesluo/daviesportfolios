"""Score the fp281 screen. 2023 entries only.

    python3 docs/agents/scripts/fp281/measure.py
"""
...
DATA = Path(os.environ.get("FP281_DATA", "/tmp/fp281/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp281-protocol.md"
NOTE = 'The finished spot close was strictly above each of the twenty-nine closes before it. The next spot open is bought and sold eleven days later. The other trade buys spot eleven days later for another eleven days. Both windows have to fill. One leg. Funding cash is not added. The null is not that eleven-day long on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'PEAK' or c.MIN_N != 30 or c.HOLD_DAYS != 11:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp281_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp281_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp281/screen_2023.json"
