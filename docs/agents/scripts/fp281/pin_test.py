...
"""Pins for the fp281 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "PEAK" or c.HOLD_DAYS != 11 or c.NULL_KIND != "other_time":
        raise SystemExit("the idea moved")
    day = c.fp5.DAY_MS
    entry = c.fp5.SCREEN_START_MS + 80 * day
    spot, um, cm = {}, {}, {}
    for k in range(1, 31):
        _put(spot, entry - k * day, 1, 100)
    _put(spot, entry - day, 1, 200)
    _put(spot, entry, 50, 1)
    _put(spot, entry + 11 * day, 55, 1)
    _put(spot, entry + 22 * day, 60, 1)
    rules = c.rule_trades(spot, um, cm)
    nulls = c.null_trades(spot, um, cm)
    if len(rules) != 1 or rules[0]["book"] != "spot" or rules[0]["side"] != "long":
        raise SystemExit("the new closing high was not bought")
    if len(nulls) != 1 or nulls[0]["entry_ms"] != entry + 11 * day:
    ...
    inside = dict(spot)
    inside[entry - day] = (inside[entry - day][0], 50.0)
    if c.rule_trades(inside, um, cm):
        raise SystemExit("a close inside the range was bought")
    thin = dict(spot)
