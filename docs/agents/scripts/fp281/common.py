"""The search fp281. The finished spot close was strictly above each of the twenty-nine closes before it. Buy spot for eleven days. The other trade buys spot eleven days later for another eleven days. Both windows have to fill. The entry open is not an input. One leg.
...
def _signal(spot, um, cm, entry) -> bool:
    closes = _closes(spot, entry - fp5.DAY_MS, 30)
    if closes is None:
        return False
    return closes[0] > max(closes[1:])
