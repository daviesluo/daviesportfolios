"""Score the fp53 screen. 2023 only.

    python3 docs/agents/scripts/fp53/measure.py
"""
...
DATA = Path(os.environ.get("FP53_DATA", "/tmp/fp53/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp53-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "HL-GAP", c.gap_trades(daily, hours), pool, c.MIN_N,
        "Hours between the high and the low, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp53_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp53_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp53/screen_2023.json"
