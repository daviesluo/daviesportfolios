"""The seventy-eighth search (fp78). The fill and the null are fp5's.

Nothing here reads the network or a file. An hourly bar is (open, close).
A daily bar is (open,). The signal is the lowest hourly close divided by
the day's open, minus 1. The low price is not stored. The fill reads a
later daily open.
"""
...
def dip_at(
    ...
    """Lowest hourly close over the day's open. None when an hour is missing."""
    if day >= end_ms:
        return None
    first = hours.get(day)
    ...
    for i in range(HOURS):
        ...
        if bar is None or len(bar) < 2 or bar[1] <= 0:
            return None
        if worst is None or bar[1] < worst:
            worst = bar[1]
    if worst is None:
        return None
    return worst / first[0] - 1.0
...
def dip_signal_days(
    ...
    return _upper(dip_prints(hours, end_ms), q)
...
def dip_trades(
    ...
    """Enter the next daily open after the worst hourly close stayed high. Hold one day."""
    trades = []
    for day in dip_signal_days(hours, q, end_ms):
