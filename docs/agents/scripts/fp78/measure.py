"""Score the fp78 screen. 2023 only.

    python3 docs/agents/scripts/fp78/measure.py
"""
...
DATA = Path(os.environ.get("FP78_DATA", "/tmp/fp78/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp78-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "DIP", c.dip_trades(daily, hours), pool, c.MIN_N,
        "Lowest hourly close over the open, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp78_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp78_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp78/screen_2023.json"
