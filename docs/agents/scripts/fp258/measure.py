"""Score the fp258 screen. 2023 entries only.

    python3 docs/agents/scripts/fp258/measure.py
"""
...
DATA = Path(os.environ.get("FP258_DATA", "/tmp/fp258/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp258-protocol.md"
NOTE = 'A finished twelve-day USDT rise. The next USDT open is sold and bought back thirteen days later. The other trade buys that open for the same thirteen days. One leg. Funding cash is not added. The null is not that short with the rise turned off.'
...
def run() -> None:
    ...
    if c.IDEA != 'FADEU' or c.MIN_N != 30 or c.HOLD_DAYS != 13:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp258_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp258_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp258/screen_2023.json"
