"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp214/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp214-research"}
...
def main() -> None:
    ...
    spot = pull_klines("spot", "BTCUSDT", (1, 4), session, (2022, 12))
    um = pull_klines("um", "BTCUSDT", (1, 4), session, (2022, 12))
    mark = pull_price((1, 4), last)
    project(spot, (0, 1), last, "spotsig/BTCUSDT.json", True)
    project(um, (0, 1), last, "umsig/BTCUSDT.json", True)
    project(mark, (0, 1), last, "mark1d/BTCUSDT.json", True)
    spot_c = project(spot, (1,), session, "spotpx/BTCUSDT.json", True)
    um_c = project(um, (1,), session, "umpx/BTCUSDT.json", True)
    add_exit(spot_c, "spot", "BTCUSDT", (4,), "spotpx/BTCUSDT.json")
    add_exit(um_c, "um", "BTCUSDT", (4,), "umpx/BTCUSDT.json")
    print("fp214 " + f"spot {len(spot)} um {len(um)} mark {len(mark)}")
