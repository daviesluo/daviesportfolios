"""Pull BTCUSDT bars for the fp49 screen.
...
No later year is requested. Hourly bars stop before the 2024-01-01 open. The daily file keeps that open because it can be an exit. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp49/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp49-research"}
...
def main() -> None:
    ...
    print(
        f"fp49 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
