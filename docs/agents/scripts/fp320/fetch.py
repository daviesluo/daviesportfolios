"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp320/fetch.py
"""
...
UA = {"User-Agent": "Mozilla/5.0 daviesportfolios-fp320-research"}
...
HISTORY = "https://www.binance.com/dapi/v1/fundingRate"
...
def save_opens(rows: dict[int, float]) -> None:
    path = DATA / "cm1d" / "ETHUSD_PERP.json"
...
def pull_funding() -> dict[int, float]:
    rows: dict[int, float] = {}
    cursor = c.FUND_FIRST - 0
    end = c.fp5.SCREEN_END_MS
    while cursor < end:
        url = f"{HISTORY}?symbol=ETHUSD_PERP&startTime={cursor}&endTime={end}&limit=1000"
    ...
    path = DATA / "fund" / "ETHUSD_PERP.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps([[t, rows[t]] for t in stamps], separators=(",", ":")))
    return rows

def pull_sofr() -> dict[int, str]:
    url = (
        "https://markets.newyorkfed.org/api/rates/secured/sofr/search.json"
        "?startDate=2022-12-01&endDate=2024-02-01&type=rate"
    )
    payload = json.loads(get(url).decode(), parse_float=Decimal)
    rows = []
    for row in payload["refRates"]:
        if row.get("type") != "SOFR":
            raise SystemExit("the SOFR file holds another rate")
        rows.append((row["effectiveDate"], format(row["percentRate"], "f")))
    rows.sort()
    paired = c.pair_sofr(rows)
    out = {pub: percent for pub, percent in paired}
    if len(out) != c.SOFR_ROWS:
        raise SystemExit("SOFR does not have the frozen row count")
    if min(out) >= c.fp5.SCREEN_START_MS:
        raise SystemExit("SOFR has no lookback")
    if max(out) >= c.fp5.SCREEN_END_MS:
        raise SystemExit("a later SOFR publication was stored")
    path = DATA / "sofr.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps([[t, out[t]] for t in sorted(out)], separators=(",", ":")))
    return out
...
def main() -> None:
    ...
    opens = pull_opens()
    fund = pull_funding()
    print(f"fp320 {len(opens)} {len(fund)}")

    sofr = pull_sofr()
    print(f"fp320 sofr {len(sofr)}")
