...
"""Pins for the fp320 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "MSOFR" or c.HOLD_DAYS != 2 or c.NULL_KIND != "other_regime":
        raise SystemExit("the idea moved")
    if c.MODE != "regime" or c.RULE_SIDE != "long" or c.RULE_BOOK != "cm":
        raise SystemExit("the leg moved")
    if c.FAIR_KIND != "sofr" or c.COIN != "ETHUSD_PERP":
