"""Score the fp73 screen. 2023 only.

    python3 docs/agents/scripts/fp73/measure.py
"""
...
DATA = Path(os.environ.get("FP73_DATA", "/tmp/fp73/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp73-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "PRE-QUOTE", c.prequote_trades(daily, hours), pool, c.MIN_N,
        "Quote share before the high, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp73_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp73_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp73/screen_2023.json"
