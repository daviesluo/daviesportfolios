"""Pull bars for the fp73 screen.

Hourly bars store the high and the quote volume.
...
No later year is requested. Hourly bars stop before the 2024-01-01 open. The daily file keeps that open because it can be an exit. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp73/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp73-research"}
...
def main() -> None:
    ...
    hourly = klines("BTCUSDT", "1h", c.HOUR_MS, hour_end, (2, 7))
    ...
    print(
        f"fp73 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
