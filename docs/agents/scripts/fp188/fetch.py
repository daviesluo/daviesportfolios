"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp188/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp188-research"}
...
def pull_close(kind: str, symbol: str, rel: str) -> dict[int, tuple]:
    rows: dict[int, tuple] = {}
    for ym in months():
        url = (
...
def main() -> None:
    ...
    spot = pull_spot((1, 4))
    um = pull_close("um", "BTCUSDT", "um1d/BTCUSDT.json")
    cm = pull_close("cm", "BTCUSD_PERP", "cm1d/BTCUSD_PERP.json")
    print("fp188 " + f"spot {len(spot)} last {max(spot)} um {len(um)} cm {len(cm)}")
