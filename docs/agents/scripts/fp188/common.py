"""The search fp188. The coin-margined close finished under the USDT close.
...
def signal_days(bars: dict[int, tuple], books: tuple[dict[int, tuple], dict[int, tuple]], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Days whose coin-margined close finished under the USDT close."""
    um, cm = books
    out = []
    for day in sorted(set(um) & set(cm)):
        ...
        if not (fp5.SCREEN_START_MS <= entry < end_ms):
            continue
        if len(um[day]) != 1 or len(cm[day]) != 1:
            raise ValueError("a perpetual bar is the close")
        if cm[day][0] < um[day][0]:
            out.append(day)
    return out
