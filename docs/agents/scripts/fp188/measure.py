"""Score the fp188 screen. 2023 entries only.

    python3 docs/agents/scripts/fp188/measure.py
"""
...
DATA = Path(os.environ.get("FP188_DATA", "/tmp/fp188/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp188-protocol.md"
NOTE = 'The coin-margined perpetual close finished strictly under the USDT perpetual close. Next day, long BTC from the open to the close.'
...
def main() -> None:
    ...
    um = load_rows(DATA / "um1d/BTCUSDT.json", 1, c.SIGNAL_LAST_MS, required=False)
    cm = load_rows(DATA / "cm1d/BTCUSD_PERP.json", 1, c.SIGNAL_LAST_MS, required=False)
    ...
    for trade in trades:
        ...
        if not (cm[day][0] < um[day][0]):
            raise SystemExit("a trade was not the coin-margined book under the USDT book")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp188_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp188_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp188/screen_2023.json"
