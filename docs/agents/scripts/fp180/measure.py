"""Score the fp180 screen. 2023 entries only.

    python3 docs/agents/scripts/fp180/measure.py
"""
...
DATA = Path(os.environ.get("FP180_DATA", "/tmp/fp180/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp180-protocol.md"
NOTE = 'Yesterday closed under its open, and today closed above that open. Next day, long BTC from the open to the close.'
...
def main() -> None:
    ...
    for trade in trades:
        ...
        if not (prev[3] < prev[0] and bars[day][3] > prev[0]):
            raise SystemExit("a trade did not close above the prior open")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp180_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp180_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp180/screen_2023.json"
