"""Score the fp86 screen. 2023 only.

    python3 docs/agents/scripts/fp86/measure.py
"""
...
DATA = Path(os.environ.get("FP86_DATA", "/tmp/fp86/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp86-protocol.md"
...
def main() -> None:
    ...
    trades = c.overlap_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "OVERLAP", trades, pool, c.MIN_N,
        "Mean overlap of consecutive hourly ranges, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp86_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp86_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp86/screen_2023.json"
