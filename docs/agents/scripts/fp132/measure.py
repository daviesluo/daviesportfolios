"""Score the fp132 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp132/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp132-protocol.md"
NOTE = "Blocks mined that UTC day, above their own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_book(DATA / "blocks/count.json")
    ...
    row = c.fp5.summarise("BLKCNT", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp132_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp132_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "block_count": sha256(DATA / "blocks/count.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp132/screen_2023.json"
