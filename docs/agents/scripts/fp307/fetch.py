"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp307/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp307-research"}
...
def pull_bitmex() -> dict[int, tuple]:
    url = (
        "https://www.bitmex.com/api/v1/trade/bucketed?binSize=1d&partial=false"
        "&symbol=XBTUSD&count=500&reverse=false"
        "&startTime=2022-11-01T00:00:00.000Z&endTime=2024-01-01T00:00:00.000Z"
    )
    rows: dict[int, tuple] = {}
    for bar in json.loads(get(url)):
        stamp = datetime.strptime(bar["timestamp"], "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=timezone.utc)
        ts = int(stamp.timestamp() * 1000)
        if not keep_fair(ts):
            continue
        rows[ts] = (float(bar["open"]), float(bar["close"]))
    check_fair(rows, "BitMEX")
    save("mx1d/XBTUSD.json", rows)
    return rows
...
def main() -> None:
    ...
    fair = pull_bitmex()
    print(f"fp307 {len(traded)} {len(fair)}")
