"""Score the fp215 screen. 2023 entries only.

    python3 docs/agents/scripts/fp215/measure.py
"""
...
DATA = Path(os.environ.get("FP215_DATA", "/tmp/fp215/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp215-protocol.md"
NOTE = 'The USDT close and the coin-margined close differ, and the gap widened. Next close to the following close, long the lower close and short the higher close. Funding cash is not added. The null is that spread on every day the closes differ.'
...
def run() -> None:
    ...
    um = load_rows(DATA / "umsig/BTCUSDT.json", 1, last, True)
    cm = load_rows(DATA / "cmsig/BTCUSD_PERP.json", 1, last, True)
    um_px = load_rows(DATA / "um1d/BTCUSDT.json", 1, end, True)
    cm_px = load_rows(DATA / "cm1d/BTCUSD_PERP.json", 1, end, True)
    trades = c.signal_trades(um, cm, um_px, cm_px)
    for trade in trades:
        check_common(trade)
        ...
        left, right = um[day][0], cm[day][0]
        prev = abs(um[yday][0] - cm[yday][0])
    ...
    pool = c.pool_nets(um, cm, um_px, cm_px)
    inputs = {
    ...
    payload = {
        ...
        "code_sha256": {
            "fp215_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp215_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp215/screen_2023.json"
