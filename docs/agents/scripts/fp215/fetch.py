"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp215/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp215-research"}
...
def main() -> None:
    ...
    um = pull_klines("um", "BTCUSDT", (4,), session, (2022, 12))
    cm = pull_klines("cm", "BTCUSD_PERP", (4,), session, (2022, 12))
    project(um, (0,), last, "umsig/BTCUSDT.json", True)
    project(cm, (0,), last, "cmsig/BTCUSD_PERP.json", True)
    add_exit(um, "um", "BTCUSDT", (4,), "um1d/BTCUSDT.json")
    add_exit(cm, "cm", "BTCUSD_PERP", (4,), "cm1d/BTCUSD_PERP.json")
    print("fp215 " + f"um {len(um)} cm {len(cm)}")
