"""Score the fp225 screen. 2023 entries only.

    python3 docs/agents/scripts/fp225/measure.py
"""
...
DATA = Path(os.environ.get("FP225_DATA", "/tmp/fp225/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp225-protocol.md"
NOTE = "The front quarterly is bought when 21 to 35 days remain and sold ten days later, before expiry. One leg. Funding cash is not added. The null is that ten-day long whenever the sale is still before expiry."
...
def run() -> None:
    ...
    for trade in trades:
        ...
        if found is None:
            raise SystemExit("the entry has no front")
        expiry, symbol = found
        dte = (expiry - trade["entry_ms"]) // c.fp5.DAY_MS
        if dte < 21 or dte > 35 or trade["exit_ms"] >= expiry:
    ...
    for entry, exit_ms in pool_spans:
        found = c.front(entry)
        if found is None or exit_ms >= found[0] or exit_ms - entry != c.HOLD_DAYS * c.fp5.DAY_MS:
            raise SystemExit("the null is not a ten-day hold before expiry")
    pool = c.pool_nets(books)
    if c.IDEA != "QFAR" or c.MIN_N != 30:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp225_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp225_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            symbol: sha256(DATA / "cm1d" / f"{symbol}.json") for _expiry, symbol in c.QUARTERS
        },
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp225/screen_2023.json"
