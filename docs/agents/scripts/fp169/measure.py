"""Score the fp169 screen. 2023 entries only.

    python3 docs/agents/scripts/fp169/measure.py
"""
...
DATA = Path(os.environ.get("FP169_DATA", "/tmp/fp169/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp169-protocol.md"
NOTE = 'An outside day. Next day, long BTC from the open to the close.'
...
def main() -> None:
    if os.environ.get("FP169_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    if c.IDEA != 'OUTNEXT' or c.MIN_N != 30:
    ...
    for trade in trades:
        ...
        if not (bars[day][1] > bars[prev][1] and bars[day][2] < bars[prev][2]):
            raise SystemExit("a trade was not an outside day")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp169_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp169_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp169/screen_2023.json"
