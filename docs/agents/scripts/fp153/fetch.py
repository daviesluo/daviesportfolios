"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp153/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp153-research"}
...
def main() -> None:
    ...
    premium = pull_premium_close()
    spot = pull_opens("BTCUSDT", "/api/v3/klines", end, "spot1d/BTCUSDT.json", 1)
    um = pull_um_open(end)
    funding = pull_funding(end - c.fp5.DAY_MS)
    print(
        "fp153 "
        + f"premium {len(premium)} spot {len(spot)} um {len(um)} funding {len(funding)}"
    )
