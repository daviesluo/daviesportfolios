"""Pull BTCUSDT daily opens and closes for the fp20 screen.
...
low, quote volume and the trade count are not stored.

    python3 docs/agents/scripts/fp20/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp20-research"}
...
def main() -> None:
    ...
    print(f"fp20 BTCUSDT 1d bars {len(rows)} first {min(rows)} last {max(rows)}")
