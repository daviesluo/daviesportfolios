"""Score the fp95 screen. 2023 only.

    python3 docs/agents/scripts/fp95/measure.py
"""
...
DATA = Path(os.environ.get("FP95_DATA", "/tmp/fp95/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp95-protocol.md"
...
def main() -> None:
    if os.environ.get("FP95_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")

    alt_path = DATA / "spot1d" / "SOLUSDT.json"
    ...
    for trade in trades:
        if trade["coin"] != "SOLUSDT":
            raise SystemExit("a fill is not SOL")
    ...
    pool = c.fp5.pool_from_bars({"SOLUSDT": alt}, ["SOLUSDT"], c.fp5.DAY_MS)
    row = c.fp5.summarise(
        "SOL-Q", trades, pool, c.MIN_N,
        "SOL's daily quote volume over BTC's, above its own trailing 90th. Next day, long SOL.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp95_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp95_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp95/screen_2023.json"
