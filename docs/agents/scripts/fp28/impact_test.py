"""The pre-registered IMPACT test. Scores 2024-01-01 through 2026-09-24.
...
when it was frozen. Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp28/impact_test.py
"""
...
PREREG = ROOT / "docs/agents/reviews/2026-09-25-fp28-prereg-impact.md"
SIDECAR = ROOT / "docs/agents/reviews/2026-09-25-fp28-prereg-impact.sha256"

# Measured 2026-09-25T02:52:42Z from data-api bookTicker BTCUSDT, bid 84273.51 / ask 84273.52.
HALF_SPREAD = 5.933062130678438e-08
...
def main() -> None:
    ...
    if not oos_hour_path.exists() or not oos_day_path.exists():
        os.environ["FP28_OOS"] = "1"
        fetch.pull_oos()
    screen_hours = load_bars(screen_hour_path, 4)
    screen_daily = load_bars(screen_day_path, 2)
    screen = c.impact_trades(screen_daily, screen_hours)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    screen_n = len(screen)
    if screen_n != 37 or abs(screen_total - 32.0371) > 0.01:
        raise SystemExit(f"2023 did not reproduce: n={screen_n} total={screen_total}")

    oos_hours = load_bars(oos_hour_path, 4)
    ...
    primary = c.impact_trades(daily, hours, 0.90, FEE, OOS_START, OOS_END)
    doubled = c.impact_trades(daily, hours, 0.90, FEE_DOUBLE, OOS_START, OOS_END)
    q80 = c.impact_trades(daily, hours, 0.80, FEE, OOS_START, OOS_END)
    q95 = c.impact_trades(daily, hours, 0.95, FEE, OOS_START, OOS_END)
    ...
    out = ROOT / "docs/agents/backtests/fp28/impact_oos.json"
