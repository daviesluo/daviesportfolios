"""Score the fp28 screen. 2023 only.

    python3 docs/agents/scripts/fp28/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp28-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "IMPACT", c.impact_trades(daily, hours), pool, c.MIN_N,
        "Hourly quote volume lined up with the absolute hourly move, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp28_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp28_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp28/screen_2023.json"
