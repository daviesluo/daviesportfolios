"""Pull bars for the fp80 screen.

Daily bars store the open, the high, the low, quote volume and base volume.
The 2024-01-01 bar keeps its open and stores the high and the low equal to
that open, with quote and base at zero, so the bar can be an exit and cannot
be a print.

    python3 docs/agents/scripts/fp80/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp80-research"}
...
def horizon(open_: float, fields: tuple[int, ...]) -> tuple:
    return (open_, open_, open_, 0.0, 0.0)


def main() -> None:
    if os.environ.get("FP80_OOS"):
        raise SystemExit("this pull does not request a later year")
    rows = klines("BTCUSDT", "1d", c.fp5.DAY_MS, c.fp5.SCREEN_END_MS, (1, 2, 3, 7, 5))
    ...
    if horizon_bar is None or horizon_bar[1] != horizon_bar[0] or horizon_bar[2] != horizon_bar[0]:
        raise SystemExit("the 2024-01-01 bar stored a high or a low")
    if horizon_bar[3] != 0.0 or horizon_bar[4] != 0.0:
    ...
    print(f"fp80 BTCUSDT 1d {len(rows)} first {min(rows)} last {max(rows)}")
