"""Score the fp108 screen. 2023 entries only. The hold is 7 days.

    python3 docs/agents/scripts/fp108/measure.py
"""
...
DATA = Path(os.environ.get("FP108_DATA", "/tmp/fp108/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp108-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "FRNG-7", trades, pool, c.MIN_N,
        "Range of BTC's three funding prints, above its own trailing 90th. "
        "Next day, long BTC for 7 days.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp108_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp108_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp108/screen_2023.json"
