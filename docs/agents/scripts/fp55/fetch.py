"""Pull BTCUSDT bars for the fp55 screen.
...
No later year is requested. The 2024-01-01 bar keeps its open. Its signal fields are masked.
    ...
    python3 docs/agents/scripts/fp55/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp55-research"}
...
def main() -> None:
    ...
    save(rows)
    print(f"fp55 BTCUSDT 1d {len(rows)} first {min(rows)} last {max(rows)}")
