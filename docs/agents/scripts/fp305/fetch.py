"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp305/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp305-research"}
...
def pull_bitstamp() -> dict[int, tuple]:
    url = "https://www.bitstamp.net/api/v2/ohlc/btcusd/?step=86400&limit=1000&start=1667260800"
    payload = json.loads(get(url))
    rows: dict[int, tuple] = {}
    for bar in payload["data"]["ohlc"]:
        ts = int(bar["timestamp"]) * 1000
        if not keep_fair(ts):
            continue
        rows[ts] = (float(bar["open"]), float(bar["close"]))
    check_fair(rows, "Bitstamp")
    save("bs1d/BTCUSD.json", rows)
    return rows
...
def main() -> None:
    ...
    fair = pull_bitstamp()
    print(f"fp305 {len(traded)} {len(fair)}")
