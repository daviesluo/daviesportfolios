"""Score the fp110 screen. 2023 entries only. The hold is 14 days.

    python3 docs/agents/scripts/fp110/measure.py
"""
...
DATA = Path(os.environ.get("FP110_DATA", "/tmp/fp110/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp110-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "FRNG-14", trades, pool, c.MIN_N,
        "Range of BTC's three funding prints, above its own trailing 90th. "
        "Next day, long BTC for 14 days.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp110_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp110_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp110/screen_2023.json"
