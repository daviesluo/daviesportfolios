"""Pull BTC funding and daily opens for the fp110 screen.

Funding stops before 2024-01-01. Daily opens run through 2024-01-14 so a
14-day hold can exit. Those later opens are not a signal. No hourly
price bar is requested. No alt quote is requested.

    python3 docs/agents/scripts/fp110/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp110-research"}
...
def main() -> None:
    ...
    bars = pull_opens()
    print(
        f"fp110 funding {len(funding)} first {min(funding)} last {max(funding)}; "
