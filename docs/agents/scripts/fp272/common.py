"""The search fp272. Spot's finished ten-day return was strictly above the USDT ten-day return. Sell the coin-margined book for twenty-four days. The other trade sells the USDT book for those same twenty-four days.
...
def _signal(spot, um, cm, entry) -> bool:
    spot_move = finished(spot, entry, 10)
    um_move = finished(um, entry, 10)
    return spot_move is not None and um_move is not None and spot_move > um_move
