"""Pull bars for the fp87 screen.

Daily bars store the open, the high, the low, the close, quote volume and base volume.
The 2024-01-01 bar keeps its open and stores the high, the low and the close equal to
that open, with quote and base at zero, so the bar can be an exit and cannot be a print.

    python3 docs/agents/scripts/fp87/fetch.py
"""
...
DATA = Path(os.environ.get("FP87_DATA", "/tmp/fp87/data"))
MARKET = "https://data-api.binance.vision"
LOOKBACK_MS = 1_664_582_400_000  # 2022-10-01 00:00 UTC
UA = {"User-Agent": "daviesportfolios-fp87-research"}
...
def horizon(open_: float, fields: tuple[int, ...]) -> tuple:
    return (open_, open_, open_, open_, 0.0, 0.0)


def main() -> None:
    if os.environ.get("FP87_OOS"):
        raise SystemExit("this pull does not request a later year")
    rows = klines("BTCUSDT", "1d", c.fp5.DAY_MS, c.fp5.SCREEN_END_MS, (1, 2, 3, 4, 7, 5))
    ...
    if horizon_bar is None or horizon_bar[1] != horizon_bar[0] or horizon_bar[2] != horizon_bar[0] or horizon_bar[3] != horizon_bar[0]:
        raise SystemExit("the 2024-01-01 bar stored a high, a low, or a close")
    if horizon_bar[4] != 0.0 or horizon_bar[5] != 0.0:
        raise SystemExit("the 2024-01-01 bar stored quote or base volume")
    if not rows:
        raise SystemExit("BTCUSDT klines came back empty")
    save("spot1d/BTCUSDT.json", rows)
    print(f"fp87 BTCUSDT 1d {len(rows)} first {min(rows)} last {max(rows)}")
