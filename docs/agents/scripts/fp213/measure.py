"""Score the fp213 screen. 2023 entries only.

    python3 docs/agents/scripts/fp213/measure.py
"""
...
DATA = Path(os.environ.get("FP213_DATA", "/tmp/fp213/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp213-protocol.md"
NOTE = 'The USDT and coin-margined 16:00 funding changes have opposite signs. Next session, long the book whose rate fell and short the book whose rate rose. Funding cash is not added. The null is that spread on every day the changes differ.'
...
def run() -> None:
    ...
    trades = c.signal_trades(um_rate, cm_rate, um_f, cm_f)
    for trade in trades:
        check_common(trade)
        ...
        left = um_rate[day] - um_rate[yday]
        right = cm_rate[day] - cm_rate[yday]
        if not ((left > 0.0 > right) or (right > 0.0 > left)):
            raise SystemExit("the changes were not opposite")
        if left < right:
    ...
    pool = c.pool_nets(um_rate, cm_rate, um_f, cm_f)
    inputs = {
    ...
    payload = {
        ...
        "code_sha256": {
            "fp213_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp213_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp213/screen_2023.json"
