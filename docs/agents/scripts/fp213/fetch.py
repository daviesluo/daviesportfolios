"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp213/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp213-research"}
...
def main() -> None:
    ...
    cm = pull_klines("cm", "BTCUSD_PERP", (1, 4), session, (2022, 12))
    project(um, (0, 1), session, "um1d/BTCUSDT.json", True)
    project(cm, (0, 1), session, "cm1d/BTCUSD_PERP.json", True)
    fum = pull_funding("um", "BTCUSDT", "funding/BTCUSDT.json")
    fcm = pull_funding("cm", "BTCUSD_PERP", "funding/BTCUSD_PERP.json")
    print("fp213 " + f"um {len(um)} cm {len(cm)} fum {len(fum)} fcm {len(fcm)} last {last}")
