"""Score the fp120 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp120/measure.py
"""
...
DATA = Path(os.environ.get("FP120_DATA", "/tmp/fp120/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp120-protocol.md"
NOTE = "Sum of BTC option volume_contracts at the day's last hour, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP120_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "options" / "vol.json")
    ...
    row = c.fp5.summarise("OPTVOL", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp120_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp120_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "vol": sha256(DATA / "options" / "vol.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp120/screen_2023.json"
