"""Pins for the fp120 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    if not near(c.signal_at({day: (23, 1.5, 23, 2.5)}, day), 4.0):
        raise SystemExit("the contract-volume sum was wrong")
    if not near(c.signal_at({day: (0, 99.0, 23, 1.5)}, day), 1.5):
        raise SystemExit("an earlier hour entered the sum")
    if not near(c.signal_at({day: (23, 0.0, 23, 0.0)}, day), 0.0):
        raise SystemExit("a zero volume was dropped")
    if c.signal_at({day: (23, -1.0)}, day) is not None:
        raise SystemExit("a negative volume was a print")
