"""Score the fp239 screen. 2023 entries only.

    python3 docs/agents/scripts/fp239/measure.py
"""
...
DATA = Path(os.environ.get("FP239_DATA", "/tmp/fp239/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp239-protocol.md"
NOTE = 'The absolute three-day USDT return finished smaller than the absolute three-day return before it. The next open is bought and sold thirteen days later. One USDT-perpetual leg. Funding cash is not added. The null is that thirteen-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    recent = c.finished(books["um"], trade["entry_ms"], 3)
    older = c.finished(books["um"], trade["entry_ms"] - 3 * c.fp5.DAY_MS, 3)
    if recent is None or older is None or not abs(recent) < abs(older):
        raise SystemExit("the absolute return did not contract")


def run() -> None:
    end = c.fp5.SCREEN_END_MS
    last = end - c.fp5.DAY_MS
    books = {"um": load_rows(DATA / "um1d/BTCUSDT.json", 2, end)}
    trades = c.signal_trades(books['um'])
    spans = c.signal_spans(books['um'])
    ...
    pool_spans = c.pool_spans(books['um'])
    pool = c.pool_nets(books['um'])
    if c.IDEA != 'QUIET' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp239_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp239_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp239/screen_2023.json"
