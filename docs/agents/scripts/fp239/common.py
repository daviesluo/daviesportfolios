"""The search fp239. The absolute three-day USDT return finished smaller than the absolute three-day return before it, then a thirteen-day long.

Nothing here reads the network or a file. Both three-day windows end before the entry open. The entry open is not an input. The comparison is closes, not a high or a low. The position is one
leg. It is not a one-day trade and it is not an overnight trade. Funding
cash is not added. The null is that same 13-day long on every
day.
"""
...
def _signal(um: dict, entry: int) -> bool:
    recent = finished(um, entry, 3)
    older = finished(um, entry - 3 * fp5.DAY_MS, 3)
    if recent is None or older is None:
        return False
    return abs(recent) < abs(older)
