"""Score the fp107 screen. 2023 entries only. The hold is 6 days.

    python3 docs/agents/scripts/fp107/measure.py
"""
...
DATA = Path(os.environ.get("FP107_DATA", "/tmp/fp107/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp107-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "FRNG-6", trades, pool, c.MIN_N,
        "Range of BTC's three funding prints, above its own trailing 90th. "
        "Next day, long BTC for 6 days.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp107_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp107_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp107/screen_2023.json"
