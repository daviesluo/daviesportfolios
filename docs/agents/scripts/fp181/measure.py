"""Score the fp181 screen. 2023 entries only.

    python3 docs/agents/scripts/fp181/measure.py
"""
...
DATA = Path(os.environ.get("FP181_DATA", "/tmp/fp181/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp181-protocol.md"
NOTE = 'A lower high and a lower low. Next day, short BTC from the open to the next open. The funding cash is not added.'
...
def main() -> None:
    ...
    for trade in trades:
        ...
        if not (bars[day][1] < prev[1] and bars[day][2] < prev[2]):
            raise SystemExit("a trade was not a lower high and a lower low")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp181_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp181_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp181/screen_2023.json"
