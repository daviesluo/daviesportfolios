"""Score the fp270 screen. 2023 entries only.

    python3 docs/agents/scripts/fp270/measure.py
"""
...
DATA = Path(os.environ.get("FP270_DATA", "/tmp/fp270/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp270-protocol.md"
NOTE = 'The finished twenty-day coin-margined return was positive and the finished five-day return was negative. The next coin-margined open is sold and bought back seventeen days later. The other trade buys that open for the same seventeen days. One leg. Funding cash is not added. The null is not that seventeen-day short on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'INUP' or c.MIN_N != 30 or c.HOLD_DAYS != 17:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp270_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp270_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp270/screen_2023.json"
