"""The search fp270. The finished twenty-day coin-margined return was positive and the finished five-day return was negative. Sell the coin-margined book for seventeen days. The other trade buys it for those same seventeen days.
...
def _signal(spot, um, cm, entry) -> bool:
    longer = finished(cm, entry, 20)
    near = finished(cm, entry, 5)
    return longer is not None and near is not None and longer > 0.0 and near < 0.0
