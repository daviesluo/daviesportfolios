"""The search fp266. Yesterday's coin-margined close finished below yesterday's spot close. Buy the coin-margined book for twelve days. The other trade buys spot for those same twelve days.
...
RULE_SIDE = 'long'
NULL_SIDE = 'long'
FILES = {'spot': ('spot1d/BTCUSDT.json', True), 'cm': ('cm1d/BTCUSD_PERP.json', True)}
PULLS = [('spot', 'BTCUSDT', 'spot1d', True), ('cm', 'BTCUSD_PERP', 'cm1d', True)]
...
def _signal(spot, um, cm, entry) -> bool:
    cm_px = _px(cm, entry - fp5.DAY_MS, 1)
    spot_px = _px(spot, entry - fp5.DAY_MS, 1)
    return cm_px is not None and spot_px is not None and cm_px < spot_px
