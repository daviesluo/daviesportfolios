...
"""Pins for the fp266 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "UNDER" or c.HOLD_DAYS != 12 or c.NULL_KIND != "other_book":
    ...
    _put(cm, entry, 30, 1)
    _put(cm, entry + 12 * day, 36, 1)
    _put(spot, entry, 40, 1)
    _put(spot, entry + 12 * day, 48, 1)
    _put(cm, missed - day, 1, 120)
    _put(spot, missed - day, 1, 100)
    _put(cm, missed, 30, 1)
    _put(cm, missed + 12 * day, 36, 1)
    _put(spot, missed, 40, 1)
    _put(spot, missed + 12 * day, 48, 1)
    rules = c.rule_trades(spot, um, cm)
    nulls = c.null_trades(spot, um, cm)
    if len(rules) != 1 or rules[0]["book"] != "cm" or rules[0]["entry_ms"] != entry:
        raise SystemExit("the cheaper book was not bought")
    if len(nulls) != 1 or nulls[0]["book"] != "spot":
