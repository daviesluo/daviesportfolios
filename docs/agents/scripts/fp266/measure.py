"""Score the fp266 screen. 2023 entries only.

    python3 docs/agents/scripts/fp266/measure.py
"""
...
DATA = Path(os.environ.get("FP266_DATA", "/tmp/fp266/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp266-protocol.md"
NOTE = "Yesterday's coin-margined close finished below yesterday's spot close. The next coin-margined open is bought and sold twelve days later. The other trade buys spot for those same twelve days. One leg. Funding cash is not added. The null is not that coin-margined long with the comparison turned off."
...
def run() -> None:
    ...
    if c.IDEA != 'UNDER' or c.MIN_N != 30 or c.HOLD_DAYS != 12:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp266_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp266_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp266/screen_2023.json"
