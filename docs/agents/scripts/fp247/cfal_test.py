"""The pre-registered CFAL test. Entries from 2024-01-01 through 2026-09-07.

The buy and the sell stay where the screen put them. The 2026-09-25 bar is
the exit open of the last allowed entry. It is not an entry. Coin-margined
closes used as the signal stop on 2026-09-06. Refuses to run until the
...
Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp247/cfal_test.py
"""
...
SCREEN = ROOT / "docs/agents/backtests/fp247/screen_2023.json"

BID = 84212.8
ASK = 84212.9
HALF_SPREAD = (ASK - BID) / (ASK + BID)
if HALF_SPREAD != 5.937336166111754e-07:
...
def fill(entry: int, um: dict, fee: float) -> dict | None:
    ...
    if not (OOS_START <= entry < ENTRY_END):
        return None
    if exit_ms != entry + 18 * c.fp5.DAY_MS or exit_ms > EXIT_OPEN:
        return None
...
def signal_trades(cm: dict, um: dict, fee: float) -> list[dict]:
    trades = []
    entry = OOS_START
    while entry < ENTRY_END:
        if c._signal(cm, um, entry):
...
def check_fill(cm: dict, um: dict, trade: dict, fee: float) -> None:
    entry = trade["entry_ms"]
    if entry >= ENTRY_END or entry > LAST_ENTRY:
        raise SystemExit("an entry is after 2026-09-07")
    ...
    used = (
        entry - c.fp5.DAY_MS,
        entry - 6 * c.fp5.DAY_MS,
        entry - 11 * c.fp5.DAY_MS,
    )
    for ts in used:
        if ts > SIGNAL_LAST:
            raise SystemExit("a signal close is after 2026-09-06")
        if c._px(cm, ts, 1) is None:
            raise SystemExit("a signal close is missing")
    if not c._signal(cm, um, entry):
        raise SystemExit("the two-decline rule did not fire")
    entry_px = um[entry][0]
    exit_px = um[trade["exit_ms"]][0]
    net = c.fp5.net_return(entry_px, exit_px, fee)
    if abs(trade["net"] - net) > 1e-12 or abs(trade["pnl"] - 100.0 * net) > 1e-9:
        raise SystemExit("a fill is not the open eighteen days later")
...
def main() -> None:
    ...
    if "84212.8, ask 84212.9" not in prereg or "5.937336166111754e-07" not in prereg:
    ...
    if sha256(common_path) != screen_doc["code_sha256"]["fp247_common"]:
        raise SystemExit("the rule file moved after the screen")
    if c.IDEA != "CFAL" or c.HOLD_DAYS != 18 or c.MIN_N != 30 or c.COIN != "BTCUSDT":
        raise SystemExit("the frozen name, hold, count or coin moved")
    if LAST_ENTRY != int(datetime(2026, 9, 7, tzinfo=timezone.utc).timestamp() * 1000):
        raise SystemExit("the last entry is not 2026-09-07")
    if ENTRY_END != int(datetime(2026, 9, 8, tzinfo=timezone.utc).timestamp() * 1000):
        raise SystemExit("the entry window does not stop on 2026-09-08")
    ...
    screen_cm = load_bars(screen_cm_path, 2, c.fp5.SCREEN_END_MS - c.fp5.DAY_MS)
    screen_um = load_bars(screen_um_path, 2, c.fp5.SCREEN_END_MS)
    if max(screen_cm) != c.fp5.SCREEN_END_MS - c.fp5.DAY_MS:
        raise SystemExit("the screen coin-margined file does not stop on 2023-12-31")
    if max(screen_um) != c.fp5.SCREEN_END_MS:
        raise SystemExit("the screen USDT file does not stop on 2024-01-01")
    screen = c.signal_trades(screen_cm, screen_um)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    if len(screen) != 64 or abs(screen_total - 485.7511) > 0.01:
    ...
    if not oos_um_path.exists() or not oos_cm_path.exists():
        oos_fetch.pull()
    oos_um = load_bars(oos_um_path, 2, EXIT_OPEN)
    oos_cm = load_bars(oos_cm_path, 2, SIGNAL_LAST)
    if min(oos_um) != OOS_START or max(oos_um) != EXIT_OPEN:
        raise SystemExit("the USDT window is wrong")
    if min(oos_cm) != OOS_START or max(oos_cm) != SIGNAL_LAST:
        raise SystemExit("the coin-margined window is wrong")
    if any(ts > SIGNAL_LAST for ts in oos_cm):
    ...
    while day <= SIGNAL_LAST:
        if day not in oos_cm:
            raise SystemExit("the coin-margined perpetual is missing a day")
        day += c.fp5.DAY_MS
    um = merge(screen_um, oos_um)
    cm = merge(screen_cm, oos_cm)
    primary = signal_trades(cm, um, FEE)
    doubled = signal_trades(cm, um, FEE_DOUBLE)
    if not primary:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp247_common": sha256(common_path),
            "fp247_cfal_test": sha256(Path(__file__)),
            "fp247_oos_fetch": sha256(Path(oos_fetch.__file__)),
        },
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp247/cfal_oos.json"
