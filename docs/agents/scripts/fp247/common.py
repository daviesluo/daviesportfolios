"""The search fp247. Two finished five-day coin-margined declines, then an eighteen-day USDT long.
...
not added. The null is that same 18-day long on every day.
"""
...
def _signal(cm: dict, um: dict, entry: int) -> bool:
    near = finished(cm, entry, 5)
    far = finished(cm, entry - 5 * fp5.DAY_MS, 5)
