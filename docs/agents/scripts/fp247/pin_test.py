...
"""Pins for the fp247 rule. No file and no return from 2023 is read."""
...
def test_cfal() -> None:
    if c.IDEA != "CFAL" or c.MIN_N != 30 or c.HOLD_DAYS != 18:
    ...
    y = entry - day
    _put(cm, y, 1, 80)
    _put(cm, y - 5 * day, 1, 100)
    _put(cm, y - 10 * day, 1, 120)
    _put(um, entry, 30, 200)
    _put(um, entry + 18 * day, 40, 1)
    py = missed - day
    _put(cm, py, 1, 120)
    _put(cm, py - 5 * day, 1, 100)
    _put(cm, py - 10 * day, 1, 80)
    _put(um, missed, 25, 90)
    _put(um, missed + 18 * day, 25, 1)
    ...
    if entry not in pool or missed not in pool:
        raise SystemExit("the null dropped an eighteen-day long")
