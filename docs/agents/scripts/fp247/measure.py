"""Score the fp247 screen. 2023 entries only.

    python3 docs/agents/scripts/fp247/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp247-protocol.md"
NOTE = 'Two finished five-day coin-margined declines, back to back. The next USDT open is bought and sold eighteen days later. One USDT-perpetual leg. Funding cash is not added. The null is that eighteen-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    ...
    near = c.finished(books["cm"], entry, 5)
    far = c.finished(books["cm"], entry - 5 * c.fp5.DAY_MS, 5)
    if near is None or far is None or not (near < 0.0 and far < 0.0):
        raise SystemExit("the two five-day declines were not already finished")
...
def run() -> None:
    ...
    trades = c.signal_trades(books['cm'], books['um'])
    spans = c.signal_spans(books['cm'], books['um'])
    ...
    pool_spans = c.pool_spans(books['cm'], books['um'])
    pool = c.pool_nets(books['cm'], books['um'])
    if c.IDEA != 'CFAL' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp247_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp247_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"cm": sha256(DATA / "cm1d/BTCUSD_PERP.json"), "um": sha256(DATA / "um1d/BTCUSDT.json")},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp247/screen_2023.json"
