"""The search fp114. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the top-trader position ratio over the top-trader account ratio, minus one. Retail is not an input. No hourly price bar is read. No funding print is read.
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Position ratio over account ratio, minus one. Retail is not an input."""
    if day >= end_ms:
        return None
    row = rows.get(day)
    if row is None or len(row) != 2 or row[0] <= 0 or row[1] <= 0:
        return None
