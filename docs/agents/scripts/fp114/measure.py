"""Score the fp114 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp114/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp114-protocol.md"
NOTE = "Top-trader position ratio over the account ratio, minus one, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_fixed(DATA / "metrics/pos_gap.json", 3, True)
    ...
    row = c.fp5.summarise("POSGAP", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp114_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp114_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "pos_gap": sha256(DATA / "metrics/pos_gap.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp114/screen_2023.json"
