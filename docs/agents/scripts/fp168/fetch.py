"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp168/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp168-research"}
...
def main() -> None:
    ...
    spot = pull_opens(end, "spot1d/BTCUSDT.json", (1,))
    print("fp168 " + f"oi {len(oi)} taker {len(taker)} spot {len(spot)} last {max(spot)}")
