...
"""Pins for the fp311 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != 'HTXCH' or c.HOLD_DAYS != 10 or c.NULL_KIND != "other_regime":
    ...
    if c.FAIR_KEY != 'hx' or c.FAIR_KEY in ("spot", "um", "cm", "cb", "db"):
        raise SystemExit("the fair value is a Binance line or a reused venue")
    if c.FAIR_SHIFT_MS != 57600000:
