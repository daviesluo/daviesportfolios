"""The pre-registered AVAX-Q test. Scores 2024-01-01 through 2026-09-24.
...
when it was frozen. Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp100/avax_test.py
"""
...
DATA = Path(os.environ.get("FP100_DATA", "/tmp/fp100/data"))
PREREG = ROOT / "docs/agents/reviews/2026-09-25-fp100-prereg-avax.md"
SIDECAR = ROOT / "docs/agents/reviews/2026-09-25-fp100-prereg-avax.sha256"

# Measured 2026-09-25T05:23:21Z from data-api bookTicker AVAXUSDT, bid 10.17300000 / ask 10.17400000.
HALF_SPREAD = 4.914729444141376e-05
...
def pool_pnl(daily: dict[int, tuple]) -> list[float]:
    ...
    for t in daily:
        trade = c.fp5._trade(
            "AVAXUSDT", t, t + c.fp5.DAY_MS, daily,
...
def main() -> None:
    ...
    screen_alt_path = DATA / "spot1d" / "AVAXUSDT.json"
    screen_btc_path = DATA / "spot1d" / "BTCUSDT.json"
    oos_alt_path = DATA / "oos_1d" / "AVAXUSDT.json"
    oos_btc_path = DATA / "oos_1d" / "BTCUSDT.json"
    if not oos_alt_path.exists() or not oos_btc_path.exists():
        os.environ["FP100_OOS"] = "1"
        fetch.pull_oos()
    screen_alt = load_bars(screen_alt_path, 3)
    screen_btc = load_bars(screen_btc_path, 3)
    screen = c.relq_trades(screen_alt, screen_btc)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    screen_n = len(screen)
    if screen_n != 77 or abs(screen_total - 80.1845) > 0.01:
        raise SystemExit(f"2023 did not reproduce: n={screen_n} total={screen_total}")

    oos_alt = load_bars(oos_alt_path, 3)
    oos_btc = load_bars(oos_btc_path, 3)
    if min(oos_alt) != OOS_START or max(oos_alt) != OOS_END:
        raise SystemExit("the AVAX out-of-sample window is wrong")
    if min(oos_btc) != OOS_START or max(oos_btc) != OOS_END:
        raise SystemExit("the BTC out-of-sample window is wrong")
    if any(t > OOS_END for t in oos_alt) or any(t > OOS_END for t in oos_btc):
        raise SystemExit("a daily bar opened after 2026-09-25")
    alt = dict(screen_alt)
    btc = dict(screen_btc)
    alt.update(oos_alt)
    btc.update(oos_btc)
    if alt[OOS_START][1] <= 0 or btc[OOS_START][1] <= 0:
        raise SystemExit("the 2024-01-01 quote is still masked")
    if alt[OOS_END][1] != 0.0 or btc[OOS_END][1] != 0.0:
        raise SystemExit("the last open stored a quote")
    primary = c.relq_trades(alt, btc, 0.90, FEE, OOS_START, OOS_END)
    doubled = c.relq_trades(alt, btc, 0.90, FEE_DOUBLE, OOS_START, OOS_END)
    q80 = c.relq_trades(alt, btc, 0.80, FEE, OOS_START, OOS_END)
    q95 = c.relq_trades(alt, btc, 0.95, FEE, OOS_START, OOS_END)
    primary_sum = summarise(primary, pool_pnl(alt))
    ...
    payload = {
        ...
        "input_sha256": {
            "screen_avax_1d": hashlib.sha256(screen_alt_path.read_bytes()).hexdigest(),
            "screen_btc_1d": hashlib.sha256(screen_btc_path.read_bytes()).hexdigest(),
            "oos_avax_1d": hashlib.sha256(oos_alt_path.read_bytes()).hexdigest(),
            "oos_btc_1d": hashlib.sha256(oos_btc_path.read_bytes()).hexdigest(),
        },
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp100/avax_oos.json"
