"""The search fp100. The fill and the null are fp5's.

Nothing here reads the network or a file. A daily bar is (open, quote).
The open is the fill on AVAXUSDT. BTC's open is not an input. The signal is
AVAX's quote volume divided by BTC's quote volume on the same day.
...
def relq_at(
    ...
    """AVAX quote over BTC quote. None when either day is missing or a quote is not positive."""
    if day >= end_ms:
        return None
    a = alt.get(day)
    b = btc.get(day)
...
def relq_trades(
    ...
    """Enter the next AVAXUSDT daily open after AVAX quote ran high versus BTC. Hold one day."""
    trades = []
