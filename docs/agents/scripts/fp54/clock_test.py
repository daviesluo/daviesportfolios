"""The pre-registered VOL-CLOCK test. Scores 2024-01-01 through 2026-09-24.
...
when it was frozen. Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp54/clock_test.py
"""
...
PREREG = ROOT / "docs/agents/reviews/2026-09-25-fp54-prereg-clock.md"
SIDECAR = ROOT / "docs/agents/reviews/2026-09-25-fp54-prereg-clock.sha256"
...
def holes(hours: dict[int, tuple]) -> int:
    if not hours:
        return 0
    step = c.HOUR_MS
    stamps = sorted(hours)
    return sum(1 for i in range(1, len(stamps)) if stamps[i] - stamps[i - 1] != step)
...
def main() -> None:
    ...
    if not oos_hour_path.exists() or not oos_day_path.exists():
        os.environ["FP54_OOS"] = "1"
        fetch.pull_oos()
    screen_hours = load_bars(screen_hour_path, 2)
    screen_daily = load_bars(screen_day_path, 2)
    screen = c.clock_trades(screen_daily, screen_hours)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    screen_n = len(screen)
    if screen_n != 38 or abs(screen_total - 25.3277) > 0.01:
        raise SystemExit(f"2023 did not reproduce: n={screen_n} total={screen_total}")

    oos_hours = load_bars(oos_hour_path, 2)
    oos_daily = load_bars(oos_day_path, 2)
    hour_last = OOS_END - c.HOUR_MS
    if min(oos_hours) != OOS_START or max(oos_hours) != hour_last:
    ...
    if min(oos_daily) != OOS_START or max(oos_daily) != OOS_END:
    ...
    primary = c.clock_trades(daily, hours, 0.90, FEE, OOS_START, OOS_END)
    doubled = c.clock_trades(daily, hours, 0.90, FEE_DOUBLE, OOS_START, OOS_END)
    q80 = c.clock_trades(daily, hours, 0.80, FEE, OOS_START, OOS_END)
    q95 = c.clock_trades(daily, hours, 0.95, FEE, OOS_START, OOS_END)
    ...
    payload = {
        ...
        "hourly_bars": len(oos_hours),
        "hourly_holes": holes(oos_hours),
        "primary": primary_sum,
    ...
    out = ROOT / "docs/agents/backtests/fp54/clock_oos.json"
