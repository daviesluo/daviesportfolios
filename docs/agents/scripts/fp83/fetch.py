"""Pull bars for the fp83 screen.

Hourly bars stop before the 2024-01-01 open. The daily file keeps that open
because it can be an exit. A later open is a bug.

    python3 docs/agents/scripts/fp83/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp83-research"}
...
def main() -> None:
    ...
    hourly = klines("BTCUSDT", "1h", c.HOUR_MS, hour_end, (1, 2, 4))
    if any(t >= c.fp5.SCREEN_END_MS for t in hourly):
        raise SystemExit("an hourly bar opened on or after 2024-01-01")
    daily = klines("BTCUSDT", "1d", c.fp5.DAY_MS, c.fp5.SCREEN_END_MS, (1,))
    ...
    print(
        f"fp83 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
