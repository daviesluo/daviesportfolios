"""Score the fp83 screen. 2023 only.

    python3 docs/agents/scripts/fp83/measure.py
"""
...
DATA = Path(os.environ.get("FP83_DATA", "/tmp/fp83/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp83-protocol.md"
...
def main() -> None:
    ...
    trades = c.highclose_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "HIGH-CLOSE", trades, pool, c.MIN_N,
        "Close of the high's hour over the day's open, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp83_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp83_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp83/screen_2023.json"
