"""Score the fp125 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp125/measure.py
"""
...
DATA = Path(os.environ.get("FP125_DATA", "/tmp/fp125/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp125-protocol.md"
NOTE = "Open-contract-weighted days from the UTC date to each BTC option's expiry date, at the day's last hour, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP125_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "options" / "dte.json")
    ...
    row = c.fp5.summarise("DTE", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp125_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp125_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "dte": sha256(DATA / "options" / "dte.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp125/screen_2023.json"
