"""Pull the public series the fp125 screen is allowed to see.
...
requested. ETH, BNB, DOGE and XRP option files are not requested.
    ...
    python3 docs/agents/scripts/fp125/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp125-research"}
SPOT_FIELDS = (1,)
...
def pull_options() -> dict[int, tuple]:
    ...
    for ymd in days:
        ...
        for row in reader:
            ...
            oi = num(row[24])
            dte = c.days_to_expiry(row[2], file_ms)
            if oi is None or dte is None:
                continue
            out_rows.append((float(hour), dte, oi))
