"""The search fp151. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the dispersion of the taker ratio. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Population standard deviation of the day's taker ratios, divided by their mean."""
    if day >= end_ms:
        return None
    row = rows.get(day)
