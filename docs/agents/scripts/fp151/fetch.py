"""Pull every positive taker long/short volume ratio in the day. The last print is not the statistic. No hourly price bar is requested.

    python3 docs/agents/scripts/fp151/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp151-research"}
...
def pull_signal() -> dict[int, tuple]:
    out: dict[int, tuple] = {}
    for file_ms, parsed, column in read_metrics(7):
        if METRICS_HEADER[column] != "sum_taker_long_short_vol_ratio":
            raise SystemExit("the taker column moved")
        values = []
        blank = False
    ...
    save("metrics/taker_std.json", out)
    return out
