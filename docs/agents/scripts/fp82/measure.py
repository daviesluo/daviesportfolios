"""Score the fp82 screen. 2023 only.

    python3 docs/agents/scripts/fp82/measure.py
"""
...
DATA = Path(os.environ.get("FP82_DATA", "/tmp/fp82/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp82-protocol.md"
...
def main() -> None:
    ...
    trades = c.noon_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "NOON-QUOTE", trades, pool, c.MIN_N,
        "Quote share of hours 12 through 17, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp82_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp82_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp82/screen_2023.json"
