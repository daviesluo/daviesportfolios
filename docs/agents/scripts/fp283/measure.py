"""Score the fp283 screen. 2023 entries only.

    python3 docs/agents/scripts/fp283/measure.py
"""
...
DATA = Path(os.environ.get("FP283_DATA", "/tmp/fp283/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp283-protocol.md"
NOTE = 'The finished eight-day coin-margined return was negative and the eight-day return that ended eight days earlier was positive. The next coin-margined open is sold and bought back eleven days later. The other trade sells it eleven days later for another eleven days. Both windows have to fill. One leg. Funding cash is not added. The null is not that eleven-day short on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'DROPCM' or c.MIN_N != 30 or c.HOLD_DAYS != 11:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp283_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp283_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp283/screen_2023.json"
