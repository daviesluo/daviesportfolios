"""The search fp283. The finished eight-day coin-margined return was negative and the eight-day return that ended eight days earlier was positive. Sell the coin-margined book for eleven days. The other trade sells it eleven days later for another eleven days. Both windows have to fill. The entry open is not an input. One leg.
...
RULE_SIDE = 'short'
NULL_SIDE = 'short'
FILES = {'cm': ('cm1d/BTCUSD_PERP.json', True)}
PULLS = [('cm', 'BTCUSD_PERP', 'cm1d', True)]
...
def _signal(spot, um, cm, entry) -> bool:
    near = finished(cm, entry, 8)
    prev = finished(cm, entry - 8 * fp5.DAY_MS, 8)
    return near is not None and prev is not None and near < 0.0 and prev > 0.0
