"""The search fp166. Base volume above the volume ten days earlier, held three days.

Nothing here reads the network or a file. Ten days is the horizon. It is
not a percentile and it is not a one-day change. The fill buys the next
open and sells the open three days later.
"""
...
def signal_days(bars: dict[int, tuple], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Days whose base volume is strictly above the volume ten days earlier."""
    out = []
    for day in sorted(bars):
        ...
        if len(bars[day]) != 2 or len(bars[prev]) != 2:
            raise ValueError("a bar is the open and the base volume")
        if bars[prev][1] > 0.0 and bars[day][1] > bars[prev][1]:
            out.append(day)
    return out
