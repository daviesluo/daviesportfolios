"""Score the fp166 screen. 2023 entries only.

    python3 docs/agents/scripts/fp166/measure.py
"""
...
DATA = Path(os.environ.get("FP166_DATA", "/tmp/fp166/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp166-protocol.md"
NOTE = 'Base volume is above the volume ten days earlier. Next open, long BTC, and sell the open three days later.'
...
def main() -> None:
    if os.environ.get("FP166_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    if c.IDEA != 'VOL3' or c.MIN_N != 30:
        raise SystemExit("the frozen name or the count moved")

    if c.HORIZON != 10 or c.HOLD_DAYS != 3:
        raise SystemExit("the horizon or the hold moved")
    end = c.fp5.SCREEN_END_MS + 2 * c.fp5.DAY_MS
    bars = load_rows(DATA / "spot1d/BTCUSDT.json", 2, end)
    trades = c.signal_trades(bars)
    for trade in trades:
        check_common(trade)
        if trade["exit_ms"] - trade["entry_ms"] != 3 * c.fp5.DAY_MS:
            raise SystemExit("the hold moved")
        day = trade["entry_ms"] - c.fp5.DAY_MS
        earlier = day - 10 * c.fp5.DAY_MS
        if not (bars[earlier][1] > 0.0 and bars[day][1] > bars[earlier][1]):
            raise SystemExit("a trade was not volume above ten days earlier")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp166_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp166_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "spot_open_volume": sha256(DATA / "spot1d/BTCUSDT.json"),
        },
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp166/screen_2023.json"
