"""Score the fp75 screen. 2023 only.

    python3 docs/agents/scripts/fp75/measure.py
"""
...
DATA = Path(os.environ.get("FP75_DATA", "/tmp/fp75/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp75-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "OPEN-X", c.openx_trades(daily, hours), pool, c.MIN_N,
        "Crosses of today's open, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp75_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp75_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp75/screen_2023.json"
