"""Score the fp93 screen. 2023 only.

    python3 docs/agents/scripts/fp93/measure.py
"""
...
DATA = Path(os.environ.get("FP93_DATA", "/tmp/fp93/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp93-protocol.md"
...
def main() -> None:
    ...
    trades = c.tailbar_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "TAIL-BAR", trades, pool, c.MIN_N,
        "Last hour's range over the day's range, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp93_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp93_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp93/screen_2023.json"
