"""Score the fp139 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp139/measure.py
"""
...
DATA = Path(os.environ.get("FP139_DATA", "/tmp/fp139/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp139-protocol.md"
NOTE = "Intraday range of the taker long/short volume ratio, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP139_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "metrics/taker_range.json")
    ...
    row = c.fp5.summarise("TAKRNG", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp139_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp139_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "taker_range": sha256(DATA / "metrics/taker_range.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp139/screen_2023.json"
