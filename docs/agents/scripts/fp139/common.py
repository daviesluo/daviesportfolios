"""The search fp139. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the day's range of the taker long/short volume ratio. The level is not an input. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Maximum minus minimum. None when a sample is not positive."""
    if day >= end_ms:
        return None
    row = rows.get(day)
