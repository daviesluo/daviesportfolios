"""The pre-registered C3 test. Entries from 2024-01-01 through 2026-09-22.
...
frozen. Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp176/c3_test.py
"""
...
DATA = Path(os.environ.get("FP176_DATA", "/tmp/fp176/data"))
PREREG = ROOT / "docs/agents/reviews/2026-09-25-fp176-prereg-c3.md"
SIDECAR = ROOT / "docs/agents/reviews/2026-09-25-fp176-prereg-c3.sha256"
SCREEN = ROOT / "docs/agents/backtests/fp176/screen_2023.json"
...
ENTRY_END = int(datetime(2026, 9, 23, tzinfo=timezone.utc).timestamp() * 1000)
EXIT_OPEN = int(datetime(2026, 9, 25, tzinfo=timezone.utc).timestamp() * 1000)
LAST_ENTRY = int(datetime(2026, 9, 22, tzinfo=timezone.utc).timestamp() * 1000)
NULL_DRAWS = 1000
NULL_SEED = 20260925
...
def check_fill(bars: dict[int, tuple], trade: dict, fee: float) -> None:
    ...
    if entry >= ENTRY_END or entry >= EXIT_OPEN:
        raise SystemExit("a bar on or after 2026-09-23 was read as an entry")
    ...
    prior = [bars[day - k * c.fp5.DAY_MS][3] for k in range(1, 4)]
    if not bars[day][3] > max(prior):
        raise SystemExit("a trade did not close above the prior three closes")
...
def main() -> None:
    ...
    if sha256(common_path) != screen_doc["code_sha256"]["fp176_common"]:
    ...
    screen_bars = load_bars(screen_path, c.fp5.SCREEN_END_MS + 2 * c.fp5.DAY_MS)
    if max(screen_bars) != c.fp5.SCREEN_END_MS + 2 * c.fp5.DAY_MS:
        raise SystemExit("the screen file does not stop on 2024-01-03")
    screen = c.signal_trades(screen_bars)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    if len(screen) != 119 or abs(screen_total - 149.937) > 0.01:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp176_common": sha256(common_path),
            "fp176_c3_test": sha256(Path(__file__)),
        },
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp176/c3_oos.json"
