"""Score the fp176 screen. 2023 entries only.

    python3 docs/agents/scripts/fp176/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp176-protocol.md"
NOTE = 'The close is strictly above each of the prior three closes. Next open, long BTC, and sell the open three days later.'
...
def main() -> None:
    if os.environ.get("FP176_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    if c.IDEA != 'C3' or c.MIN_N != 30:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp176_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp176_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp176/screen_2023.json"
