"""Pins for the fp124 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    row = (23, 0.5, 4.0, 23, 0.5, 4.0, 23, -1.0, 4.0, 23, 0.5, 0.0, 0, 9.0, 9.0)
    if not near(c.signal_at({day: row}, day), 4.0):
        raise SystemExit("gamma times open contracts was wrong")
    if near(c.signal_at({day: row}, day), 0.5):
        raise SystemExit("the mean replaced the sum")
