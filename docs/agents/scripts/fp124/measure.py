"""Score the fp124 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp124/measure.py
"""
...
DATA = Path(os.environ.get("FP124_DATA", "/tmp/fp124/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp124-protocol.md"
NOTE = "Sum of gamma times open contracts at the day's last hour, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP124_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "options" / "gamma.json")
    ...
    row = c.fp5.summarise("GAMMA", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp124_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp124_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "gamma": sha256(DATA / "options" / "gamma.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp124/screen_2023.json"
