"""Score the fp161 screen. 2023 entries only.

    python3 docs/agents/scripts/fp161/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp161-protocol.md"
NOTE = 'The three funding rates rise through the day. Next open, long BTC for one day. The funding cash is not added.'
...
def main() -> None:
    ...
    if c.IDEA != 'FNCLIMB' or c.MIN_N != 30:
    ...
    for trade in trades:
        ...
        if not (rate_00 < rate_08 < rate_16):
            raise SystemExit("a trade was not a rising funding path")
        entry_px = bars[trade["entry_ms"]][0]
        exit_px = bars[trade["exit_ms"]][0]
        if abs(trade["gross"] - (exit_px / entry_px - 1.0)) > 1e-12:
            raise SystemExit("the gross is not the two opens")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp161_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp161_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp161/screen_2023.json"
