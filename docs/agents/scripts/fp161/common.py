"""The search fp161. A rising funding path, then a one-day long.

Nothing here reads the network or a file. The 00:00 rate is strictly under
the 08:00 rate, and that rate is strictly under the 16:00 rate. No percentile
is used. The fill is one day long. The funding cash is not added.
"""
...
def signal_days(funding: dict[int, tuple], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Days whose three rates strictly rise through the day."""
    out = []
    for day in sorted(funding):
        if day >= end_ms:
            continue
        rate_00, rate_08, rate_16 = funding[day]
        if rate_00 < rate_08 < rate_16:
            out.append(day)
    return out
