"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp161/fetch.py
"""
...
DATA = Path(os.environ.get("FP161_DATA", "/tmp/fp161/data"))
VISION = "https://data.binance.vision"
MARKET = "https://data-api.binance.vision"
LOOKBACK_MS = 1_664_582_400_000  # 2022-10-01 00:00 UTC
UA = {"User-Agent": "daviesportfolios-fp161-research"}
...
def pull_opens(end_ms: int, rel: str, indexes: tuple[int, ...]) -> dict[int, tuple]:
    cursor = LOOKBACK_MS
    rows: dict[int, tuple] = {}
    while cursor <= end_ms:
        url = (
        ...
        if len(batch) < 1000:
            break
    assert_span(rows, end_ms, "BTCUSDT 1d", True)
    save(rel, rows)
    return rows
...
def pull_metric_ends(last_ymd: str, fields: str) -> dict[int, tuple]:
    """`fields` is `oi` or `taker_ends`. One row per UTC day: the first print and the last."""
    ...
    if stop > datetime.datetime(2023, 12, 31, tzinfo=datetime.timezone.utc):
    ...
    while day <= stop:
        ...
        elif fields == "taker_ends":
            rows[open_ms] = (
                float(body[0][idx["sum_taker_long_short_vol_ratio"]]),
                float(body[-1][idx["sum_taker_long_short_vol_ratio"]]),
            )
        else:
    ...
    rel = "oi/ends.json" if fields == "oi" else "taker/ends.json"
    save(rel, rows)
    return rows

def main() -> None:
    if os.environ.get("FP161_OOS"):
        raise SystemExit("this pull does not request a later year")
    price_end = c.fp5.SCREEN_END_MS
    funding = pull_funding(c.fp5.SCREEN_END_MS - c.fp5.DAY_MS)
    spot = pull_opens(price_end, "spot1d/BTCUSDT.json", (1,))
    print("fp161 " + f"funding {len(funding)} spot {len(spot)} last {max(spot)}")
