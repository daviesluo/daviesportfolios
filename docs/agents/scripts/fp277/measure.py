"""Score the fp277 screen. 2023 entries only.

    python3 docs/agents/scripts/fp277/measure.py
"""
...
DATA = Path(os.environ.get("FP277_DATA", "/tmp/fp277/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp277-protocol.md"
NOTE = 'A finished twenty-day coin-margined rise was strictly smaller than the twenty-day rise that ended twenty days earlier. The next coin-margined open is bought and sold eight days later. The other trade buys that book eight days later for another eight days. Both windows have to fill. One leg. Funding cash is not added. The null is not that eight-day long on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'COOLED' or c.MIN_N != 30 or c.HOLD_DAYS != 8:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp277_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp277_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp277/screen_2023.json"
