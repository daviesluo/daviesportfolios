"""The search fp277. A finished twenty-day coin-margined rise was strictly smaller than the twenty-day rise that ended twenty days earlier. Buy the coin-margined book for eight days. The other trade buys it eight days later for another eight days. Both windows have to fill. The entry open is not an input. One leg.
...
def _signal(spot, um, cm, entry) -> bool:
    near = finished(cm, entry, 20)
    prev = finished(cm, entry - 20 * fp5.DAY_MS, 20)
    return near is not None and prev is not None and near > 0.0 and near < prev
