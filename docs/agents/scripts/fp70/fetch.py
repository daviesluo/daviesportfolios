"""Pull bars for the fp70 screen.

The 2024-01-01 high, low and close are stored equal to the open.

No later year is requested. The 2024-01-01 bar keeps its open. Its signal fields are masked.

    python3 docs/agents/scripts/fp70/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp70-research"}
...
def horizon(open_: float, fields: tuple[int, ...]) -> tuple:
    # fields (1, 2, 3, 4): high, low and close stored equal to the open
    return (open_, open_, open_, open_)


def main() -> None:
    if os.environ.get("FP70_OOS"):
        raise SystemExit("this pull does not request a later year")
    rows = klines("BTCUSDT", "1d", c.fp5.DAY_MS, c.fp5.SCREEN_END_MS, (1, 2, 3, 4))
    ...
    if horizon_bar is None or horizon_bar != (horizon_bar[0], horizon_bar[0], horizon_bar[0], horizon_bar[0]):
    ...
    print(f"fp70 BTCUSDT 1d {len(rows)} first {min(rows)} last {max(rows)}")
