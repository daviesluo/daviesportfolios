"""Score the fp45 screen. 2023 only.

    python3 docs/agents/scripts/fp45/measure.py
"""
...
DATA = Path(os.environ.get("FP45_DATA", "/tmp/fp45/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp45-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "YDAY-HIGH", c.yhigh_trades(daily), pool, c.MIN_N,
        "Close versus yesterday's high, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp45_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp45_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp45/screen_2023.json"
