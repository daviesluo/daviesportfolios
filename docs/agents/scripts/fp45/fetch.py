"""Pull daily bars for the fp45 screen.

A bar is the open, the high and the close. The 2024-01-01 high and close are stored as the open.
...
open is a bug.
    ...
    python3 docs/agents/scripts/fp45/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp45-research"}
...
def main() -> None:
    ...
    for symbol in ("BTCUSDT",):
        ...
        save(symbol, rows)
        print(f"fp45 {symbol} 1d {len(rows)} first {min(rows)} last {max(rows)}")
