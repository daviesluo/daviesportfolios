"""Score the fp264 screen. 2023 entries only.

    python3 docs/agents/scripts/fp264/measure.py
"""
...
DATA = Path(os.environ.get("FP264_DATA", "/tmp/fp264/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp264-protocol.md"
NOTE = 'Spot and the USDT perpetual both finished a five-day decline. The next USDT open is bought and sold fourteen days later. The other trade buys the coin-margined book for those same fourteen days. One leg. Funding cash is not added. The null is not that USDT long with the two declines turned off.'
...
def run() -> None:
    ...
    if c.IDEA != 'AFTER' or c.MIN_N != 30 or c.HOLD_DAYS != 14:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp264_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp264_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp264/screen_2023.json"
