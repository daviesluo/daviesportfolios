"""The search fp264. After spot and the USDT book both finished a five-day decline, buy the USDT book for fourteen days. The other trade buys the coin-margined book for those same fourteen days.
...
def _signal(spot, um, cm, entry) -> bool:
    s = finished(spot, entry, 5)
    u = finished(um, entry, 5)
    return s is not None and u is not None and s < 0.0 and u < 0.0
