"""Score the fp94 screen. 2023 only.

    python3 docs/agents/scripts/fp94/measure.py
"""
...
DATA = Path(os.environ.get("FP94_DATA", "/tmp/fp94/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp94-protocol.md"
...
def main() -> None:
    ...
    trades = c.rest_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "REST", trades, pool, c.MIN_N,
        "Last close over the first hour's close, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp94_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp94_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp94/screen_2023.json"
