"""Score the fp207 screen. 2023 entries only.

    python3 docs/agents/scripts/fp207/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp207-protocol.md"
NOTE = "The USDT close finished strictly above the index close. The next spot close is sold and the following spot close is bought. The null is that close-to-close short on every day."
...
def main() -> None:
    ...
    um = load_rows(DATA / "umcls/BTCUSDT.json", 1, c.SIGNAL_LAST_MS, required=True)
    index = load_rows(DATA / "index1d/BTCUSDT.json", 1, c.SIGNAL_LAST_MS, required=True)
    spot = load_rows(DATA / "spot1d/BTCUSDT.json", 1, end, required=True)
    trades = c.signal_trades(um, index, spot)
    for trade in trades:
        check_common(trade)
        day = trade["entry_ms"] - c.fp5.DAY_MS
        if day > c.SIGNAL_LAST_MS or day not in um or day not in index:
            raise SystemExit("the signal day is past the window")
        if not um[day][0] > index[day][0]:
            raise SystemExit("a trade was not above the index")
        if trade["exit_ms"] != trade["entry_ms"] + c.fp5.DAY_MS:
            raise SystemExit("the hold moved")
        entry_px = spot[trade["entry_ms"]][0]
    ...
    payload = {
        ...
        "code_sha256": {
            "fp207_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp207_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp207/screen_2023.json"
