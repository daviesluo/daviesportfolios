"""The search fp207. The USDT close finished above the index close.

Nothing here reads the network or a file. Both closes are known when the
signal day closes. The next spot close is sold and the following spot
...
def signal_days(
    um: dict[int, tuple], index: dict[int, tuple], end_ms: int = fp5.SCREEN_END_MS,
) -> list[int]:
    """Days whose USDT close finished strictly above the index close."""
    out = []
    for day in sorted(set(um) & set(index)):
        if day > SIGNAL_LAST_MS:
            continue
        entry = day + fp5.DAY_MS
        if not (fp5.SCREEN_START_MS <= entry < end_ms):
            continue
        if len(um[day]) != 1 or len(index[day]) != 1:
            raise ValueError("the input is the close")
        if um[day][0] > index[day][0]:
            out.append(day)
    return out
...
def signal_trades(
    um: dict[int, tuple], index: dict[int, tuple], spot: dict[int, tuple],
    fee: float = fp5.FEE, start_ms: int = fp5.SCREEN_START_MS,
    end_ms: int = fp5.SCREEN_END_MS,
) -> list[dict]:
    trades = []
    for day in signal_days(um, index, end_ms):
        entry = day + fp5.DAY_MS
        if not (start_ms <= entry < end_ms):
            continue
        trade = _close_short(spot, entry, fee)
...
def pool_nets(
    spot: dict[int, tuple], fee: float = fp5.FEE,
    start_ms: int = fp5.SCREEN_START_MS, end_ms: int = fp5.SCREEN_END_MS,
) -> list[float]:
    """A spot close-to-close short on every in-screen day."""
    pool = []
    day = start_ms
    while day < end_ms:
        trade = _close_short(spot, day, fee)
        if trade is not None:
