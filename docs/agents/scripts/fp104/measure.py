"""Score the fp104 screen. 2023 entries only. The hold is 3 days.

    python3 docs/agents/scripts/fp104/measure.py
"""
...
DATA = Path(os.environ.get("FP104_DATA", "/tmp/fp104/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp104-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "FRNG-3", trades, pool, c.MIN_N,
        "Range of BTC's three funding prints, above its own trailing 90th. "
        "Next day, long BTC for 3 days.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp104_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp104_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp104/screen_2023.json"
