"""Pins for the fp122 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    row = (23, 1, 6, 23, 1, 6, 23, 0, 4)
    if not near(c.signal_at({day: row}, day), 3.0):
        raise SystemExit("put over call was wrong")
    if near(c.signal_at({day: row}, day), 4.0 / 12.0):
        raise SystemExit("the reciprocal replaced the ratio")
    early = (0, 1, 100, 0, 0, 1, 23, 1, 2, 23, 0, 10)
    if not near(c.signal_at({day: early}, day), 0.2):
        raise SystemExit("an earlier hour entered the ratio")
    if c.signal_at({day: (23, 1, 6)}, day) is not None:
        raise SystemExit("a one-sided book was a print")
    if c.signal_at({day: (23, 1, 0, 23, 0, 4)}, day) is not None:
        raise SystemExit("a zero leg was a print")
    if c.signal_at({day: (23, 2, 6, 23, 0, 4)}, day) is not None:
        raise SystemExit("a bad side was a print")
...
def test_tail_and_fill() -> None:
    ...
    for i in range(121):
        ...
        rows[day] = (23, 1, 1, 23, 0, 5)
    last = start + 120 * c.fp5.DAY_MS
    rows[last] = (23, 1, 9, 23, 0, 10)
