"""Score the fp122 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp122/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp122-protocol.md"
NOTE = "BTC put volume_contracts over call volume_contracts at the day's last hour, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_book(DATA / "options" / "pc_vol.json")
    ...
    row = c.fp5.summarise("PCVOL", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp122_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp122_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "pc_vol": sha256(DATA / "options" / "pc_vol.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp122/screen_2023.json"
