"""Score the fp182 screen. 2023 entries only.

    python3 docs/agents/scripts/fp182/measure.py
"""
...
DATA = Path(os.environ.get("FP182_DATA", "/tmp/fp182/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp182-protocol.md"
NOTE = "The low undercut yesterday's low and the close finished above yesterday's close. Next day, long BTC from the open to the close."
...
def main() -> None:
    if os.environ.get("FP182_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    if c.IDEA != 'DIP' or c.MIN_N != 30:
    ...
    for trade in trades:
        ...
        if not (bars[day][2] < prev[2] and bars[day][3] > prev[3]):
            raise SystemExit("a trade was not a lower low and a higher close")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp182_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp182_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp182/screen_2023.json"
