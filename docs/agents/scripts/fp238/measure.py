"""Score the fp238 screen. 2023 entries only.

    python3 docs/agents/scripts/fp238/measure.py
"""
...
DATA = Path(os.environ.get("FP238_DATA", "/tmp/fp238/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp238-protocol.md"
NOTE = "Yesterday's USDT open finished above the close of the day before. The next open is bought and sold six days later. One USDT-perpetual leg. Funding cash is not added. The null is that six-day long on every day."
...
def check_signal(trade: dict, books: dict) -> None:
    ...
    opened = c._px(books["um"], entry - c.fp5.DAY_MS, 0)
    prior = c._px(books["um"], entry - 2 * c.fp5.DAY_MS, 1)
    if opened is None or prior is None or not opened > prior:
        raise SystemExit("yesterday did not open above the prior close")
...
def run() -> None:
    ...
    if c.IDEA != 'GAP' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp238_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp238_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp238/screen_2023.json"
