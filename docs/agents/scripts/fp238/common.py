"""The search fp238. Yesterday's open finished above the close of the day before, then a six-day USDT long.

Nothing here reads the network or a file. Yesterday's open and the prior close have already printed. The entry open is not an input. The position is one
leg. It is not a one-day trade and it is not an overnight trade. Funding
cash is not added. The null is that same 6-day long on every
day.
"""
...
def _signal(um: dict, entry: int) -> bool:
    opened = _px(um, entry - fp5.DAY_MS, 0)
    prior = _px(um, entry - 2 * fp5.DAY_MS, 1)
    if opened is None or prior is None:
        return False
    return opened > prior
