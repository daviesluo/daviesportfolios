"""The search fp248. Yesterday's USDT close above the prior close and below the close five days earlier, then a twenty-one-day spot long.
...
not added. The null is that same 21-day long on every day.
"""
...
def _signal(um: dict, spot: dict, entry: int) -> bool:
    ...
    yesterday = _px(um, end, 1)
    prior = _px(um, end - fp5.DAY_MS, 1)
    older = _px(um, end - 5 * fp5.DAY_MS, 1)
    if yesterday is None or prior is None or older is None:
        return False
    return yesterday > prior and yesterday < older
