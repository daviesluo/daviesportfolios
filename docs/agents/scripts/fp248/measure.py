"""Score the fp248 screen. 2023 entries only.

    python3 docs/agents/scripts/fp248/measure.py
"""
...
DATA = Path(os.environ.get("FP248_DATA", "/tmp/fp248/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp248-protocol.md"
NOTE = "Yesterday's USDT close finished above the prior close and below the close five days earlier. The next spot open is bought and sold twenty-one days later. One spot leg. Funding cash is not added. The null is that twenty-one-day long on every day."
...
def check_signal(trade: dict, books: dict) -> None:
    end = trade["entry_ms"] - c.fp5.DAY_MS
    yesterday = c._px(books["um"], end, 1)
    prior = c._px(books["um"], end - c.fp5.DAY_MS, 1)
    older = c._px(books["um"], end - 5 * c.fp5.DAY_MS, 1)
    if yesterday is None or prior is None or older is None or not (yesterday > prior and yesterday < older):
        raise SystemExit("the bounce was not already finished")
...
def run() -> None:
    ...
    books = {
        ...
        "spot": load_rows(DATA / "spot1d/BTCUSDT.json", 2, end),
    }
    trades = c.signal_trades(books['um'], books['spot'])
    spans = c.signal_spans(books['um'], books['spot'])
    ...
    for trade in trades:
        ...
        entry_px = books["spot"][trade["entry_ms"]][0]
        exit_px = books["spot"][trade["exit_ms"]][0]
    ...
    pool_spans = c.pool_spans(books['um'], books['spot'])
    pool = c.pool_nets(books['um'], books['spot'])
    if c.IDEA != 'BOUN' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp248_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp248_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"um": sha256(DATA / "um1d/BTCUSDT.json"), "spot": sha256(DATA / "spot1d/BTCUSDT.json")},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp248/screen_2023.json"
