...
because it can be an exit. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp85/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp85-research"}
...
def main() -> None:
    ...
    hourly = klines("BTCUSDT", "1h", c.HOUR_MS, hour_end, (1, 4))
    ...
    print(
        f"fp85 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
