"""Score the fp85 screen. 2023 only.

    python3 docs/agents/scripts/fp85/measure.py
"""
...
DATA = Path(os.environ.get("FP85_DATA", "/tmp/fp85/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp85-protocol.md"
...
def main() -> None:
    ...
    trades = c.medopen_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "MED-OPEN", trades, pool, c.MIN_N,
        "Lower median hourly close over the day's open, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp85_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp85_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp85/screen_2023.json"
