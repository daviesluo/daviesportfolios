"""Score the fp96 screen. 2023 only.

    python3 docs/agents/scripts/fp96/measure.py
"""
...
DATA = Path(os.environ.get("FP96_DATA", "/tmp/fp96/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp96-protocol.md"
...
def main() -> None:
    if os.environ.get("FP96_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")

    alt_path = DATA / "spot1d" / "BNBUSDT.json"
    ...
    for trade in trades:
        if trade["coin"] != "BNBUSDT":
            raise SystemExit("a fill is not BNB")
    ...
    pool = c.fp5.pool_from_bars({"BNBUSDT": alt}, ["BNBUSDT"], c.fp5.DAY_MS)
    row = c.fp5.summarise(
        "BNB-Q", trades, pool, c.MIN_N,
        "BNB's daily quote volume over BTC's, above its own trailing 90th. Next day, long BNB.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp96_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp96_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp96/screen_2023.json"
