"""Score the fp183 screen. 2023 entries only.

    python3 docs/agents/scripts/fp183/measure.py
"""
...
DATA = Path(os.environ.get("FP183_DATA", "/tmp/fp183/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp183-protocol.md"
NOTE = "An up day whose body is strictly wider than yesterday's body. Next day, long BTC from the open to the close."
...
def main() -> None:
    ...
    for trade in trades:
        ...
        today = bars[day]
        if not (today[3] > today[0] and abs(today[3] - today[0]) > abs(prev[3] - prev[0])):
            raise SystemExit("a trade was not a wider up body")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp183_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp183_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp183/screen_2023.json"
