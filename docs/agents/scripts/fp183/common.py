"""The search fp183. An up day with a body wider than yesterday's body.
...
def signal_days(bars: dict[int, tuple], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Up days whose body is strictly wider than the prior day's body."""
    out = []
    for day in sorted(bars):
        ...
        if close_px > open_px and abs(close_px - open_px) > abs(prior_close - prior_open):
            out.append(day)
    return out
