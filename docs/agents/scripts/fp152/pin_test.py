...
"""Pins for the fp152 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    rows = {prev: (0.005,), day: (0.02,)}
    if not near(c.signal_at(rows, day), 0.015):
        raise SystemExit("the close jump was not the signal")
    if near(c.signal_at(rows, day), 0.02):
        raise SystemExit("the close replaced the jump")
    if near(c.signal_at(rows, day), 4.0):
        raise SystemExit("the ratio replaced the jump")
    if not near(c.signal_at({prev: (-0.01,), day: (-0.04,)}, day), -0.03):
        raise SystemExit("a negative close was dropped")
    if c.signal_at({day: (0.02,)}, day) is not None:
        raise SystemExit("a day with no yesterday was a print")
    if not near(c.signal_at({prev: (0.01,), day: (0.01,)}, day), 0.0):
        raise SystemExit("a flat jump was dropped")
    if c.signal_prints({prev: (0.01,), day: (0.01,)}) != [(day, 0.0)]:
...
def test_tail_and_fill() -> None:
    ...
    for i in range(121):
        ...
        rows[day] = (1.0,) if i == 120 else (0.0,)
