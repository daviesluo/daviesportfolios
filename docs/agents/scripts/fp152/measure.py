"""Score the fp152 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp152/measure.py
"""
...
DATA = Path(os.environ.get("FP152_DATA", "/tmp/fp152/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp152-protocol.md"
NOTE = "The BTC premium-index close minus yesterday's close, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP152_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "premium/close.json")
    ...
    row = c.fp5.summarise("BASJMP", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp152_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp152_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp152/screen_2023.json"
