"""The search fp152. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the jump in the premium close. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Today's premium close minus yesterday's. A close may be negative."""
    if day >= end_ms:
        return None
    today = rows.get(day)
