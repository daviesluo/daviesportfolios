"""Pull the premium-index close. The high and the low are not stored. A missing day is not filled. No hourly price bar is requested.

    python3 docs/agents/scripts/fp152/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp152-research"}
...
def pull_signal() -> dict[int, tuple]:
    ...
    if any(len(row) != 1 for row in out.values()):
        raise SystemExit("a premium bar stored the high or the low")
