"""Pins for the fp97 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    if not near(c.relq_at(alt, btc, day), 0.4):
        raise SystemExit("XRP quote over BTC quote was wrong")
    ...
    if near(c.relq_at(alt, btc, day), 100.0 / 40.0):
        raise SystemExit("BTC quote over XRP quote replaced the ratio")
    if c.relq_at({day: (7.0, 0.0)}, btc, day) is not None:
        raise SystemExit("a day with no XRP quote was a print")
...
def test_tail_and_fill() -> None:
    ...
    if filled[0]["coin"] != c.COIN:
        raise SystemExit("the fill is not XRP")
