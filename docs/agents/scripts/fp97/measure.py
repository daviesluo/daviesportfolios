"""Score the fp97 screen. 2023 only.

    python3 docs/agents/scripts/fp97/measure.py
"""
...
DATA = Path(os.environ.get("FP97_DATA", "/tmp/fp97/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp97-protocol.md"
...
def main() -> None:
    if os.environ.get("FP97_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")

    alt_path = DATA / "spot1d" / "XRPUSDT.json"
    ...
    for trade in trades:
        if trade["coin"] != "XRPUSDT":
            raise SystemExit("a fill is not XRP")
    ...
    pool = c.fp5.pool_from_bars({"XRPUSDT": alt}, ["XRPUSDT"], c.fp5.DAY_MS)
    row = c.fp5.summarise(
        "XRP-Q", trades, pool, c.MIN_N,
        "XRP's daily quote volume over BTC's, above its own trailing 90th. Next day, long XRP.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp97_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp97_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp97/screen_2023.json"
