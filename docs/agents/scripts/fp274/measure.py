"""Score the fp274 screen. 2023 entries only.

    python3 docs/agents/scripts/fp274/measure.py
"""
...
DATA = Path(os.environ.get("FP274_DATA", "/tmp/fp274/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp274-protocol.md"
NOTE = "Yesterday's USDT close finished above yesterday's spot close. The next USDT open is sold and bought back eighteen days later. The other trade sells spot for those same eighteen days. One leg. Funding cash is not added. The null is not that USDT short with the comparison turned off."
...
def run() -> None:
    ...
    if c.IDEA != 'ABOVE' or c.MIN_N != 30 or c.HOLD_DAYS != 18:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp274_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp274_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp274/screen_2023.json"
