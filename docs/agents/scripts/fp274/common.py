"""The search fp274. Yesterday's USDT close finished above yesterday's spot close. Sell the USDT book for eighteen days. The other trade sells spot for those same eighteen days. The entry open is not an input. One leg.
...
def _signal(spot, um, cm, entry) -> bool:
    um_px = _px(um, entry - fp5.DAY_MS, 1)
    spot_px = _px(spot, entry - fp5.DAY_MS, 1)
    return um_px is not None and spot_px is not None and um_px > spot_px
