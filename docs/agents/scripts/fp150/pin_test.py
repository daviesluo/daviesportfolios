...
"""Pins for the fp150 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    rows = {prev: (100.0, 100.0), day: (300.0, 100.0)}
    if not near(c.signal_at(rows, day), 2.0):
        raise SystemExit("the volume jump was not the signal")
    if near(c.signal_at(rows, day), 3.0):
        raise SystemExit("the level replaced the jump")
    if c.signal_at({day: (300.0, 100.0)}, day) is not None:
        raise SystemExit("a day with no yesterday was a print")
    hole = day + c.fp5.DAY_MS
    if c.signal_at({prev: (100.0, 100.0), hole: (100.0, 100.0)}, hole) is not None:
        raise SystemExit("a missing yesterday was filled")
    flat = {prev: (200.0, 100.0), day: (100.0, 50.0)}
    if not near(c.signal_at(flat, day), 0.0):
        raise SystemExit("equal ratios were dropped")
    if c.signal_prints(flat) != [(day, 0.0)]:
        raise SystemExit("a zero jump left the history")
    quiet = None

    later = c.fp5.SCREEN_END_MS
    if c.signal_at({later: (300.0, 100.0)}, later) is not None:
...
def test_tail_and_fill() -> None:
    ...
    for i in range(121):
        ...
        rows[day] = (300.0, 100.0) if i == 120 else (100.0, 100.0)
