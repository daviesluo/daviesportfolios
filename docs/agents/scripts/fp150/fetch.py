"""Pull USDT-margined base volume and coin-margined quote volume. The August gap is not filled. No hourly price bar is requested.

    python3 docs/agents/scripts/fp150/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp150-research"}
...
def pull_leg(market: str, symbol: str, index: int, unit_check: bool) -> dict[int, float]:
    out: dict[int, float] = {}
    ...
    for ym in months():
        ...
        for row in csv.reader(io.StringIO(zf.read(zf.namelist()[0]).decode())):
            if not row or row[0] == "open_time" or len(row) <= 7:
                continue
            open_ms = int(row[0])
            if open_ms < LOOKBACK_MS or open_ms > end_ms:
                continue
            if unit_check:
                contracts = float(row[5])
                btc = float(row[7])
                if contracts > 0 and btc > 0 and not btc < contracts:
                    raise SystemExit("CM quote_volume is not the BTC amount")
                value = btc
            else:
...
def pull_signal() -> dict[int, tuple]:
    um = pull_leg("um", "BTCUSDT", 5, False)
    cm = pull_leg("cm", "BTCUSD_PERP", 7, True)
    out = {day: (um[day], cm[day]) for day in um if day in cm}
    stop = c.fp5.SCREEN_END_MS - c.fp5.DAY_MS
    if not out or min(out) != LOOKBACK_MS or max(out) != stop:
        raise SystemExit("the volume ratio does not span the window")
    if len(out) < 440:
        raise SystemExit("volume days are too few")
    if any(t >= c.fp5.SCREEN_END_MS for t in out):
        raise SystemExit("a volume day is in 2024")
    save("volume/both.json", out)
    return out
