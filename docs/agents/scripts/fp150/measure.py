"""Score the fp150 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp150/measure.py
"""
...
DATA = Path(os.environ.get("FP150_DATA", "/tmp/fp150/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp150-protocol.md"
NOTE = "USDT-margined BTC volume over coin-margined BTC volume, minus yesterday's ratio, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP150_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "volume/both.json")
    ...
    row = c.fp5.summarise("UMCMJMP", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp150_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp150_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "volumes": sha256(DATA / "volume/both.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp150/screen_2023.json"
