"""The search fp150. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the jump in USDT-margined volume over coin-margined volume. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Today's volume ratio minus yesterday's. Yesterday is one day earlier."""
    if day >= end_ms:
        return None
    today = rows.get(day)
    ...
    if today is None or prev is None or len(today) != 2 or len(prev) != 2:
        return None
    um, cm = today
    prev_um, prev_cm = prev
    if um <= 0 or cm <= 0 or prev_um <= 0 or prev_cm <= 0:
        return None
    return um / cm - prev_um / prev_cm
