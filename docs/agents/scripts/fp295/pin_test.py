...
"""Pins for the fp295 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != 'GAPCM' or c.HOLD_DAYS != 10 or c.NULL_KIND != "other_regime":
        raise SystemExit("the idea moved")
    if c.DISCOUNT is not None or c.RULE_SIDE != "short" or c.RULE_BOOK != 'cm':
    ...
    _put(traded, entry - day - 6 * day, 1, 100)
    _put(fair, entry - day - 6 * day, 1, 100)
    _put(traded, entry, 50, 1)
    _put(traded, entry + 10 * day, 45, 1)
    for ts in (off - day, off - day - 6 * day):
        _put(traded, ts, 1, 100)
        _put(fair, ts, 1, 100)
    _put(traded, off, 40, 1)
    _put(traded, off + 10 * day, 42, 1)
