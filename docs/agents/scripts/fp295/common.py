"""The search fp295. The basis is the coin-margined close over the USDT close. That basis finished positive and strictly above the basis six days earlier. Sell the coin-margined book for ten days. The other trade sells it for ten days when that wider premium is absent.
...
it is not an overnight trade. Funding cash is not added. The null is the same
leg with the price edge turned off. It is not the opposite side and it is not
...
RULE_SIDE = 'short'
NULL_SIDE = 'short'
FILES = {'um': ('um1d/BTCUSDT.json', False), 'cm': ('cm1d/BTCUSD_PERP.json', True)}
PULLS = [('um', 'BTCUSDT', 'um1d', False), ('cm', 'BTCUSD_PERP', 'cm1d', True)]
...
def _closes(book: dict, end_ts: int, n: int) -> list[float] | None:
    out = []
    for k in range(n):
        close = _px(book, end_ts - k * fp5.DAY_MS, 1)
        if close is None:
            return None
        out.append(close)
    return out
...
def _basis(left, right, entry, lag=0):
    ts = entry - fp5.DAY_MS - lag * fp5.DAY_MS
    a = _px(left, ts, 1)
    b = _px(right, ts, 1)
...
def _widen(spot, um, cm, entry):
    now = _basis(cm, um, entry, 0)
    old = _basis(cm, um, entry, 6)
    if now is None or old is None:
        return None
    return now > old and now > 0.0


def _rule_signal(spot, um, cm, entry) -> bool:
    return _widen(spot, um, cm, entry) is True


def _null_signal(spot, um, cm, entry) -> bool:
    return _widen(spot, um, cm, entry) is False
