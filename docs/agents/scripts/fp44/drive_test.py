"""The pre-registered OPEN-DRIVE test. Scores 2024-01-01 through 2026-09-24.
...
when it was frozen. Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp44/drive_test.py
"""
...
DATA = Path(os.environ.get("FP44_DATA", "/tmp/fp44/data"))
PREREG = ROOT / "docs/agents/reviews/2026-09-25-fp44-prereg-drive.md"
SIDECAR = ROOT / "docs/agents/reviews/2026-09-25-fp44-prereg-drive.sha256"

# Measured 2026-09-25T03:28:08Z from data-api bookTicker BTCUSDT, bid 84200.00000000 / ask 84200.01000000.
HALF_SPREAD = 5.938241924546982e-08
...
def main() -> None:
    ...
    screen_hour_path = DATA / "spot1h" / "BTCUSDT.json"
    screen_day_path = DATA / "spot1d" / "BTCUSDT.json"
    oos_hour_path = DATA / "oos_1h" / "BTCUSDT.json"
    oos_day_path = DATA / "oos_1d" / "BTCUSDT.json"
    if not oos_hour_path.exists() or not oos_day_path.exists():
        os.environ["FP44_OOS"] = "1"
        fetch.pull_oos()
    screen_hours = load_bars(screen_hour_path, 3)
    screen_daily = load_bars(screen_day_path, 2)
    screen = c.drive_trades(screen_daily, screen_hours)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    screen_n = len(screen)
    if screen_n != 36 or abs(screen_total - 29.0716) > 0.01:
        raise SystemExit(f"2023 did not reproduce: n={screen_n} total={screen_total}")

    oos_hours = load_bars(oos_hour_path, 3)
    oos_daily = load_bars(oos_day_path, 2)
    ...
    if min(oos_hours) != OOS_START or max(oos_hours) != OOS_END - c.HOUR_MS:
        raise SystemExit("the out-of-sample hours are not the locked window")
    if any(t >= OOS_END for t in oos_hours):
        raise SystemExit("an hourly bar opened on or after 2026-09-25")
    if len(oos_daily[OOS_END]) != 1:
        raise SystemExit("the 2026-09-25 bar stored more than its open")
    hours = dict(screen_hours)
    hours.update(oos_hours)
    daily = dict(screen_daily)
    daily.update(oos_daily)
    primary = c.drive_trades(daily, hours, 0.90, FEE, OOS_START, OOS_END)
    doubled = c.drive_trades(daily, hours, 0.90, FEE_DOUBLE, OOS_START, OOS_END)
    q80 = c.drive_trades(daily, hours, 0.80, FEE, OOS_START, OOS_END)
    q95 = c.drive_trades(daily, hours, 0.95, FEE, OOS_START, OOS_END)
    ...
    payload = {
        ...
        "input_sha256": {
            "screen_btc_1h": hashlib.sha256(screen_hour_path.read_bytes()).hexdigest(),
            "screen_btc_1d": hashlib.sha256(screen_day_path.read_bytes()).hexdigest(),
            "oos_btc_1h": hashlib.sha256(oos_hour_path.read_bytes()).hexdigest(),
            "oos_btc_1d": hashlib.sha256(oos_day_path.read_bytes()).hexdigest(),
        },
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp44/drive_oos.json"
