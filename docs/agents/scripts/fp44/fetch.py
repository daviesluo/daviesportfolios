"""Pull BTCUSDT bars for the fp44 screen.

Hourly bars store the open and the close. High, low, volume and trade count are not stored.
...
entry. A later open is a bug.

    python3 docs/agents/scripts/fp44/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp44-research"}
...
def pull_oos() -> None:
    ...
    if not os.environ.get("FP44_OOS"):
    ...
    hourly = klines("1h", c.HOUR_MS, end_ms - 1, (1, 4), start_ms)
...
def main() -> None:
    ...
    print(
        f"fp44 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
        f"1d {len(daily)} first {min(daily)} last {max(daily)}"
    )
