"""The search fp234. A finished four-day rise, then a finished four-day fall, short eleven days.

Nothing here reads the network or a file. Both four-day windows end before the entry open. The entry open is not an input. The position is one
leg. It is not a one-day trade and it is not an overnight trade. Funding
cash is not added. The null is that same 11-day short on every
day.
"""
...
def _signal(um: dict, entry: int) -> bool:
    fall = finished(um, entry, 4)
    rise = finished(um, entry - 4 * fp5.DAY_MS, 4)
    if fall is None or rise is None:
        return False
    return rise > 0.0 and fall < 0.0
