"""Score the fp234 screen. 2023 entries only.

    python3 docs/agents/scripts/fp234/measure.py
"""
...
DATA = Path(os.environ.get("FP234_DATA", "/tmp/fp234/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp234-protocol.md"
NOTE = 'A finished four-day rise was followed by a finished four-day fall. The next USDT open is sold and bought back eleven days later. One USDT-perpetual leg. Funding cash is not added. The null is that eleven-day short on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    um = books["um"]
    fall = c.finished(um, trade["entry_ms"], 4)
    rise = c.finished(um, trade["entry_ms"] - 4 * c.fp5.DAY_MS, 4)
    if fall is None or rise is None or not (rise > 0.0 and fall < 0.0):
        raise SystemExit("the rise then the fall was not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'UMSH' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp234_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp234_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp234/screen_2023.json"
