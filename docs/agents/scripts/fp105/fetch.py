"""Pull BTC funding and daily opens for the fp105 screen.

Funding stops before 2024-01-01. Daily opens run through 2024-01-14 so a
4-day hold can exit. Those later opens are not a signal. No hourly
price bar is requested. No alt quote is requested.

    python3 docs/agents/scripts/fp105/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp105-research"}
...
def main() -> None:
    ...
    bars = pull_opens()
    print(
        f"fp105 funding {len(funding)} first {min(funding)} last {max(funding)}; "
