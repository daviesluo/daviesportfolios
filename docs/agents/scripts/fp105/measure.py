"""Score the fp105 screen. 2023 entries only. The hold is 4 days.

    python3 docs/agents/scripts/fp105/measure.py
"""
...
DATA = Path(os.environ.get("FP105_DATA", "/tmp/fp105/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp105-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "FRNG-4", trades, pool, c.MIN_N,
        "Range of BTC's three funding prints, above its own trailing 90th. "
        "Next day, long BTC for 4 days.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp105_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp105_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp105/screen_2023.json"
