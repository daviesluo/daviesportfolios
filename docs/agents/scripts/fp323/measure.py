"""Score the fp323 screen. 2023 entries only.

    python3 docs/agents/scripts/fp323/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp323-protocol.md"
NOTE = (
    "Fair value is the standing par of one BTC per WBTC. The previous WBTCBTC "
    "close is at least one basis point under that par. Buy the spot book for two "
    "days. The other trade buys it for two days when that discount is absent. "
    "Same prices, same side, the edge turned off. One leg. The traded open is "
    "the fill only. The entry day's close is not an input. A daily print is not "
    "an input. A book ticker is not read."
)
...
def run() -> None:
    ...
    if c.IDEA != "WBPAR" or c.MIN_N != 30 or c.HOLD_DAYS != 2 or c.THRESHOLD_BPS != Decimal("1"):
    ...
    payload = {
        ...
        "code_sha256": {
            "fp323_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp323_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp323/screen_2023.json"
