"""Score the fp271 screen. 2023 entries only.

    python3 docs/agents/scripts/fp271/measure.py
"""
...
DATA = Path(os.environ.get("FP271_DATA", "/tmp/fp271/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp271-protocol.md"
NOTE = 'The finished thirty-day spot return was negative. The next spot open is sold and bought back four days later. The other trade sells spot four days later for another four days. Both windows have to fill. One leg. Funding cash is not added. The null is not that four-day short on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'BACK' or c.MIN_N != 30 or c.HOLD_DAYS != 4:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp271_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp271_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp271/screen_2023.json"
