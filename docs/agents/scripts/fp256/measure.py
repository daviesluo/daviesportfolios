"""Score the fp256 screen. 2023 entries only.

    python3 docs/agents/scripts/fp256/measure.py
"""
...
DATA = Path(os.environ.get("FP256_DATA", "/tmp/fp256/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp256-protocol.md"
NOTE = 'A finished six-day coin-margined rise ended four days before the entry, and the finished four-day spot return was positive. The next USDT open is bought and sold twenty-five days later. One USDT-perpetual leg. Funding cash is not added. The null is that twenty-five-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    if not c._signal(books["cm"], books["spot"], books["um"], trade["entry_ms"]):
        raise SystemExit("the older rise and the later rise were not already finished")
...
def run() -> None:
    ...
    trades = c.signal_trades(books["cm"], books["spot"], books["um"])
    spans = c.signal_spans(books["cm"], books["spot"], books["um"])
    ...
    pool_spans = c.pool_spans(books["cm"], books["spot"], books["um"])
    pool = c.pool_nets(books["cm"], books["spot"], books["um"])
    if c.IDEA != 'ECHO' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp256_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp256_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp256/screen_2023.json"
