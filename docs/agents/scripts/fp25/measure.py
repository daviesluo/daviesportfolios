"""Score the fp25 screen. 2023 only.

    python3 docs/agents/scripts/fp25/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp25-protocol.md"
...
def main() -> None:
    ...
    stock_path = DATA / "SplyExNtv.json"
    day_path = DATA / "spot1d" / "BTCUSDT.json"
    if not stock_path.is_file() or not day_path.is_file():
        raise SystemExit("missing series")
    points = load_points(stock_path)
    ...
    row = c.fp5.summarise(
        "EXCH-BAL", c.stock_trades(daily, points), pool, c.MIN_N,
        "Exchange-held BTC above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp25_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp25_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"stock": sha256(stock_path), "btc_1d": sha256(day_path)},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp25/screen_2023.json"
