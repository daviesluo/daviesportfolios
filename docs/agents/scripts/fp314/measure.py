"""Score the fp314 screen. 2023 entries only.

    python3 docs/agents/scripts/fp314/measure.py
"""
...
DATA = Path(os.environ.get("FP314_DATA", "/tmp/fp314/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp314-protocol.md"
NOTE = "Fair value is the price index inside Binance's funding formula for BTCUSD_PERP. The last settled funding rate is negative, so that contract finished below the index. Buy the coin-margined book for two days. The other trade buys it for two days when that rate is not negative. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side. The traded open is the fill only. Another venue's close is not an input. A book ticker is not read."
...
def run() -> None:
    book = load_opens(DATA / "cm1d" / "BTCUSD_PERP.json")
    fund = load_fund(DATA / "fund" / "BTCUSD_PERP.json")
    inputs = {
        "cm": sha256(DATA / "cm1d" / "BTCUSD_PERP.json"),
        "fund": sha256(DATA / "fund" / "BTCUSD_PERP.json"),
    }
    sofr = {}
    ...
    if c.IDEA != "CZERO" or c.MIN_N != 30 or c.HOLD_DAYS != 2 or c.FAIR_KIND != "index":
    ...
    payload = {
        ...
        "code_sha256": {
            "fp314_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp314_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp314/screen_2023.json"
