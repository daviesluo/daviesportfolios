"""The search fp263. After a finished two-day spot rise that followed a two-day window that was not a rise, buy spot for fifteen days. The other trade buys spot for fifteen days starting fifteen days later.
...
def _signal(spot, um, cm, entry) -> bool:
    near = finished(spot, entry, 2)
    prev = finished(spot, entry - 2 * fp5.DAY_MS, 2)
