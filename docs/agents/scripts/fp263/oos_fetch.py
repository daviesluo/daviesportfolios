"""Pull the out-of-sample inputs named in the LATER pre-registration.

Spot runs from 2024-01-01 through the 2026-09-25 open. That last bar is an
exit open of the later trade. It is not an entry. A bar on 2026-09-26 is
not requested. Refuses to run until the pre-registration's sha256 matches.

    python3 docs/agents/scripts/fp263/oos_fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp263-research"}
...
def live_exit_open(rows: dict[int, tuple]) -> None:
    ...
    page = f"{LIVE_SPOT}?symbol=BTCUSDT&interval=1d&startTime={EXIT_OPEN}&endTime={end}&limit=5"
...
def pull() -> None:
    require_frozen()
    if c.HOLD_DAYS != 15 or c.IDEA != "LATER" or c.NULL_KIND != "other_time":
    ...
    print(f"fp263 oos spot {len(rows)}", file=sys.stderr)
