"""Score the fp191 screen. 2023 entries only.

    python3 docs/agents/scripts/fp191/measure.py
"""
...
DATA = Path(os.environ.get("FP191_DATA", "/tmp/fp191/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp191-protocol.md"
NOTE = "The spot close finished strictly above that day's average trade. Next day, long BTC from the open to the close."
...
def main() -> None:
    ...
    bars = load_rows(DATA / "spot1d/BTCUSDT.json", 4, end, required=True)
    ...
    for trade in trades:
        ...
        if not (base > 0 and close_px > quote / base):
            raise SystemExit("a trade was not a close above the average trade")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp191_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp191_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp191/screen_2023.json"
