"""Score the fp316 screen. 2023 entries only.

    python3 docs/agents/scripts/fp316/measure.py
"""
...
DATA = Path(os.environ.get("FP316_DATA", "/tmp/fp316/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp316-protocol.md"
NOTE = "Fair value is the price index inside Binance's funding formula for BNBUSDT. The last settled funding rate is negative, so that contract finished below the index. Buy the BNB USDT book for two days. The other trade buys it for two days when that rate is not negative. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side. The traded open is the fill only. Another venue's close is not an input. A book ticker is not read."
...
def run() -> None:
    book = load_opens(DATA / "um1d" / "BNBUSDT.json")
    fund = load_fund(DATA / "fund" / "BNBUSDT.json")
    inputs = {
        "um": sha256(DATA / "um1d" / "BNBUSDT.json"),
        "fund": sha256(DATA / "fund" / "BNBUSDT.json"),
    }
    sofr = {}
    ...
    if c.IDEA != "BZERO" or c.MIN_N != 30 or c.HOLD_DAYS != 2 or c.FAIR_KIND != "index":
    ...
    payload = {
        ...
        "code_sha256": {
            "fp316_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp316_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp316/screen_2023.json"
