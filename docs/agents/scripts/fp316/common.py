"""The search fp316. Fair value is the price index inside Binance's funding formula for BNBUSDT. The last settled funding rate is negative, so that contract finished below the index. Buy the BNB USDT book for two days. The other trade buys it for two days when that rate is not negative.
