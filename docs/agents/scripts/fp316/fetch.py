"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp316/fetch.py
"""
...
UA = {"User-Agent": "Mozilla/5.0 daviesportfolios-fp316-research"}
...
def pull_funding() -> dict[int, float]:
    ...
    while cursor < end:
        url = f"{HISTORY}?symbol=BNBUSDT&startTime={cursor}&endTime={end}&limit=1000"
...
def main() -> None:
    ...
    opens = pull_opens()
    fund = pull_funding()
    print(f"fp316 {len(opens)} {len(fund)}")
