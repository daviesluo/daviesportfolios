"""Score the fp243 screen. 2023 entries only.

    python3 docs/agents/scripts/fp243/measure.py
"""
...
DATA = Path(os.environ.get("FP243_DATA", "/tmp/fp243/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp243-protocol.md"
NOTE = "Spot's finished seven-day return was above the coin-margined seven-day return. The next USDT open is bought and sold twelve days later. One USDT-perpetual leg. Funding cash is not added. The null is that twelve-day long on every day."
...
def check_signal(trade: dict, books: dict) -> None:
    spot_move = c.finished(books["spot"], trade["entry_ms"], 7)
    cm_move = c.finished(books["cm"], trade["entry_ms"], 7)
    if spot_move is None or cm_move is None or not spot_move > cm_move:
        raise SystemExit("spot did not finish ahead of the coin-margined book")
...
def run() -> None:
    ...
    books = {
        ...
        "cm": load_rows(DATA / "cm1d/BTCUSD_PERP.json", 2, last),
        "um": load_rows(DATA / "um1d/BTCUSDT.json", 2, end),
    }
    trades = c.signal_trades(books['spot'], books['cm'], books['um'])
    spans = c.signal_spans(books['spot'], books['cm'], books['um'])
    ...
    for trade in trades:
        ...
        entry_px = books["um"][trade["entry_ms"]][0]
        exit_px = books["um"][trade["exit_ms"]][0]
    ...
    pool_spans = c.pool_spans(books['spot'], books['cm'], books['um'])
    pool = c.pool_nets(books['spot'], books['cm'], books['um'])
    if c.IDEA != 'AHEAD' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp243_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp243_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"spot": sha256(DATA / "spot1d/BTCUSDT.json"), "cm": sha256(DATA / "cm1d/BTCUSD_PERP.json"), "um": sha256(DATA / "um1d/BTCUSDT.json")},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp243/screen_2023.json"
