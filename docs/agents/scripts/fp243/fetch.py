"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp243/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp243-research"}
...
def main() -> None:
    ...
    counts.append(len(pull("spot", "BTCUSDT", last, "spot1d")))
    counts.append(len(pull("cm", "BTCUSD_PERP", last, "cm1d")))
    counts.append(len(pull("um", "BTCUSDT", c.fp5.SCREEN_END_MS, "um1d")))
    print("fp243 " + " ".join(str(n) for n in counts))
