"""The search fp243. Spot's finished seven-day return above the coin-margined return, then a twelve-day USDT long.
...
not added. The null is that same 12-day long on every day.
"""
...
def _signal(spot: dict, cm: dict, um: dict, entry: int) -> bool:
    spot_move = finished(spot, entry, 7)
    cm_move = finished(cm, entry, 7)
    if spot_move is None or cm_move is None:
        return False
    return spot_move > cm_move
