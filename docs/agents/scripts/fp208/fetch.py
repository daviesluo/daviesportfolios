"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp208/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp208-research"}
...
def main() -> None:
    ...
    um = pull_klines("um", "BTCUSDT", "um1d/BTCUSDT.json", (1, 4), last, (2022, 12))
    cm = pull_klines("cm", "BTCUSD_PERP", "cm1d/BTCUSD_PERP.json", (4,), session, (2022, 12))
    cm = add_exit(cm, "cm", "BTCUSD_PERP", (4,), "cm1d/BTCUSD_PERP.json")
    print("fp208 " + f"mark {len(mark)} um {len(um)} cm {len(cm)} last {max(cm)}")
