"""Score the fp32 screen. 2023 only.

    python3 docs/agents/scripts/fp32/measure.py
"""
...
DATA = Path(os.environ.get("FP32_DATA", "/tmp/fp32/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp32-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "LATE-VOL", c.late_trades(daily, hours), pool, c.MIN_N,
        "Last six hours' quote volume over the first six, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp32_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp32_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp32/screen_2023.json"
