"""Pull BTCUSDT hourly quote volume and daily opens for the fp32 screen.
...
entry. A later open is a bug. Open, high, low, close, base volume and trade
count are not stored.

    python3 docs/agents/scripts/fp32/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp32-research"}
...
def main() -> None:
    ...
    print(
        f"fp32 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
