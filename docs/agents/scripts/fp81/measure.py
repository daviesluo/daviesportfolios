"""Score the fp81 screen. 2023 only.

    python3 docs/agents/scripts/fp81/measure.py
"""
...
DATA = Path(os.environ.get("FP81_DATA", "/tmp/fp81/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp81-protocol.md"
...
def main() -> None:
    ...
    trades = c.maxgap_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "MAX-GAP", trades, pool, c.MIN_N,
        "Largest gap from an hourly close to the next open, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp81_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp81_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp81/screen_2023.json"
