"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp160/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp160-research"}
...
def main() -> None:
    ...
    price_end = c.fp5.SCREEN_END_MS + 4 * c.fp5.DAY_MS
    taker = pull_metric_ends("2024-01-04", "taker")
    spot = pull_opens("BTCUSDT", "/api/v3/klines", price_end, "spot1d/BTCUSDT.json", 1)
    print("fp160 " + f"taker {len(taker)} spot {len(spot)} last {max(spot)}")
