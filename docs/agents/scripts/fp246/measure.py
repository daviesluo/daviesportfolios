"""Score the fp246 screen. 2023 entries only.

    python3 docs/agents/scripts/fp246/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp246-protocol.md"
NOTE = 'Three finished two-day spot rises, back to back. The next coin-margined open is bought and sold three days later. One coin-margined leg. Funding cash is not added. The null is that three-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    ...
    first = c.finished(books["spot"], entry, 2)
    second = c.finished(books["spot"], entry - 2 * c.fp5.DAY_MS, 2)
    third = c.finished(books["spot"], entry - 4 * c.fp5.DAY_MS, 2)
    if first is None or second is None or third is None or not (first > 0.0 and second > 0.0 and third > 0.0):
        raise SystemExit("the three two-day rises were not already finished")
...
def run() -> None:
    ...
    trades = c.signal_trades(books['spot'], books['cm'])
    spans = c.signal_spans(books['spot'], books['cm'])
    ...
    pool_spans = c.pool_spans(books['spot'], books['cm'])
    pool = c.pool_nets(books['spot'], books['cm'])
    if c.IDEA != 'TRIP' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp246_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp246_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp246/screen_2023.json"
