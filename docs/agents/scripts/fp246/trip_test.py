"""The pre-registered TRIP test. Entries from 2024-01-01 through 2026-09-22.
...
used as the signal stop on 2026-09-21. Refuses to run until the
...
Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp246/trip_test.py
"""
...
SCREEN = ROOT / "docs/agents/backtests/fp246/screen_2023.json"

BID = 84187.9
ASK = 84188.0
HALF_SPREAD = (ASK - BID) / (ASK + BID)
if HALF_SPREAD != 5.939092233854178e-07:
...
def fill(entry: int, cm: dict, fee: float) -> dict | None:
    ...
    if not (OOS_START <= entry < ENTRY_END):
        return None
    if exit_ms != entry + 3 * c.fp5.DAY_MS or exit_ms > EXIT_OPEN:
        return None
...
def signal_trades(spot: dict, cm: dict, fee: float) -> list[dict]:
    trades = []
    entry = OOS_START
...
def check_fill(spot: dict, cm: dict, trade: dict, fee: float) -> None:
    entry = trade["entry_ms"]
    if entry >= ENTRY_END or entry > LAST_ENTRY:
        raise SystemExit("an entry is after 2026-09-22")
...
def main() -> None:
    ...
    if "84187.9, ask 84188.0" not in prereg or "5.939092233854178e-07" not in prereg:
    ...
    if sha256(common_path) != screen_doc["code_sha256"]["fp246_common"]:
        raise SystemExit("the rule file moved after the screen")
    if c.IDEA != "TRIP" or c.HOLD_DAYS != 3 or c.MIN_N != 30 or c.COIN != "BTCUSDT":
        raise SystemExit("the frozen name, hold, count or coin moved")
    if LAST_ENTRY != int(datetime(2026, 9, 22, tzinfo=timezone.utc).timestamp() * 1000):
        raise SystemExit("the last entry is not 2026-09-22")
    if ENTRY_END != int(datetime(2026, 9, 23, tzinfo=timezone.utc).timestamp() * 1000):
        raise SystemExit("the entry window does not stop on 2026-09-23")
    ...
    screen_spot = load_bars(screen_spot_path, 2, c.fp5.SCREEN_END_MS - c.fp5.DAY_MS)
    ...
    if max(screen_cm) != c.fp5.SCREEN_END_MS:
        raise SystemExit("the screen coin-margined file does not stop on 2024-01-01")
    screen = c.signal_trades(screen_spot, screen_cm)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    if len(screen) != 55 or abs(screen_total - 98.165) > 0.01:
    ...
    if any(ts > SIGNAL_LAST for ts in oos_spot):
    ...
    primary = signal_trades(spot, cm, FEE)
    doubled = signal_trades(spot, cm, FEE_DOUBLE)
    if not primary:
    ...
    if [t["entry_ms"] for t in signal_trades(wrecked_spot, wrecked_cm, FEE)] != [t["entry_ms"] for t in primary]:
        raise SystemExit("a later close or the entry open moved a buy")
    for trade in primary:
        check_fill(spot, cm, trade, FEE)
    for trade in doubled:
        check_fill(spot, cm, trade, FEE_DOUBLE)
    ...
    if holds != {c.HOLD_DAYS * c.fp5.DAY_MS}:
        raise SystemExit("the rule hold is not three days")
    pool_entries = set()
    entry = OOS_START
    while entry < ENTRY_END:
        trade = fill(entry, cm, FEE)
    ...
    payload = {
        ...
        "code_sha256": {
            "fp246_common": sha256(common_path),
            "fp246_trip_test": sha256(Path(__file__)),
            "fp246_oos_fetch": sha256(Path(oos_fetch.__file__)),
        },
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp246/trip_oos.json"
