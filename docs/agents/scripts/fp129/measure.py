"""Score the fp129 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp129/measure.py
"""
...
DATA = Path(os.environ.get("FP129_DATA", "/tmp/fp129/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp129-protocol.md"
NOTE = "Average block size of the UTC day, in megabytes, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP129_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "blocks/size.json")
    ...
    row = c.fp5.summarise("BLKSZ", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp129_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp129_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "block_size": sha256(DATA / "blocks/size.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp129/screen_2023.json"
