"""Score the fp205 screen. 2023 entries only.

    python3 docs/agents/scripts/fp205/measure.py
"""
...
DATA = Path(os.environ.get("FP205_DATA", "/tmp/fp205/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp205-protocol.md"
NOTE = "The USDT close finished strictly above the mark close. Next day, short the USDT perpetual from the open to the next open. The null is that short on every day."
...
def main() -> None:
    ...
    closes = load_rows(DATA / "umcls/BTCUSDT.json", 1, c.SIGNAL_LAST_MS, required=True)
    mark = load_rows(DATA / "mark1d/BTCUSDT.json", 1, c.SIGNAL_LAST_MS, required=True)
    opens = load_rows(DATA / "um1d/BTCUSDT.json", 1, end, required=True)
    trades = c.signal_trades(closes, mark, opens)
    for trade in trades:
        check_common(trade)
        day = trade["entry_ms"] - c.fp5.DAY_MS
        if day > c.SIGNAL_LAST_MS or day not in closes or day not in mark:
            raise SystemExit("the signal day is past the window")
        if not closes[day][0] > mark[day][0]:
            raise SystemExit("a trade was not above the mark")
        if trade["exit_ms"] != trade["entry_ms"] + c.fp5.DAY_MS:
            raise SystemExit("the hold moved")
        entry_px = opens[trade["entry_ms"]][0]
        exit_px = opens[trade["exit_ms"]][0]
        ...
        if abs(entry_px - closes[day][0]) > 1e-9 and abs(trade["net"] - c.short_net(closes[day][0], mark[day][0])) <= 1e-12:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp205_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp205_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "umcls": sha256(DATA / "umcls/BTCUSDT.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp205/screen_2023.json"
