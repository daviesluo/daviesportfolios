"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp205/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp205-research"}
...
def main() -> None:
    ...
    closes = pull_klines("um", "BTCUSDT", "umcls/BTCUSDT.json", (4,), last, (2022, 12))
    mark = pull_price("mark", (4,), "mark1d/BTCUSDT.json", last)
    opens = pull_klines("um", "BTCUSDT", "um1d/BTCUSDT.json", (1,), session, (2022, 12))
    opens = add_exit(opens, "um", "BTCUSDT", (1,), "um1d/BTCUSDT.json")
    print("fp205 " + f"closes {len(closes)} mark {len(mark)} opens {len(opens)}")
