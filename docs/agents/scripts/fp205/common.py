"""The search fp205. The USDT close finished above the mark close.

Nothing here reads the network or a file. Both closes are known when the
signal day closes. The next USDT open is sold and the following open is
bought. The close is an input, so it is not the fill. The null is that
...
def signal_days(
    ...
    """Days whose USDT close finished strictly above the mark close."""
    out = []
    for day in sorted(set(um) & set(mark)):
        ...
        if not (fp5.SCREEN_START_MS <= entry < end_ms):
            continue
        if len(um[day]) != 1 or len(mark[day]) != 1:
            raise ValueError("the input is the close")
        if um[day][0] > mark[day][0]:
            out.append(day)
    return out
...
def signal_trades(
    um: dict[int, tuple], mark: dict[int, tuple], opens: dict[int, tuple],
    ...
    for day in signal_days(um, mark, end_ms):
        ...
        if not (start_ms <= entry < end_ms):
            continue
        trade = _open_short(opens, entry, fee)
...
def pool_nets(
    opens: dict[int, tuple], fee: float = fp5.FEE,
    start_ms: int = fp5.SCREEN_START_MS, end_ms: int = fp5.SCREEN_END_MS,
) -> list[float]:
    """A USDT open-to-next-open short on every in-screen day."""
    pool = []
    day = start_ms
    while day < end_ms:
        trade = _open_short(opens, day, fee)
        if trade is not None:
