...
"""Pins for the fp148 rule. No file and no return from 2023 is read."""
...
def test_formula() -> None:
    ...
    rows = {prev: (100.0, 100.0), day: (110.0, 121.0)}
    if not near(c.signal_at(rows, day), 0.11):
        raise SystemExit("the return gap was not the signal")
    if near(c.signal_at(rows, day), 0.10):
        raise SystemExit("the spot return replaced the gap")
    if near(c.signal_at(rows, day), 0.21):
        raise SystemExit("the perpetual return replaced the gap")
    if c.signal_at({day: (110.0, 121.0)}, day) is not None:
        raise SystemExit("a day with no yesterday was a print")
    flat = {prev: (100.0, 100.0), day: (200.0, 200.0)}
    if not near(c.signal_at(flat, day), 0.0):
        raise SystemExit("equal returns were dropped")
