"""Score the fp148 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp148/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp148-protocol.md"
NOTE = "The USDT perpetual's close-to-close return minus spot's, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_book(DATA / "close/both.json")
    ...
    row = c.fp5.summarise("PXGAP", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp148_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp148_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "closes": sha256(DATA / "close/both.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp148/screen_2023.json"
