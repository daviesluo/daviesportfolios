"""The search fp148. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the perpetual's return minus spot's. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Perpetual close-to-close return minus the spot close-to-close return."""
    if day >= end_ms:
        return None
    today = rows.get(day)
    ...
    if spot <= 0 or um <= 0 or prev_spot <= 0 or prev_um <= 0:
        return None
    return (um / prev_um - 1.0) - (spot / prev_spot - 1.0)
