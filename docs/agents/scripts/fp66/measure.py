"""Score the fp66 screen. 2023 only.

    python3 docs/agents/scripts/fp66/measure.py
"""
...
DATA = Path(os.environ.get("FP66_DATA", "/tmp/fp66/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp66-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "AFTER-HIGH", c.after_trades(daily, hours), pool, c.MIN_N,
        "Return of the hour after the high, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp66_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp66_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp66/screen_2023.json"
