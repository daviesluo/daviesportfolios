"""The search fp236. A finished two-day rise inside a finished eight-day fall, held ten days.

Nothing here reads the network or a file. Both windows end at yesterday's
close. The entry open is not an input. The position is one leg. It is not a
one-day trade and it is not an overnight trade. Funding cash is not added.
The null is that same ten-day long on every day.
"""
...
def _signal(um: dict, entry: int) -> bool:
    bounce = finished(um, entry, 2)
    decline = finished(um, entry, 8)
    if bounce is None or decline is None:
        return False
    return bounce > 0.0 and decline < 0.0
