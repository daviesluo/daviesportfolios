"""Score the fp236 screen. 2023 entries only.

    python3 docs/agents/scripts/fp236/measure.py
"""
...
DATA = Path(os.environ.get("FP236_DATA", "/tmp/fp236/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp236-protocol.md"
NOTE = 'A finished two-day rise sat inside a finished eight-day fall. The next USDT open is bought and sold ten days later. One USDT-perpetual leg. Funding cash is not added. The null is that ten-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    bounce = c.finished(books["um"], trade["entry_ms"], 2)
    decline = c.finished(books["um"], trade["entry_ms"], 8)
    if bounce is None or decline is None or bounce <= 0.0 or decline >= 0.0:
        raise SystemExit("the bounce inside the fall was not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'BNC' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp236_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp236_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp236/screen_2023.json"
