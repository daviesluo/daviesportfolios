"""Score the fp260 screen. 2023 entries only.

    python3 docs/agents/scripts/fp260/measure.py
"""
...
DATA = Path(os.environ.get("FP260_DATA", "/tmp/fp260/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp260-protocol.md"
NOTE = "Spot's finished eight-day return was above the coin-margined eight-day return. The next USDT open is bought and sold sixteen days later. The other trade buys the coin-margined book for those same sixteen days. One leg. Funding cash is not added. The null is not that USDT long with the comparison turned off."
...
def run() -> None:
    ...
    if c.IDEA != 'CMLAG' or c.MIN_N != 30 or c.HOLD_DAYS != 16:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp260_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp260_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp260/screen_2023.json"
