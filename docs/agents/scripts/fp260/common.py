"""The search fp260. When the finished eight-day spot return is above the finished eight-day coin-margined return, buy the USDT book for sixteen days. The other trade buys the coin-margined book for those same sixteen days.
...
def _signal(spot, um, cm, entry) -> bool:
    s = finished(spot, entry, 8)
    m = finished(cm, entry, 8)
    return s is not None and m is not None and s > m
