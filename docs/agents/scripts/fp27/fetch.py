"""Pull the count of BTC addresses with a balance, and BTCUSDT daily opens.

No later year is requested. A count stamp on or after 2024-01-01 is not
...
the open only.
    ...
    python3 docs/agents/scripts/fp27/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp27-research"}
METRIC = "AdrBalCnt"
    ...
    while url:
        ...
        for row in body.get("data") or []:
            ...
            if ts < LOOKBACK_MS or ts >= c.fp5.SCREEN_END_MS:
                continue
            if ts in rows:
                raise RuntimeError(f"duplicate count stamp {ts}")
...
def main() -> None:
    ...
    counts = pull_counts()
    if any(row[0] >= c.fp5.SCREEN_END_MS for row in counts):
        raise SystemExit("a count stamp is on or after 2024-01-01")
    daily = pull_opens()
    if any(t > c.fp5.SCREEN_END_MS for t in daily):
        raise SystemExit("a daily bar opened after 2024-01-01")
    if not counts or not daily:
        raise SystemExit("the count series or the daily opens came back empty")
    DATA.mkdir(parents=True, exist_ok=True)
    (DATA / "AdrBalCnt.json").write_text(json.dumps(counts, separators=(",", ":")))
    spot = DATA / "spot1d"
    ...
    print(
        f"fp27 AdrBalCnt {len(counts)} first {counts[0][0]} last {counts[-1][0]}; "
