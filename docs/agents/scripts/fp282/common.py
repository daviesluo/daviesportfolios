"""The search fp282. The finished seven-day USDT return was positive and the seven-day return that ended seven days earlier was negative. Buy the USDT book for eight days. The other trade buys it eight days later for another eight days. Both windows have to fill. The entry open is not an input. One leg.
...
RULE_SIDE = 'long'
NULL_SIDE = 'long'
FILES = {'um': ('um1d/BTCUSDT.json', True)}
PULLS = [('um', 'BTCUSDT', 'um1d', True)]
...
def _signal(spot, um, cm, entry) -> bool:
    near = finished(um, entry, 7)
    prev = finished(um, entry - 7 * fp5.DAY_MS, 7)
