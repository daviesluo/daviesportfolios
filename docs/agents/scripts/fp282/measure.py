"""Score the fp282 screen. 2023 entries only.

    python3 docs/agents/scripts/fp282/measure.py
"""
...
DATA = Path(os.environ.get("FP282_DATA", "/tmp/fp282/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp282-protocol.md"
NOTE = 'The finished seven-day USDT return was positive and the seven-day return that ended seven days earlier was negative. The next USDT open is bought and sold eight days later. The other trade buys it eight days later for another eight days. Both windows have to fill. One leg. Funding cash is not added. The null is not that eight-day long on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'REBND' or c.MIN_N != 30 or c.HOLD_DAYS != 8:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp282_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp282_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp282/screen_2023.json"
