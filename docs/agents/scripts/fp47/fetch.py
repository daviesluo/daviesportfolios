"""Pull BTCUSDT bars for the fp47 screen.
...
No later year is requested. Hourly bars stop before the 2024-01-01 open. The daily file keeps that open because it can be an exit. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp47/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp47-research"}
...
def main() -> None:
    ...
    hourly = klines("1h", c.HOUR_MS, hour_end, (2,))
    ...
    print(
        f"fp47 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
