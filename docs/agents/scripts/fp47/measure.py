"""Score the fp47 screen. 2023 only.

    python3 docs/agents/scripts/fp47/measure.py
"""
...
DATA = Path(os.environ.get("FP47_DATA", "/tmp/fp47/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp47-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "NEW-HIGHS", c.newhigh_trades(daily, hours), pool, c.MIN_N,
        "Count of hours that break the day's running high, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp47_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp47_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp47/screen_2023.json"
