"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp312/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp312-research"}
...
def pull_whitebit() -> dict[int, tuple]:
    start = c.LOOKBACK_MS // 1000
    end = c.FAIR_LAST_MS // 1000
    rows: dict[int, tuple] = {}
    cursor = start
    while cursor <= end:
        nxt = min(cursor + 90 * 86400, end)
        url = (
            "https://whitebit.com/api/v1/public/kline?market=BTC_USDT&interval=1d"
            f"&start={cursor}&end={nxt}"
        )
        payload = json.loads(get(url))
        if not payload.get("success"):
            raise SystemExit("WhiteBIT refused the candles")
        ...
        cursor = nxt + 86400
        time.sleep(0.15)
    check_fair(rows, "WhiteBIT")
    save("wb1d/BTC_USDT.json", rows)
    return rows
...
def main() -> None:
    ...
    fair = pull_whitebit()
    print(f"fp312 {len(traded)} {len(fair)}")
