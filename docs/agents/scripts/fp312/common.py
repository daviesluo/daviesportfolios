"""The search fp312. Fair value is the WhiteBIT BTC_USDT daily close. The finished Binance spot close was at least five basis points under that close, so the next Binance spot open is a price that finished cheap to WhiteBIT. Buy Binance spot for sixteen days. The other trade buys Binance spot for sixteen days on these same prices when that discount is absent.
...
FILES = {
    ...
    FAIR_KEY: ('wb1d/BTC_USDT.json', False),
}
PULLS = [
    ('spot', 'BTCUSDT', 'spot1d', True),
    ('wb', 'BTC_USDT', 'wb1d', False),
]
