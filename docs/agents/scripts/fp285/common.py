"""The search fp285. The finished nine-day coin-margined return was negative and strictly below the USDT nine-day return. Sell the coin-margined book for fifteen days. The other trade sells the USDT book for those same fifteen days. The entry open is not an input. One leg.
...
RULE_SIDE = 'short'
NULL_SIDE = 'short'
FILES = {'um': ('um1d/BTCUSDT.json', True), 'cm': ('cm1d/BTCUSD_PERP.json', True)}
PULLS = [('um', 'BTCUSDT', 'um1d', True), ('cm', 'BTCUSD_PERP', 'cm1d', True)]
...
def _signal(spot, um, cm, entry) -> bool:
    cm_ret = finished(cm, entry, 9)
    um_ret = finished(um, entry, 9)
    return cm_ret is not None and um_ret is not None and cm_ret < 0.0 and cm_ret < um_ret
