"""Score the fp285 screen. 2023 entries only.

    python3 docs/agents/scripts/fp285/measure.py
"""
...
DATA = Path(os.environ.get("FP285_DATA", "/tmp/fp285/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp285-protocol.md"
NOTE = 'The finished nine-day coin-margined return was negative and strictly below the USDT nine-day return. The next coin-margined open is sold and bought back fifteen days later. The other trade sells the USDT book for those same fifteen days. One leg. Funding cash is not added. The null is not that coin-margined short with the comparison turned off.'
...
def run() -> None:
    ...
    if c.IDEA != 'SOFTER' or c.MIN_N != 30 or c.HOLD_DAYS != 15:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp285_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp285_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp285/screen_2023.json"
