"""The search fp147. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the curvature of BTC funding. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """08:00 rate minus the average of the 00:00 and 16:00 rates."""
    if day >= end_ms:
        return None
    row = rows.get(day)
