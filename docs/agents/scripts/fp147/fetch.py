"""Pull the three USDT-margined funding rates. The coin-margined book is not requested. No hourly price bar is requested.

    python3 docs/agents/scripts/fp147/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp147-research"}
...
def pull_signal() -> dict[int, tuple]:
    ...
    for ym in ym_list:
        ...
        for row in csv.DictReader(io.StringIO(text)):
            ...
            if bucket is None or bucket < LOOKBACK_MS or bucket >= c.fp5.SCREEN_END_MS:
                continue
    ...
    for day, items in by_day.items():
        ...
        if stamps != [day, day + eight, day + 2 * eight]:
            continue
        out[day] = (items[0][1], items[1][1], items[2][1])
