"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp162/fetch.py
"""
...
DATA = Path(os.environ.get("FP162_DATA", "/tmp/fp162/data"))
VISION = "https://data.binance.vision"
MARKET = "https://data-api.binance.vision"
LOOKBACK_MS = 1_664_582_400_000  # 2022-10-01 00:00 UTC
UA = {"User-Agent": "daviesportfolios-fp162-research"}
...
def main() -> None:
    if os.environ.get("FP162_OOS"):
        raise SystemExit("this pull does not request a later year")
    oi = pull_metric_ends("2023-12-31", "oi")
    spot = pull_opens(c.fp5.SCREEN_END_MS, "spot1d/BTCUSDT.json", (1, 4))
    print("fp162 " + f"oi {len(oi)} spot {len(spot)} last {max(spot)}")
