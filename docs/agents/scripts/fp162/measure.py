"""Score the fp162 screen. 2023 entries only.

    python3 docs/agents/scripts/fp162/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp162-protocol.md"
NOTE = 'Dollar open interest rose and the candle closed under its open. Next open, long BTC for one day.'
...
def main() -> None:
    ...
    if max(oi) >= end:
        raise SystemExit("an open-interest day is in 2024")
    ...
    for trade in trades:
        ...
        if not (first > 0.0 and last > first and close_px < open_px):
            raise SystemExit("a trade was not inventory up on a down candle")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp162_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp162_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "oi_ends": sha256(DATA / "oi/ends.json"),
            "spot_open_close": sha256(DATA / "spot1d/BTCUSDT.json"),
        },
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp162/screen_2023.json"
