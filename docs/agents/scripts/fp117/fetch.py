"""Pull the public series the fp117 screen is allowed to see.

The signal is USDT-margined BTC volume over coin-margined BTC volume. Contract counts are not stored. Spot bars store only the open. Both perpetual series stop before 2024-01-01. No hourly price bar is requested. No funding print is requested.
No alt quote is requested. No bookDepth file is requested. Count long/short
and taker columns are not stored.

    python3 docs/agents/scripts/fp117/fetch.py
"""
...
DATA = Path(os.environ.get("FP117_DATA", "/tmp/fp117/data"))
VISION = "https://data.binance.vision"
MARKET = "https://data-api.binance.vision"
LOOKBACK_MS = 1_664_582_400_000  # 2022-10-01 00:00 UTC
UA = {"User-Agent": "daviesportfolios-fp117-research"}
SPOT_FIELDS = (1,)
...
def pull_cm() -> dict[int, float]:
    return pull_vision("cm", "BTCUSD_PERP", 7, "cm/base.json", True)

def main() -> None:
    if os.environ.get("FP117_OOS"):
        raise SystemExit("this pull does not request a later year")
    bars = pull_spot()
    um = pull_um()
    cm = pull_cm()
    print("fp117 " + f"BTCUSDT 1d {len(bars)} first {min(bars)} last {max(bars)}" + "; " + f"um {len(um)} first {min(um)} last {max(um)}" + "; " + f"cm {len(cm)} first {min(cm)} last {max(cm)}")
