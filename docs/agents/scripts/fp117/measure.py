"""Score the fp117 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp117/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp117-protocol.md"
NOTE = "USDT-margined BTC volume over coin-margined BTC volume, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    um = load_float(DATA / "um/base.json", True)
    cm = load_float(DATA / "cm" / "base.json", True)
    ...
    if max(um) >= c.fp5.SCREEN_END_MS or max(cm) >= c.fp5.SCREEN_END_MS:
        raise SystemExit("a perpetual bar is in 2024")

    trades = c.signal_trades(um, cm, bars)
    for trade in trades:
    ...
    row = c.fp5.summarise("UMCM", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp117_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp117_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "um_base": sha256(DATA / "um/base.json"),
            "cm_base": sha256(DATA / "cm" / "base.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp117/screen_2023.json"
