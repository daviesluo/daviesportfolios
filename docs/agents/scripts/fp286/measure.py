"""Score the fp286 screen. 2023 entries only.

    python3 docs/agents/scripts/fp286/measure.py
"""
...
DATA = Path(os.environ.get("FP286_DATA", "/tmp/fp286/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp286-protocol.md"
NOTE = 'Three non-overlapping finished five-day spot returns went up, then down, then up. The next spot open is bought and sold five days later. The other trade buys spot five days later for another five days. Both windows have to fill. One leg. Funding cash is not added. The null is not that five-day long on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'RESUME' or c.MIN_N != 30 or c.HOLD_DAYS != 5:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp286_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp286_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp286/screen_2023.json"
