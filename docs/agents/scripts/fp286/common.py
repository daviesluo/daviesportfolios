"""The search fp286. Three non-overlapping finished five-day spot returns went up, then down, then up. Buy spot for five days. The other trade buys spot five days later for another five days. Both windows have to fill. The entry open is not an input. One leg.
...
def _signal(spot, um, cm, entry) -> bool:
    newest = finished(spot, entry, 5)
    middle = finished(spot, entry - 5 * fp5.DAY_MS, 5)
    oldest = finished(spot, entry - 10 * fp5.DAY_MS, 5)
    return (
        newest is not None
