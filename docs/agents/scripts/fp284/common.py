"""The search fp284. Spot, the USDT book and the coin-margined book each finished a ten-day rise. Buy the coin-margined book for twenty-one days. The other trade buys spot for those same twenty-one days. The entry open is not an input. One leg.
...
RULE_SIDE = 'long'
NULL_SIDE = 'long'
FILES = {'spot': ('spot1d/BTCUSDT.json', True), 'um': ('um1d/BTCUSDT.json', False), 'cm': ('cm1d/BTCUSD_PERP.json', True)}
PULLS = [('spot', 'BTCUSDT', 'spot1d', True), ('um', 'BTCUSDT', 'um1d', False), ('cm', 'BTCUSD_PERP', 'cm1d', True)]
...
if RULE_SIDE != NULL_SIDE:
    raise RuntimeError("the sides did not match")
...
def _signal(spot, um, cm, entry) -> bool:
    spot_ret = finished(spot, entry, 10)
    um_ret = finished(um, entry, 10)
    cm_ret = finished(cm, entry, 10)
    return (
        spot_ret is not None
        and um_ret is not None
        and cm_ret is not None
        and spot_ret > 0.0
        and um_ret > 0.0
        and cm_ret > 0.0
    )
