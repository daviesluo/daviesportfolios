"""Score the fp284 screen. 2023 entries only.

    python3 docs/agents/scripts/fp284/measure.py
"""
...
DATA = Path(os.environ.get("FP284_DATA", "/tmp/fp284/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp284-protocol.md"
NOTE = 'Spot, the USDT book and the coin-margined book each finished a ten-day rise. The next coin-margined open is bought and sold twenty-one days later. The other trade buys spot for those same twenty-one days. One leg. Funding cash is not added. The null is not that coin-margined long with the other two books ignored.'
...
def run() -> None:
    ...
    if c.IDEA != 'TRIO' or c.MIN_N != 30 or c.HOLD_DAYS != 21:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp284_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp284_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp284/screen_2023.json"
