"""Score the fp275 screen. 2023 entries only.

    python3 docs/agents/scripts/fp275/measure.py
"""
...
DATA = Path(os.environ.get("FP275_DATA", "/tmp/fp275/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp275-protocol.md"
NOTE = 'The finished six-day coin-margined return was negative. The next coin-margined open is sold and bought back sixteen days later. The other trade sells that book sixteen days later for another sixteen days. Both windows have to fill. One leg. Funding cash is not added. The null is not that sixteen-day short on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'DEEPCM' or c.MIN_N != 30 or c.HOLD_DAYS != 16:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp275_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp275_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp275/screen_2023.json"
