...
"""Pins for the fp275 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "DEEPCM" or c.HOLD_DAYS != 16 or c.NULL_KIND != "other_time":
    ...
    _put(cm, entry - 7 * day, 1, 100)
    _put(cm, entry, 40, 1)
    _put(cm, entry + 16 * day, 36, 1)
    _put(cm, entry + 32 * day, 30, 1)
    ...
    if len(rules) != 1 or rules[0]["entry_ms"] != entry or rules[0]["side"] != "short":
        raise SystemExit("the decline was not sold")
    if len(nulls) != 1 or nulls[0]["entry_ms"] != entry + 16 * day:
        raise SystemExit("the other trade did not start sixteen days later")
    if abs(rules[0]["net"] - float(_short(Decimal(40), Decimal(36)))) > 1e-12:
        raise SystemExit("the short moved")
    if abs(nulls[0]["net"] - float(_short(Decimal(36), Decimal(30)))) > 1e-12:
        raise SystemExit("the later short moved")
    thin = dict(cm)
    del thin[entry + 32 * day]
