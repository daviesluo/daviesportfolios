"""Pull the public series the fp111 screen is allowed to see.

The signal is the last dollar open interest of the UTC day. The change is not stored. No hourly price bar is requested. No funding print is requested.
No alt quote is requested. No bookDepth file is requested. Count long/short
and taker columns are not stored.

    python3 docs/agents/scripts/fp111/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp111-research"}
SPOT_FIELDS = (1,)
...
COLUMNS = (3,)
METRICS_REL = "metrics/oi_usd.json"
METRICS_MODE = "last"
...
def main() -> None:
    ...
    bars = pull_spot()
    print("fp111 " + f"metrics {len(metrics)} first {min(metrics)} last {max(metrics)}" + "; " + f"BTCUSDT 1d {len(bars)} first {min(bars)} last {max(bars)}")
