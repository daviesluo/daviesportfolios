"""Score the fp140 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp140/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp140-protocol.md"
NOTE = "Today's spot quote over the USDT perpetual quote, minus yesterday's ratio, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_book(DATA / "mix/both.json")
    ...
    row = c.fp5.summarise("MIXJMP", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp140_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp140_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "mix_both": sha256(DATA / "mix/both.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp140/screen_2023.json"
