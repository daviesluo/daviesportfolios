"""The search fp140. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is today's spot-quote over perpetual-quote, minus yesterday's ratio. The level is not an input. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Today's quote ratio minus yesterday's. Yesterday is one day earlier."""
    if day >= end_ms:
        return None
    today = rows.get(day)
    prev = rows.get(day - fp5.DAY_MS)
    if today is None or prev is None or len(today) != 2 or len(prev) != 2:
        return None
    spot, um = today
    prev_spot, prev_um = prev
    if spot <= 0 or um <= 0 or prev_spot <= 0 or prev_um <= 0:
        return None
    return spot / um - prev_spot / prev_um
