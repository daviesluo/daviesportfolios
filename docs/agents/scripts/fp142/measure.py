"""Score the fp142 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp142/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp142-protocol.md"
NOTE = "Last aligned USDT-margined funding minus coin-margined funding, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_book(DATA / "funding/gap.json")
    ...
    row = c.fp5.summarise("FUNDGAP", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp142_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp142_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "funding_gap": sha256(DATA / "funding/gap.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp142/screen_2023.json"
