"""The search fp142. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the last aligned USDT-margined funding rate minus the coin-margined rate. Neither level is an input. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """USDT-margined rate minus coin-margined rate. A rate may be negative or zero."""
    if day >= end_ms:
        return None
    row = rows.get(day)
