"""Score the fp102 screen. 2023 only.

    python3 docs/agents/scripts/fp102/measure.py
"""
...
DATA = Path(os.environ.get("FP102_DATA", "/tmp/fp102/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp102-protocol.md"
...
def main() -> None:
    if os.environ.get("FP102_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")

    alt_path = DATA / "spot1d" / "LTCUSDT.json"
    ...
    for trade in trades:
        if trade["coin"] != "LTCUSDT":
            raise SystemExit("a fill is not LTC")
    ...
    pool = c.fp5.pool_from_bars({"LTCUSDT": alt}, ["LTCUSDT"], c.fp5.DAY_MS)
    row = c.fp5.summarise(
        "LTC-Q", trades, pool, c.MIN_N,
        "LTC's daily quote volume over BTC's, above its own trailing 90th. Next day, long LTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp102_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp102_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp102/screen_2023.json"
