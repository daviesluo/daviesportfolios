"""Score the fp133 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp133/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp133-protocol.md"
NOTE = "On-chain output volume that UTC day, in BTC, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_book(DATA / "chain/output_btc.json")
    ...
    row = c.fp5.summarise("OUTBTC", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp133_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp133_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "output_btc": sha256(DATA / "chain/output_btc.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp133/screen_2023.json"
