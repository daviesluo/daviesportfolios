"""The search fp187. Spot finished under the index.
...
def signal_days(bars: dict[int, tuple], index: dict[int, tuple], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Days whose spot close finished under the index close."""
    out = []
    for day in sorted(index):
        if day > SIGNAL_LAST_MS:
            continue
        entry = day + fp5.DAY_MS
        if not (fp5.SCREEN_START_MS <= entry < end_ms) or day not in bars:
            continue
        if len(bars[day]) < 2 or len(index[day]) != 1:
            raise ValueError("spot is the open and the close; the index is the close")
        if bars[day][1] < index[day][0]:
            out.append(day)
    return out
