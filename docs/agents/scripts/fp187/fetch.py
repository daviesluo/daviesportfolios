"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp187/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp187-research"}
...
def pull_index() -> dict[int, tuple]:
    rows: dict[int, tuple] = {}
    for ym in months():
        url = (
            f"{VISION}/data/futures/um/monthly/indexPriceKlines/BTCUSDT/1d/"
            f"BTCUSDT-1d-{ym}.zip"
        )
        for row in read_zip_rows(url):
            ...
            rows[ts] = (float(row[4]),)
    if not rows:
        raise SystemExit("the index came back empty")
    if max(rows) > c.SIGNAL_LAST_MS:
        raise SystemExit("the index goes past the signal window")
    save("index1d/BTCUSDT.json", rows)
    return rows
...
def main() -> None:
    ...
    spot = pull_spot((1, 4))
    index = pull_index()
    print("fp187 " + f"spot {len(spot)} last {max(spot)} index {len(index)} last {max(index)}")
