"""Score the fp187 screen. 2023 entries only.

    python3 docs/agents/scripts/fp187/measure.py
"""
...
DATA = Path(os.environ.get("FP187_DATA", "/tmp/fp187/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp187-protocol.md"
NOTE = 'The Binance spot close finished strictly under the index close. Next day, long BTC from the open to the close.'
...
def main() -> None:
    ...
    index = load_rows(DATA / "index1d/BTCUSDT.json", 1, c.SIGNAL_LAST_MS, required=False)
    ...
    for trade in trades:
        ...
        if not (bars[day][1] < index[day][0]):
            raise SystemExit("a trade was not spot under the index")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp187_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp187_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp187/screen_2023.json"
