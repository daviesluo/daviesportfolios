"""Score the fp300 screen. 2023 entries only.

    python3 docs/agents/scripts/fp300/measure.py
"""
...
DATA = Path(os.environ.get("FP300_DATA", "/tmp/fp300/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp300-protocol.md"
NOTE = 'Fair value is the Coinbase Exchange BTC-USD daily close. The basis is the finished USDT close over that fair value. It finished positive and strictly above the basis five days earlier, so the next USDT open sells a premium to Coinbase that has widened. Sell the USDT book for eight days. The other trade sells the USDT book for eight days on these same prices when that wider premium is absent. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side. The traded open is the fill only.'
...
def run() -> None:
    ...
    if c.IDEA != 'CBWDN' or c.MIN_N != 30 or c.HOLD_DAYS != 8 or c.NULL_KIND != "other_regime":
        raise SystemExit("the frozen name or the count moved")
    if c.FAIR_KEY in ("spot", "um", "cm"):
        raise SystemExit("the fair value is a Binance line")
    pool = [trade["net"] for trade in nulls]
    row = c.fp5.summarise(c.IDEA, trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp300_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp300_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp300/screen_2023.json"
