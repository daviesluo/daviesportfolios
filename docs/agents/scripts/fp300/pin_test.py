...
"""Pins for the fp300 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != 'CBWDN' or c.HOLD_DAYS != 8 or c.NULL_KIND != "other_regime":
        raise SystemExit("the idea moved")
    if c.MODE != "regime" or c.RULE_SIDE != 'short' or c.RULE_BOOK != 'um':
        raise SystemExit("the leg moved")
    if c.EDGE != 'widen' or c.DISCOUNT != 0.0 or c.WIDEN_LAG != 5:
        raise SystemExit("the edge moved")
    if c.FAIR_KEY != 'cb' or c.FAIR_KEY in ("spot", "um", "cm"):
