"""Pull BTCUSDT daily bars for the fp21 screen.

No later year is requested. The 2024-01-01 open is kept because it can be
the exit of the last 2023 entry. A bar opened after that is a bug. High,
low and the trade count are not stored.

    python3 docs/agents/scripts/fp21/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp21-research"}
...
def main() -> None:
    ...
    while cursor <= end_ms:
        ...
        for k in batch:
            if len(k) < 8:
                raise SystemExit("a kline has no quote volume")
            open_ms = int(k[0])
            if open_ms < LOOKBACK_MS:
                continue
            if open_ms > end_ms:
                raise SystemExit(f"a bar opened after 2024-01-01: {open_ms}")
            rows[open_ms] = (float(k[1]), float(k[4]), float(k[5]), float(k[7]))
    ...
    print(f"fp21 BTCUSDT 1d bars {len(rows)} first {min(rows)} last {max(rows)}")
