"""Pull daily bars for the fp37 screen.

BTCUSDT and ETHUSDT bars are the open and the close. The 2024-01-01 close is stored as the open on both.
...
open is a bug.
    ...
    python3 docs/agents/scripts/fp37/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp37-research"}
...
def main() -> None:
    ...
    for symbol in ("BTCUSDT", "ETHUSDT"):
        ...
        save(symbol, rows)
        print(f"fp37 {symbol} 1d {len(rows)} first {min(rows)} last {max(rows)}")
