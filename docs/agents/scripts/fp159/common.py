"""The search fp159. Long the next day when the 00:00 funding rate is the richest.

Nothing here reads the network or a file. The 00:00 rate has to be strictly
above both later rates that day. A percentile is not used. The fill is one
day long, open to the next open.
"""
...
def signal_days(funding: dict[int, tuple], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Days whose 00:00 rate is strictly above the 08:00 rate and the 16:00 rate."""
    out = []
    for day in sorted(funding):
        if day >= end_ms:
            continue
        rate_00, rate_08, rate_16 = funding[day]
        if rate_00 > rate_08 and rate_00 > rate_16:
            out.append(day)
    return out
