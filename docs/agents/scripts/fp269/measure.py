"""Score the fp269 screen. 2023 entries only.

    python3 docs/agents/scripts/fp269/measure.py
"""
...
DATA = Path(os.environ.get("FP269_DATA", "/tmp/fp269/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp269-protocol.md"
NOTE = 'A nine-day spot rise had already finished, and the latest nine-day spot return was not positive. The next coin-margined open is bought and sold five days later. The other trade buys the USDT book for those same five days. One leg. Funding cash is not added. The null is not that coin-margined long with the two windows turned off.'
...
def run() -> None:
    ...
    if c.IDEA != 'STALE' or c.MIN_N != 30 or c.HOLD_DAYS != 5:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp269_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp269_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp269/screen_2023.json"
