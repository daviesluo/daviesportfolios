"""Score the fp257 screen. 2023 entries only.

    python3 docs/agents/scripts/fp257/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp257-protocol.md"
NOTE = 'A finished four-day spot decline. The next coin-margined open is bought and sold nine days later. The other trade buys spot for those same nine days. One leg. Funding cash is not added. The null is not that coin-margined long with the decline turned off.'
...
def run() -> None:
    ...
    if c.IDEA != 'PICK' or c.MIN_N != 30 or c.HOLD_DAYS != 9:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp257_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp257_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp257/screen_2023.json"
