"""Score the fp84 screen. 2023 only.

    python3 docs/agents/scripts/fp84/measure.py
"""
...
DATA = Path(os.environ.get("FP84_DATA", "/tmp/fp84/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp84-protocol.md"
...
def main() -> None:
    ...
    trades = c.wtd_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "WTD-RET", trades, pool, c.MIN_N,
        "Quote-weighted mean of the 24 hourly returns, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp84_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp84_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp84/screen_2023.json"
