"""Pull daily bars for the fp42 screen.

A bar is the open, the close and quote volume. The 2024-01-01 close is stored as the open and its quote volume as 0.
...
open is a bug.
    ...
    python3 docs/agents/scripts/fp42/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp42-research"}
...
def main() -> None:
    ...
    for symbol in ("BTCUSDT",):
        ...
        save(symbol, rows)
        print(f"fp42 {symbol} 1d {len(rows)} first {min(rows)} last {max(rows)}")
