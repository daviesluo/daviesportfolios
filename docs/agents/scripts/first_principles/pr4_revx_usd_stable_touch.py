"""PR4 — PR3's rule on Revolut X's USDC/USD and USDT/USD books, as pre-registered in
prereg_revx_usd_stable_touch.md (frozen 2026-09-23T11:44:24Z, sha256 407d3288…). Generated from
pr3_revx_gbp_stable_touch.py by the substitutions listed in that pre-registration: fair = the book's own
24-hour median, no exchange rate (X = 1), quotes all week, the book's own half-spread on a taker stop.
...
usage: pr4_revx_usd_stable_touch.py <out.json>
"""
...
TICK = 0.0001
HALF_SPREAD_BY_BOOK = {"USDC-USD": 0.00005, "USDT-USD": 0.0001}
CAPITAL = 2 * 2 * 3 * 100.0
...
def simulate(book, bars, fairU, X, stress=False, full=False, collect=None):
    HALF_SPREAD = HALF_SPREAD_BY_BOOK[book]
...
def exit_only(book, bars, fairU, X, i0, side, usd):
    ...
    taker = 0.0009 + HALF_SPREAD_BY_BOOK[book]
...
def main():
    ...
    fU = {b: fair_usd(b) for b in BOOKS}
    XY = (lambda t: 1.0)
    is_days = (OOS0 - IS0) / 86400000; oos_days = (OOS1 - OOS0) / 86400000
    total_days = (bars[BOOKS[0]][-1][0] - bars[BOOKS[0]][0][0]) / 86400000
    R = {"prereg": "prereg_revx_usd_stable_touch.md", "capital_usd": CAPITAL, "is_days": round(is_days, 3), "oos_days": round(oos_days, 3)}
    arms = {"primary": dict(X=XY), "stress": dict(X=XY, stress=True), "full_100_fills": dict(X=XY, full=True)}
