"""Score the fp198 screen. 2023 entries only.

    python3 docs/agents/scripts/fp198/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp198-protocol.md"
NOTE = 'Coin open interest finished strictly above its first print. Next day, short the USDT perpetual from the open to the close. Funding cash is not added. The null is that short on every day.'
...
def main() -> None:
    ...
    oi = load_rows(DATA / "oi/coin.json", 2, c.SIGNAL_LAST_MS, required=True)
    ...
    for trade in trades:
        ...
        first, last_oi = oi[day]
        if not (first > 0.0 and last_oi > first):
            raise SystemExit("a trade was not rising coin open interest")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp198_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp198_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"um": sha256(DATA / "um1d/BTCUSDT.json"), "oi": sha256(DATA / "oi/coin.json")},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp198/screen_2023.json"
