"""Score the fp255 screen. 2023 entries only.

    python3 docs/agents/scripts/fp255/measure.py
"""
...
DATA = Path(os.environ.get("FP255_DATA", "/tmp/fp255/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp255-protocol.md"
NOTE = 'Two finished down days on spot, back to back. The next spot open is sold and bought back nineteen days later. One spot leg. Funding cash is not added. The null is that nineteen-day short on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    if not c._signal(books["spot"], trade["entry_ms"]):
        raise SystemExit("the two down days were not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'TWOD' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp255_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp255_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp255/screen_2023.json"
