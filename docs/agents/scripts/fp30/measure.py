"""Score the fp30 screen. 2023 only.

    python3 docs/agents/scripts/fp30/measure.py
"""
...
DATA = Path(os.environ.get("FP30_DATA", "/tmp/fp30/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp30-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "TURN", c.turn_trades(daily, hours), pool, c.MIN_N,
        "Sign changes of the hourly return, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp30_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp30_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp30/screen_2023.json"
