...
and it is not the Binance spot, coin-margined or USDT book. The entry open is
not an input. A fill is an open on the contract being traded. The position is
one leg. It is not a one-day trade and it is not an overnight trade. Funding
cash is not added. The null is the same leg with the price edge turned off.
It is not the opposite side and it is not an unconditional hold.
"""
...
WIDEN_LAG = 6
FAIR_KEY = 'db'
FAIR_LAST_MS = fp5.SCREEN_END_MS - fp5.DAY_MS + fp5.EIGHT_H_MS
RULE_BOOK = 'um'
NULL_BOOK = 'um'
RULE_SIDE = 'short'
NULL_SIDE = 'short'
FILES = {
    RULE_BOOK: ('um1d/BTCUSDT.json', True),
    FAIR_KEY: ('db1d/BTC-PERPETUAL.json', False),
}
PULLS = [
    ('um', 'BTCUSDT', 'um1d', True),
    ('db', 'BTC-PERPETUAL', 'db1d', False),
]
...
if FAIR_KEY in ("spot", "um", "cm") or FAIR_KEY == RULE_BOOK:
    raise RuntimeError("the fair value is a Binance line of this contract")
if EDGE not in ("cheap", "rich", "widen"):
    raise RuntimeError("the edge is not a price")
if EDGE == "widen" and WIDEN_LAG <= 0:
    raise RuntimeError("the basis lag is missing")
