"""Score the fp113 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp113/measure.py
"""
...
DATA = Path(os.environ.get("FP113_DATA", "/tmp/fp113/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp113-protocol.md"
NOTE = "Coin open interest over spot base volume, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP113_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    oi = load_fixed(DATA / "metrics/oi_coin.json", 2, True)
    ...
    if bars[c.fp5.SCREEN_END_MS][1] != 0.0:
        raise SystemExit("the horizon stored a base volume")
    ...
    row = c.fp5.summarise("OITURN", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp113_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp113_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "oi_coin": sha256(DATA / "metrics/oi_coin.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp113/screen_2023.json"
