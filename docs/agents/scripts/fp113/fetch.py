"""Pull the public series the fp113 screen is allowed to see.

The signal is the last coin open interest over that day's spot base volume. Dollar open interest is not stored. The 2024-01-01 base volume is stored as zero. No hourly price bar is requested. No funding print is requested.
No alt quote is requested. No bookDepth file is requested. Count long/short
and taker columns are not stored.

    python3 docs/agents/scripts/fp113/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp113-research"}
SPOT_FIELDS = (1, 5)
...
COLUMNS = (2,)
METRICS_REL = "metrics/oi_coin.json"
METRICS_MODE = "last"
...
def main() -> None:
    ...
    bars = pull_spot()
    print("fp113 " + f"metrics {len(metrics)} first {min(metrics)} last {max(metrics)}" + "; " + f"BTCUSDT 1d {len(bars)} first {min(bars)} last {max(bars)}")
