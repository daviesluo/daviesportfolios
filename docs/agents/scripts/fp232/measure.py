"""Score the fp232 screen. 2023 entries only.

    python3 docs/agents/scripts/fp232/measure.py
"""
...
DATA = Path(os.environ.get("FP232_DATA", "/tmp/fp232/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp232-protocol.md"
NOTE = 'The USDT close finished above its close nine days earlier. The next open is sold and bought back nine days later. One USDT-perpetual leg. Funding cash is not added. The null is that nine-day short on every day.'
...
def check_common(trade: dict) -> None:
    ...
    if trade["exit_ms"] - trade["entry_ms"] != 9 * c.fp5.DAY_MS:
...
def assert_subset(trades, pool, spans, pool_spans) -> None:
    ...
    for entry, exit_ms in pool_spans:
        if exit_ms - entry != 9 * c.fp5.DAY_MS:
...
def run() -> None:
    ...
    for trade in trades:
        check_common(trade)
        move = c.finished(book, trade["entry_ms"], 9)
    ...
    payload = {
        ...
        "code_sha256": {
            "fp232_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp232_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp232/screen_2023.json"
