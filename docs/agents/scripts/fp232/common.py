"""The search fp232. A finished nine-day rise, then a nine-day short.

Nothing here reads the network or a file. The return uses the USDT close
yesterday and the close nine days before that. The entry open is not an
input. The position is one USDT-perpetual leg, sold at the next open and
bought back nine days later. It is not a one-day trade and it is not an
overnight trade. Funding cash is not added. The null is that same nine-day
short on every day.
"""
...
def signal_trades(um: dict) -> list[dict]:
    ...
    for entry, exit_ms in signal_spans(um):
        ...
        if trade["exit_ms"] - trade["entry_ms"] != HOLD_DAYS * fp5.DAY_MS:
            raise RuntimeError("the hold is not nine days")
        trades.append(trade)
    return trades
