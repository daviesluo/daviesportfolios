"""Score the fp79 screen. 2023 only.

    python3 docs/agents/scripts/fp79/measure.py
"""
...
DATA = Path(os.environ.get("FP79_DATA", "/tmp/fp79/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp79-protocol.md"
...
def main() -> None:
    ...
    trades = c.busy_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "BUSY-RET", trades, pool, c.MIN_N,
        "Return of the earliest maximum-quote hour, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp79_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp79_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp79/screen_2023.json"
