"""The search fp268. The absolute three-day spot return finished larger than the absolute three-day return that ended three days earlier. Buy the USDT book for twenty-one days. The other trade buys it when the recent absolute return finished smaller. Equals go to neither.
...
def _rule_signal(spot, um, cm, entry) -> bool:
    recent = finished(spot, entry, 3)
    prior = finished(spot, entry - 3 * fp5.DAY_MS, 3)
    return recent is not None and prior is not None and abs(recent) > abs(prior)
