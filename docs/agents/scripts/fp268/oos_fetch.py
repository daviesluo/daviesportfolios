"""Pull the out-of-sample inputs named in the LOUDER pre-registration.

Spot is the signal. It runs from 2024-01-01 through the 2026-09-03 close.
A close after that is not a signal and is not requested. The USDT perpetual
runs from 2024-01-01 through the 2026-09-25 open. That last bar is an exit
open. It is not an entry. A bar on 2026-09-26 is not requested. Refuses to
run until the pre-registration's sha256 matches.

    python3 docs/agents/scripts/fp268/oos_fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp268-research"}
...
SIGNAL_LAST = int(datetime(2026, 9, 3, tzinfo=timezone.utc).timestamp() * 1000)
...
def pull_book(kind: str, symbol: str, label: str, last_ms: int, last_day: int, live_last: bool) -> dict[int, tuple]:
    rows: dict[int, tuple] = {}
    for ym in months():
        ...
        if not pull_zip(rows, url, last_ms):
    ...
    for day in range(1, last_day + 1):
        ...
        if kind == "spot":
            ...
            url = f"{VISION}/data/futures/{kind}/daily/klines/{symbol}/1d/{symbol}-1d-{stamp}.zip"
        missing_ok = live_last and day == last_day
        if not pull_zip(rows, url, last_ms, missing_ok=missing_ok) and not missing_ok:
            raise SystemExit(f"{stamp} did not return a {label} bar")
    if live_last and EXIT_OPEN not in rows:
        live_exit_open(rows)
    complete(rows, last_ms, label)
    return rows


def pull() -> None:
    require_frozen()
    if c.HOLD_DAYS != 21 or c.IDEA != "LOUDER" or c.NULL_KIND != "other_regime":
        raise SystemExit("the frozen hold, name or null moved")
    if SIGNAL_LAST != EXIT_OPEN - 22 * c.fp5.DAY_MS:
        raise SystemExit("the last signal close moved")
    um = pull_book("um", "BTCUSDT", "the USDT perpetual", EXIT_OPEN, 25, True)
    spot = pull_book("spot", "BTCUSDT", "spot", SIGNAL_LAST, 3, False)
    save("oos_um1d/BTCUSDT.json", um)
    save("oos_spot1d/BTCUSDT.json", spot)
    print(f"fp268 oos spot {len(spot)} um {len(um)}", file=sys.stderr)
