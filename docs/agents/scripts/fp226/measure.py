"""Score the fp226 screen. 2023 entries only.

    python3 docs/agents/scripts/fp226/measure.py
"""
...
DATA = Path(os.environ.get("FP226_DATA", "/tmp/fp226/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp226-protocol.md"
NOTE = "Spot's close finished above its close twenty days earlier and below its close five days earlier. The next open is bought and sold ten days later. One spot leg. Funding cash is not added. The null is that ten-day spot long on every day."
...
def check_common(trade: dict) -> None:
    ...
    if trade["exit_ms"] - trade["entry_ms"] != 10 * c.fp5.DAY_MS:
...
def assert_subset(trades, pool, spans, pool_spans) -> None:
    ...
    for entry, exit_ms in pool_spans:
        if exit_ms - entry != 10 * c.fp5.DAY_MS:
...
def run() -> None:
    ...
    for trade in trades:
        check_common(trade)
        slow = c.finished(book, trade["entry_ms"], 20)
        fast = c.finished(book, trade["entry_ms"], 5)
        if slow is None or fast is None or slow <= 0.0 or fast >= 0.0:
            raise SystemExit("the pullback was not already finished")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp226_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp226_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp226/screen_2023.json"
