"""Pull bars for the fp77 screen.

The 2024-01-01 quote and base volume are stored as zero.

No later year is requested. The 2024-01-01 bar keeps its open. Its quote and base volume are masked.

    python3 docs/agents/scripts/fp77/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp77-research"}
...
def horizon(open_: float, fields: tuple[int, ...]) -> tuple:
    # fields (1, 7, 5): quote and base stored as zero
    return (open_, 0.0, 0.0)


def main() -> None:
    if os.environ.get("FP77_OOS"):
        raise SystemExit("this pull does not request a later year")
    rows = klines("BTCUSDT", "1d", c.fp5.DAY_MS, c.fp5.SCREEN_END_MS, (1, 7, 5))
    ...
    if horizon_bar is None or horizon_bar[1] != 0.0 or horizon_bar[2] != 0.0:
    ...
    print(f"fp77 BTCUSDT 1d {len(rows)} first {min(rows)} last {max(rows)}")
