"""Score the fp127 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp127/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp127-protocol.md"
NOTE = "Last mempool size of the UTC day, in bytes, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    signal = load_book(DATA / "mempool" / "bytes.json")
    ...
    row = c.fp5.summarise("MEMBYTE", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp127_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp127_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "mempool": sha256(DATA / "mempool" / "bytes.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp127/screen_2023.json"
