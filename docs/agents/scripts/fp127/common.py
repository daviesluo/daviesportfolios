"""The search fp127. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the last mempool size of the UTC day. The
day's maximum and the day's mean are not the signal. No hourly price bar
is read. No funding print is read. No option file is read. The hold is
one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Last mempool size that UTC day. None when a sample is missing or not positive."""
    if day >= end_ms:
        return None
    row = rows.get(day)
    ...
    for i in range(0, len(row), 2):
        ...
        if value <= 0:
            return None
        if last_t is None or stamp > last_t:
            last_t = stamp
            last_v = value
    return last_v
