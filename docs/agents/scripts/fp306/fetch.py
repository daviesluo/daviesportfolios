"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp306/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp306-research"}
...
def pull_bitfinex() -> dict[int, tuple]:
    url = (
        "https://api-pub.bitfinex.com/v2/candles/trade:1D:tBTCUSD/hist"
        "?limit=1000&sort=1&start=1667260800000&end=1704067200000"
    )
    rows: dict[int, tuple] = {}
    for mts, open_px, close_px, _high, _low, _vol in json.loads(get(url)):
        ts = int(mts)
        if not keep_fair(ts):
            continue
        rows[ts] = (float(open_px), float(close_px))
    check_fair(rows, "Bitfinex")
    save("bf1d/tBTCUSD.json", rows)
    return rows
...
def main() -> None:
    ...
    fair = pull_bitfinex()
    print(f"fp306 {len(traded)} {len(fair)}")
