"""The search fp306. Fair value is the Bitfinex tBTCUSD daily close. The finished USDT close was strictly above that close, so the next USDT open is a price that finished rich to Bitfinex. Sell the USDT book for twelve days. The other trade sells the USDT book for twelve days on these same prices when that premium is absent.
...
FILES = {
    ...
    FAIR_KEY: ('bf1d/tBTCUSD.json', False),
}
PULLS = [
    ('um', 'BTCUSDT', 'um1d', True),
    ('bf', 'tBTCUSD', 'bf1d', False),
]
