...
"""Pins for the fp302 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != 'DBRCH' or c.HOLD_DAYS != 16 or c.NULL_KIND != "other_regime":
        raise SystemExit("the idea moved")
    if c.MODE != "regime" or c.RULE_SIDE != 'short' or c.RULE_BOOK != 'cm':
        raise SystemExit("the leg moved")
    if c.EDGE != 'rich' or c.DISCOUNT != 0.0005 or c.WIDEN_LAG != 0:
        raise SystemExit("the edge moved")
    if c.FAIR_KEY != 'db' or c.FAIR_KEY in ("spot", "um", "cm"):
    ...
    shift = 0 if c.FAIR_KEY == "cb" else c.fp5.EIGHT_H_MS
    ...
    if c.EDGE == "cheap":
        ...
        rule_px, rule_fair = 102.0, 100.0
        off_px, off_fair = 100.0, 100.0
        old_at = entry - c.WIDEN_LAG * day
        _put(traded, old_at - day, 1, 100.2)
        _put(fair, fair_open(old_at), 1, 100.0)
    else:
    ...
    if c.EDGE == "widen":
        del late[fair_open(entry - c.WIDEN_LAG * day)]
