"""Pull the public series the fp112 screen is allowed to see.

The signal is the last top-trader position long/short. Account-count ratios are not stored. No hourly price bar is requested. No funding print is requested.
No alt quote is requested. No bookDepth file is requested. Count long/short
and taker columns are not stored.

    python3 docs/agents/scripts/fp112/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp112-research"}
SPOT_FIELDS = (1,)
...
COLUMNS = (5,)
METRICS_REL = "metrics/top_pos.json"
METRICS_MODE = "last"
...
def main() -> None:
    ...
    bars = pull_spot()
    print("fp112 " + f"metrics {len(metrics)} first {min(metrics)} last {max(metrics)}" + "; " + f"BTCUSDT 1d {len(bars)} first {min(bars)} last {max(bars)}")
