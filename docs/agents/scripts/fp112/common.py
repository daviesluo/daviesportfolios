"""The search fp112. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the last top-trader position long/short ratio. The account-count ratio is not an input. No hourly price bar is read. No funding print is read.
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Last top-trader position long/short. None when the day is missing or the ratio is not positive."""
    if day >= end_ms:
        return None
    row = rows.get(day)
