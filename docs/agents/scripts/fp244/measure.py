"""Score the fp244 screen. 2023 entries only.

    python3 docs/agents/scripts/fp244/measure.py
"""
...
DATA = Path(os.environ.get("FP244_DATA", "/tmp/fp244/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp244-protocol.md"
NOTE = 'The absolute nine-day coin-margined return finished smaller than the absolute nine-day return before it. The next open is bought and sold six days later. One coin-margined leg. Funding cash is not added. The null is that six-day long on every day.'
...
def check_signal(trade: dict, books: dict) -> None:
    near = c.finished(books["cm"], trade["entry_ms"], 9)
    far = c.finished(books["cm"], trade["entry_ms"] - 9 * c.fp5.DAY_MS, 9)
    if near is None or far is None or not abs(near) < abs(far):
        raise SystemExit("the later nine-day move was not the quieter one")
...
def run() -> None:
    ...
    trades = c.signal_trades(books['cm'])
    spans = c.signal_spans(books['cm'])
    ...
    pool_spans = c.pool_spans(books['cm'])
    pool = c.pool_nets(books['cm'])
    if c.IDEA != 'CALM' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp244_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp244_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp244/screen_2023.json"
