"""The search fp244. A smaller absolute nine-day coin-margined return, then a six-day coin-margined long.
...
not added. The null is that same 6-day long on every day.
"""
...
def _signal(cm: dict, entry: int) -> bool:
    near = finished(cm, entry, 9)
    far = finished(cm, entry - 9 * fp5.DAY_MS, 9)
