...
"""Pins for the fp244 rule. No file and no return from 2023 is read."""
...
def test_calm() -> None:
    if c.IDEA != "CALM" or c.MIN_N != 30 or c.HOLD_DAYS != 6:
    ...
    y = entry - day
    _put(cm, y, 1, 101)
    _put(cm, y - 9 * day, 1, 100)
    _put(cm, y - 18 * day, 1, 80)
    _put(cm, entry, 30, 200)
    _put(cm, entry + 6 * day, 40, 1)
    py = missed - day
    _put(cm, py, 1, 150)
    _put(cm, py - 9 * day, 1, 100)
    _put(cm, py - 18 * day, 1, 90)
    _put(cm, missed, 25, 90)
    _put(cm, missed + 6 * day, 25, 1)
    trades = c.signal_trades(cm)
    if len(trades) != 1 or trades[0]["entry_ms"] != entry:
        raise SystemExit("the coin-margined leg was bought when the later window was not quieter")
