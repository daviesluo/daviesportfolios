"""Score the fp92 screen. 2023 only.

    python3 docs/agents/scripts/fp92/measure.py
"""
...
DATA = Path(os.environ.get("FP92_DATA", "/tmp/fp92/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp92-protocol.md"
...
def main() -> None:
    ...
    trades = c.ends_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "ENDS", trades, pool, c.MIN_N,
        "Opening-hour quote over the last hour's quote, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp92_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp92_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp92/screen_2023.json"
