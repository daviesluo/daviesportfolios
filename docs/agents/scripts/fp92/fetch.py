...
because it can be an exit. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp92/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp92-research"}
...
def main() -> None:
    ...
    hourly = klines("BTCUSDT", "1h", c.HOUR_MS, hour_end, (7,))
    ...
    print(
        f"fp92 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
