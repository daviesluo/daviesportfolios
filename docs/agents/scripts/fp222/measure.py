"""Score the fp222 screen. 2023 entries only.

    python3 docs/agents/scripts/fp222/measure.py
"""
...
DATA = Path(os.environ.get("FP222_DATA", "/tmp/fp222/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp222-protocol.md"
NOTE = "Friday's open is bought and the next Friday's open is sold. One USDT-perpetual leg. Funding cash is not added. The null is that seven-day long on every day."
...
def run() -> None:
    ...
    for trade in trades:
        ...
        if stamp.weekday() != 4:
            raise SystemExit("the entry is not a Friday")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp222_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp222_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp222/screen_2023.json"
