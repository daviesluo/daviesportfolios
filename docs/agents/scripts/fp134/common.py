"""The search fp134. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is intraday range of the unspent-output count. No hourly price bar is read.
No funding print is read. No option file is read. No mempool chart is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Maximum minus minimum that UTC day. None when a sample is bad."""
    if day >= end_ms:
        return None
    row = rows.get(day)
    if row is None or len(row) < 4 or len(row) % 2 != 0:
        return None
    vals = []
