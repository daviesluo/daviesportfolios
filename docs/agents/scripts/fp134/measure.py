"""Score the fp134 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp134/measure.py
"""
...
DATA = Path(os.environ.get("FP134_DATA", "/tmp/fp134/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp134-protocol.md"
NOTE = "Intraday range of the unspent-output count, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP134_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "utxo/samples.json")
    ...
    row = c.fp5.summarise("UTXORNG", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp134_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp134_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "utxo_samples": sha256(DATA / "utxo/samples.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp134/screen_2023.json"
