...
bar is requested. No funding print is requested. No mempool chart is requested.
    ...
    python3 docs/agents/scripts/fp134/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp134-research"}
SPOT_FIELDS = (1,)
...
CHART = "https://api.blockchain.info/charts/utxo-count"
UNIT = "Number of Outputs"
...
def pull_signal() -> dict[int, tuple]:
    ...
    for point in values:
        ...
        if stamp_s >= c.fp5.SCREEN_END_MS // 1000:
            continue
        if stamp_s in seen:
            raise SystemExit("the chart repeats a timestamp")
        seen.add(stamp_s)
        day = stamp_s * 1000 // c.fp5.DAY_MS * c.fp5.DAY_MS
        value = as_number(point["y"], "a chart point")
        buckets.setdefault(day, []).extend((float(stamp_s * 1000), value))
    out = {day: tuple(pairs) for day, pairs in buckets.items()}
    assert_span(out, c.fp5.SCREEN_END_MS - c.fp5.DAY_MS, "signal")
    save("utxo/samples.json", out)
    return out
