"""Score the fp196 screen. 2023 entries only.

    python3 docs/agents/scripts/fp196/measure.py
"""
...
DATA = Path(os.environ.get("FP196_DATA", "/tmp/fp196/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp196-protocol.md"
NOTE = 'The coin-margined close finished strictly above the spot close. Next day, long spot and short that perpetual, open to close. Funding cash is not added. The null is that hedge on every day.'
...
def main() -> None:
    ...
    spot = load_rows(DATA / "spot1d/BTCUSDT.json", 2, end, required=True)
    ...
    for trade in trades:
        ...
        if not cm[day][1] > spot[day][1]:
            raise SystemExit("a trade was not a richer coin-margined close")
        ...
        short_leg = c.short_net(cm_open, cm_close)
    ...
    payload = {
        ...
        "code_sha256": {
            "fp196_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp196_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp196/screen_2023.json"
