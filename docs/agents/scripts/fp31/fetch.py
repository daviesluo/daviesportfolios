"""Pull BTCUSDT hourly opens and closes, and daily opens, for the fp31 screen.
...
entry. A later open is a bug. High, low, volume and trade count are not stored.
    ...
    python3 docs/agents/scripts/fp31/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp31-research"}
...
def main() -> None:
    ...
    print(
        f"fp31 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
