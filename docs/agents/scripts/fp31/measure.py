"""Score the fp31 screen. 2023 only.

    python3 docs/agents/scripts/fp31/measure.py
"""
...
DATA = Path(os.environ.get("FP31_DATA", "/tmp/fp31/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp31-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "RET-HHI", c.hhi_trades(daily, hours), pool, c.MIN_N,
        "Herfindahl of absolute hourly returns, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp31_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp31_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp31/screen_2023.json"
