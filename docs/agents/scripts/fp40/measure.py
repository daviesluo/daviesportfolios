"""Score the fp40 screen. 2023 only.

    python3 docs/agents/scripts/fp40/measure.py
"""
...
DATA = Path(os.environ.get("FP40_DATA", "/tmp/fp40/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp40-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "HI-LOWS", c.lows_trades(daily, hours), pool, c.MIN_N,
        "Count of hours with a higher low, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp40_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp40_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp40/screen_2023.json"
