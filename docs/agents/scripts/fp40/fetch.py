"""Pull BTCUSDT bars for the fp40 screen.

Hourly bars store the low. Open, high, close, volume and trade count are not stored.
...
entry. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp40/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp40-research"}
...
def main() -> None:
    ...
    hourly = klines("1h", c.HOUR_MS, hour_end, (3,))
    ...
    print(
        f"fp40 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
