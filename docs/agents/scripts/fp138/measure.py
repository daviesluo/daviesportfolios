"""Score the fp138 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp138/measure.py
"""
...
DATA = Path(os.environ.get("FP138_DATA", "/tmp/fp138/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp138-protocol.md"
NOTE = "Premium-index high minus low, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP138_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "premium/hl.json")
    ...
    if len(signal) < 440:
        raise SystemExit("the signal has too few days")
    if min(signal) > 1_664_582_400_000 + 5 * c.fp5.DAY_MS:
        raise SystemExit("the signal starts too late")
    if max(signal) < c.fp5.SCREEN_END_MS - 3 * c.fp5.DAY_MS:
        raise SystemExit("the signal ends too early")
    ...
    row = c.fp5.summarise("BASERNG", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp138_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp138_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "premium_hl": sha256(DATA / "premium/hl.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp138/screen_2023.json"
