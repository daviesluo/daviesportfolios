...
def test_formula() -> None:
    ...
    if not near(c.signal_at({day: (2.0, -1.0)}, day), 3.0):
        raise SystemExit("the width was not the signal")
    if near(c.signal_at({day: (2.0, -1.0)}, day), 2.0):
        raise SystemExit("the high replaced the width")
    if near(c.signal_at({day: (2.0, -1.0)}, day), -1.0):
        raise SystemExit("the low replaced the width")
    if not near(c.signal_at({day: (1.0, 1.0)}, day), 0.0):
        raise SystemExit("a zero width was dropped")
    ...
    if c.signal_at({day: (1.0,)}, day) is not None:
        raise SystemExit("one field was a print")
    later = c.fp5.SCREEN_END_MS
    if c.signal_at({later: (2.0, -1.0)}, later) is not None:
        raise SystemExit("a 2024 day was a print")
    quiet = day - c.fp5.DAY_MS
    if c.signal_prints({quiet: (1.0, 1.0)}) != [(quiet, 0.0)]:
        raise SystemExit("a zero width left the history")

"""Pins for the fp138 rule. No file and no return from 2023 is read."""
...
def test_tail_and_fill() -> None:
    ...
    for i in range(121):
        ...
        rows[day] = (3.0, 0.0) if i == 120 else (0.0, 0.0)
