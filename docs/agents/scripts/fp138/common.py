"""The search fp138. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the premium-index high minus the low. The close is not an input. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """High minus low. The premium may be negative. None when high sits under low."""
    if day >= end_ms:
        return None
    row = rows.get(day)
