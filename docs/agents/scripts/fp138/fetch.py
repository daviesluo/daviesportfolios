"""Pull the BTC premium-index daily high and low for the fp138 screen. The close is not stored. No funding print is requested. No hourly price bar is requested.

    python3 docs/agents/scripts/fp138/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp138-research"}
...
def pull_signal() -> dict[int, tuple]:
    ...
    if ym_list[0] != "2022-10" or ym_list[-1] != "2023-12":
        raise SystemExit("the premium window moved")
    out: dict[int, tuple] = {}
    for ym in ym_list:
        if ym >= "2024-01":
            raise SystemExit("a 2024 premium file was requested")
        url = (
        ...
        for row in reader:
            if not row or row[0] == "open_time":
                continue
            if len(row) < 4:
                raise SystemExit("a premium bar is short")
            open_ms = int(row[0])
            if open_ms > 10**14:
            ...
            low = float(row[3])
            if high < low:
                raise SystemExit("a premium high sits under its low")
            if open_ms in out:
                raise SystemExit(f"two premium bars share {open_ms}")
            out[open_ms] = (high, low)
    stop = c.fp5.SCREEN_END_MS - c.fp5.DAY_MS
    if not out or min(out) != LOOKBACK_MS:
        raise SystemExit("premium does not start on 2022-10-01")
    if max(out) < stop - 2 * c.fp5.DAY_MS:
        raise SystemExit("premium ends too early")
    ...
    if any(len(row) != 2 for row in out.values()):
        raise SystemExit("a premium bar stored the close")
    save("premium/hl.json", out)
    return out
