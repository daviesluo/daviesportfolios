"""The pre-registered AVG-CHG test. Scores 2024-01-01 through 2026-09-24.
...
when it was frozen. Re-running the 2023 screen is a check, not the bar.
    ...
    python3 docs/agents/scripts/fp48/size_test.py
"""
...
PREREG = ROOT / "docs/agents/reviews/2026-09-25-fp48-prereg-size.md"
SIDECAR = ROOT / "docs/agents/reviews/2026-09-25-fp48-prereg-size.sha256"

# Measured 2026-09-25T03:47:48Z from data-api bookTicker BTCUSDT, bid 84180.31000000 / ask 84180.32000000.
HALF_SPREAD = 5.939630903800506e-08
...
def main() -> None:
    ...
    if not oos_day_path.exists():
        os.environ["FP48_OOS"] = "1"
        fetch.pull_oos()
    screen_daily = load_bars(screen_day_path, 4)
    screen = c.size_trades(screen_daily)
    screen_total = round(sum(t["pnl"] for t in screen), 4)
    screen_n = len(screen)
    if screen_n != 48 or abs(screen_total - 37.2656) > 0.01:
        raise SystemExit(f"2023 did not reproduce: n={screen_n} total={screen_total}")

    oos_daily = load_bars(oos_day_path, 4)
    ...
    if len(horizon) != 3 or horizon[1] != 0 or horizon[2] != 0:
        raise SystemExit("the 2026-09-25 bar stored a quote or a trade count")
    daily = dict(screen_daily)
    daily.update(oos_daily)
    primary = c.size_trades(daily, 0.90, FEE, OOS_START, OOS_END)
    doubled = c.size_trades(daily, 0.90, FEE_DOUBLE, OOS_START, OOS_END)
    q80 = c.size_trades(daily, 0.80, FEE, OOS_START, OOS_END)
    q95 = c.size_trades(daily, 0.95, FEE, OOS_START, OOS_END)
    ...
    out = ROOT / "docs/agents/backtests/fp48/size_oos.json"
