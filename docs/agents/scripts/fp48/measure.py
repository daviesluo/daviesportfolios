"""Score the fp48 screen. 2023 only.

    python3 docs/agents/scripts/fp48/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp48-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "AVG-CHG", c.size_trades(daily), pool, c.MIN_N,
        "Rise in quote per trade, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp48_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp48_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp48/screen_2023.json"
