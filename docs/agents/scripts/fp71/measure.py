"""Score the fp71 screen. 2023 only.

    python3 docs/agents/scripts/fp71/measure.py
"""
...
DATA = Path(os.environ.get("FP71_DATA", "/tmp/fp71/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp71-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "OPEN-IN", c.openin_trades(daily), pool, c.MIN_N,
        "Open inside today's range, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp71_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp71_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp71/screen_2023.json"
