"""Pull bars for the fp71 screen.

The 2024-01-01 high and low are stored equal to the open.

No later year is requested. The 2024-01-01 bar keeps its open. Its high and low are masked.

    python3 docs/agents/scripts/fp71/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp71-research"}
...
def main() -> None:
    ...
    print(f"fp71 BTCUSDT 1d {len(rows)} first {min(rows)} last {max(rows)}")
