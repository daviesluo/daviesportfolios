"""Score the fp293 screen. 2023 entries only.

    python3 docs/agents/scripts/fp293/measure.py
"""
...
DATA = Path(os.environ.get("FP293_DATA", "/tmp/fp293/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp293-protocol.md"
NOTE = 'Fair value of the coin-margined book is the USDT close. The finished coin-margined close was strictly under that fair value. The next coin-margined open is bought and sold eleven days later. The other trade buys the coin-margined book for eleven days when it is not cheap to the USDT book. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side.'
...
def run() -> None:
    ...
    if c.IDEA != 'CHEAPC' or c.MIN_N != 30 or c.HOLD_DAYS != 11 or c.NULL_KIND != 'other_regime':
    ...
    payload = {
        ...
        "code_sha256": {
            "fp293_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp293_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp293/screen_2023.json"
