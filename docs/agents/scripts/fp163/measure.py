"""Score the fp163 screen. 2023 entries only.

    python3 docs/agents/scripts/fp163/measure.py
"""
...
DATA = Path(os.environ.get("FP163_DATA", "/tmp/fp163/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp163-protocol.md"
NOTE = 'The taker ratio rose through the day. Next day, buy the open, sell at 2% above it when the high reaches it, otherwise sell the close.'
...
def main() -> None:
    ...
    for trade in trades:
        ...
        first, last = taker[day]
        if not last > first:
            raise SystemExit("a trade was not a rising taker path")
        open_px, high, close_px = bars[trade["entry_ms"]]
        target = open_px * 1.02
        exit_px = target if high >= target else close_px
        if abs(trade["gross"] - (exit_px / open_px - 1.0)) > 1e-12:
            raise SystemExit("the gross is not the target or the close")
        if abs(trade["net"] - c.fp5.net_return(open_px, exit_px)) > 1e-12:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp163_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp163_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp163/screen_2023.json"
