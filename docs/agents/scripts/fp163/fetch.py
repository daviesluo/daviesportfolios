"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp163/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp163-research"}
...
def main() -> None:
    ...
    taker = pull_metric_ends("2023-12-31", "taker_ends")
    spot = pull_opens(c.fp5.SCREEN_END_MS, "spot1d/BTCUSDT.json", (1, 2, 4))
    print("fp163 " + f"taker {len(taker)} spot {len(spot)} last {max(spot)}")
