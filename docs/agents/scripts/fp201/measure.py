"""Score the fp201 screen. 2023 entries only.

    python3 docs/agents/scripts/fp201/measure.py
"""
...
DATA = Path(os.environ.get("FP201_DATA", "/tmp/fp201/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp201-protocol.md"
NOTE = "The taker-buy average finished strictly above the mark close. Next day, short the coin-margined perpetual from the open to the close. The null is that session short on every day."
...
def main() -> None:
    ...
    um = load_rows(DATA / "um1d/BTCUSDT.json", 2, last, required=True)
    mark = load_rows(DATA / "mark1d/BTCUSDT.json", 1, last, required=True)
    cm = load_rows(DATA / "cm1d/BTCUSD_PERP.json", 2, session, required=True)
    ...
    for trade in trades:
        ...
        if day > last or day not in um or day not in mark:
            raise SystemExit("the signal day is past the window")
        if not (um[day][1] / um[day][0] > mark[day][0]):
            raise SystemExit("a trade was not taker-rich")
        open_px, close_px = cm[trade["entry_ms"]]
    ...
    payload = {
        ...
        "code_sha256": {
            "fp201_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp201_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp201/screen_2023.json"
