"""The search fp179. A close above yesterday's close and under yesterday's high.
...
def signal_days(bars: dict[int, tuple], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Days that closed above yesterday's close and under yesterday's high."""
    out = []
    for day in sorted(bars):
        ...
        close_px = bars[day][3]
        prior_high = bars[prev][1]
        prior_close = bars[prev][3]
        if close_px > prior_close and close_px < prior_high:
            out.append(day)
    return out
