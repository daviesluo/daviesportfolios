"""Score the fp179 screen. 2023 entries only.

    python3 docs/agents/scripts/fp179/measure.py
"""
...
DATA = Path(os.environ.get("FP179_DATA", "/tmp/fp179/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp179-protocol.md"
NOTE = "The close finished above yesterday's close and under yesterday's high. Next day, long BTC from the open to the close."
...
def main() -> None:
    ...
    for trade in trades:
        ...
        if not (bars[day][3] > prev[3] and bars[day][3] < prev[1]):
            raise SystemExit("a trade was not between yesterday's close and high")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp179_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp179_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp179/screen_2023.json"
