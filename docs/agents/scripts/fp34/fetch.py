"""Pull daily bars for the fp34 screen.

A bar is the open and the trade count. The 2024-01-01 count is stored as 0.
...
open is a bug.
    ...
    python3 docs/agents/scripts/fp34/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp34-research"}
...
def main() -> None:
    ...
    for symbol in ("BTCUSDT",):
        ...
        save(symbol, rows)
        print(f"fp34 {symbol} 1d {len(rows)} first {min(rows)} last {max(rows)}")
