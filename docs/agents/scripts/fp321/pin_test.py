...
"""Pins for the fp321 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "EREAL" or c.HOLD_DAYS != 2 or c.NULL_KIND != "other_regime":
        raise SystemExit("the idea moved")
    if c.MODE != "regime" or c.RULE_SIDE != "long" or c.RULE_BOOK != "cm":
        raise SystemExit("the leg moved")
    if c.FAIR_KIND != "realized" or c.COIN != "ETHUSD_PERP" or c.THRESHOLD_BPS != Decimal("1"):
    ...
    # Exactly one basis point under the fair value. The entry day's close is rich.
    fair[entry - 86_400_000] = Decimal("100.01")
    # A later stamp sitting on the entry must not replace that print.
    fair[entry] = Decimal("1")
    # Half a basis point is the edge turned off, even though the entry close is cheap.
    fair[off - 86_400_000] = Decimal("100.005")
