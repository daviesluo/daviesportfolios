"""The search fp197. The 16:00 funding rate finished above the 08:00 rate.

Nothing here reads the network or a file. Both prints have happened at
16:00. That day's close is sold and the next open is bought. The close is
the fill, not an input. Funding cash is not added. The null is that
overnight short on every day, not an unconditional long.
"""
...
def signal_days(funding: dict[int, float], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Days whose 16:00 funding rate is strictly above the same day's 08:00 rate."""
    out = []
    for bucket in sorted(funding):
        if bucket % fp5.DAY_MS != fp5.EIGHT_H_MS:
            continue
        day = bucket - fp5.EIGHT_H_MS
        if day >= end_ms or day > ENTRY_LAST_MS:
            continue
        afternoon = day + 2 * fp5.EIGHT_H_MS
        if afternoon not in funding:
            continue
        if funding[afternoon] > funding[bucket]:
            out.append(day)
    return out
...
def pool_nets(
    ...
    """An overnight short on every in-screen day."""
    pool = []
    day = start_ms
    while day < end_ms:
        trade = _overnight(bars, day, fee)
