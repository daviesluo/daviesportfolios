"""The search fp130. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is median confirmation time of the UTC day. No hourly price bar is read.
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """The day's single published value. None when it is missing or not positive."""
    if day >= end_ms:
        return None
    row = rows.get(day)
