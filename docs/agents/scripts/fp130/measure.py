"""Score the fp130 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp130/measure.py
"""
...
DATA = Path(os.environ.get("FP130_DATA", "/tmp/fp130/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp130-protocol.md"
NOTE = "Median confirmation time of the UTC day, in minutes, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP130_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "confirm/median.json")
    ...
    row = c.fp5.summarise("MEDCONF", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp130_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp130_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "median_conf": sha256(DATA / "confirm/median.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp130/screen_2023.json"
