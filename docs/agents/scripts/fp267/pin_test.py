...
"""Pins for the fp267 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "FASTER" or c.HOLD_DAYS != 8 or c.NULL_KIND != "other_time":
    ...
    _put(um, entry - 4 * day, 1, 100)
    _put(um, entry - 7 * day, 1, 100)
    _put(um, entry, 50, 1)
    _put(um, entry + 8 * day, 40, 1)
    _put(um, entry + 16 * day, 45, 1)
    _put(um, missed - day, 1, 110)
    _put(um, missed - 4 * day, 1, 100)
    _put(um, missed - 7 * day, 1, 100)
    _put(um, missed, 50, 1)
    _put(um, missed + 8 * day, 40, 1)
    _put(um, missed + 16 * day, 45, 1)
    ...
    if len(rules) != 1 or rules[0]["entry_ms"] != entry or rules[0]["side"] != "short":
        raise SystemExit("the faster decline was not sold")
    ...
    if abs(nulls[0]["net"] - float(_short(Decimal(40), Decimal(45)))) > 1e-12:
