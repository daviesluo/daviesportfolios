"""Score the fp267 screen. 2023 entries only.

    python3 docs/agents/scripts/fp267/measure.py
"""
...
DATA = Path(os.environ.get("FP267_DATA", "/tmp/fp267/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp267-protocol.md"
NOTE = 'A finished three-day USDT decline was strictly below zero and strictly below the three-day window that ended three days earlier. The next USDT open is sold and bought back eight days later. The other trade sells that book eight days later for another eight days. Both windows have to fill. One leg. Funding cash is not added. The null is not that eight-day short on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'FASTER' or c.MIN_N != 30 or c.HOLD_DAYS != 8:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp267_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp267_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp267/screen_2023.json"
