"""Score the fp116 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp116/measure.py
"""
...
DATA = Path(os.environ.get("FP116_DATA", "/tmp/fp116/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp116-protocol.md"
NOTE = "Spot trade count over the USDT perpetual's trade count, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    ...
    row = c.fp5.summarise("TRDMIX", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp116_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp116_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "um_count": sha256(DATA / "um/count.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp116/screen_2023.json"
