"""The search fp116. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the spot trade count divided by the USDT perpetual's trade count. No hourly price bar is read. No funding print is read.
...
def signal_at(
    ...
    """Spot trade count over USDT-perpetual trade count. The level is not the statistic."""
    if day >= end_ms:
        return None
    bar = spot.get(day)
    quote = perp.get(day)
