"""Pull the public series the fp116 screen is allowed to see.

The signal is the spot trade count over the USDT perpetual's trade count. Quote volume is not stored. The 2024-01-01 trade count is stored as zero. The perpetual stops before 2024-01-01. No hourly price bar is requested. No funding print is requested.
No alt quote is requested. No bookDepth file is requested. Count long/short
and taker columns are not stored.

    python3 docs/agents/scripts/fp116/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp116-research"}
SPOT_FIELDS = (1, 8)
...
def main() -> None:
    ...
    bars = pull_spot()
    um = pull_um()
    print("fp116 " + f"BTCUSDT 1d {len(bars)} first {min(bars)} last {max(bars)}" + "; " + f"um {len(um)} first {min(um)} last {max(um)}")
