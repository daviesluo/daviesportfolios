"""Pull the inputs this rule stores. No later year is requested as an entry.
    ...
    python3 docs/agents/scripts/fp309/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp309-research"}
...
def pull_gate() -> dict[int, tuple]:
    start = c.LOOKBACK_MS // 1000
    end = c.FAIR_LAST_MS // 1000
    rows: dict[int, tuple] = {}
    cursor = start
    while cursor <= end:
        nxt = min(cursor + 200 * 86400, end)
        url = (
            "https://api.gateio.ws/api/v4/spot/candlesticks?currency_pair=BTC_USDT"
            f"&interval=1d&from={cursor}&to={nxt}"
        )
        for row in json.loads(get(url)):
            ts = int(row[0]) * 1000
            if str(row[7]).lower() != "true":
                continue
            if not keep_fair(ts):
                continue
            rows[ts] = (float(row[5]), float(row[2]))
        if nxt == end:
            break
        cursor = nxt
        time.sleep(0.15)
    check_fair(rows, "Gate.io")
    save("gt1d/BTC_USDT.json", rows)
    return rows
...
def main() -> None:
    ...
    fair = pull_gate()
    print(f"fp309 {len(traded)} {len(fair)}")
