"""Score the fp279 screen. 2023 entries only.

    python3 docs/agents/scripts/fp279/measure.py
"""
...
DATA = Path(os.environ.get("FP279_DATA", "/tmp/fp279/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp279-protocol.md"
NOTE = 'The finished fifteen-day spot return was positive. The next spot open is bought and sold three days later. The other trade buys spot three days later for another three days. Both windows have to fill. One leg. Funding cash is not added. The null is not that three-day long on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'RISE' or c.MIN_N != 30 or c.HOLD_DAYS != 3:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp279_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp279_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp279/screen_2023.json"
