...
"""Pins for the fp279 rule. No file and no return from 2023 is read."""
...
def test_rule() -> None:
    if c.IDEA != "RISE" or c.HOLD_DAYS != 3 or c.NULL_KIND != "other_time":
    ...
    _put(spot, entry + 3 * day, 55, 1)
    _put(spot, entry + 6 * day, 60, 1)
    ...
    if len(rules) != 1 or rules[0]["entry_ms"] != entry or rules[0]["side"] != "long":
        raise SystemExit("the rise was not bought")
    if rules[0]["exit_ms"] - rules[0]["entry_ms"] != 3 * day:
        raise SystemExit("the hold is not three days")
    if len(nulls) != 1 or nulls[0]["entry_ms"] != entry + 3 * day:
        raise SystemExit("the other trade did not start three days later")
