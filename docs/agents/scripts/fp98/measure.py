"""Score the fp98 screen. 2023 only.

    python3 docs/agents/scripts/fp98/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp98-protocol.md"
...
def main() -> None:
    ...
    alt_path = DATA / "spot1d" / "DOGEUSDT.json"
    ...
    for trade in trades:
        if trade["coin"] != "DOGEUSDT":
            raise SystemExit("a fill is not DOGE")
    ...
    pool = c.fp5.pool_from_bars({"DOGEUSDT": alt}, ["DOGEUSDT"], c.fp5.DAY_MS)
    row = c.fp5.summarise(
        "DOGE-Q", trades, pool, c.MIN_N,
        "DOGE's daily quote volume over BTC's, above its own trailing 90th. Next day, long DOGE.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp98_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp98_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp98/screen_2023.json"
