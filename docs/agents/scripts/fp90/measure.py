"""Score the fp90 screen. 2023 only.

    python3 docs/agents/scripts/fp90/measure.py
"""
...
DATA = Path(os.environ.get("FP90_DATA", "/tmp/fp90/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp90-protocol.md"
...
def main() -> None:
    ...
    trades = c.lastsize_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "LAST-SIZE", trades, pool, c.MIN_N,
        "Last hour's average trade versus the day's, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp90_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp90_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp90/screen_2023.json"
