"""Pull bars for the fp76 screen.

Hourly bars store the high and the low.
...
No later year is requested. Hourly bars stop before the 2024-01-01 open. The daily file keeps that open because it can be an exit. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp76/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp76-research"}
...
def main() -> None:
    ...
    print(
        f"fp76 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
