"""Score the fp76 screen. 2023 only.

    python3 docs/agents/scripts/fp76/measure.py
"""
...
DATA = Path(os.environ.get("FP76_DATA", "/tmp/fp76/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp76-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "MAX-BAR", c.maxbar_trades(daily, hours), pool, c.MIN_N,
        "Widest hour over the day's range, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp76_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp76_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp76/screen_2023.json"
