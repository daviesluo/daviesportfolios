"""Score the fp74 screen. 2023 only.

    python3 docs/agents/scripts/fp74/measure.py
"""
...
DATA = Path(os.environ.get("FP74_DATA", "/tmp/fp74/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp74-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "MORNING", c.morning_trades(daily, hours), pool, c.MIN_N,
        "Return from hour 0 to hour 6, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp74_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp74_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp74/screen_2023.json"
