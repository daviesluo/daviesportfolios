"""Pull the spot trade count and the USDT perpetual trade count. Quote volume is not stored. No hourly price bar is requested.

    python3 docs/agents/scripts/fp149/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp149-research"}
...
def pull_signal() -> dict[int, tuple]:
    ...
    while cursor <= end_ms:
        ...
        for k in batch:
            if len(k) < 9:
                raise SystemExit("a spot kline is missing the trade count")
            open_ms = int(k[0])
            if LOOKBACK_MS <= open_ms <= end_ms:
                count = float(k[8])
                if count <= 0:
                    raise SystemExit("a spot trade count is not positive")
                spot[open_ms] = count
        nxt = int(batch[-1][0]) + c.fp5.DAY_MS
        if nxt <= cursor:
            break
    ...
    for ym in months():
        ...
        for row in csv.reader(io.StringIO(zf.read(zf.namelist()[0]).decode())):
            if not row or row[0] == "open_time" or len(row) < 9:
                continue
            open_ms = int(row[0])
            ...
            count = float(row[8])
            if count <= 0:
                raise SystemExit("a perpetual trade count is not positive")
            if open_ms in um:
