"""Score the fp228 screen. 2023 entries only.

    python3 docs/agents/scripts/fp228/measure.py
"""
...
DATA = Path(os.environ.get("FP228_DATA", "/tmp/fp228/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp228-protocol.md"
NOTE = 'The USDT close finished below its close thirty days earlier. The next open is bought and sold five days later. One USDT-perpetual leg. Funding cash is not added. The null is that five-day long on every day.'
...
def check_common(trade: dict) -> None:
    ...
    if trade["exit_ms"] - trade["entry_ms"] != 5 * c.fp5.DAY_MS:
...
def assert_subset(trades, pool, spans, pool_spans) -> None:
    ...
    for entry, exit_ms in pool_spans:
        if exit_ms - entry != 5 * c.fp5.DAY_MS:
...
def run() -> None:
    ...
    book = load_rows(DATA / "um1d/BTCUSDT.json", 2, end)
    ...
    for trade in trades:
        check_common(trade)
        move = c.finished(book, trade["entry_ms"], 30)
        if move is None or move >= 0.0:
            raise SystemExit("the decline was not already finished")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp228_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp228_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp228/screen_2023.json"
