"""Pull BTC total native fees and BTCUSDT daily opens for the fp24 screen.

No later year is requested. A fee stamp on or after 2024-01-01 is not
...
the open only.
    ...
    python3 docs/agents/scripts/fp24/fetch.py
"""
...
DATA = Path(os.environ.get("FP24_DATA", "/tmp/fp24/data"))
...
UA = {"User-Agent": "daviesportfolios-fp24-research"}
METRIC = "FeeTotNtv"
    ...
    while url:
        ...
        for row in body.get("data") or []:
            ...
            if ts < LOOKBACK_MS or ts >= c.fp5.SCREEN_END_MS:
                continue
            if ts in rows:
                raise RuntimeError(f"duplicate fee stamp {ts}")
...
def main() -> None:
    if os.environ.get("FP24_OOS"):
        raise SystemExit("this pull does not request a later year")
    fees = pull_fees()
    if any(row[0] >= c.fp5.SCREEN_END_MS for row in fees):
        raise SystemExit("a fee stamp is on or after 2024-01-01")
    daily = pull_opens()
    if any(t > c.fp5.SCREEN_END_MS for t in daily):
        raise SystemExit("a daily bar opened after 2024-01-01")
    if not fees or not daily:
        raise SystemExit("the fee series or the daily opens came back empty")
    DATA.mkdir(parents=True, exist_ok=True)
    (DATA / "FeeTotNtv.json").write_text(json.dumps(fees, separators=(",", ":")))
    spot = DATA / "spot1d"
    ...
    print(
        f"fp24 FeeTotNtv {len(fees)} first {fees[0][0]} last {fees[-1][0]}; "
