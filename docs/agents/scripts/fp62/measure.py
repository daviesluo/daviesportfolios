"""Score the fp62 screen. 2023 only.

    python3 docs/agents/scripts/fp62/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp62-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "UNDER", c.under_trades(daily), pool, c.MIN_N,
        "Low under yesterday's close, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp62_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp62_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp62/screen_2023.json"
