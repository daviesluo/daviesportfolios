"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp211/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp211-research"}
...
def main() -> None:
    ...
    um = pull_klines("um", "BTCUSDT", (1,), session, (2022, 12))
    cm = pull_klines("cm", "BTCUSD_PERP", (1,), session, (2022, 12))
    ...
    print("fp211 " + f"um {len(um)} cm {len(cm)} fum {len(fum)} fcm {len(fcm)}")
