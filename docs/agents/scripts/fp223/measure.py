"""Score the fp223 screen. 2023 entries only.

    python3 docs/agents/scripts/fp223/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp223-protocol.md"
NOTE = "The 16:00 funding rate finished strictly below the 16:00 rate seven days earlier. The next open is bought and sold seven days later. One USDT-perpetual leg. Funding cash is not added. The null is that seven-day long on every day both prints exist."
...
def load_rows(path: Path, width: int, last_ms: int) -> dict[int, tuple]:
    ...
    for row in rows:
        ...
        ts = int(row[0])
        if ts > last_ms or ts > c.fp5.SCREEN_END_MS:
...
def run() -> None:
    ...
    um = load_rows(DATA / "um1d/BTCUSDT.json", 1, c.fp5.SCREEN_END_MS)
    fund = load_rows(DATA / "funding/BTCUSDT.json", 1, last)
    if any(ts >= c.fp5.SCREEN_END_MS for ts in fund):
        raise SystemExit("a 2024 funding print was stored")
    trades = c.signal_trades(um, fund)
    spans = c.signal_spans(um, fund)
    ...
    for trade in trades:
        ...
        if later >= c.fp5.SCREEN_END_MS or earlier >= c.fp5.SCREEN_END_MS:
            raise SystemExit("a 2024 funding print was a signal")
        if fund[later][0] >= fund[earlier][0]:
            raise SystemExit("the 16:00 rate did not fall")
        entry_px = um[entry][0]
    ...
    pool_spans = c.pool_spans(um, fund)
    pool = c.pool_nets(um, fund)
    if c.IDEA != "FWK" or c.MIN_N != 30:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp223_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp223_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp223/screen_2023.json"
