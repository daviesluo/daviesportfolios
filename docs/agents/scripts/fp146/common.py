"""The search fp146. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the intraday change in dollar open interest. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Last dollar open interest over the first, minus one. The middle of the day is not an input."""
    if day >= end_ms:
        return None
    row = rows.get(day)
