"""Score the fp291 screen. 2023 entries only.

    python3 docs/agents/scripts/fp291/measure.py
"""
...
DATA = Path(os.environ.get("FP291_DATA", "/tmp/fp291/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp291-protocol.md"
NOTE = 'Fair value of spot is the USDT close. The finished spot close was strictly under that fair value. The next spot open is bought and sold fourteen days later. The other trade buys spot for fourteen days when it is not cheap to the USDT book. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side.'
...
def run() -> None:
    ...
    if c.IDEA != 'CHEAPS' or c.MIN_N != 30 or c.HOLD_DAYS != 14 or c.NULL_KIND != 'other_regime':
    ...
    payload = {
        ...
        "code_sha256": {
            "fp291_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp291_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp291/screen_2023.json"
