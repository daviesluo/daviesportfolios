"""The search fp253. A finished down day and then a finished up day, then a twenty-two-day spot long.
...
not added. The null is that same 22-day long on every day.
"""
...
def _signal(spot: dict, entry: int) -> bool:
    ...
    near = finished(spot, entry, 1)
    prev = finished(spot, entry - fp5.DAY_MS, 1)
    if near is None or prev is None:
        return False
    return near > 0.0 and prev < 0.0
