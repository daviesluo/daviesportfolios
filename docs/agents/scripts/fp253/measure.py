"""Score the fp253 screen. 2023 entries only.

    python3 docs/agents/scripts/fp253/measure.py
"""
...
DATA = Path(os.environ.get("FP253_DATA", "/tmp/fp253/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp253-protocol.md"
NOTE = "Yesterday's spot close finished above the prior close, and that prior close finished below the close before it. The next spot open is bought and sold twenty-two days later. One spot leg. Funding cash is not added. The null is that twenty-two-day long on every day."
...
def check_signal(trade: dict, books: dict) -> None:
    if not c._signal(books["spot"], trade["entry_ms"]):
        raise SystemExit("the up day after a down day was not already finished")
...
def run() -> None:
    ...
    if c.IDEA != 'FLIP' or c.MIN_N != 30 or c.HOLD_DAYS != HOLD:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp253_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp253_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp253/screen_2023.json"
