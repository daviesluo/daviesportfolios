"""Score the fp141 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp141/measure.py
"""
...
DATA = Path(os.environ.get("FP141_DATA", "/tmp/fp141/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp141-protocol.md"
NOTE = "Today's high-low over the open, divided by yesterday's, minus one, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP141_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "range/ohl.json")
    ...
    row = c.fp5.summarise("RNGRAT", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp141_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp141_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "range_ohl": sha256(DATA / "range/ohl.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp141/screen_2023.json"
