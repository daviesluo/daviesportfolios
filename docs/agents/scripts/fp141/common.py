"""The search fp141. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is today's high-low over the open, divided by yesterday's, minus one. The range level is not an input. No hourly price bar is read.
The hold is one day.
"""
...
def _width(row: tuple | None) -> float | None:
    if row is None or len(row) != 3:
        return None
    open_, high, low = row
    if open_ <= 0 or high <= 0 or low <= 0 or high < low:
        return None
    return (high - low) / open_


def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """Today's range over yesterday's, minus one. Yesterday is one day earlier."""
    if day >= end_ms:
        return None
    today = _width(rows.get(day))
    prev = _width(rows.get(day - fp5.DAY_MS))
