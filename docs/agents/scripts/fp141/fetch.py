"""Pull BTCUSDT daily open, high and low for the fp141 screen. The close is not stored. The fill file stores the open only. No hourly price bar is requested.

    python3 docs/agents/scripts/fp141/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp141-research"}
...
def pull_bars() -> tuple[dict[int, tuple], dict[int, tuple]]:
    ...
    while cursor <= end_ms:
        ...
        for k in batch:
            if len(k) < 4:
                raise SystemExit("a daily kline is missing the range")
            open_ms = int(k[0])
            if open_ms < LOOKBACK_MS:
                continue
            if open_ms > end_ms:
    ...
    for day, row in signal.items():
        if row[0] != opens[day][0]:
            raise SystemExit("the signal open is not the fill open")
        if len(row) != 3:
            raise SystemExit("a range row stored the close")
    save("spot1d/BTCUSDT.json", opens)
    save("range/ohl.json", signal)
    return opens, signal
