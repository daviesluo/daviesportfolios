"""Score the fp106 screen. 2023 entries only. The hold is 5 days.

    python3 docs/agents/scripts/fp106/measure.py
"""
...
DATA = Path(os.environ.get("FP106_DATA", "/tmp/fp106/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp106-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "FRNG-5", trades, pool, c.MIN_N,
        "Range of BTC's three funding prints, above its own trailing 90th. "
        "Next day, long BTC for 5 days.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp106_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp106_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp106/screen_2023.json"
