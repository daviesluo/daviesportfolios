"""Score the fp89 screen. 2023 only.

    python3 docs/agents/scripts/fp89/measure.py
"""
...
DATA = Path(os.environ.get("FP89_DATA", "/tmp/fp89/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp89-protocol.md"
...
def main() -> None:
    ...
    trades = c.worst_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "WORST-RET", trades, pool, c.MIN_N,
        "The worst hourly return, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp89_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp89_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp89/screen_2023.json"
