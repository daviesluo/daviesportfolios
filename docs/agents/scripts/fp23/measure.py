"""Score the fp23 screen. 2023 only.

    python3 docs/agents/scripts/fp23/measure.py
"""
...
DATA = Path(os.environ.get("FP23_DATA", "/tmp/fp23/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp23-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "UP-SHARE", c.share_trades(daily, hours), pool, c.MIN_N,
        "Share of the day's hours that closed up, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp23_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp23_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp23/screen_2023.json"
