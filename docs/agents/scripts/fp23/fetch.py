"""Pull BTCUSDT hourly opens and closes, and daily opens, for the fp23 screen.
...
entry. A later open is a bug. Volume is not stored.

    python3 docs/agents/scripts/fp23/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp23-research"}
...
def main() -> None:
    ...
    print(
        f"fp23 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
