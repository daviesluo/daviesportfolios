"""Score the fp171 screen. 2023 entries only.

    python3 docs/agents/scripts/fp171/measure.py
"""
...
DATA = Path(os.environ.get("FP171_DATA", "/tmp/fp171/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp171-protocol.md"
NOTE = "The close finished strictly above yesterday's high. Next day, long BTC from the open to the close."
...
def main() -> None:
    ...
    for trade in trades:
        ...
        if not bars[day][3] > bars[day - c.fp5.DAY_MS][1]:
            raise SystemExit("a trade did not close above yesterday's high")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp171_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp171_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp171/screen_2023.json"
