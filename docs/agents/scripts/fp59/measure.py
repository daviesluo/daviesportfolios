"""Score the fp59 screen. 2023 only.

    python3 docs/agents/scripts/fp59/measure.py
"""
...
DATA = Path(os.environ.get("FP59_DATA", "/tmp/fp59/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp59-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "STEP-BASE", c.stepbase_trades(daily, hours), pool, c.MIN_N,
        "Share of base volume in hours that closed above the prior hour, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp59_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp59_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp59/screen_2023.json"
