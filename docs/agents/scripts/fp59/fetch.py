"""Pull BTCUSDT bars for the fp59 screen.
...
No later year is requested. Hourly bars stop before the 2024-01-01 open. The daily file keeps that open because it can be an exit. A later open is a bug.
    ...
    python3 docs/agents/scripts/fp59/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp59-research"}
...
def main() -> None:
    ...
    hourly = klines("1h", c.HOUR_MS, hour_end, (4, 5))
    ...
    print(
        f"fp59 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
