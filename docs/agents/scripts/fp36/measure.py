"""Score the fp36 screen. 2023 only.

    python3 docs/agents/scripts/fp36/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp36-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "OPEN-LOC", c.loc_trades(daily), pool, c.MIN_N,
        "Where the open sits in yesterday's range, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp36_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp36_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp36/screen_2023.json"
