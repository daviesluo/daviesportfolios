"""Score the fp273 screen. 2023 entries only.

    python3 docs/agents/scripts/fp273/measure.py
"""
...
DATA = Path(os.environ.get("FP273_DATA", "/tmp/fp273/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp273-protocol.md"
NOTE = 'Two non-overlapping finished fourteen-day USDT rises. The next USDT open is bought and sold nine days later. The other trade buys that book nine days later for another nine days. Both windows have to fill. One leg. Funding cash is not added. The null is not that nine-day long on every day.'
...
def run() -> None:
    ...
    if c.IDEA != 'DRIFT' or c.MIN_N != 30 or c.HOLD_DAYS != 9:
    ...
    payload = {
        ...
        "code_sha256": {
            "fp273_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp273_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp273/screen_2023.json"
