"""The search fp273. Two non-overlapping finished fourteen-day USDT rises. Buy the USDT book for nine days. The other trade buys it nine days later for another nine days. Both windows have to fill. The entry open is not an input. One leg.
...
def _signal(spot, um, cm, entry) -> bool:
    near = finished(um, entry, 14)
    prev = finished(um, entry - 14 * fp5.DAY_MS, 14)
