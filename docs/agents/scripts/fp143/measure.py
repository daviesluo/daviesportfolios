"""Score the fp143 screen. 2023 entries only. The hold is one day.

    python3 docs/agents/scripts/fp143/measure.py
"""
...
DATA = Path(os.environ.get("FP143_DATA", "/tmp/fp143/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp143-protocol.md"
NOTE = "Spot close over the previous close, minus one, above its own trailing 90th. Next day, long BTC for one day."
...
def main() -> None:
    if os.environ.get("FP143_OOS"):
        raise SystemExit("this scorer is the 2023 screen")
    if not PROTOCOL.is_file():
        raise SystemExit("the protocol file is not on disk; do not score without it")
    signal = load_book(DATA / "close/btc.json")
    ...
    row = c.fp5.summarise("DRET", trades, pool, c.MIN_N, NOTE)
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp143_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp143_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {
            "btc_close": sha256(DATA / "close/btc.json"),
    ...
    out = ROOT / "docs/agents/backtests/fp143/screen_2023.json"
