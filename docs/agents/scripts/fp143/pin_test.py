...
def test_formula() -> None:
    ...
    rows = {prev: (100.0,), day: (110.0,)}
    if not near(c.signal_at(rows, day), 110.0 / 100.0 - 1.0):
        raise SystemExit("the return was not the signal")
    if near(c.signal_at(rows, day), 110.0):
        raise SystemExit("the close replaced the return")
    if near(c.signal_at(rows, day), 100.0):
        raise SystemExit("yesterday's close replaced the return")
    up = {prev: (100.0,), day: (200.0,)}
    if not near(c.signal_at(up, day), 1.0):
        raise SystemExit("a double was not one")
    flat = {prev: (100.0,), day: (100.0,)}
    if c.signal_prints(flat) != [(day, 0.0)]:
        raise SystemExit("a zero return left the history")
    if c.signal_at({day: (110.0,)}, day) is not None:
        raise SystemExit("a day with no yesterday was a print")
    if c.signal_at({prev: (0.0,), day: (1.0,)}, day) is not None:
        raise SystemExit("a zero close was a print")
    later = c.fp5.SCREEN_END_MS
    if c.signal_at({later - c.fp5.DAY_MS: (100.0,), later: (110.0,)}, later) is not None:
        raise SystemExit("a 2024 day was a print")

"""Pins for the fp143 rule. No file and no return from 2023 is read."""
...
def test_tail_and_fill() -> None:
    ...
    for i in range(121):
        ...
        rows[day] = (200.0,) if i == 120 else (100.0,)
