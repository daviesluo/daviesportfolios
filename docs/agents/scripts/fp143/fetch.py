"""Pull BTCUSDT daily closes for the fp143 screen. The fill file stores the open only. No second coin is requested. No hourly price bar is requested.

    python3 docs/agents/scripts/fp143/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp143-research"}
...
def pull_bars() -> tuple[dict[int, tuple], dict[int, tuple]]:
    end_ms = c.fp5.SCREEN_END_MS
    cursor = LOOKBACK_MS
    opens: dict[int, tuple] = {}
    signal: dict[int, tuple] = {}
    while cursor <= end_ms:
        url = (
            f"{MARKET}/api/v3/klines?symbol=BTCUSDT&interval=1d"
            f"&startTime={cursor}&endTime={end_ms}&limit=1000"
        ...
        for k in batch:
            if len(k) < 5:
                raise SystemExit("a daily kline is missing the close")
            open_ms = int(k[0])
            if open_ms < LOOKBACK_MS:
                continue
            if open_ms > end_ms:
    ...
    assert_span(signal, end_ms - c.fp5.DAY_MS, "signal")
    same = sum(1 for day, row in signal.items() if row[0] == opens[day][0])
    if same == len(signal):
        raise SystemExit("the close file is the open")
    if any(len(row) != 1 for row in signal.values()):
        raise SystemExit("a close row stored another field")
    save("spot1d/BTCUSDT.json", opens)
    save("close/btc.json", signal)
    return opens, signal
