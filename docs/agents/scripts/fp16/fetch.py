"""Pull BTCUSDT 1h and 1d bars for the fp16 screen. No later year is requested.

Hourly bars stop before the 2024-01-01 open. The daily file keeps that open
because it can be the exit of the last 2023 entry. A later open is a bug.

    python3 docs/agents/scripts/fp16/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp16-research"}
...
def klines(interval: str, step: int, end_ms: int) -> dict[int, tuple]:
    cursor = LOOKBACK_MS
    rows: dict[int, tuple] = {}
    while cursor <= end_ms:
        url = (
...
def main() -> None:
    ...
    # The last hourly open that can still belong to 2023-12-31 is 23:00.
    hour_end = c.fp5.SCREEN_END_MS - 1
    hourly = klines("1h", c.HOUR_MS, hour_end)
    if any(t >= c.fp5.SCREEN_END_MS for t in hourly):
        raise SystemExit("an hourly bar opened on or after 2024-01-01")
    daily = klines("1d", c.fp5.DAY_MS, c.fp5.SCREEN_END_MS)
    ...
    print(
        f"fp16 BTCUSDT 1h {len(hourly)} first {min(hourly)} last {max(hourly)}; "
