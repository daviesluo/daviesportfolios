"""Score the fp91 screen. 2023 only.

    python3 docs/agents/scripts/fp91/measure.py
"""
...
DATA = Path(os.environ.get("FP91_DATA", "/tmp/fp91/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp91-protocol.md"
...
def main() -> None:
    ...
    trades = c.hlquote_trades(daily, hours)
    ...
    row = c.fp5.summarise(
        "HL-QUOTE", trades, pool, c.MIN_N,
        "Quote in the hour of the high over quote in the hour of the low, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp91_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp91_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp91/screen_2023.json"
