"""Score the fp290 screen. 2023 entries only.

    python3 docs/agents/scripts/fp290/measure.py
"""
...
DATA = Path(os.environ.get("FP290_DATA", "/tmp/fp290/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp290-protocol.md"
NOTE = 'Fair value of the USDT book is the spot close. The finished USDT close was strictly above that fair value. The next USDT open is sold and bought back twelve days later. The other trade sells the USDT book for twelve days when it is not rich to spot. Same prices, same side, the edge turned off. One leg. Funding cash is not added. The null is not an unconditional hold and it is not the opposite side.'
...
def run() -> None:
    ...
    if c.IDEA != 'RICHU' or c.MIN_N != 30 or c.HOLD_DAYS != 12 or c.NULL_KIND != 'other_regime':
    ...
    payload = {
        ...
        "code_sha256": {
            "fp290_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp290_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp290/screen_2023.json"
