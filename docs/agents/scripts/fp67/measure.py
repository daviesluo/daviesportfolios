"""Score the fp67 screen. 2023 only.

    python3 docs/agents/scripts/fp67/measure.py
"""
...
DATA = Path(os.environ.get("FP67_DATA", "/tmp/fp67/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp67-protocol.md"
...
def main() -> None:
    ...
    row = c.fp5.summarise(
        "CHOP", c.chop_trades(daily, hours), pool, c.MIN_N,
        "Hourly range sum over the day's range, above its own trailing 90th. Next day, long BTC.",
    )
    payload = {
        "protocol_sha256": sha256(PROTOCOL),
        "code_sha256": {
            "fp67_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp67_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp67/screen_2023.json"
