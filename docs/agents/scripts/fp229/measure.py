"""Score the fp229 screen. 2023 entries only.

    python3 docs/agents/scripts/fp229/measure.py
"""
...
DATA = Path(os.environ.get("FP229_DATA", "/tmp/fp229/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp229-protocol.md"
NOTE = "Spot's finished ten-day return was strictly above the USDT return. The next spot open is sold and bought back fourteen days later. One spot leg. The USDT book is a signal. Funding cash is not added. The null is that fourteen-day spot short on every day."
...
def check_common(trade: dict) -> None:
    ...
    if trade["exit_ms"] - trade["entry_ms"] != 14 * c.fp5.DAY_MS:
...
def assert_subset(trades, pool, spans, pool_spans) -> None:
    ...
    for entry, exit_ms in pool_spans:
        if exit_ms - entry != 14 * c.fp5.DAY_MS:
...
def run() -> None:
    ...
    book = load_rows(DATA / "spot1d/BTCUSDT.json", 2, end)
    um = load_rows(DATA / "um1d/BTCUSDT.json", 1, last)
    trades = c.signal_trades(book, um)
    spans = c.signal_spans(book, um)
    if [t["entry_ms"] for t in trades] != [entry for entry, _exit in spans]:
        raise SystemExit("the fills and the spans diverged")
    for trade in trades:
        check_common(trade)
        spot_move = c.finished(book, trade["entry_ms"], 1)
        um_move = c.finished(um, trade["entry_ms"], 0)
        if spot_move is None or um_move is None or spot_move <= um_move:
            raise SystemExit("spot did not lead on a finished close")
        if trade["entry_ms"] in um and trade["entry_ms"] > last:
            raise SystemExit("a 2024 USDT close was a signal")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp229_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp229_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp229/screen_2023.json"
