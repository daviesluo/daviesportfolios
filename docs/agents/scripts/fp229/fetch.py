"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp229/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp229-research"}
...
def pull(kind: str, symbol: str, indexes: tuple[int, ...], missing_ok: bool, end_ms: int, folder: str) -> dict[int, tuple]:
    rows: dict[int, tuple] = {}
    for ym in months():
        if kind == "spot":
...
def main() -> None:
    ...
    spot = pull("spot", "BTCUSDT", (1, 4), False, c.fp5.SCREEN_END_MS, "spot1d")
    um = pull("um", "BTCUSDT", (4,), False, last, "um1d")
    print(f"fp229 spot {len(spot)} um {len(um)}")
