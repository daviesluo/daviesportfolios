"""The search fp144. The fill and the null are fp5's.

Nothing here reads the network or a file. A fill bar's first field is the
BTCUSDT open. The signal is the 16:00 funding rate minus the 00:00 rate. The level is not an input. No hourly price bar is read.
The hold is one day.
"""
...
def signal_at(rows: dict[int, tuple], day: int, end_ms: int = fp5.SCREEN_END_MS) -> float | None:
    """16:00 rate minus 00:00 rate. A rate may be negative or zero."""
    if day >= end_ms:
        return None
    row = rows.get(day)
