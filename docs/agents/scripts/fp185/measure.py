"""Score the fp185 screen. 2023 entries only.

    python3 docs/agents/scripts/fp185/measure.py
"""
...
DATA = Path(os.environ.get("FP185_DATA", "/tmp/fp185/data"))
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp185-protocol.md"
NOTE = 'Bid notional rose and ask notional fell through the day. Next day, long BTC from the open to the close.'
...
def main() -> None:
    ...
    for trade in trades:
        ...
        if not (bid1 > bid0 and ask1 < ask0):
            raise SystemExit("a trade was not bid notional up and ask notional down")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp185_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp185_measure": sha256(Path(__file__)),
    ...
    out = ROOT / "docs/agents/backtests/fp185/screen_2023.json"
