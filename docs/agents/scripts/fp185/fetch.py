"""Pull the inputs this rule stores. No later year is requested as a signal.
    ...
    python3 docs/agents/scripts/fp185/fetch.py
"""
...
UA = {"User-Agent": "daviesportfolios-fp185-research"}
...
def main() -> None:
    ...
    spot = pull_spot((1, 4))
    book = pull_book()
    print("fp185 " + f"spot {len(spot)} last {max(spot)} book {len(book)} last {max(book)}")
