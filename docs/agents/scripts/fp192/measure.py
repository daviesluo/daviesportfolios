"""Score the fp192 screen. 2023 entries only.

    python3 docs/agents/scripts/fp192/measure.py
"""
...
PROTOCOL = ROOT / "docs/agents/reviews/2026-09-25-fp192-protocol.md"
NOTE = "The USDT perpetual's average trade finished strictly under the mark close. Next day, long BTC from the open to the close."
...
def main() -> None:
    ...
    mark = load_rows(DATA / "mark1d/BTCUSDT.json", 1, c.SIGNAL_LAST_MS, required=False)
    trades = c.signal_trades(bars, (um, mark))
    ...
    for trade in trades:
        ...
        base, quote = um[day]
        if not (base > 0 and quote / base < mark[day][0]):
            raise SystemExit("a trade was not an average under the mark")
    ...
    payload = {
        ...
        "code_sha256": {
            "fp192_common": sha256(Path(__file__).resolve().parent / "common.py"),
            "fp192_measure": sha256(Path(__file__)),
            "fp5_common": sha256(Path(__file__).resolve().parents[1] / "fp5" / "common.py"),
        },
        "input_sha256": {"spot": sha256(DATA / "spot1d/BTCUSDT.json"), "um": sha256(DATA / "um1d/BTCUSDT.json"), "mark": sha256(DATA / "mark1d/BTCUSDT.json")},
        "ideas": [row],
        "passed": [row["idea"]] if row["passes_screen"] else [],
    }
    text = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    out = ROOT / "docs/agents/backtests/fp192/screen_2023.json"
