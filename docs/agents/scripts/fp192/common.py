"""The search fp192. The perpetual's average trade finished under the mark.
...
def signal_days(bars: dict[int, tuple], books: tuple[dict[int, tuple], dict[int, tuple]], end_ms: int = fp5.SCREEN_END_MS) -> list[int]:
    """Days whose perpetual average trade finished under the mark close."""
    um, mark = books
    out = []
    for day in sorted(set(um) & set(mark)):
        ...
        if not (fp5.SCREEN_START_MS <= entry < end_ms):
            continue
        if len(um[day]) != 2 or len(mark[day]) != 1:
            raise ValueError("the perpetual bar is volume and quote; the mark is the close")
        base, quote = um[day]
        if base > 0 and quote / base < mark[day][0]:
            out.append(day)
    return out
