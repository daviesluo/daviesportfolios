...
**fp313–fp320 are frozen. No return of these rules has been computed.** Fair value is the price index inside Binance's funding formula when the last settled funding rate is negative, or that index grossed up by already-published SOFR when the settled rate is strictly below the eight-hour SOFR carry. The series is the Binance funding history on www.binance.com and the New York Fed SOFR search. Publication of SOFR is 13:00 UTC on the next effective date, not the effective date. Another venue's close is not an input. The fill is that contract's own open. Funding cash is not added. The other trade is the same book and the same side with the edge off. The hold is two days. Counts with no profit computed: UZERO 36/328, CZERO 37/321, EZERO 40/318, BZERO 102/262, USOFR 126/238, CSOFR 76/282, ESOFR 120/244, MSOFR 84/274. ETHUSDT funding below zero had 27 spans and was not shipped. BNBUSDT funding below SOFR was the longer set, 294 against 70, and was not shipped. The short side was not taken. Option end-of-hour files were not used. The coin-margined gap 2023-08-28 through 2023-08-31 is not filled in. No testing row is added. The next eight are not frozen. The count stays 30. LS-FADE's 60-trade reproduction, the control, doubled costs, and the rule that no month is above 40% of the profit are not relaxed. Reference §3.360. No pull request. Main was not pushed.
