...
**fp54–fp61 are frozen and not yet scored.** Eight screens, each long BTC the next day, fill fp5's `net_return`. Protocols `docs/agents/reviews/2026-09-25-fp54-protocol.md` through `fp61`, written before any return. VOL-CLOCK is the quote-weighted hour of the day. BOUNCE is the close divided by the day's low, minus 1. The high does not enter. CLOSE-HIGHS counts hours whose close breaks the day's running close. BASE-CHG is the change in base volume, and a day that did not grow stays in the history. LAST-EXT is the later of the hour of the high and the hour of the low, earliest tie. STEP-BASE is the share of base volume, in hours 1 through 23, that closed above the prior hour. Hour 0's base volume does not enter. OPEN-YLOW is today's open divided by yesterday's low, minus 1. LAST-HOUR is the 23:00 hour's close divided by that hour's open, minus 1. Pins passed on synthetic bars. A one-element kline field was written as `(7)` and `(4)`, which is not a tuple. That is fixed before any pull. The statistic is unchanged. Score only after that fix is on the remote: fetch, measure twice, compare the JSON files. A 2023 pass still needs a pre-registration before any 2024 price. A rule rejected without a scored run does not count. Do not lower the count of 30. Do not flip a sign. Do not shrink the window. Do not score an 80th or a 95th on a failed screen. fp46–fp53 stay dead (§3.75–§3.82), including AVG-CHG and its positive 95th. Do not score a close under the day's average, fewer new highs, a day the average size did not rise, fewer rising hourly closes, an earlier high, fewer hours above the open, a day quote volume did not rise, or a smaller clock gap. UP-STEP beat its null on 25 trades. The count stays 30. fp30–fp45 stay dead, including STREAK and OPEN-DRIVE and their positive neighbours. PEAK stays dead (§3.58). IMPACT stays dead, including the later years (§3.57). Do not promote its 80th or its 95th. BAL-CHG stays dead (§3.56). BODY stays dead, including the later years (§3.55). Do not promote its 80th. Do not reopen EXCH-BAL, FEE-HIGH, the net exchange flow, UP-SHARE, VOL-HHI, VWAP-PREM, GAP-UP, CLOSE-LOC, PAIR-CORR, AVG-SIZE, AC-HOUR, RV-RATIO, the quiet-day range, the taker share, dispersion, or skew. fp5 through fp53 stay closed, including LS-FADE. No testing row. No pull request. No push to main.
