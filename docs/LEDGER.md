...
**Fair-value files were downloaded and counted before any cause was written. None was frozen and none was scored.** Davies, 2026-09-25: do not retest leveraged-token net asset value, the staking exchange rate, PEOPLE, ANT, or a coin-swap ratio with no overlapping open. A rule starts by downloading the fair-value file and counting at least 30 days of overlap with that contract's daily bars. The cause is written after that count. The rule is frozen after the cause. Overlap under 30 days is not one of the eight. The eight causes have to differ, and they cannot be redemption, par, realised price, another venue's close, a funding sign, or purchasing-power parity. The fill is that contract's own open. The discount known before entry has to exceed the 20 bp round trip. The threshold stays at least 25 bp. A negative mean, or a gap inside 20 bps, is not taken out of sample. Cambridge's open download `https://ccaf.io/cbeci/api/v1.4.0/download/data` is power in GW and consumption in TWh, 365 dates in 2023, not USD per BTC. The cost-of-minting chart URL returns HTTP 400, invalid recaptcha. A third-party file of "Estimated cost of minting USD" overlaps BTCUSDT on 365 days. Its central estimate is 25 bp cheap on 0 of 364 days. The richest discount is −3095.0454 bps on 2023-01-01. The upper bound was not substituted. FRED `APU000072610` is 12 monthly USD-per-kWh prints. Digiconomist publishes one current annual electricity total, not a daily coin price. No return was computed. Nothing is taken out of sample. No testing row. Reference §3.388. No pull request. Main was not pushed.
