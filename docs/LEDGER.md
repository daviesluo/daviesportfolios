...
000000000000000. **fp5 reviewed (2026-09-26): nothing to put money on; what is worth running, in order**
   ...
   UK order book for a queue model (RECORDING since 2026-09-26, `0057`; pre-register the queue model before reading
   it; four weeks run to ~10-24). 6. PR5's live path after its dry-run (Davies' go). Each needs its own
...
00000000000000. **HANDOVER, 2026-09-24 ~20:30 UTC: ONE Cursor session on this repository, rotating between Opus 5.5 and
   ...
   - **G2 — RW's verdict, on or after 2026-10-09 00:05 UTC** (the procedure is in item 000000000000, step 3). Then the
     ...
     **RW-E is judged with it** (`docs/agents/reviews/2026-09-26-polymarket-rw-end-prereg.md`, frozen 2026-09-26):
     RW without the markets that end on the day they are quoted, on 09-27 → 10-08, by an exact replay of the stored
     `pm_rw_*` tables through `agents/pmrw.ts`'s own `stepRw`. Build the replay before then (export the tables day by
     day; the check that it reproduces RW's own `pm_rw_days` comes first). Do not read the twelve days' fills market by
     market before that: a narrower variant (stop quoting once a temperature bucket is decided) still needs its own
     pre-registration, frozen before anyone has.
