...
2. **RW (Polymarket reward quotes, paper) and RW-E: the verdict on or after 2026-10-09 00:05 UTC.** RW runs by itself
   ...
   and `e`, into `pm_rw_e_days`; RW's page shows both.
...
5. **Davies' to decide or to do; nothing waits on them:**
   - Jev on the two paper rows, `trend-1h` and `momentum-1d`: keep the v2 gate at 0.45, which they ask now, or shadow them
     (`params.jevGate: false`). Neither row's own wording cleared the bar (reference §4.21, §4.28).
