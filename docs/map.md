...
| `docs/agents/backtests/polymarket/` | The Polymarket searches: fp4's scripts, inputs and results, listed in `MANIFEST.json` (`scripts/rw_golden.py` cuts RW's replay fixture); fp5's VOL test (`scripts/vol_inputs.py`, `scripts/vol_test.py`, `inputs/vol_inputs.json.gz`, `results/vol_run.json`) and copy-wallet test (`scripts/copy_inputs.py`, `scripts/copy_test.py`, `inputs/copy_inputs.json.gz`, `results/copy_run.json`), both of which fail their bars; fp5's round-strike test (`scripts/round_inputs.py`, `scripts/round_test.py`, `inputs/round_inputs.json.gz`, `results/round_run.json`), which fails its bar; fp5's fade test (`scripts/fade_inputs.py`, `scripts/fade_test.py`, `inputs/fade_inputs.json.gz`, `results/fade_run.json`), which fails its bar; fp5's post-count test (`scripts/post_inputs.py`, `scripts/post_test.py`, `inputs/post_inputs.json.gz`, `results/post_run.json`), which fails its bar; and fp5's hit-rate, Poisson and pace tests (`scripts/hits_inputs.py`, `scripts/hits_test.py`, `inputs/hits_inputs.json.gz`, `results/hits_run.json`, `scripts/poisson_inputs.py`, `scripts/poisson_test.py`, `inputs/poisson_inputs.json.gz`, `results/poisson_run.json`, `scripts/pace_inputs.py`, `scripts/pace_test.py`, `inputs/pace_inputs.json.gz`, `results/pace_run.json`), all of which fail their bars. fp5's next eight, pre-registered and not yet run: the median bracket (`scripts/med_test.py`, `reviews/2026-09-25-polymarket-fp5-prereg-med.md`), the variance bracket (`scripts/var_test.py`, `reviews/2026-09-25-polymarket-fp5-prereg-var.md`), the neighbor of the dearest bracket (`scripts/near_test.py`, `reviews/2026-09-25-polymarket-fp5-prereg-near.md`), NO on the dearest bracket (`scripts/rich_test.py`, `reviews/2026-09-25-polymarket-fp5-prereg-rich.md`), yesterday's temperature bucket (`scripts/yday_test.py`, `reviews/2026-09-25-polymarket-fp5-prereg-yday.md`), the month's median temperature bucket (`scripts/clim_test.py`, `reviews/2026-09-25-polymarket-fp5-prereg-clim.md`), the extrapolated temperature bucket (`scripts/jump_test.py`, `reviews/2026-09-25-polymarket-fp5-prereg-jump.md`) and the bucket above that median (`scripts/hot_test.py`, `reviews/2026-09-25-polymarket-fp5-prereg-hot.md`). The shared books are `scripts/ladder_hold.py` and `scripts/city_hold.py`; ladder prints are fetched by `scripts/ladder_pick.py`. |
