...
| `docs/agents/backtests/polymarket/` | The Polymarket searches: fp4's scripts, inputs and results, listed in `MANIFEST.json` (`scripts/rw_golden.py` cuts RW's replay fixture); fp5's VOL test (`scripts/vol_inputs.py`, `scripts/vol_test.py`, `inputs/vol_inputs.json.gz`, `results/vol_run.json`) and copy-wallet test (`scripts/copy_inputs.py`, `scripts/copy_test.py`, `inputs/copy_inputs.json.gz`, `results/copy_run.json`), both of which fail their bars; fp5's round-strike test (`scripts/round_inputs.py`, `scripts/round_test.py`, `inputs/round_inputs.json.gz`, `results/round_run.json`), which fails its bar; fp5's fade test (`scripts/fade_inputs.py`, `scripts/fade_test.py`, `inputs/fade_inputs.json.gz`, `results/fade_run.json`), which fails its bar; fp5's post-count test (`scripts/post_inputs.py`, `scripts/post_test.py`, `inputs/post_inputs.json.gz`, `results/post_run.json`), which fails its bar; and fp5's hit-rate, Poisson and pace tests (`scripts/hits_inputs.py`, `scripts/hits_test.py`, `inputs/hits_inputs.json.gz`, `results/hits_run.json`, `scripts/poisson_inputs.py`, `scripts/poisson_test.py`, `inputs/poisson_inputs.json.gz`, `results/poisson_run.json`, `scripts/pace_inputs.py`, `scripts/pace_test.py`, `inputs/pace_inputs.json.gz`, `results/pace_run.json`), all of which fail their bars. fp5's next eight, each run on the full sample and each failing its bar: the median bracket (`scripts/med_test.py`, `inputs/med_inputs.json.gz`, `results/med_run.json`), the variance bracket (`scripts/var_test.py`, `inputs/var_inputs.json.gz`, `results/var_run.json`), the neighbor of the dearest bracket (`scripts/near_test.py`, `inputs/near_inputs.json.gz`, `results/near_run.json`), NO on the dearest bracket (`scripts/rich_test.py`, `inputs/rich_inputs.json.gz`, `results/rich_run.json`), yesterday's temperature bucket (`scripts/yday_test.py`, `results/yday_run.json`), the month's median temperature bucket (`scripts/clim_test.py`, `results/clim_run.json`), the extrapolated temperature bucket (`scripts/jump_test.py`, `results/jump_run.json`) and the bucket above that median (`scripts/hot_test.py`, `results/hot_run.json`). The temperature four read `inputs/wx_inputs.json.gz`. The shared books are `scripts/ladder_hold.py` and `scripts/city_hold.py`; ladder prints are fetched by `scripts/ladder_pick.py`. The next eight were run on the full weather sample and fail: other cities (`scripts/peer_test.py`, `results/peer_run.json`), the daily range (`scripts/diurnal_test.py`, `results/diurnal_run.json`), the change histogram (`scripts/shift_test.py`, `results/shift_run.json`), the off-diagonal transition (`scripts/next_test.py`, `results/next_run.json`), one bucket toward the month (`scripts/back_test.py`, `results/back_run.json`), the three-day mean (`scripts/mean3_test.py`, `results/mean3_run.json`), the majority step (`scripts/drift_test.py`, `results/drift_run.json`) and the two estimates agreeing (`scripts/agree_test.py`, `results/agree_run.json`). The shared checks are `scripts/city_more.py`. fp5's Premier League full-time rules were run on the full sample and fail, in `scripts/epl_score.py`, `scripts/epl_hold.py`, `scripts/epl_inputs.py`, `scripts/epl_pick.py`, `scripts/elo_test.py`, `scripts/form_test.py`, `scripts/table_test.py`, `scripts/rest_test.py`, `scripts/venue_test.py`, `scripts/streak_test.py`, `scripts/draw_test.py`, `scripts/h2h_test.py`, `scripts/congest_test.py` and `scripts/margin_test.py`, with `results/elo_run.json`, `results/form5_run.json`, `results/table_run.json`, `results/rest_run.json`, `results/venue_run.json`, `results/streak3_run.json`, `results/drawbase_run.json`, `results/h2h_run.json` and `results/margin_run.json`. fp5's referee, shots-on-target, fouls, yellow-card, full-time -1.5, both-teams-to-score, over-3.5 and corner rules were run on the full sample and fail, in `scripts/stat_score.py`, `scripts/stat_inputs.py`, `scripts/stat_pick.py`, `scripts/ref_test.py`, `scripts/sot_test.py`, `scripts/foul_test.py`, `scripts/card_test.py`, `scripts/spread15_test.py`, `scripts/btts_test.py`, `scripts/over35_test.py` and `scripts/corner_test.py`, with `results/ref_run.json`, `results/sot_run.json`, `results/foul_run.json`, `results/card_run.json`, `results/spread15_run.json`, `results/btts_run.json`, `results/over35_run.json` and `results/corners_run.json`. fp5's halftime-lead, halftime-draw, corner-gap, half-time-lead-to-win, clean-sheet, second-half-goals, team-total-1.5 and first-half-over-0.5 rules were run on the full sample and fail, in `scripts/half_score.py`, `scripts/half_inputs.py`, `scripts/htlead_test.py`, `scripts/htdraw_test.py`, `scripts/corn6_test.py`, `scripts/htwin_test.py`, `scripts/clean_test.py`, `scripts/shwin_test.py`, `scripts/team15_test.py` and `scripts/fh05_test.py`, with `results/htlead_run.json`, `results/htdraw_run.json`, `results/corn6_run.json`, `results/htwin_run.json`, `results/clean_run.json`, `results/shwin_run.json`, `results/team15_run.json` and `results/fh05_run.json`. fp5's Major League Baseball rules are frozen and not yet scored, in `scripts/mlb_score.py`, `scripts/mlb_inputs.py`, `scripts/mlb_pick.py`, `scripts/arm_test.py`, `scripts/walk_test.py`, `scripts/k9_test.py`, `scripts/tired_test.py`, `scripts/yrfi_test.py`, `scripts/xin_test.py`, `scripts/hr05_test.py` and `scripts/k35_test.py`. |
