...
| `docs/agents/scripts/fp321/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | Fair value is the Coin Metrics realized ETH price in USD. The previous ETHUSD_PERP close is at least one basis point under that price. Buy the coin-margined book for two days. The other trade buys it for two days when that discount is absent. The open is the fill only. The entry day's close is not an input. A book ticker is not read. The 2023-08-28 through 2023-08-31 gap is not filled in. The protocol is `docs/agents/reviews/2026-09-25-fp321-protocol.md`. |
| `docs/agents/scripts/fp322/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | Fair value is Binance's published WBETH exchange rate, ETH per WBETH. The previous WBETHETH close is at least twenty-five basis points under that rate. Buy the spot book for two days. The other trade buys it for two days when that discount is absent. The open is the fill only. The entry day's close is not an input. A book ticker is not read. April 2023 is not filled in. The protocol is `docs/agents/reviews/2026-09-25-fp322-protocol.md`. |
| `docs/agents/scripts/fp323/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | Fair value is the standing par of one BTC per WBTC. The previous WBTCBTC close is at least one basis point under that par. Buy the spot book for two days. The other trade buys it for two days when that discount is absent. The open is the fill only. The entry day's close is not an input. A book ticker is not read. The protocol is `docs/agents/reviews/2026-09-25-fp323-protocol.md`. |
