...
| `docs/agents/backtests/polymarket/` | The Polymarket searches: fp4's scripts, inputs and results, listed in `MANIFEST.json` (`scripts/rw_golden.py` cuts RW's replay fixture), and fp5's pre-registered VOL test (`scripts/vol_inputs.py`, `scripts/vol_test.py`). |
