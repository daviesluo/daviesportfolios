...
| `docs/agents/scripts/fp332/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | The buyer of the front BTCUSDT quarterly pays more than the Binance spot open for a coin the contract meets at delivery. The result is the change in that basis between the entry opens and the expiry-day opens, minus forty basis points. The spot move is not the result. The previous close basis is at least 100 basis points. The same package is the other trade when it is not. The protocol is `docs/agents/reviews/2026-09-25-fp332-protocol.md`. The screen is `docs/agents/backtests/fp332/screen_2023.json`. The pre-registration is `docs/agents/reviews/2026-09-25-fp332-prereg-spqtr.md`. |
...
| `docs/agents/scripts/fp334/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | A negative TRBUSDT funding rate is cash the short pays the long, on a long-perpetual short-spot package. The result is that cash minus forty basis points. The coin's move is not added. The signal is the completed-interval premium, at least 50 basis points cheap, and the print is not the signal. The same package is the other trade when that premium is not that cheap. The protocol is `docs/agents/reviews/2026-09-25-fp334-protocol.md`. The screen is `docs/agents/backtests/fp334/screen_2023.json`. The pre-registration is `docs/agents/reviews/2026-09-25-fp334-prereg-fdcsh.md`. |
