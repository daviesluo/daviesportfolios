...
| `docs/agents/scripts/fp161/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | The three BTC funding rates rise through the day: long BTC for one day. The funding cash is not added. |
| `docs/agents/scripts/fp162/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | Dollar open interest rose and the candle closed under its open: long BTC for one day. |
| `docs/agents/scripts/fp163/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | The taker ratio rose through the day: buy the next open, sell at 2% above it when the high reaches it, otherwise sell the close. |
| `docs/agents/scripts/fp164/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | A break of yesterday's high that is also the widest range of four days: buy yesterday's high, or the open if it is already through, and sell the close. |
| `docs/agents/scripts/fp165/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | A break that closed back under yesterday's high: short BTC the next day from the open to the next open. The funding cash is not added. |
| `docs/agents/scripts/fp166/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | Base volume above the volume ten days earlier: long BTC from the next open to the open three days later. |
| `docs/agents/scripts/fp167/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | The widest range of four days: long BTC the next day from the open to the close. |
| `docs/agents/scripts/fp168/common.py`, `pin_test.py`, `fetch.py`, `measure.py` | Dollar open interest rose and the taker ratio rose: long BTC from the next open to the open two days later. |
