...
- **Agents (crypto auto-trading, paper first)** — an "Agents" page behind
  the ☰ menu: four rulebooks (4h trend following, its 1h variant, 30-day
  momentum, relative-strength rotation over BTC/ETH/SOL/XRP) across two
  venues — Revolut X executes at 0 % maker and reads Kraken's candles for
  its signals; Kraken runs the slow rules and paper twins that pay its real
  0.40 % — deciding on closed bars every five minutes with TypeSafe's
  **Jev 1.13** decision model as a veto/advice node inside a deterministic
  ...
  between the venues is recorded every turn and shown on the page: there is
  no arbitrage to run between them, and the record keeps saying so.
  ...
  strategy into its decisions (the words the model saw, what it answered,
  what the rule said) and orders. Every strategy starts in paper and a
...
|---|---|
| `agents` | The crypto agents' server half. `POST ?action=tick` (pg_cron every 5 min, migration `0037`; cron bearer or admin token) runs one turn of `tick.ts`: it reads both venues' public quotes for every symbol in play and records the basis between them (`agent_basis`); for every strategy that is not paused it refreshes the candles from that strategy's `signal_venue` (the decision bar's series and 130 daily candles) and the execution venue's latest closed 5-minute candle, touch and pair limits; settles open orders (a `pending` row — written before a live order was sent — is reconciled against the venue's active orders by client id; a paper order fills when the venue's last 5-minute candle traded through its price and pays the venue's maker fee; a live order is re-read from the venue; either is cancelled after three unfilled turns, a live one only after the venue confirms and any partial fill is read back); then, on a NEWLY closed decision bar and with no order in flight, builds the categorical state (`_shared/agents_strategy.ts`; the rotation rule ranks the whole cross-section first), asks Jev (`_shared/jev.ts`: OpenRouter first, TypeSafe direct as fallback, `provider: none` when both fail — which means hold), applies the rulebook, applies the risk gate (per-order cap, exposure and order count per venue and mode, a daily loss limit that blocks new risk but never an exit, global pause — every number from `agent_risk`), CLAIMS the bar by inserting the decision (a unique index on strategy, symbol and bar makes a second claim fail, so overlapping ticks cannot both order), and places the order it allows as a resting post-only limit at the touch. A live order additionally needs `agent_strategies.mode = 'live'`, `agent_risk.live_confirmed_at` set, and credentials for that venue. `GET ?action=dashboard` (admin or ro) computes what the page shows — positions and P&L from fills via `positionFromFills`, marked at each venue's mid, the book split by venue, the 24 h basis per symbol, each strategy's next decision time, plus venue health, caps, Jev spend and recent decisions / orders; `GET ?action=log&strategy=` pages more. `GET ?action=probe` (cron or admin) is the read-only credential self-check: signed Revolut X balances / pairs / candles-with-query, Kraken balances / fee tier / open orders / `AddOrder validate=true` (the venue checks and places nothing), Jev on both transports. Venues behind one interface (`_shared/venue.ts`): `_shared/revx.ts` (Ed25519-signed; keyless public market data) and `_shared/kraken.ts` (HMAC-SHA512-signed, test vector pinned; OHLC capped at 720 candles). **Auth: cron bearer or `x-app-token`**, so it deploys `--no-verify-jwt` (in `PUBLIC_FNS`). Pinned by `strategy.test.ts`, `momentum.test.ts`, `jev.test.ts`, `revx.test.ts`, `kraken.test.ts`, `tick.test.ts`. `backtest.ts` is the walk-forward backtester run by hand (same functions as the loop) that writes `docs/agents/backtests/`. |
