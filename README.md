...
| `eslint.config.js` | Flat ESLint config (ESLint 9). Deliberately a **bug** gate, not a formatting one: errors on `react-hooks/rules-of-hooks` (the hook-ordering class that shipped a React-#310 crash once) + `js.recommended` (with `allowEmptyCatch` for the codebase's best-effort-swallow pattern); `react-hooks/exhaustive-deps` is a non-blocking warning since a few effects intentionally narrow their dep set (documented inline). Stylistic / unused-locals are left to `tsc` + `knip`. `npm run lint`. |
