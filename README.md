...
- The layout is a recursive binary-split treemap. Area is the contract
  — always proportional to market value — but *where* the sorted run
  gets cut is a free choice, and taking the first cut past the halfway
  mark meant a run of near-equal holdings always split down the exact
  middle. The halving then repeated, so the biggest positions came out
  as a rigid grid of near-identical rectangles while only the uneven
  tail below them looked like a map. The cut point is now chosen from
  every index inside a balanced band, using a per-branch golden-ratio
  target pushed away from the middle, with tile size as a hard veto
  (nothing may be cut down below the 24 × 22 px the ticker and % need)
  and elongation as a price the chooser weighs against the variety it
  buys. Deterministic — the same book always draws the same map.
...
| `heatmap.jsx` | One tile per holding, sized by market value, colored by day-change. Tile labels go through `displayTicker` (now in `formatters.js`, re-exported here) — strips the Yahoo exchange suffix (`XFAB.PA` → `XFAB`) and maps the display aliases (every `2DG` variant — `.F` / `.SG` / `.DE` — → `SIVE`, Sivers Semiconductors' home-market symbol). Shared with the tactics-board chips and Top Movers rows so all three read the same; the **chart modal is deliberately the one place that still shows the true Yahoo symbol**, so a tap resolves the right instrument and the user can see which listing they hold. Display-only — the real ticker stays on the data, keys and click handlers. Under the extended-hours toggle a tile with **no** extended figure yet (a US name in the overnight window before a broker quote lands) shows an em dash rather than `+0.00%` — `computeMetrics` flags it `dayPctUnknown`, because painting "nobody knows" as "unchanged" states a fact that isn't one, and a board of flat zeros reads as a broken toggle. **`treemap`** (exported for its pin tests) is the recursive binary split that places the tiles. Each call hands the two halves exactly the share of the box their value deserves — area is proportional to market value and nothing below bends that — but the cut point itself is free, and taking the first index past the halfway mark meant a run of near-equal holdings split down the exact middle every time. Repeated down the recursion that drew the top of the board as a grid: on a 338×612 canvas the eight largest all landed within 15 % of 110×170 px, three to a row, while only the uneven tail below them varied. The cut is now chosen from every index whose group-1 fraction lands in a balanced band (0.22–0.78), scored against a per-branch target — a golden-ratio low-discrepancy sequence over the recursion tree (left `2s+1`, right `2s+2`, so siblings differ), pushed away from the middle because a near-half cut is exactly the cut that draws the grid. Two guards, deliberately of different kinds: tile SIZE is a hard veto (27×25 px, the render gate plus the component's 3 px gutter — below it a holding shows neither ticker nor %), while ELONGATION is only a price added to the score. Vetoing shape is what lost the CN fund in the browser sweep — the cut that would have given it a readable 141×34 strip was refused for being 4.15:1 and the fallback stranded it in a 275×18 band with no label at all — so a guard with no comparison is how a layout gets worse. A child holding exactly two nodes is looked at one step ahead, since a pair has no freedom left and a dominant holding paired with a small one always splinters the small one. Measured over seven differently-shaped books at five canvas sizes: the longest run of look-alike tiles among the top eight holdings 5.31 → 3.97, worst aspect ratio anywhere 4.39:1 → 3.96:1, tiles too small to label 4/605 → 1/605. Fully deterministic — the same book always draws the same map. |
