...
- **Yahoo-Finance-equivalent YTD chart math** — per-lot purchase
  ...
  the basis for in-year lots, with cash counted in both value and
  basis so the PORTFOLIO line is the whole-account return (and the 1D
  live point matches the scoreboard's DAY CHANGE % to the basis point
  rather than overstating it by ignoring cash); pinned by a vitest
  suite so the formula can't quietly regress.
...
- **P/E YTD view** — sixth range button on the ticker chart modal
  ...
  parallel P/S YTD button below instead.
- **P/S YTD view** — same range-button slot for **loss-makers**
  (pre-profit companies where Yahoo / Finnhub publish no trailing
  P/E because EPS ≤ 0). Mutually exclusive with the P/E button:
  profitable tickers see P/E YTD, unprofitable tickers see P/S
...
- **Range buttons**: 1D / 1W / 1M / 3M / YTD plus an optional
  **P/E YTD** for stocks with positive trailing EPS and the three
  ...
  Loss-makers (pre-profit companies) get a parallel **P/S YTD**
  ...
  holdings only show the daily ranges (1M / 3M / YTD) since they
...
| `utils.js` | Thin re-export barrel after the 933-LoC split. The public surface (Storage, formatters, fx, metrics, market hours, proxy chain, live-price fetch, historical batch, POSITION_COORDS) is unchanged — consumers keep `import { … } from './utils.js'` until they migrate to direct imports from the focused modules below. |
| `storage.js` | `Storage` namespace (`dp.auth` / `dp.prefs` / `dp.schema` / `dp.marketCache` / `dp.opsErrorAck`) with single-version schema migration. `loadMarketCache` / `saveMarketCache` persist the last successful `fetchTickers` reply (FX pairs + MC tickers) and hydrate the next cold start so the portfolio total + MC cards paint correct values on the first frame instead of flashing 1:1-USD fallbacks. Chart caches (`dp.tickerChart` / `dp.maCache` / `dp.ytd`) live in IndexedDB via `chart_store.js` to escape the localStorage 5 MB quota; this module only carries the small / low-churn rows that stay under it. Wrapped writes include a quota-fallback that halves the largest dp.* row and retries before giving up. |
| `market_hours.js` | DST-aware time helpers (`londonTimeParts`, `usMarketPhase`, `ukTzAbbr`, `usMarketHoursUtc`, `lseIsOpen`). All `Intl`-based so DST cutovers are resolved by the runtime. `lseIsOpen` gates the LSE-ticker ext-hours toggle in `metrics.js` — every `.L` ticker (currently VUAA.L / SAEM.L on the T212 sync, plus any user-added London listing) shows 0 % when LSE is closed (US after-hours / overnight) and the live LSE pct only when LSE is actually trading (US pre-market / first hour of RTH). |
...
| `ytd.js` | Pure chart math. `buildTickerSeries`, `computeAt` (sums per-lot value/basis across non-cash holdings, then adds cash to BOTH so the % is the whole-account return — keeps the PerfChart PORTFOLIO line in agreement with the header's cash-inclusive DAY CHANGE), `lotsFor`, `closeOn`, `RANGES`, `fetchParamsFor`, `maFetchParamsFor` (per-range wider-history params for the MA overlay; honours the `dailyOnly` override so CN funds / `.PVT` get 1d-only history), `filterToLatestDay`, `filterToLast24h`. Decoupled from React so it's unit-testable. |
...
| `pitch.jsx` | Football-pitch SVG rendering. Position chips, captain armband, hot-mover ball, inline subtitle rename in edit mode. (The drag-to-move scaffolding was removed once it became clear the drag SOURCES had been deleted long ago but the drop targets stayed orphan, and HTML5 DnD doesn't fire on touch anyway.) |
...
| `trading212.js` / `trading212.test.js` | `fetchTrading212Holdings()` hits the `trading212` Edge Function and returns `{ 'VUAA.L': { shares, cost }, 'SAEM.L': { ... } }` or null; `applyTrading212(holdings, t212, today)` overlays the result on the live portfolio by replacing each matching ticker's `lots` with a single synthetic lot dated today. Fired in parallel with the live-prices / MC / ext-series fetches inside `doRefresh`. Best-effort: a null T212 response leaves the lots alone, so a transient upstream error doesn't wipe what the user last saved manually. The user can still edit lots in EditTickerModal; the next auto-refresh tick just overwrites the manual edit with whatever T212 reports. |
...
| `trading212` | `GET /functions/v1/trading212` → `{ holdings: { 'VUAA.L': { shares, cost }, 'SAEM.L': { ... } }, prices: { 'VUAA.L': 98.4, 'AAPL': 234.5, … }, updatedAt, source: 'cache' \| 'live' \| 'stale' \| 'disabled' }`. Two maps with different jobs. **`holdings`** is the auto-sync allow-list (VUAA.L / SAEM.L, the two USD-denominated UCITS ETFs the user DCAs into via T212's cashback + Spare-Change auto-invest — both settle USD, which is why the GBP-denominated VUAG.L / SEGM.L were swapped out; auto-invest in USD against GBP ETFs added an FX-haircut per micro-buy). Only these get shares/cost mirrored: `averagePrice` taken as **per-share USD AC** (settle currency, not GBp/pence), folded client-side into a single synthetic lot. **`prices`** is `currentPrice` (USD) for EVERY recognised T212 holding via a generic ticker map (`AAPL_US_EQ → AAPL`, `VUAAl_EQ → VUAA.L`), no extra API call — it comes in the same `/equity/portfolio` row. The client (`trading212.js` → `applyTrading212NightPrice`) uses `prices` ONLY as an **overnight ("night market") quote**: during the 20:00–04:00 ET overnight window (`usMarketPhase() === 'overnight'`) with the Extended Hours toggle on, for any **US equity** the user also holds in T212 it swaps T212's price into `extPrice` (+ `extPriceTrusted`, `extDayPct` vs the RTH close) so metrics shows the broker's overnight print. Regular / pre-market / after-hours keep the original Yahoo logic untouched; the LSE ETFs (not US equities) and any T212 stock not on the board are skipped, so HOOD (held on Robinhood, never in the T212 response) is unaffected. Calls `https://live.trading212.com/api/v0/equity/portfolio` with single-key auth — T212's documented public API takes the API key as the raw `Authorization` header value (no `Bearer ` prefix, no Basic encoding; see [t212public-api-docs.redoc.ly](https://t212public-api-docs.redoc.ly/)). `T212_API_KEY` is required, `T212_API_SECRET` is optional — if both are set and the single-key call returns 401 the function retries once with HTTP Basic Auth (`key:secret` base64). Both keys are Supabase secrets, never in the client bundle. `cost` is **per-share AC**, matching the convention `lot.cost` / `h.cost` use elsewhere (computeMetrics multiplies by `shares` for total cost). Client-side, `fx.js` keeps a `TICKER_CURRENCY_OVERRIDES` map pinning `VUAA.L`/`SAEM.L` → USD, because the default `.L` → GBP suffix rule in `detectCurrency` would otherwise mis-convert their USD prices by the GBPUSD rate (~1.27×). **Server-side 30 s cache** in `trading212_cache`, in lockstep with the regular-hours auto-refresh tick. An **atomic Postgres claim** (`try_claim_t212_refresh` RPC in migration `0008`, a conditional UPSERT that bumps `updated_at` only when the row is older than the TTL and returns whether it actually wrote) guarantees at most ONE upstream T212 call per 30 s window across every device — within T212's 1-req-per-30-s limit on `/equity/portfolio` even though the TTL equals that window (the old 120 s TTL carried only slow-changing shares/cost; the live price needs to keep pace). The cached `data` is `{ holdings, prices }`; `unpackCache` treats a legacy holdings-map row as `{ holdings, prices: {} }` so a deploy doesn't blank the response before the first live refresh. Only the claim winner calls T212; losers re-read the cache. Stale-tolerant for 5 min on T212 errors. **Token-gated**: requires the same HMAC-signed `x-app-token` the `data` / `ops-error` functions use — both admin and ro tokens accepted but anonymous callers get 401. Holdings are PII; an open endpoint would leak the owner's share counts and cost basis. Returns `source: 'disabled'` + empty maps when `T212_API_KEY` is absent so the function deploys safely before the secret is configured. |
...
| `0007_trading212_cache.sql` | `trading212_cache` table — single-row (id=1) server-side cache of the Trading 212 portfolio snapshot (120 s TTL in the Edge Function, 4× T212's 1-req-per-30-s rate-limit window). Lets the `trading212` Edge Function fan-out to N visitors without each visit firing a fresh upstream call. RLS-deny default. Apply once via Supabase SQL Editor — the edge-functions deploy workflow doesn't run migrations. |
...
npm test              # vitest (193 client-side cases — YTD math, indicators (incl. isPriceAxis live-tail gate + extPriceIsRealAh ext-trade verdict), cache + ratio-suffix key pins, ticker shape, fetch strategy, portfolio metrics + FX, lot sanitisation, market-cache + legacy fallback, SW banner suppression window, ops-badge desktop gate, portfolio fingerprint diffing, fmtMoney magnitude tiers, applyTrading212 lots-merge). Plus the `fundamentals` Edge Function helpers (TTM rollover, USD-anchor rescale, pickPsFields ADR guard, computeForwardGrowth blend, computePeg) and the `trading212` helpers (T212 → portfolio shape, pence → GBP, cache TTL) on the Deno side.
