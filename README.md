...
- The layout is **bands**. Area is the contract — always
  proportional to market value — but the arrangement is free, and it
  has been wrong in both directions. A recursive binary split cutting
  at the halfway mark made a run of near-equal holdings split down the
  exact middle every time, so the biggest positions drew a grid of
  near-identical rectangles. Moving the cut off-centre broke the
  repetition and replaced it with a mess: seams that stop halfway and
  restart 70 px lower. Varied-but-orderly is a statement about the
  SEAMS, so each pass now lays one band across the full width (or
  height) of whatever space is left, and the variety is in how many
  holdings share a band — two, then three, then a column of three down
  the side. The last handful of holdings goes to the nesting split
  instead, because a band can only span the full width and a 1 % tail
  becomes a row of slivers no label fits.
...
| `heatmap.jsx` | One tile per holding, sized by market value, colored by day-change. Tile labels go through `displayTicker` (now in `formatters.js`, re-exported here) — strips the Yahoo exchange suffix (`XFAB.PA` → `XFAB`) and maps the display aliases (every `2DG` variant — `.F` / `.SG` / `.DE` — → `SIVE`, Sivers Semiconductors' home-market symbol). Shared with the tactics-board chips and Top Movers rows so all three read the same; the **chart modal is deliberately the one place that still shows the true Yahoo symbol**, so a tap resolves the right instrument and the user can see which listing they hold. Display-only — the real ticker stays on the data, keys and click handlers. Under the extended-hours toggle a tile with **no** extended figure yet (a US name in the overnight window before a broker quote lands) shows an em dash rather than `+0.00%` — `computeMetrics` flags it `dayPctUnknown`, because painting "nobody knows" as "unchanged" states a fact that isn't one, and a board of flat zeros reads as a broken toggle. **`treemap`** (exported for its pin tests) places the tiles. Area is proportional to market value and nothing bends that; the ARRANGEMENT is free, and it has been wrong twice in opposite directions. It began as a recursive binary split cutting where the running total first passed half — for a run of near-equal holdings that is the exact middle every time, the halving repeated, and on a 338×612 canvas the eight largest tiles all landed within 15 % of 110×170 px, three to a row. Moving the cut off-centre broke the repetition and replaced it with a mess: a split landing at 0.38 here and 0.62 there leaves seams that stop halfway and restart 70 px lower, and near-miss edges read as chaos rather than variety. Varied-but-orderly is a statement about the SEAMS, so the layout is now BANDS: each pass lays one band across the full width (or full height) of the space left, which makes every seam run edge to edge inside its own region, and the leftover is a rectangle the next band fills the same way. Bands run across the SHORTER side so the leftover stays as square as it can, which is also what turns the lower half into columns. Variety is in the band: how many holdings it carries, hence how thick it is and how wide its tiles are. The count is scored against a shape the band is ASKED for — tile width over band thickness, walking 0.62 (tall tiles) to 1.75 (wide ones) along a golden-ratio sequence, so consecutive bands never come out the same shape. Counts that would put a tile under the label size, past 3:1, or leave a strip too thin for another band are not offered; the count that takes everything left is not exempt, since left competing on score it swallowed a whole book and splintered the smallest holding. Below `NEST_BELOW` (90 px of short side) or `NEST_COUNT` (6 holdings left) the remainder goes to `nest`, the ORIGINAL binary split, because a band can only span the full width and a 1 % tail becomes a row of slivers — measured, four tiles under 13 px wide — while a nested corner can still give them squarish tiles. That corner is also the part of the old board that always looked right. Measured over eight differently-shaped books at eight canvas sizes, against the shipped off-centre split: worst aspect ratio anywhere 21.2:1 → 5.4:1, tiles too small to label 19 → 17, and the longest run of look-alike tiles among the top eight 3.44 → 3.93 (against 4.68 for the original rule). Fully deterministic — the same book always draws the same map. |
