...
| `trading212` | `GET /functions/v1/trading212` → `{ holdings: { 'VUAA.L': { shares, cost }, 'SAEM.L': { ... } }, updatedAt, source: 'cache' \| 'live' \| 'stale' \| 'disabled' }`. Mirrors a hard-coded allow-list of tickers from the owner's Trading 212 account — VUAA.L and SAEM.L, two USD-denominated UCITS ETFs on LSE that the user DCAs into via T212's cashback + Spare-Change auto-invest (both settle in USD, which is why the previous GBP-denominated VUAG.L / SEGM.L were swapped out — auto-invest in USD against GBP ETFs added an FX-haircut to every micro-buy). Calls `https://live.trading212.com/api/v0/equity/portfolio` with single-key auth — T212's documented public API takes the API key as the raw `Authorization` header value (no `Bearer ` prefix, no Basic encoding; see [t212public-api-docs.redoc.ly](https://t212public-api-docs.redoc.ly/)). `T212_API_KEY` is required, `T212_API_SECRET` is optional — if both are set and the single-key call returns 401 the function retries once with HTTP Basic Auth (`key:secret` base64), so accounts on a non-standard two-key flavour still work. Both keys are Supabase secrets, never in the client bundle. Filters the response to the allow-listed tickers via the internal `VUAAl_EQ` / `SAEMl_EQ` → Yahoo-ticker map, takes T212's `averagePrice` as **per-share USD** (T212 reports VUAA.L / SAEM.L in their settle currency USD, since they're USD-denominated UCITS), and returns `{ shares: quantity, cost: averagePriceUSD }`. `cost` is **per-share AC**, matching the convention `lot.cost` / `h.cost` use elsewhere (the lot editor's AC math + `computeMetrics` both multiply by `shares` to get total cost — passing total cost there would inflate cost basis by the share count). Client-side, `fx.js` keeps a `TICKER_CURRENCY_OVERRIDES` map that pins `VUAA.L`/`SAEM.L` → USD, because the default `.L` → GBP suffix rule in `detectCurrency` would otherwise force the FX path to mis-convert USD prices by the GBPUSD rate (~1.27×). **Server-side 120 s cache** in `trading212_cache`: T212 rate-limits `/equity/portfolio` at 1 req / 30 s, and 120 s sits 4× the rate-limit window. The TTL window keeps the steady-state rate safe but doesn't kill the boundary race (two workers passing the freshness check in the same ms), so refresh is also gated by an **atomic Postgres claim** (`try_claim_t212_refresh` RPC in migration `0008`, a conditional UPSERT that bumps `updated_at` only when the row is stale and returns whether it actually wrote). Only the claim winner calls T212; losers re-read the cache and serve whatever the winner wrote. Stale-tolerant for 5 min: T212 errors fall back to the cached row when one exists. **Token-gated**: requires the same HMAC-signed `x-app-token` the `data` / `ops-error` functions use — both admin and ro tokens are accepted (the read-only "screenshot" view sees synced holdings the same as the rest of the dashboard) but anonymous callers get 401. Holdings are PII; an open endpoint would have leaked the owner's share counts and cost basis to anyone with the function URL. Returns `source: 'disabled'` + empty holdings when `T212_API_KEY` is absent so the function deploys safely even before the secret is configured. |
