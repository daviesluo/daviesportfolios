...
| `ticker_class.js` — `tradesAllWeekdayHours` | New alongside the other shape predicates: does this instrument print right through a WEEKDAY night? Futures and spot FX do, so the 3M chart draws them on the same four-hour London grid the portfolio panel uses. An index (`^GSPC` / `^VIX` / `^TNX`) or a listed equity does not — on that grid it would land on two live prices a day and carry the previous close through the other four, so it keeps its hourly session bars. **Crypto is excluded on purpose** although it trades the most: the grid skips weekends, which is right for anything whose venue shuts on Friday and wrong for a 24/7 tape. |
