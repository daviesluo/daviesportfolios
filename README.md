...
| `trading212` | `GET /functions/v1/trading212` → `{ holdings: { 'VUAG.L': { shares, cost }, 'SEGM.L': { ... } }, updatedAt, source: 'cache' \| 'live' \| 'stale' \| 'disabled' }`. Mirrors a hard-coded allow-list of tickers from the owner's Trading 212 account — VUAG.L and SEGM.L, the two GBP-denominated ETFs the user DCAs into daily and would otherwise have to type into EditTickerModal one buy at a time. Calls `https://live.trading212.com/api/v0/equity/portfolio` with `Authorization: $TRADING212_API_KEY` (Supabase secret, never in client bundle), filters the response to the allow-listed tickers via the internal `VUAGl_EQ` / `SEGMl_EQ` → Yahoo-ticker map, divides `averagePrice` by 100 for `.L` listings (T212 reports LSE in pence/GBX, matching what Yahoo does for the chart side via the GBp normalisation in `utils.js`), and returns `{ shares: quantity, cost: averagePriceGBP }`. `cost` is **per-share AC**, matching the convention `lot.cost` / `h.cost` use elsewhere (the lot editor's AC math + `computeMetrics` both multiply by `shares` to get total cost — passing total cost there would inflate cost basis by the share count). **Server-side 60 s cache** in `trading212_cache`: T212 rate-limits `/equity/portfolio` at 1 req / 30 s, so 60 s leaves a 30 s safety margin in case two Edge Function workers race past the freshness check at the same instant. Every visitor hits this function on every doRefresh, so without the cache two concurrent visitors would punch through the rate limit. Stale-tolerant for 5 min: T212 errors fall back to the cached row when one exists; the error path also re-reads the cache before returning empty so a concurrent winner's just-written row isn't missed on a cold-cache race. No token gate (all visitors sync — the user's read-only "screenshot" mode shares the same view). Returns `source: 'disabled'` + empty holdings when `TRADING212_API_KEY` isn't set so the function deploys safely even before the secret is configured. |
...
| `0007_trading212_cache.sql` | `trading212_cache` table — single-row (id=1) server-side 60 s cache of the Trading 212 portfolio snapshot (1 req / 30 s T212 rate limit + 30 s safety margin). Lets the `trading212` Edge Function fan-out to N visitors without each visit firing a fresh upstream call. RLS-deny default. Apply once via Supabase SQL Editor — the edge-functions deploy workflow doesn't run migrations. |
