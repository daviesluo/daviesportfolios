...
- **1D chart spans 24 h** — both in-session and ext-hours views show
  ...
  **official** regular close (Yahoo `regularMarketPrice` = the same
  ...
  Crypto (`-USD`) reads the same **"since previous close"** basis as
  every US stock (anchored at Yahoo's `regularMarketPreviousClose`); it
  has no regular close / ext session, so the Extended-Hours toggle never
  changes that basis (it never flips to a 0.00% live-anchored headline).
