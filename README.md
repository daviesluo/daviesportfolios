...
- **Fast cold start** — a cache-primed `portfolio` (`storage.js`'s
  ...
  (`.github/workflows/healthcheck.yml`, every 10 min) and a
  ...
  cold session) — together these were the fix for "the first refresh
  after opening the app takes tens of seconds, but a second refresh
  inside the same session is fast" (Supabase Edge Functions have no
  documented keep-warm config, so an idle isolate going cold, chained
  across the several functions the initial load depends on, was the
  dominant cause).
