...
- **Multi-currency holdings** — USD / GBP / EUR / CNY / HKD with live FX
  ...
  to USD. The Investment Performance deposit line is actual capital
  deployed: manual lots plus T212 fill quantity × price at the real fill
  timestamp, net of sells. T212 Card top-ups/spending are excluded.
  Native→USD deposit rates freeze once; market value keeps live FX.
  A holding whose price is unknown is skipped, not counted as $0. A
  closed leftover lot (still long on the ledger) is put back on the
  board so PORTFOLIO matches the shares that are actually owned.
...
| `formatters.js` / `fx.js` / `metrics.js` / `lots.js` | The pure pieces lifted out of `utils.js`. `formatters.js`: `fmtMoney` / `fmtPct` / `fmtPrice` / `fmtShares` (share count, configurable decimal cap — default ≤2, no padded zeros) + `fmtSharesFor(n, ticker)` (the per-ticker policy: crypto `-USD` shows ≤3 decimals so small coin balances like 0.043 BTC aren't rounded to "0.04", everything else ≤2) / `pctColor` / `maskDigits` / `formatAgo`. `fx.js`: `detectCurrency` / `currencySymbol` / `fxRateToUSD` (the version that returns `{ rate, missing }` so a 1:1 fallback can be surfaced) / `fxToUSD` (back-compat shim) / `depositFxRate` (missing frozen rate is `null`, never 1:1). `metrics.js`: `computeMetrics` / `detectFormation` — the ext-hours gate treats crypto (`-USD`) like a US equity (the prices Edge Function pre-anchors it to the last US close), so BTC's heatmap / scoreboard day change matches a US stock's, and its always-real 24/7 `extPrice` bypasses the ±5% bogus-quote cap; a non-cash row with no positive price is skipped (unknown is not $0, and does not book a fake 100% loss); `tickerCount` (the header's "N tickers") is **board-scoped** — distinct non-cash tickers referenced by a position, so a fully-sold holding (kept in `holdings` with `closed: true` for its transaction ledger, but removed from the board) stops counting the moment it's sold out. `lots.js`: `cleanLots` / `totalShares` / `weightedAvgCost` (lot-input sanitisation, which used to be inline in modals.jsx and silently kept negative cost values). All pinned by `utils.metrics.test.js` (37 cases) and `lots.test.js` (14 cases) so the on-screen portfolio numbers can't quietly regress. `formatters.js` also owns `normalizeDecimalInput` (every decimal field in the modals runs its keystrokes through it, so a bare `.5` reads back as `0.5`; it only touches a leading dot, since a half-typed `0.` or `1.` has to survive) plus two shared display predicates: `displayTicker` (heatmap tiles / tactics chips / Top Movers all label a ticker the same way) and **`pctIsFlat`** — ONE "did this actually move" threshold (|pct| < 0.005, i.e. anything that prints as 0.00 %). The heatmap already painted those tiles neutral while Top Movers ranked on a bare `> 0` / `< 0`, so a -0.004 % row was simultaneously a dark "no change" tile and a red LOSERS entry reading "-0.00%". |
| `data.js` | `INITIAL_PORTFOLIO` seed for first-load demo state. |
| `deposit_math.js` | Current source of truth for Investment Performance Deposited (supersedes the older transaction-history description in the legacy `ytd.js` row below): reconciles board shares with manual lots, combines unmatched other-broker rows with actual T212 fills, subtracts sells, and never consumes T212 Card cash transactions. A short machine ledger keeps its known lots/sells and appends only the missing residual — replacing the whole book with one 1970 lot erased real August dates. Other lots longer than the residual (a blended board lot) still stand in residual cash only. |
...
| `trading212.js` / `trading212.test.js` | T212 positions and prices are fetched on refresh. The allow-list ETFs remember their T212 slice as `t212Shares` / `t212Cost`, so a position shared with another broker applies only the T212 delta and can never shrink the whole board row. User `lots` / `sells` are never replaced by machine history; non-allow-list fills do not mutate the portfolio at all. `fetchTrading212Orders()` reads complete, immutable fills from `t212_orders`; Investment Performance combines their real quantity × price × timestamp with unmatched manual lots, or an inferred pre-window residual when the old overwrite already destroyed those rows. Cash/card transactions are not used. `syncTrading212History()` advances orders only. Overnight price overlay remains price-only and best-effort. |
...
| `snapshot-record` | `POST /functions/v1/snapshot-record` — **cron-triggered** 24/7 (`*/5 * * * *`, migration `0026`), NOT user-facing. Loads `board_data`, overlays T212 `currentPrice` on Yahoo quotes, converts VALUE with live FX, and upserts `{ ts, value_usd, deposit_usd }`. Value includes leftover closed lots that still net long, matching the scoreboard heal, without writing `board_data`. Deposited uses actual T212 fills plus manual/inferred lots, net of sells and board cash; T212 Card cash rows are excluded. Native deposit FX freezes in-memory from live quotes when the board row has none (still never written back). Incomplete T212 history writes Value with `deposit_usd` NULL. Missing live FX/price skips the tick. |
...
| `20260818180000_nullable_snapshot_deposit.sql` | Drops `NOT NULL` on `portfolio_snapshots.deposit_usd` so cron can write a Value-only row while T212 history is still walking. The chart treats NULL deposit as derived-only; it must not invent 0. |
