...
| `heatmap.jsx` | One tile per holding, sized by market value, colored by day-change, laid out by a recursive binary-split `treemap` (exported + pinned in `heatmap.test.js`). A region that bottoms out at exactly **3 tiles** is special-cased, instead of taking the generic split's lopsided "two-beside-one" (two side-by-side tiles of very different size, which read as unbalanced): a tall/square region becomes three full-width rows (top / middle / bottom, heights by value); a wide region puts the largest tile in a full-height left column and **stacks** the other two in a right column. Splitting a wide region into three proportional columns instead — the first attempt — turned the two smaller tiles into unreadable vertical slivers next to a dominant one, so the left column is clamped to 40–70 % of the width and the right column's stack to 30–70 % of the height; no tile can collapse into a thin band. Tile labels go through `displayTicker` — strips the Yahoo exchange suffix (`XFAB.PA` → `XFAB`) and maps `HEATMAP_TICKER_ALIASES` (e.g. `2DG` → `SIVE`, Sivers Semiconductors' home-market symbol) — display-only; the real ticker stays everywhere else in the app and on the tile's key/click. |
