...
| `trading212.js` / `trading212.test.js` | `fetchTrading212Holdings()` hits the `trading212` Edge Function and returns `{ holdings: { 'VUAA.L': { shares, cost }, … }, prices: { 'VUAA.L': 98.4, 'AAPL': 234.5, … } }` or null; `applyTrading212(holdings, t212, today)` overlays the shares/cost allow-list on the live portfolio by replacing each matching ticker's `lots` with a single synthetic lot dated today, while `applyTrading212NightPrice(holdings, prices, active)` overlays the `prices` map as overnight quotes (US equities only, when `active`). Fired in parallel with the live-prices / MC / ext-series fetches inside `doRefresh`. Best-effort: a null T212 response leaves the lots alone, so a transient upstream error doesn't wipe what the user last saved manually. The user can still edit lots in EditTickerModal; the next auto-refresh tick just overwrites the manual edit with whatever T212 reports. |
