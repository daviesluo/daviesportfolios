...
a "Performance vs S&P 500" chart with 24H / 1W / 1M / 3M / YTD ranges.
...
- **Multi-currency holdings** — USD / GBP / EUR / CNY / HKD with live FX
  ...
  and avg-cost render in the native currency; market value converts
  to USD. The Investment Performance deposit line is money paid in
  as recorded (T212 deposit − withdraw once that history is complete,
  otherwise lot costs) and is not revalued at live FX.
...
- **Yahoo-Finance-equivalent YTD chart math** — per-lot purchase
  ...
  the basis for in-year lots. The **24H** range (internal `1D`) still
  forces every holding's basis to `prevClose` (`prevCloseBasis`) so
  T212-synced lots (re-dated `today` each refresh) don't leak the
  whole position's gain into the window; the panel then rebases both
  lines to the first point, so the displayed reading is a trailing
  24 h, not the scoreboard's DAY CHANGE. Pinned by a vitest suite so
  the formula can't quietly regress.
...
- **Overnight line chart** — Yahoo has no overnight bars and T212
  ...
  One-time setup: enable `pg_cron`/`pg_net`, store `cron_secret` in
  Vault matching the function's `CRON_SECRET` (see `0021`).
- **24/7 portfolio snapshots** — Investment Performance's recorded
  series is sampled every 5 minutes by `snapshot-record` on pg_cron
  (`*/5 * * * *`, 24/7 — not the overnight-only UTC 0–9 window). The
  original client sampler only ran while an admin tab was in the
  foreground, so a day of "every five minutes" produced a handful of
  clusters. Real samples overlay the vs-S&P timestamp grid and push
  the derived/faded handover left until there is none of it left.
  Table prune matches the chart's density: 5-minute points only for
  the trailing ~26 h (the 24H window); older rows keep the last
  sample per 30 min / 1 h / 4 h / 1 day, then drop after ~400 days.
  A 30-day wipe like overnight-points would erase the 1M / 3M / YTD
  replacement series. The admin tab still writes a backup sample
  during US regular hours; after hours the server is the only writer.
...
### Holding list + Sectors list + Transaction history (☰ menu)

- The **☰ menu** to the right of EDIT (both desktop + mobile) opens a
  dropdown with three items: **Holding list**, **Sectors list** and
  **Transaction history**.
...
- **Copy / save screenshot** — two icon buttons next to the close
  ...
  loaded only on click — out of the main bundle (gate untouched)
...
- **Top Movers · Today** — the largest **actual** winners and losers by
  % change, up to five each. Only names that really moved rank: a row
  pinned at 0 — a no-US-ext venue suppressed during the overnight
  (SFTBY / `.L` / euro / CN fund) or a genuinely flat stock — pads
  neither column (the overnight LOSERS list used to be five red 0.00 %
  rows); an em-dash marks a side with no movers.
...
- **Backend** — Supabase (Postgres + Edge Functions, Deno runtime).
  Ten functions: `auth`, `data`, `prices`, `chart`, `fundamentals`,
  `ops-error`, `trading212`, plus `overnight-record` / `overnight-fetch`
  (the cron-driven overnight intraday recorder) and `snapshot-record`
  (24/7 5-minute portfolio value + deposit samples). Migration files
  `0001`–`0026`; `0005`/`0006` are a historical create/drop pair for the
...
| `ytd.js` | Pure chart math. `buildTickerSeries`, `computeAt` (sums per-lot value/basis; restricted to **board-scoped** holdings — only tickers referenced by a position, matching `computeMetrics` so an orphaned holding can't split the chart from the scoreboard; with `prevCloseBasis` — passed for 1D — every held lot's basis is forced to `prevClose` and no-prevClose holdings are counted FLAT in the denominator, so T212 lots don't leak lifetime gain into the 24H window; the panel then rebases, so the displayed right edge is trailing-24 h, not the scoreboard DAY CHANGE; cash is also added to both value and basis), `lotsFor`, `closeOn`, `RANGES`, `panelRangeLabel` (the vs-S&P / Investment row says **24H** for the internal `1D` key; the ticker modal keeps 1D), `fetchParamsFor`, `maFetchParamsFor` (per-range wider-history params for the MA overlay; honours the `dailyOnly` override so CN funds / `.PVT` get 1d-only history), `filterToLatestDay`, `filterToLastHours` (trailing-N-hours cut; `filterToLast24h` is the 24 h specialisation), `applyVariantFilter` (the 1D fetch-variant → display-window dispatch — `closed` → latest day, `reg`/`ext` → last 24 h — that `perf_chart.jsx` / `use_ticker_chart_data.js` / `prefetch.js` had inlined at 8 sites). Crypto 1D US-session windows: `windowSinceLastUsClose` (ext OFF + market **open** → from the last 16:00-ET close to now) and `windowBetweenLastTwoUsCloses` (ext OFF + market **closed** → the last complete close-to-close day, ending AT the last close so, like a US stock after hours, the overnight move is hidden). `fillVenueSessionGrid(points, session, nowMs)` lays a sparse-tape venue listing's Yahoo bars onto a fixed 5-min session grid (per venue-local day, DST-safe via Intl), carrying the last close flat through no-trade gaps — the 2DG.F 1D frame that always spans 07:00–21:00 UK (live day capped at "now"; leading slots backfill flat; out-of-frame prints dropped). Decoupled from React so it's unit-testable.  **Range windows**: 1D is one trailing 24 h in every variant — `closed` used to keep the latest CALENDAR day, so with the Extended Hours toggle off the window's length changed with the phase and disagreed with what the same button showed with the toggle on (the benchmark and whether pre/post bars are fetched still follow the toggle; only the length is now common). 1W fetches a MONTH and trims to the trailing 168 h — Yahoo's `5d` is five TRADING sessions, Mon→Fri = 4.3 days, so the button was showing well under the week it claimed, and Yahoo has no range between `5d` and `1mo`. Both 1W variants trim, including PerfChart's ext-on `1w-ext`, which previously passed through untouched (correct while the fetch was already 5 days, a whole month once it wasn't).  `investmentPointAt` / `netDepositNow` back the Investment Performance chart and are deliberately NOT `computeAt`: that one is board-scoped and ignores sells, so a stock sold last month would contribute nothing to any PAST point and a partially-sold position would keep counting shares no longer owned. Here every holding participates, closed ones included, and the share count at a date is buys minus sells up to it. `netDeposit` is money in — cumulative buy cash minus sale proceeds — so selling at a profit drops it by more than the cost of what was sold, which is what makes the gap between the two lines the account's actual gain. Cash lifts both lines equally (matching the scoreboard's PORTFOLIO) rather than reading as profit. Once T212 `/equity/history/transactions` has been walked to completion, `t212MoneyInAt` replaces lot-cost deposit for every ticker that has stored fills (deposit − withdraw, fees/interest/transfers ignored) so idle cash is not counted twice; an unfinished newest-first walk keeps the ledger formula. `priceAtOrCarried` prices each holding at a date, carrying the EARLIEST known close backwards when the history doesn't reach that far: `closeOn` only looks at or before the date, so before a ticker's first bar it returns null and the holding used to drop out of the value while its deposit kept counting — it read as "not owned yet" when the truth is "owned, price unknown". The damage was large and systematic: on a 1D window over a weekend NO bar is at or before the window's own start, so every holding fell out and the value line sat at the cash balance ($6.7k under a $181k book) until the recorded samples took over; and on any range where a few tickers' history starts later than the others, those stepped in one by one as their first bar arrived, drawing a week that really moved +1.8% as +7.6%. A flat book now draws flat on every range — pinned by a browser harness that reads +247% against the same fixture with the carry removed. The holding's own `lastPrice` is the last resort, for a ticker with no usable series at all (a CN fund or `.PVT` on an intraday range, or a name sold long enough ago that nothing fetches its history). `computeAt`'s no-price handling was reworked off the back of the same closed-form comparison: it now carries a series' FIRST known close backwards before that series starts (interpolating from lot cost there invents a move the stock never made — a window that really did 5.83 % read 4.2 %), and a pre-anchor lot with no baseline at all now counts FLAT instead of being skipped. That `continue` removed the holding from the value as well as the basis, so a position with no fetchable price history — a CN fund or `.PVT` on an intraday range — was simply not in the portfolio as far as either chart was concerned; it understated the Investment chart's value line by the whole holding and skewed the vs-S&P percentage by leaving it out of the denominator too (5 % on a window that really did 4.17 %). Flat, not absent — the same treatment cash already gets. `historyLotsFor` is the lot source for that walk: the walk starts at `shares = 0` and adds only what the lots say, so a holding whose lots were never filled in contributed NOTHING to either line at every date — it read as an account that didn't own the thing, not as an unknown, and the deposit line started far below what had been paid in. The stand-in says "held since before any window we chart, at the recorded average cost": assuming a position predates the chart is the least-wrong reading, where dating it today would redraw the entire past as if the money had only just arrived. Unlike `lotsFor` it keeps present-but-drifted lots, because a sold-out position's lots legitimately don't sum to its zero `shares` and that history is the whole point here. |
...
| `investment_chart.jsx` / `investment_chart.test.jsx` | Investment Performance — the account in dollars rather than percent: portfolio value against net deposited, the gap between them being what it made. Shares the panel slot with the vs-S&P chart (`PerfPanel`'s ⇄ swap; the range carries across so flipping the view doesn't also change the period). The derived half SEEDS synchronously from the per-ticker bucket the vs-S&P chart and the background prefetch fill, then fetches to revalidate and writes the trimmed result back into the same bucket (per-ticker, never through `savePerfCache`, which replaces the whole bucket and would drop the S&P anchor row). Seeding is what stops the panel loading every single time it is opened — it used to start from an empty `hist` and show "Insufficient data" until a fetch returned, on first open and on every range switch. Fetching anyway is what stops it depending on render order, the ext toggle or the market phase for whether that bucket exists at all. Cash follows the scoreboard's board scope on both lines (a cash holding not referenced by a position counts on neither), while securities deliberately don't: a sold-out position is off the board but its money moved, and that history is the point. The value line is `computeAt().value` sampled on `benchmarkWindow()` — the very same call the vs-S&P chart makes for its PORTFOLIO line, on the same merged marketData and the same dates. Same values on the same grid is what makes the two views answer with one voice: sampling the same quantity on a different grid drew a differently-shaped line for the same week, and on 1D a different WINDOW entirely (the cash index is regular-session-only, so that chart covers a session where this one was covering a trailing 24 h). With deposits flat the two lines are now pinned to the same point count and the same normalised shape to within a pixel. `withLivePoint` then pins the right-hand end to the board's current figures — samples land every five minutes and derived points stop at the last fetched bar, so the legend's Value trailed the scoreboard sitting next to it by up to five minutes. It inherits the trailing stretch's `estimated` flag, or the line splits one point from its right edge into a one-vertex segment that draws nothing. That is the point: this chart IS that line in dollars, so it has no business arriving at a different number for the same moment, and two independent reconstructions of one quantity drift (they did — one carried the last close over a missing bar, the other interpolated from lot cost, and the two charts disagreed about the same week). The deposit stays with `investmentPointAt`, a different question — money in, not market value — which needs the sells `computeAt` has no reason to track. A point whose value or deposit comes out non-finite is dropped rather than drawn: one NaN takes the whole axis with it. `mergeSeries` overlays stored 5-minute samples onto the vs-S&P timestamp grid (same bar count, same x-spacing) rather than concatenating them: appending today's 5-minute points onto a daily/hourly axis stretched the last calendar day across half the 1W / 1M / 3M / YTD chart because index spacing gives every point equal width. Samples still win on VALUE wherever they exist and fill only OLDER gaps from the ledger — interleaving a recomputation with recorded figures makes the line jitter between two answers for the same moment, and a book predating the sampler still charts its whole history. Samples after the last bar fold into the last point so today's mark is live without extra columns. The handover is one-way: every sample recorded pushes the boundary left, so the reconstructed stretch shrinks on its own until there is none of it left. Derived points carry `estimated: true` and the chart draws that stretch faded, with a dotted rule at the point recording takes over and a `~` on the crosshair chips — a reconstruction from the lot ledger plus today's price history must not read as a record of what the account actually showed, particularly where the ledger can't know the real purchase dates. Carries the same axis furniture as the vs-S&P chart, in the same 300×106 box with the same padding so the ⇄ swap moves no chrome: round-dollar y gridlines, a sampled time/date x axis, and a DOM-ref crosshair (vertical line, a dot on each series, a date pill under the axis, a money chip beside each dot — refs + `setAttribute` inside a rAF, because a state update per mousemove would reconcile a ~290-point path every frame; the native non-passive `touchmove` listener is what makes the finger drag work on iOS, where `touch-action` on an inline `<svg>` inside a scrollable ancestor is ignored). `niceMoneyTicks` snaps the axis to the FINEST round step that still fits the tick budget — rounding the ideal spacing up instead read the padded range and drew $0/$100k/$200k/$300k for a $0–$216k window — and refuses to pad an all-positive account below zero, which had spent half the height on money that was never owed. `axisMoneyLabel` takes its unit from the axis magnitude (so a column never mixes `$950` with `$1.0k`) and its decimals from the step (a $2,500 step must read `$132.5k`, or two rows both say `$132k`). Both the axis labels and the crosshair chips honour `hideValues` — masked, not dropped, so the chart keeps its shape. The band between the lines is filled and tinted by the result: the enclosed area IS the money made — which is also why the legend no longer carries a lifetime-gain figure. Instead each line reports its move ACROSS THE WINDOW the range buttons select, and the VALUE figure has the window's deposits netted out of it first: paying $10k into a $30k account does not make it up 33 % — the account got bigger, it did not perform — and reading it that way is what put +43 % beside a window the vs-S&P view called +7.5 %. Only that netted figure is tinted green / red; the deposit line's own percentage sits beside it uncoloured, so the size change is still on screen without claiming to be a result. A window opening on an empty account has no percentage to report rather than a division by zero. Both charts are pinned against closed-form arithmetic by a browser harness over four book shapes (nothing moving, a purchase mid-window, a holding the chart endpoint returns nothing for, a holding whose history starts mid-window); this chart's value line endpoints and percentage are correct in all four. The vs-S&P view was only correct in the first of those four until the `computeAt` pricing fixes below landed; it now agrees in three. The remaining difference is a convention, not a defect: on a window containing a purchase this chart measures the gain against the capital it OPENED with (10 %) while the vs-S&P line money-weights against the capital deployed by the end (7.5 %). Its legend uses `inv-`-prefixed classes; reusing the `perf-` ones silently restyled the vs-S&P legend, whose round dots had become 6×2 dashes. |
| `portfolio_snapshots.js` / `portfolio_snapshots.test.js` | Client side of that series, cached per range in `YtdStore` and warmed for every range by `prefetchAllChartData` — so the ⇄ swap and the range buttons inside it are cache hits, the same way the vs-S&P chart's ranges already are (the warm is deliberately NOT awaited: it must not hold up the prefetch's prune behind a request for a panel that may never be opened). `RANGE_BUCKET_SECONDS` coarsens the point interval with the window, and a coarse read that can't make a line retries at the finest bucket — on day one two samples five minutes apart fold into a single 30-minute bucket, and 1W and up read "Insufficient data" with the data sitting right there. An empty read is never cached — it's ambiguous between "no samples yet" and "request failed", and caching it would blank a panel that had good points. `saveSnapshot` is the US-RTH backup write (the 24/7 series is `snapshot-record` on pg_cron, same machinery as overnight-record) and refuses a not-yet-loaded board, since recording a zero would punch a hole in the chart at the moment the app opened. `dropDepositSpikes` repairs the read: money paid in is a STEP — it moves once and stays — so a figure that jumps and comes straight back on the next sample is a bad reading, not a cash movement. There was a real one on 17 Aug (deleted) before the sampler learned to skip a tick with a missing FX pair (a CNY position converted at a fallback 1:1 instead of ~0.14 lands seven times its size). Deliberately narrow: the neighbours must agree with EACH OTHER to within 1 % before the middle is judged, so a genuine deposit — which makes the two sides disagree permanently — is never touched. Display-side repair still stands for any future one-sample jump. `loadSnapshots` requires an actual number per column rather than coercing — `Number(null)` is 0 and passes `isFinite`, which would have read as a real zero. Admin only: a read-only viewer must not write to the owner's book. |
| `perf_chart.jsx` | `<PerfChart>` (the chart) + `<PerfPanel>` (chrome wrapper). 5 ranges, dual fetch effect (S&P alone + portfolio batch in parallel), background prefetch effect for the other ranges, DOM-ref crosshair, CLOSE/OPEN markers in 1D, ^GSPC RTH filter + ES=F ETH filter. **Night-market (overnight) inclusion on 1D + 1W**: when the Extended Hours toggle is on, the S&P reference switches to the ES=F future on BOTH 1D and 1W (`spSymbolFor` — the cash index ^GSPC is RTH-only and can't carry pre/post or overnight), 1W pulls pre/post bars via the `1w-ext` variant (`perfFetchParams`; passes through `applyVariantFilter` untrimmed), and whenever the toggle is on the ES=F `[4 AM,8 PM) ET` session filter is lifted so the futures line runs continuously through 20:00–04:00 ET. The portfolio line is sampled at the S&P series' timestamps, so to draw a real overnight curve (not a flat carry-forward) each ticker's Yahoo bars are spliced with the server-recorded overnight points (`mergeOvernightSeries` over `overnight-fetch`'s T212 samples, same source the ticker modal draws; `NIGHT_BAR_INTERVAL_MS` matches recorded-point density to the 5/30-min bar cadence). Gated on the toggle, NOT the live overnight phase — so last night's overnight stays drawn through the next trading day; the 1D anchor stays phase-aware (prevClose during RTH), the correct baseline for the whole 24 h window since the overnight happened after that close. `spSymbolFor` / `perfVariantKey` / `perfFetchParams` are exported + pinned by `perf_chart.test.jsx`.  Both lines are **rebased to 0 % at the window's first point** on every range, including the panel's shortest window — labelled **24H** (internal key still `1D` so cache/fetch/prefetch don't fork; the ticker modal keeps saying 1D). There is no DAY / 24H toggle and no previous-close reading on this panel: a trailing 24 h is the only 1-day view, which is what the ext-hours / futures title is for. They used to be measured against a basis outside the window (the S&P against the last close before it, the portfolio against cost / prevClose), so a chart of "the last week" opened at whatever the move happened to be and the two lines started at different heights — you couldn't read "who is ahead over this window" off the left edge, which is the point of an indexed comparison. Rebasing is a shift, not a reshape: `computeAt` still folds in-period buys into the basis, so a mid-window purchase doesn't read as performance. On 24H the right edge is "since this point 24 h ago", not the scoreboard's since-previous-close DAY CHANGE — those two numbers will disagree by the overnight gap, and that is the reading he asked for. |
...
| `trading212.js` / `trading212.test.js` | `fetchTrading212Holdings()` hits the `trading212` Edge Function and returns `{ holdings: { 'VUAA.L': { shares, cost }, … }, prices: { 'VUAA.L': 98.4, 'AAPL': 234.5, … } }` or null; `applyTrading212(holdings, t212, prices, today)` overlays the shares/cost allow-list on the live portfolio by replacing each matching ticker's `lots` with a single synthetic lot carrying the EARLIEST date that holding already had (today's only on the first ever sync). The broker reports a position, not a purchase history, so the synthetic lot can only approximate — but re-stamping today's date on every sync made the position read as bought this morning, every morning, and anything reconstructing the past from the ledger (Investment Performance's derived half, and the net-deposit figure the sampler records) then saw the money arriving today, which is a large part of why the deposit line started from nothing — AND, when a `prices` map is supplied, uses the broker's live `currentPrice` (USD) as the allow-list ETFs' regular-session `lastPrice`, recomputing `dayPct` against the (Yahoo) prevClose and pinning currency USD. This is because Yahoo's free LSE feed lags ~15-20 min at the 08:00 UK open (VUAA.L / SAEM.L would otherwise sit on a stale close after a Refresh), so the broker's own quote is both fresher and already in USD; the price updates in lockstep with the shares on every auto-refresh + manual Refresh. `applyTrading212NightPrice(holdings, prices, active)` overlays the same `prices` map as OVERNIGHT quotes for US equities instead (only tickers with `hasOvernightSession` — US equities excluding OTC ADRs like SFTBY — and only when `active`). Both fired in parallel with the live-prices / MC / ext-series fetches inside `doRefresh`, applied AFTER the Yahoo merge so they see the just-set prevClose. Best-effort: a null T212 response (or a ticker missing from `prices`) leaves the lots + Yahoo price alone, so a transient upstream error doesn't wipe what the user last saved manually. The user can still edit lots in EditTickerModal; the next auto-refresh tick just overwrites the manual edit with whatever T212 reports. **Real purchase dates**: `fetchTrading212Orders()` reads the backfilled fills from `t212_orders` (client-cached 10 min — the history is immutable, so this costs nothing on the 30-second tick, and an empty read is never cached over a good one), and `lotsFromOrders()` rebuilds a ticker's `lots` + `sells` from them, merging both T212 accounts since the board has one row per ticker. `applyTrading212` uses that in place of the synthetic lot whenever the backfill has reached the ticker, and falls back to the single lot otherwise — an unfinished backfill must not empty a position's ledger. `syncTrading212History()` drives the backfill one page per call (fills first, then cash movements — the two History endpoints share a tight per-minute budget so they must not run in the same call); app.jsx pages it every 20 s until the server reports both walks finished, then stops. `complete` latches server-side, so later sessions make one request that tops up the staler stream and ends. Admin only. The stored keys already have Portfolio, History: orders and History: transactions — a 403 is not a prompt to regenerate. History items are nested `{ fill, order }` (fill.id / price / quantity / filledAt, order.instrument.ticker / order.side); a flat ticker/filledQuantity shaper parsed every page to 0 rows and still advanced the cursor, walking the history into the void. A page in an unknown envelope refuses to advance; a page of recognised `{ order }` rows that were all cancelled / never filled advances so the walk can reach cash history. The shaper unwraps `{ fill, order }` and order-only envelopes. Deposit amounts are not revalued at live FX. |
...
| `snapshot-record` | `POST /functions/v1/snapshot-record` — **cron-triggered** 24/7 (`*/5 * * * *`, migration `0026`), NOT user-facing. Loads `board_data`, overlays T212 `currentPrice` (overnight US + live LSE) on Yahoo quotes from `prices`, converts FX, and upserts `{ ts, value_usd, deposit_usd }` into `portfolio_snapshots`. Skips the tick when an FX pair is missing or a positioned ticker has no print — a 1:1 fallback writes a permanent spike. Deposit uses T212 cash history once that walk is complete, otherwise lots + board cash (same question `netDepositNow` answers on the client). Either way the deposit figure is not multiplied by live FX — a past GBP lot or deposit does not wiggle when GBPUSD ticks. Same Vault `cron_secret` + `--no-verify-jwt` as overnight-record. Pure helpers (live-price pick, MV, deposit, prune density) pinned by `index.test.ts`. |
...
| `0025_t212_transactions.sql` | Cash movements from Trading 212 (`t212_transactions` + per-account `t212_transactions_sync`). `/equity/history/transactions` is the deposit/withdraw history; once both accounts have been walked, the Investment Performance deposit line is money paid in (deposit − withdraw) rather than summed lot costs. Fees/interest/transfers are stored and ignored by the deposit line. RLS-denied; the `trading212` Edge Function is the only path in. |
...
| `.github/workflows/check.yml` | CI: typecheck → lint → vitest → build → size-limit → knip → dependency audit, all hard gates, on every push + PR. The size gate reads `.size-limit.json` (`assets/app-*.js`, gzipped). It is a doubling detector, not a ratchet — raise it in the same commit when the growth is intentional, and treat a jump that isn't explained by the diff as the bug it is. Current: **122 KB**, raised from 115 KB by the Investment Performance chart (≈113.6 → 118.3 KB across the view, its snapshot data layer, and its axes + crosshair). |
...
| `APP_AUTH_SECRET` | `auth`, `data`, `snapshot-record` | Long random string (`openssl rand -hex 32`). `snapshot-record` mints a short-lived admin token so it can call `prices` the same way the client does. |
...
| `CRON_SECRET` | `overnight-record`, `snapshot-record` | Bearer secret pg_cron presents to both recorders (they deploy `--no-verify-jwt`, so this is their only auth gate). Must equal the Vault secret named `cron_secret` (migration `0021`; `ALTER DATABASE … SET app.cron_secret` is permission-denied on this project). `openssl rand -hex 32`. |
