...
| `snapshot-record` | `POST /functions/v1/snapshot-record` — **cron-triggered** 24/7 (`*/5 * * * *`, migration `0026`), NOT user-facing. Loads `board_data`, overlays T212 `currentPrice` on Yahoo quotes, converts VALUE with live FX, and upserts `{ ts, value_usd, deposit_usd }`. Deposited uses actual T212 fills plus manual/inferred lots, net of sells and board cash; T212 Card cash rows are excluded. Native deposit FX is persisted once while value continues to use live FX. Missing FX/price skips the tick. |
