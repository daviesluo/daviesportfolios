...
- **Agents** is the crypto strategies page. Headline: **TOTAL REALIZED
  ...
  a countdown to the next bar close and a running dot that goes dim when
  the loop has missed two bars. Under the table, the **cross-venue basis**
  over the last 24 h per symbol (now, |basis| p50 / p95 / max, counts above
  20 / 40 / 80 bps) with the verdict the fees impose.
  Clicking a row opens its detail: description and parameters, positions
  ...
  (`?action=dashboard`); the page only formats.
...
| `agents.jsx` / `agents.js` / `agents.test.js` | The Agents page (☰ menu → Agents), a `React.lazy` chunk prefetched like the other modals. `agents.js` is the pure half: `fetchAgentsDashboard` / `fetchAgentsLog` (token-gated calls to the `agents` Edge Function), `strategyRows` (one overview row per strategy: cost, value, unrealised, realised, return on capital, orders, last decision, next decision), `strategyStatus` (running only while the loop has decided within two bars), `totalsView`, `venueRows` (the per-venue split and share), `basisRows` (the 24 h cross-venue basis), `untilText` (the countdown), `decisionView` (the categorical state as text, the rule's and the final action, the model's P(healthy) and caution), `orderView`, `backtestRows` / `rotationBacktestRows` (per rulebook per venue from `docs/agents/backtests/summary.json`, which the chunk imports at build time). `agents.jsx` renders the headline block, the venue split, the caps / Jev strip, the strategy table, the basis table, the "how it works" text and the per-strategy detail (positions, decisions, orders, backtest). Nothing is computed client-side that could disagree with the server. |
