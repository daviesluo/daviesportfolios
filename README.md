...
- **1D chart spans 24 h** — both in-session and ext-hours views show
  ...
  **official** regular close (Yahoo `regularMarketPrice` = the same
  ...
  Crypto (`-USD`) follows the **same ext-hours logic as a US stock**:
  ext-OFF it's "since previous close" (`regularMarketPreviousClose`),
  ext-ON it's the move **since the last US close** (16:00 ET). The one
  source difference is that crypto anchors ext-ON at the 16:00 ET close
  BAR, not at `regularMarketPrice` — Yahoo doesn't freeze that for a 24/7
  asset, so it's the live price (anchoring there would read a flat ~0.00%).
...
| `chart_modal_geometry.js` / `chart_modal_geometry.test.js` | Pure geometry for the ticker chart modal: anchorClose selection (regular / ext-mode fallback chain to lastPrice / prevClose / series[0]; ext-ON, `anchorRefs.isCrypto` skips the `lastPriceAny` step — crypto's `regularMarketPrice` is the LIVE 24/7 price, not a frozen close — so it anchors at the 16:00 ET close BAR like a stock's after-hours move), `hasData` gate, `xOfIdx` / `yOf` scale fns, y-range containment for PE/PS 3yAvg + MA + VWAP + the overnight-dot price, "nice step" y-tick spacing, and index-spaced x-ticks (+ the "now" tick at the live dot's true-time x). Lifted out of `ticker_chart_modal.jsx` in PR #158 — the inline block had grown to ~200 lines that needed to be touched in 8 different positions during PR #157's SFTBY / `.PVT` saga. Pinned by 13 vitest cases. |
