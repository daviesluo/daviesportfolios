...
| `market_hours.js` | DST-aware time helpers (`londonTimeParts`, `usMarketPhase`, `ukTzAbbr`, `usMarketHoursUtc`, `lseIsOpen`). All `Intl`-based so DST cutovers are resolved by the runtime. `lseIsOpen` gates the LSE-ticker ext-hours toggle in `metrics.js` — VUAG.L / SEGM.L show 0 % when LSE is closed (US after-hours / overnight) and the live LSE pct only when LSE is actually trading (US pre-market / first hour of RTH). |
...
| `trading212.js` / `trading212.test.js` | `fetchTrading212Holdings()` hits the `trading212` Edge Function and returns `{ 'VUAG.L': { shares, cost }, 'SEGM.L': { ... } }` or null; `applyTrading212(holdings, t212, today)` overlays the result on the live portfolio by replacing each matching ticker's `lots` with a single synthetic lot dated today. Fired in parallel with the live-prices / MC / ext-series fetches inside `doRefresh`. Best-effort: a null T212 response leaves the lots alone, so a transient upstream error doesn't wipe what the user last saved manually. The user can still edit lots in EditTickerModal; the next auto-refresh tick just overwrites the manual edit with whatever T212 reports. |
