...
| `t212_fills.js` / `t212_fills.test.js` | Reconciles Trading 212's executed fills against the hand-kept lot ledger, for the lot editor. `brokerLedgerFor(orders, ticker)` is that ticker's fills in `{ lots, sells }` shape; `splitAgainstLedger` decides which of them the ledger already carries — same date, same shares to 1e-6, same price to a cent or 0.5% — and **consumes** each match, so two identical same-day fills need two ledger rows to both count (RKLB really does carry two 0.5-share rows at 39.50 on one day); `reconcileFills` returns every fill newest-first flagged `known`, plus the missing ones; `withFillsApplied` returns the ledger with the missing rows folded in. Purely additive — a lot the broker has no record of (SPCX's 50 + 21 bought elsewhere) is never touched. Nothing here writes `shares` or `cost`: the board is the source of truth for quantity. |
