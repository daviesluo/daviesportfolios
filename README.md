...
>
> Every dollar amount and share count in the screenshots below is masked
> by the site's own **hide-values** switch (the eye icon beside
> PORTFOLIO), the feature I built so the page can be shown or shared
> without showing any amount; the mask keeps each number's length.
> Percentages, tickers and market data are real and unmasked.
...
- **Performance vs S&P 500** — both lines rebased to 0 % at the start of
  the window, over 24H / 1W / 1M / 3M / YTD. A second tab,
  **Investment**, shows the book's value against the net money paid in,
  on the same grid. Prices are recorded server-side every five
...
- Two passwords: an admin password that can edit and a read-only one for
  ...
  session.
- Hide-values mode masks every dollar figure and share count and keeps
  the percentages, which is how the screenshots above were taken.
...
Two paths share one database: requests from the page, and scheduled jobs
that keep recording whether or not anyone has the page open.
...
| `supabase_config.js` | Shared `SB_URL`, `SB_ANON`, `EDGE_AUTH_URL`, `EDGE_DATA_URL`, `EDGE_TRADING212_URL`, `EDGE_AGENTS_URL`. |
...
| `overnight-fetch` | `GET /functions/v1/overnight-fetch?tickers=NVDA,AAPL` → `{ "NVDA": [{date,close,volume:0}, …], … }` — the recorded overnight points per ticker over the last ~26 h, oldest-first, in the chart's standard series shape. Anon-readable: the client sends only the anon Bearer (unlike `chart`, which now requires the app token), since the recorded overnight quotes are market data. The modal splices these onto the Yahoo series to draw a real overnight **line** instead of the single heartbeat dot. The PostgREST IN(…) query is **paginated** (`paginateRows` loops `?limit=&offset=` until a short page returns) because a single ~40-holding fetch covers ~12 k rows, well over PostgREST's default 1000-row page — and the query orders by `ticker.asc`, so without pagination the alphabetically-later names (NVDA / ORCL / TSM …) were silently truncated and the modal showed "dot until the single-ticker fetch caught up" only for them. Pure helpers (`parseTickers` charset/cap, `groupRows`, `paginateRows`) pinned. |
...
(issue side) and `data` / `prices` / `chart` / `fundamentals` /
`trading212` / `ops-error` / `agents` (verify side) import, and that
`snapshot-record` uses to mint itself a short-lived admin token, so
issue and verify can't drift apart (each function re-exports what it uses so its own
...
`reportServerError`, the best-effort `ops_errors` insert most
functions' top-level catch uses (not `ops-error`, `overnight-fetch` or
`overnight-record`); `us_market_calendar.ts` is the rule-based
...
US-session anchoring skips holidays/weekends), `overnight-record` (skips
recording on holidays) and `snapshot-record`. A byte-equivalent copy of the rules lives in the
...
trading day open or a holiday closed. The agents' shared modules live
there too: `agents_strategy.ts` (the rulebooks, the state words and
`combineDecision`), `jev.ts` (the decision model's two transports),
`venue.ts` (the venue interface), `revx.ts` (Revolut X, Ed25519-signed),
`kraken.ts` (Kraken, HMAC-SHA512-signed) and `bytes.ts` (the byte and
base64 helpers both signers share).
...
| `0004_ops_errors_retention.sql` | `pg_cron` job at 03:00 UTC daily that drops `ops_errors` rows older than 30 days. Keeps the table bounded and the badge's `?action=summary` scan tight. Applied by `migrations.yml` like every file here. |
...
| `0011_auth_attempts_search_path_escalation.sql` | Hardens the `bump_auth_attempt` RPC from `0001`: explicit `search_path = public, pg_temp` (closes the schema-shadow attack the original didn't lock; brings 0001 to parity with 0008) AND escalating lockout — each repeat threshold-cross doubles the lockout window (24 h → 48 h → 96 h ...), capped at 30 days, via a new `lockouts` column on `auth_attempts`. A brute-forcer who patiently waited the original 24 h could try 3 wrong / wait 24 h forever; doubling makes their expected time superlinear while a legitimate user (who only triggers a lockout once) pays the unchanged 24 h. Applied after `0001`. |
| `0012_av_quota.sql` | `av_quota` table + `try_claim_av_call(max_calls int) → boolean` RPC — atomic per-UTC-day counter. The `fundamentals` Edge Function calls it before every live Alpha Vantage request and refuses to fire when the cap (20, 5-call buffer under AV's 25/day free tier) is reached. Protects against the cache-bypass scenario (RLS regression, env typo) where every Edge invocation goes cold and drains the daily budget in an hour. RLS-deny default. Applied by `migrations.yml`. |
| `0013_board_data_version.sql` | Adds a `version bigint not null default 0` column to `board_data` + a `save_board_data(_data jsonb, _if_match bigint) → jsonb` RPC for optimistic concurrency. The `data` Edge Function passes the client's `If-Match` header through as `_if_match`; the RPC returns `{ ok: false, conflict: true, current_version }` when the supplied version doesn't match the row's actual version (another tab / device wrote in between), and `{ ok: true, version }` after a successful atomic UPSERT + version-bump. `_if_match IS NULL` skips the check — backward-compatible path for pre-0013 clients and one-off scripts. Cold-project special case: if no row exists yet, `_if_match = 0` matches. The client (`portfolio_remote.js`) caches the version returned from every load/save and surfaces the conflict via a banner in `app.jsx` instead of overwriting. Applied by `migrations.yml`. |
...
| `0045_maker_probe_cascade.sql` | `agent_maker_probes` is a THIRD child of `agent_strategies`, and `0044` did not know it. `0042` gave it a `strategy_id` reference with no `on delete` action, so the delete survived only on luck: the three probe rows that existed belonged to `trend-1h`, which stayed. Had a retired row owned one, `0044` would have failed on a foreign key for the SECOND time in a day. Probes now CASCADE — a measurement notebook, not a record of money, and a cascade cannot be forgotten. `agent_orders` and `agent_decisions` stay EXPLICIT on purpose: a fill that is gone is a position that never existed, so deleting one should take a statement someone wrote deliberately, and a forgotten one should fail loudly. `order_id`'s `on delete set null` is untouched, so a probe outlives the order beside it. |
| `0046_delete_kraken_twin.sql` | Deletes `trend-4h-kraken` and its records — 3 orders, 50 decisions, 326 observations, 0 probes, 0 backtests — on Davies' word. The row existed for ONE job, measuring the live rulebook's post-only fills on the second venue, and did not do it: joined on symbol and bar start **50 of 50 paired decisions matched `trend-4h` exactly**, because both carried `signal_venue = 'kraken'` and the same rulebook, so it could differ only through the fill path; nothing read that path; and the path is a simulation whose accuracy was measured keylessly in 18 minutes (a resting order at Kraken's touch is reached 79 % within a minute and 91 % within five, against the loop's 82 % and 92 %). Meanwhile it paid **4.44× the fee for identical fills**. Children first and all of them, the lesson `0044` learned twice. Kraken is now a SIGNAL venue only — every rule still reads its candles — and the browser sweep drives that shape. Reference §4.22. |
| `0047_jev_question_v2.sql` | Sets `enterMin` 0.60 → 0.45 on all three rows, with the loop switched to the v2 Jev question in the same push. The v1 question listed an "established uptrend" checklist stricter than the rulebook and the model applied it to the letter; 0.60 sat where its high-volatility answers were a coin flip. 0.45 was chosen from the model's measured replies to every entry state before any backtest; on a trend entry it refuses a weak trend in high volatility and nothing else (reference §4.21). |
...
| `20260818044956_drop_aug17_fx_spike_snapshot.sql` | Same, for the out-of-band DELETE of the 17 Aug FX-spike snapshot row. |
...
| `src/eslint.config.js` | Flat ESLint config (ESLint 10). It lives in `src/` because that is all it lints: ESLint 10 finds a config from each file's own folder, and `package.json`'s `knip` block points knip at it. Deliberately a **bug** gate, not a formatting one: errors on `react-hooks/rules-of-hooks` (the hook-ordering class that shipped a React-#310 crash once) + `js.recommended` (with `allowEmptyCatch` for the codebase's best-effort-swallow pattern); `react-hooks/exhaustive-deps` is a non-blocking warning since a few effects intentionally narrow their dep set (documented inline). Stylistic / unused-locals are left to `tsc` + `knip`. `npm run lint`. |
| `package.json` | Scripts, dependencies and the Node engines, plus two tool configs: `knip` (the entry points the dead-code scan starts from) and `size-limit` (a 122 KB gzipped cap on `dist/assets/app-*.js`). |
| `wrangler.jsonc` | Cloudflare Pages config. `pages_build_output_dir: ./dist` makes `dist/` the only directory the site publishes. |
| `.nvmrc` | Node 22, the version CI uses. |
| `.github/SECURITY.md` | How to report a vulnerability, what is in scope, and what is already hardened. |
| `supabase/functions/.env.example` | Every Edge Function secret by name, with what it powers; the real values live only in Supabase. |
| `src/public/_headers` / `src/public/robots.txt` | Cloudflare Pages cache rules (deliberately NO long-lived rule for `/assets/*`: a rule applies to every response on the path, and Pages' not-found fallback for a chunk that does not exist yet inherited "immutable for a year" on 2026-09-21 — see Hosting above), `X-Robots-Tag: noindex, nofollow, noarchive, nosnippet, noimageindex` on every path with a matching `robots.txt` (a password-gated personal ledger should not be indexable, quotable or summarisable from a link, by a search engine or by an AI assistant following one — the header covers the JS bundle, which `robots.txt` alone does not reliably reach; neither is access control, and a fetcher that ignores the directive still gets the bytes — what stops a stranger reading the PORTFOLIO is the token gate on every Edge Function, and what stops them reading the SOURCE is that `dist/` holds only the minified bundle, no `src/` and no sourcemaps), **plus** the production security headers — Content-Security-Policy (scoped to the exact upstream origins the app talks to: Supabase, Yahoo, Eastmoney, Danjuanapp, Xueqiu, and the five public CORS proxies), Strict-Transport-Security (2-year preload), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` denying camera/mic/geolocation/payment. `index.html` / `sw.js` always revalidate, and `/assets/*` carries no long-lived rule (Pages revalidates it with ETags) so a chunk requested mid-deploy is never cached as the HTML fallback. Source-of-truth lives at `src/public/_headers`; Vite copies it into `dist/` on build, where Pages reads it. |
| `manifest.webmanifest` | PWA install metadata (name, theme color; the icon list is still empty). |
| `.github/workflows/check.yml` | CI on every PR and every push to `main`: bundle freshness (a `src/` change must ship a rebuilt `dist/`) → typecheck → lint → vitest → build → the browser sweep → size-limit → knip → dependency audit, all hard gates. |
| `.github/workflows/edge-functions.yml` | Edge Function CI/CD. Runs `deno test supabase/functions/` on every PR; on push to `main` it also deploys every function with a changed non-test file in its folder (all of them when `_shared/` changed) via the Supabase CLI (requires `SUPABASE_ACCESS_TOKEN` + `SUPABASE_PROJECT_REF` repo secrets). `PUBLIC_FNS` (`auth data trading212 ops-error overnight-record snapshot-record agents`) deploy `--no-verify-jwt` because their own gate is not a Supabase JWT. Replaces the manual paste-into-dashboard workflow that CLAUDE.md still mentions — once those secrets are set the workflow takes over and the dashboard step is optional. |
...
| `docs/improvement-plan.md` | The whole-repository review of 2026-09-05, as an ordered plan: 28 items in five tiers (0–4), each with what it costs, what it risks, how it is verified, and whether it needs Davies at a console. Five tiers (0–4). Mostly a proposal: item 1 (the site publishing the whole repository) was fixed on 2026-09-18 by serving only `dist/`, and item 24 (lazy modal chunks) is done. `LEDGER.md`'s what-remains list is the short version and points here for the reasoning. Kept free of anything attack-shaped. |
| `docs/handover.md` | The ledger's **archive**: the append-only decision log and the raw session transcripts, about 37k lines. Closed operations move here verbatim from `LEDGER.md`. Opened only when a closed item is reopened or audited. Quotes real positions — do not copy balances out of it. |
...
| `bin/hooks/pre-commit` | The ledger's two gates, extracted from `.ledger/SKILL.md`: a commit with substantive changes must carry `LEDGER.md` or follow one that did, and a new history section must open with a well-formed source header. Reached via `core.hooksPath`, which — like `ledger.path` — is per-clone config a rebuilt container loses; `LEDGER.md`'s machine-setup section has both commands. Hatch is `LEDGER_OK=1 git commit`, not `--no-verify`. |
| `bin/setup.sh` | One-time setup for a fresh clone or a rebuilt container, safe to re-run: `core.hooksPath bin/hooks` (without it the ledger hook is silently off), the ledger path, and `npm ci`. It does not set a commit identity; that is each person's own. |
| `bin/gates.sh` | Every gate CI runs, in CI's order, stopping at the first failure: typecheck, lint, vitest, build, the browser sweep, size-limit, knip, the npm audit, `deno check`, `deno test`. Warns when the ledger hook is off in the clone. |
