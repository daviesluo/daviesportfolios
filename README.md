...
- **Agents (crypto auto-trading, paper first)** — an "Agents" page behind
  the ☰ menu: two rulebooks (4h trend following, 30-day momentum), each run
  on Revolut X and on Kraken, deciding on closed bars every five minutes
  with TypeSafe's **Jev 1.13** decision model as a veto/advice node inside a
  deterministic rulebook and a database-held risk gate with the last word.
...
- **Agents** is the crypto strategies page. Headline: **TOTAL REALIZED
  ...
  and fees beside it. A strip shows each venue (fees at this account's
  tier, funding, key health), the caps from `agent_risk` and the last 24 h
  of Jev calls and spend. The table has one row per strategy — name,
  rulebook, venue, mode badge (LIVE / PAPER / PAUSED), cost, value,
  unrealised, realised, return on capital, orders today, the last decision
  and a running dot that goes dim when the loop has missed two bars.
  Clicking a row opens its detail: description and parameters, positions
  (size, average cost, mark, value, unrealised, realised, fees), the
  ...
  post-only limits, fills, fees, mode) with a "Load full history" button,
  and the walk-forward backtest for that rulebook on that venue with the
  other venue's figure beside it. Refreshes every minute while open; the
...
| `agents.jsx` / `agents.js` / `agents.test.js` | The Agents page (☰ menu → Agents), a `React.lazy` chunk prefetched like the other modals. `agents.js` is the pure half: `fetchAgentsDashboard` / `fetchAgentsLog` (token-gated calls to the `agents` Edge Function), `strategyRows` (one overview row per strategy: cost, value, unrealised, realised, return on capital, orders, last decision), `strategyStatus` (running only while the loop has decided within two bars), `totalsView`, `decisionView` (the categorical state as text, the rule's and the final action, the model's P(healthy) and caution), `orderView`, `backtestRows` (per rulebook per venue from `docs/agents/backtests/summary.json`, which the chunk imports at build time). `agents.jsx` renders the headline block, the venue / caps / Jev strip, the strategy table, the "how it works" text and the per-strategy detail (positions, decisions, orders, backtest). Nothing is computed client-side that could disagree with the server. |
...
|---|---|
| `agents` | The crypto agents' server half. `POST ?action=tick` (pg_cron every 5 min, migration `0037`; cron bearer or admin token) runs one turn of `tick.ts`: for every strategy that is not paused it refreshes that strategy's OWN venue's 4h / daily candles, latest closed 5-minute candle, touch and pair limits; settles open orders (a paper order fills when the venue's last 5-minute candle traded through its price and pays the venue's maker fee; a live order is re-read from the venue; either is cancelled after three unfilled turns, a live one only after the venue confirms and any partial fill is read back); then, on a NEWLY closed decision bar and with no order in flight, builds the categorical state (`_shared/agents_strategy.ts`), asks Jev (`_shared/jev.ts`: OpenRouter first, TypeSafe direct as fallback, `provider: none` when both fail — which means hold), applies the rulebook, applies the risk gate (per-order cap, per-venue exposure, daily loss limit, per-venue order count, global pause — every number from `agent_risk`), records the decision with everything it saw, and places the order it allows as a resting post-only limit at the touch. A live order additionally needs `agent_strategies.mode = 'live'`, `agent_risk.live_confirmed_at` set, and credentials for that venue. `GET ?action=dashboard` (admin or ro) computes what the page shows — positions and P&L from fills via `positionFromFills`, marked at each venue's mid, plus venue health, caps, Jev spend and recent decisions / orders; `GET ?action=log&strategy=` pages more. `GET ?action=probe` (cron or admin) is the read-only credential self-check: signed Revolut X balances / pairs / candles-with-query, Kraken balances / fee tier / open orders / `AddOrder validate=true` (the venue checks and places nothing), Jev on both transports. Venues behind one interface (`_shared/venue.ts`): `_shared/revx.ts` (Ed25519-signed; keyless public market data) and `_shared/kraken.ts` (HMAC-SHA512-signed, test vector pinned; OHLC capped at 720 candles). **Auth: cron bearer or `x-app-token`**, so it deploys `--no-verify-jwt` (in `PUBLIC_FNS`). Pinned by `strategy.test.ts`, `momentum.test.ts`, `jev.test.ts`, `revx.test.ts`, `kraken.test.ts`, `tick.test.ts`. `backtest.ts` is the walk-forward backtester run by hand (same functions as the loop) that writes `docs/agents/backtests/`. |
...
| `0037_agents.sql` | The agents feature's schema: `agent_strategies` (one row per rulebook per venue — `kind`, `venue`, symbols, `mode` paper / live / paused, capital, params; seeded with `trend-4h`, `momentum-1d`, `trend-4h-kraken`, `momentum-1d-kraken`, all PAPER), `agent_risk` (one row of caps, per venue account: max order $20, max exposure $100, daily loss $5, 40 orders a day, global pause, and `live_confirmed_at`, null until Davies says go — the tick refuses every live order while it is null), `agent_decisions` (every decision with the categorical state, the numbers behind it, the questions, the answers, the rule's and the final action, the risk verdict), `agent_orders` (every order, paper or live, with the request as placed and the venue's reply), `agent_candles` (the venue candles the loop reads, keyed by venue) and `agent_backtests`. RLS on everything; the function uses the service role. Schedules `agents-tick-every-5min` → `POST /functions/v1/agents?action=tick` with the Vault `cron_secret`, same machinery as `snapshot-record`. Same cross-environment warning as `0016` / `0026`: the cron URL is production's host. |
