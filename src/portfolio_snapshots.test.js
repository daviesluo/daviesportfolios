...
describe('loadSnapshots', () => {
  ...
  it('keeps a Value-only row whose deposit is NULL', async () => {
    globalThis.fetch = /** @type {any} */ (vi.fn(async () => ({
      ok: true,
      json: async () => ({ snapshots: [
        { ts: '2026-08-14T14:30:00Z', value_usd: 105, deposit_usd: 100 },
        { ts: '2026-08-14T14:35:00Z', value_usd: 110, deposit_usd: null },
      ] }),
    })));
    const out = await loadSnapshots(Date.parse('2026-08-14T00:00:00Z'));
    expect(out).toEqual([
      { ts: Date.parse('2026-08-14T14:30:00Z'), value: 105, deposit: 100 },
      { ts: Date.parse('2026-08-14T14:35:00Z'), value: 110, deposit: NaN },
    ]);
  });
...
describe('dropDepositSpikes', () => {
  ...
  it('does not judge a NULL deposit as a spike', () => {
    const rows = [row(1, 100), row(2, NaN), row(3, 100)];
    expect(dropDepositSpikes(rows)).toEqual(rows);
  });
